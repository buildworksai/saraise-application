      ...
      {rings.map((ring, index) => (
        <div key={ring.name} className="flex items-center gap-2">
          ...
          <div
            className={cn(
              ...
              ring.status === "completed" &&
                "bg-green-500/20 border-green-500 dark:bg-green-400/20 dark:border-green-400",
              ring.status === "in_progress" &&
                "bg-primary-main/20 border-primary-main animate-pulse",
          ...
          >
            ...
            {ring.status === "in_progress" && <Clock className="w-6 h-6 text-primary-main" />}
            {ring.status === "failed" && <XCircle className="w-6 h-6 text-destructive" />}
