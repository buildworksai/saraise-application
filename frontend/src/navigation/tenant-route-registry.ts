...
export function buildTenantSidebarTree(
  ...
  const routesById = new Map(routes.map((route) => [route.id, route]));
  for (const route of routes) {
    const contextualNavItem = ["process_mining", "customization_framework", "integration_platform", "email_marketing", "ai_agent_management"].includes(route.module) && route.navigation.type === "contextual";
