...
describe("notification UX states", () => {
  ...
  it("renders an explicit permission-denied deep-link state", () => {
    render(
      <GovernedError
        error={new ApiError("Denied", 403, undefined, "permission_denied", "corr-403")}
      />
    );
  ...
  it("does not fabricate mutation success", () => {
    render(
      <MutationError
        error={new ApiError("Adapter offline", 503, undefined, "adapter_unavailable", "corr-503")}
      />
    );
    ...
    render(
      <MemoryRouter>
        <EmptyPanel
          ...
          description="No durable evidence matches."
          action={{ label: "Dispatch", to: "/notifications/deliveries/new" }}
        />
        <StatusPill value="retry_wait" />
      </MemoryRouter>
    );
