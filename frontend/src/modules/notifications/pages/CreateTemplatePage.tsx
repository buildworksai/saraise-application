...
export function CreateTemplatePage() {
  ...
  const create = useMutation({
    mutationFn: (input: TemplateCreateInput) => notificationService.templates.create(input),
    onSuccess: (item) => navigate(PATHS.TEMPLATE_DETAIL(item.id)),
  });
  return (
    <PageShell
      title="Create notification template"
      description="Define safe variables and verify the sandboxed render before creating version 1."
    ...
    >
      <TemplateForm
        ...
        onPreview={(draft) =>
          notificationService.templates.previewDraft({
            context: {},
            draft: {
              name: draft.name,
