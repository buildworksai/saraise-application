...
export const RecoveryPointListPage = () => {
  ...
  const query = useQuery({
    queryKey: ["bdr", "recovery-points", status, search],
    queryFn: () =>
      backupDisasterRecoveryService.listRecoveryPoints({
  ...
  return (
    <PageShell>
      ...
      <PageHeader
        ...
        description="Immutable restore sources with provider-confirmed integrity evidence."
        actions={
          <Button onClick={() => navigate(MODULE_PATHS.backupNew)}>
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_14rem]">
        <label className="relative">
          <span className="sr-only">Search recovery points</span>
          <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
          <input
            className={`${inputClass} pl-9`}
            value={search}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Search job or scope…"
          />
        </label>
        <label>
          ...
          <select
            ...
            value={status}
            onChange={(event) => setStatus(event.target.value as RecoveryPointStatus | "")}
          >
            <option value="">All statuses</option>
            {["discovered", "verifying", "available", "corrupt", "expired", "deleted"].map(
              (value) => (
      ...
      {points.length === 0 ? (
        <DomainEmptyState
          icon={DatabaseBackup}
          title="No recovery points match"
          description="Request a backup to create a recovery point, or clear filters to inspect existing protection evidence."
        ...
        <ResponsiveTable
          ...
          headers={["Scope", "Backup type", "Status", "Captured", "Expires", "Size"]}
        >
          {points.map((point) => (
            <tr
              key={point.id}
            ...
            >
              <td className="px-4 py-3">
                <button
                  ...
                  onClick={() => navigate(`${MODULE_PATHS.recoveryPoints}/${point.id}`)}
                >
                  {point.scope_ref}
                </button>
                <p className="text-xs text-muted-foreground">{point.scope_type}</p>
              </td>
              <td className="px-4 py-3 capitalize">{point.backup_type}</td>
              <td className="px-4 py-3">
                <StatusPill status={point.status} presentation={presentation} />
              </td>
              <td className="px-4 py-3">{formatDateTime(point.captured_at)}</td>
              <td className="px-4 py-3">{formatDateTime(point.expires_at)}</td>
              <td className="px-4 py-3">{formatBytes(point.size_bytes, presentation)}</td>
            </tr>
          ))}
        </ResponsiveTable>
      )}
