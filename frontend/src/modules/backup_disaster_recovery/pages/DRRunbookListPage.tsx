...
export const DRRunbookListPage = () => {
  ...
  const query = useQuery({
    queryKey: ["bdr", "runbooks", status, search],
    queryFn: () =>
      backupDisasterRecoveryService.listRunbooks({
  ...
  return (
    <PageShell>
      ...
      <PageHeader
        title="DR runbooks"
        description="Versioned recovery plans with governed steps, targets, and failure policies."
        actions={
          <Button onClick={() => navigate(MODULE_PATHS.runbookNew)}>
      ...
      <div className="grid gap-3 sm:grid-cols-[1fr_14rem]">
        ...
        <select
          ...
          value={status}
          onChange={(event) => setStatus(event.target.value as RunbookStatus | "")}
        >
      ...
      {runbooks.length === 0 ? (
        <DomainEmptyState
          ...
          description="Create a runbook to turn recovery knowledge into a repeatable, measurable operating procedure."
          actionLabel="Create runbook"
          onAction={() => navigate(MODULE_PATHS.runbookNew)}
        />
      ) : (
        <ResponsiveTable
          label="DR runbooks"
          headers={["Runbook", "Status", "Scope", "RPO target", "RTO target", "Updated"]}
        >
          {runbooks.map((book) => (
            <tr key={book.id} className="hover:bg-muted/40">
              <td className="px-4 py-3">
                <button
                  ...
                  onClick={() => navigate(`${MODULE_PATHS.runbooks}/${book.id}`)}
                >
                  {book.name}
                </button>
                <p className="text-xs text-muted-foreground">
                  {book.slug} · v{book.version}
                </p>
              </td>
              <td className="px-4 py-3">
              ...
              <td className="px-4 py-3">
                {book.scope_type}: {book.scope_ref}
              </td>
              <td className="px-4 py-3">{formatDuration(book.rpo_target_seconds, presentation)}</td>
              <td className="px-4 py-3">{formatDuration(book.rto_target_seconds, presentation)}</td>
              <td className="px-4 py-3">{formatDateTime(book.updated_at)}</td>
            </tr>
          ))}
        </ResponsiveTable>
      )}
