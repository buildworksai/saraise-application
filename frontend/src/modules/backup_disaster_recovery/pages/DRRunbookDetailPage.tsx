...
export const DRRunbookDetailPage = () => {
  ...
  if (!id) return <ResourceLookup title="Open DR runbook" description="Open a tenant-owned runbook version by its UUID." label="Runbook UUID" destination={(value) => `${MODULE_PATHS.runbooks}/${value}`} />;
  ...
  return <PageShell><BackgroundProgress active={query.isFetching || stepsQuery.isFetching || pending} /><PageHeader title={`${book.name} · v${book.version}`} description={book.description || 'No description provided.'} parentLabel="DR runbooks" parentPath={MODULE_PATHS.runbooks} actions={<><StatusPill status={book.status} presentation={presentation} />{book.status === 'draft' ? <><Button variant="outline" onClick={() => navigate(`${MODULE_PATHS.runbooks}/${book.id}/edit`)}><Edit3 className="mr-2 h-4 w-4" />Edit steps</Button><Button disabled={pending || steps.length === 0} onClick={() => publish.mutate()}><Send className="mr-2 h-4 w-4" />Publish</Button></> : null}{book.status === 'published' ? <><Button variant="outline" disabled={pending} onClick={() => clone.mutate()}><Copy className="mr-2 h-4 w-4" />Clone draft</Button><Button variant="outline" disabled={pending} onClick={() => retire.mutate()}>Retire</Button></> : null}</>} />
    <MutationError error={mutationError instanceof Error ? mutationError : null} />
    <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{[['Scope',`${book.scope_type}: ${book.scope_ref}`],['RPO target',formatDuration(book.rpo_target_seconds, presentation)],['RTO target',formatDuration(book.rto_target_seconds, presentation)],['Last updated',formatDateTime(book.updated_at)]].map(([label,value]) => <Card key={label}><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">{label}</CardTitle></CardHeader><CardContent className="font-semibold">{value}</CardContent></Card>)}</section>
    <Card><CardHeader><CardTitle>Recovery sequence</CardTitle></CardHeader><CardContent>{steps.length ? <ol className="space-y-3">{steps.map((step) => <li key={step.id} className="grid grid-cols-[2.5rem_1fr_auto] items-start gap-3 rounded-lg border p-4"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-sm font-bold text-primary-foreground">{step.position}</span><div><p className="font-medium">{step.name}</p><p className="text-sm text-muted-foreground">{step.description}</p></div><StatusPill status={step.action_type} presentation={presentation} /></li>)}</ol> : <p className="text-sm text-muted-foreground">This draft has no active steps. Add at least one validated step before publishing.</p>}</CardContent></Card>
