...
export const BackupExecutionCreatePage = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (receipt) =>
      navigate(`${MODULE_PATHS.recoveryPoints}?backup_job_id=${receipt.backup_job_id}`),
  });
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!scopeRef.trim()) {
      setValidation("Enter the canonical scope reference.");
      return;
    }
  ...
  return (
    <PageShell>
      <PageHeader
        title="Request a backup"
        description="Capture a protected artifact through the registered backup catalog. The request continues as a durable background job."
        parentLabel="Overview"
        parentPath={MODULE_PATHS.overview}
      />
      <MutationError error={mutationError} />
      <FormCard
        title="Backup scope"
        description="Choose the smallest canonical scope that meets your recovery objective."
        onSubmit={submit}
        footer={
          <>
            <Button type="button" variant="outline" onClick={() => navigate(MODULE_PATHS.overview)}>
              Cancel
            </Button>
            <Button type="submit" disabled={mutation.isPending} aria-disabled={mutation.isPending}>
              {mutation.isPending ? "Queuing backup…" : "Queue backup"}
            </Button>
          </>
        }
      >
        ...
        <FormField id="backup-type" label="Backup type">
          <select
            id="backup-type"
            className={inputClass}
            value={backupType}
        ...
        <div className="md:col-span-2">
          <FormField
            id="scope-ref"
            label="Canonical scope reference"
            error={validation || fieldError(mutationError, "scope_ref")}
            hint="Use a registered logical identifier—never a credential, file URL, or connection string."
          >
            <input
              id="scope-ref"
              className={inputClass}
      ...
      <p aria-live="polite" className="sr-only">
        {mutation.isPending ? "Backup submission in progress" : ""}
      </p>
    </PageShell>
  );
};
