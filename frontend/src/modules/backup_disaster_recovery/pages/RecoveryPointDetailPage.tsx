...
import { BackgroundProgress, MODULE_PATHS, ModuleErrorState, MutationError, PageHeader, PageShell, PageSkeleton, StatusPill, createIdempotencyKey, formatBytes, formatDateTime } from '../components/ModuleUi';
...
export const RecoveryPointDetailPage = () => {
  ...
  const query = useQuery({ queryKey: ['bdr', 'recovery-point', id], queryFn: () => id ? backupDisasterRecoveryService.getRecoveryPoint(id) : Promise.reject(new Error('Recovery point not found')), enabled: Boolean(id), refetchInterval: (state) => state.state.data?.status === 'verifying' ? 4_000 : false });
