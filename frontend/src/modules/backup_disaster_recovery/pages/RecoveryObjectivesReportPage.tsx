...
export const RecoveryObjectivesReportPage = () => {
  ...
  const [bucket,setBucket] = useState<ObjectiveBucket>('week');
  const [runbookId,setRunbookId] = useState('');
  const [validation,setValidation] = useState('');
  const [filters,setFilters] = useState<ObjectiveReportFilters>({ from: new Date(`${startDate}T00:00:00`).toISOString(), to: new Date(`${endDate}T23:59:59`).toISOString(), bucket: 'week' });
  const query = useQuery({ queryKey: ['bdr','objectives',filters], queryFn: () => backupDisasterRecoveryService.getObjectiveReport(filters) });
