...
export const RecoveryObjectivesReportPage = () => {
  ...
  const applyFilters = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!from || !to || !bucket || new Date(from) > new Date(to)) {
      setValidation("Choose a valid date range and configured bucket with From before To.");
      return;
    }
    setValidation("");
    setFilters({
  ...
  return (
    <PageShell>
      ...
      <PageHeader
        ...
        description="RPO and RTO outcomes include failed restores and remain grouped by immutable runbook version."
        actions={
      ...
      <Card>
        <CardContent className="pt-6">
          <form onSubmit={applyFilters} className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <FormField id="bucket" label="Bucket">
              <select
                id="bucket"
                className={inputClass}
                value={bucket}
                onChange={(event) => setBucket(event.target.value as ObjectiveBucket)}
              >
                {configuration.data.document.reports.allowed_buckets.map((value) => (
            ...
            <FormField id="runbook" label="Runbook ID">
              <input
                id="runbook"
                className={inputClass}
                value={runbookId}
                onChange={(event) => setRunbookId(event.target.value)}
                placeholder="All runbooks"
              />
            </FormField>
        ...
        <>
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <MetricCard
              ...
              value={`${report.rpo_compliance_percent.toFixed(1)}%`}
              hint="Across all completed and failed restores"
            />
            <MetricCard
              label="RTO compliance"
              value={`${report.rto_compliance_percent.toFixed(1)}%`}
              hint="Across all completed and failed restores"
            />
            <MetricCard
              label="Total restores"
              value={String(report.total_restores)}
              hint={`${formatDateTime(report.from)} to ${formatDateTime(report.to)}`}
            />
            <MetricCard
          ...
          {report.buckets.length ? (
            <ResponsiveTable
              label="Objective report by period and runbook version"
              headers={[
                "Period",
                "Runbook version",
                "Restores",
            ...
            >
              {report.buckets.map((item) => (
                <tr key={`${item.period_start}-${item.runbook_id}-${item.runbook_version}`}>
                  ...
                  <td className="px-4 py-3">
                    {item.runbook_name} · v{item.runbook_version}
                  </td>
                  <td className="px-4 py-3">{item.restore_count}</td>
                  <td className="px-4 py-3">{item.failed_restore_count}</td>
                  <td className="px-4 py-3">{item.rpo_compliance_percent.toFixed(1)}%</td>
                  <td className="px-4 py-3">{item.rto_compliance_percent.toFixed(1)}%</td>
                </tr>
              ))}
            </ResponsiveTable>
          ) : (
            <DomainEmptyState
              icon={Activity}
              title="No objective measurements"
              description="Complete a restore or recovery exercise in this reporting period to generate RPO and RTO evidence."
            />
          )}
        </>
      ) : null}
