...
export const BackupDisasterRecoveryConfigurationPage = () => {
  ...
  const versions = useQuery({
    queryKey: ["bdr", "configuration", "versions"],
    queryFn: backupDisasterRecoveryService.listConfigurationVersions,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: backupDisasterRecoveryService.updateConfiguration,
    onSuccess: invalidate,
  });
  const importMutation = useMutation({
    mutationFn: backupDisasterRecoveryService.importConfiguration,
    onSuccess: () => {
      setImportMode(false);
      invalidate();
    },
  ...
  const rollbackMutation = useMutation({
    mutationFn: backupDisasterRecoveryService.rollbackConfiguration,
    onSuccess: invalidate,
  });
  const exportMutation = useMutation({
    mutationFn: backupDisasterRecoveryService.exportConfiguration,
    onSuccess: (result) =>
      downloadDocument(`backup-disaster-recovery-config-v${result.version}.json`, result),
  });
  ...
  const readImport = (event: ChangeEvent<HTMLInputElement>): void => {
    ...
    reader.onload = () => {
      ...
      try {
        ...
        if (
          typeof record.rollout === "object" &&
          record.rollout !== null &&
          !Array.isArray(record.rollout)
        ) {
          const importedRollout = record.rollout as Readonly<Record<string, unknown>>;
          setRolloutEnabled(importedRollout.enabled === true);
          if (Array.isArray(importedRollout.roles))
            setRoles(
              importedRollout.roles
                .filter((item): item is string => typeof item === "string")
                .join(", ")
            );
          if (Array.isArray(importedRollout.cohorts))
            setCohorts(
              importedRollout.cohorts
                .filter((item): item is string => typeof item === "string")
                .join(", ")
            );
        }
  ...
  return (
    <PageShell>
      ...
      <PageHeader
        title="Disaster recovery configuration"
        description="Tenant-scoped, versioned policy and operational controls. Preview every change before it is applied."
        actions={
          <>
            <input
              ref={fileInput}
      ...
      {importMode ? (
        <p role="status" className="rounded-md border border-primary/30 bg-primary/5 p-3 text-sm">
          Imported values are staged only. Preview the diff, then apply the import transaction.
        </p>
      ) : null}
      <Card>
        <CardHeader>
          <CardTitle>Configuration document · version {configuration.data.version}</CardTitle>
          <p className="text-sm text-muted-foreground">
            All values are validated again by the server. Secrets and provider credentials are never
            accepted here.
          </p>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="grid gap-5 md:grid-cols-2">
            <FormField
              ...
              label="Environment"
              hint="Selects tenant values without creating environment-specific code paths."
            >
              <input
                id="configuration-environment"
            ...
            <FormField id="configuration-rollout" label="Phased rollout">
              <label className="flex h-10 items-center gap-3 rounded-md border border-input px-3">
                ...
                />
                Enabled for selected roles or cohorts
              </label>
            </FormField>
            <FormField
              id="configuration-roles"
              label="Rollout roles"
              hint="Comma-separated role allow-list."
            >
              <input
                id="configuration-roles"
            ...
            <FormField
              id="configuration-cohorts"
              label="Rollout cohorts"
              hint="Comma-separated cohort allow-list."
            >
              <input
                id="configuration-cohorts"
                disabled={!rolloutEnabled}
          ...
          <FormField
            ...
            hint="Every setting exposed here has the same RBAC-gated API representation."
          >
            <textarea
              id="configuration-document"
              spellCheck={false}
              className={`${textareaClass} min-h-[32rem] font-mono text-xs`}
              value={documentText}
              onChange={(event) => {
          ...
          <div className="flex flex-wrap justify-end gap-3">
            <Button variant="outline" disabled={previewMutation.isPending} onClick={requestPreview}>
              <FileDiff className="mr-2 h-4 w-4" />
              {previewMutation.isPending ? "Validating…" : "Preview changes"}
            </Button>
            <Button disabled={!canApply || busy} onClick={apply}>
              <Save className="mr-2 h-4 w-4" />
              {importMode ? "Apply import" : "Apply configuration"}
            </Button>
          </div>
        </CardContent>
      </Card>
      {preview ? (
        <Card>
          <CardHeader>
            <CardTitle>Validated change preview</CardTitle>
          </CardHeader>
          <CardContent>
            {preview.changes.length ? (
              <ResponsiveTable label="Configuration changes" headers={["Path", "Before", "After"]}>
                {preview.changes.map((change) => (
                  <tr key={change.path}>
                    <td className="px-4 py-3 font-mono text-xs">{change.path}</td>
                    <td className="max-w-sm break-words px-4 py-3 font-mono text-xs">
              ...
              <p className="text-sm text-muted-foreground">
                No effective changes. Apply remains safe and idempotent.
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Every entry records actor, correlation ID, prior value, new value, and rollback
            ancestry.
          </p>
        </CardHeader>
        <CardContent>
          {versions.data?.length ? (
            <ResponsiveTable
              label="Configuration version history"
              headers={["Version", "Changed", "Actor", "Correlation ID", "Origin", "Action"]}
            >
              {versions.data.map((version) => (
                <tr key={version.id}>
                  <td className="px-4 py-3 font-semibold">v{version.version}</td>
                  ...
                  <td className="px-4 py-3">
                    {version.rollback_of ? `Rollback of ${version.rollback_of}` : "Direct change"}
                  </td>
                  <td className="px-4 py-3">
                    <Button
                      variant="outline"
                      size="sm"
                      disabled={version.version === configuration.data.version || busy}
                      onClick={() => rollbackMutation.mutate({ version: version.version })}
                    >
            ...
            <p className="text-sm text-muted-foreground">
              The initial immutable version appears after the first saved change.
            </p>
          )}
        </CardContent>
      </Card>
