...
export const DRRunbookCreatePage = () => {
  ...
  const mutation = useMutation({
    mutationFn: backupDisasterRecoveryService.createRunbook,
    onSuccess: (runbook) => navigate(`${MODULE_PATHS.runbooks}/${runbook.id}/edit`),
  });
  ...
  return (
    <PageShell>
      <PageHeader
        title="Create a DR runbook"
        description="Define recovery objectives first, then build and validate its ordered steps."
        parentLabel="DR runbooks"
        parentPath={MODULE_PATHS.runbooks}
      />
      <MutationError error={error} />
      <RunbookForm
        initial={blankRunbookValues(configuration.data.document.runbooks)}
        submitting={mutation.isPending}
        serverError={error}
        submitLabel="Create draft"
        onCancel={() => navigate(MODULE_PATHS.runbooks)}
