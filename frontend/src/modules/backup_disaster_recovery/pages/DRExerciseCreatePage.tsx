...
export const DRExerciseCreatePage = () => {
  ...
  const mutation = useMutation({
    mutationFn: backupDisasterRecoveryService.createExercise,
  ...
  return (
    <PageShell>
      <PageHeader
        title="Schedule a DR exercise"
        description="Rehearse a published plan under the tenant's configured environment policy."
        parentLabel="DR exercises"
        parentPath={MODULE_PATHS.exercises}
      />
      <MutationError error={error} />
      <ExerciseForm
        initial={defaultExerciseValues(configuration.data.document.exercises)}
        submitting={mutation.isPending}
        serverError={error}
        submitLabel="Schedule exercise"
        idempotencyKey={key}
        productionEnabled={configuration.data.document.exercises.production_enabled}
        onCancel={() => navigate(MODULE_PATHS.exercises)}
