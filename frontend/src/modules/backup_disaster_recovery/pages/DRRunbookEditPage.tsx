...
import type { BDRStepConfiguration, RunbookActionType, RunbookStepCreateRequest, RunbookStepParameters } from '../contracts';
...
import { BackgroundProgress, FormField, MODULE_PATHS, ModuleErrorState, MutationError, PageHeader, PageShell, PageSkeleton, ResourceLookup, StatusPill, inputClass, textareaClass } from '../components/ModuleUi';
import { useBackupDisasterRecoveryConfiguration } from '../hooks/useBackupDisasterRecoveryConfiguration';

const buildParameters = (action: RunbookActionType, target: string, detail: string, configuration: BDRStepConfiguration): RunbookStepParameters => {
  switch (action) {
    ...
    case 'verify': return { action_type: action, checks: configuration.allowed_verification_checks };
...
export const DRRunbookEditPage = () => {
  ...
  if (!id) return <ResourceLookup title="Edit DR runbook" description="Open an editable tenant-owned runbook draft by its UUID." label="Runbook UUID" destination={(value) => `${MODULE_PATHS.runbooks}/${value}/edit`} />;
  if (bookQuery.isLoading || stepsQuery.isLoading || configuration.isLoading) return <PageSkeleton />;
  ...
  const addStep = (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload: RunbookStepCreateRequest = { runbook_id: book.id, step_key: stepKey.trim().toLowerCase(), position: steps.length + 1, name: stepName.trim(), description: description.trim(), action_type: action, extension_action_key: action === 'extension' ? target.trim() : undefined, approval_permission: action === 'manual_approval' ? target.trim() : undefined, parameters: buildParameters(action, target, detail, stepConfiguration), timeout_seconds: stepConfiguration.default_timeout_seconds, retry_limit: stepConfiguration.default_retry_limit, on_failure: stepConfiguration.default_on_failure };
    setStepValidation('');
