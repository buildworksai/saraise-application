...
import type { DRExerciseCreateRequest, ExerciseEnvironment, ExerciseType } from '../contracts';
...
export const defaultExerciseValues = (): ExerciseFormValues => ({ name: '', runbookId: '', recoveryPointId: '', exerciseType: 'tabletop', environment: 'isolated', scheduledFor: new Date(Date.now() + 3600_000).toISOString().slice(0,16) });

export const ExerciseForm = ({ initial, submitting, serverError, submitLabel, idempotencyKey, editMode = false, onCancel, onSubmit }: { initial: ExerciseFormValues; submitting: boolean; serverError: Error | null; submitLabel: string; idempotencyKey: string; editMode?: boolean; onCancel: () => void; onSubmit: (payload: DRExerciseCreateRequest) => void }) => {
  ...
  return <FormCard title="Exercise plan" description="Exercises run only in isolated or standby environments; production is intentionally unavailable." onSubmit={submit} footer={<><Button type="button" variant="outline" onClick={onCancel}>Cancel</Button><Button type="submit" disabled={submitting}>{submitting ? 'Saving exercise…' : submitLabel}</Button></>}>
