...
export const formatDuration = (
  seconds: number | null,
  presentation: BDRPresentationConfiguration
): string => {
  if (seconds === null) return "Not measured";
  if (seconds < presentation.duration_minute_seconds) return `${seconds}s`;
  if (seconds < presentation.duration_hour_seconds)
    return `${Math.round(seconds / presentation.duration_minute_seconds)}m`;
...
export const BackgroundProgress = ({
  ...
  <div aria-live="polite" aria-atomic="true" className="h-1">
    {active ? (
      <div role="status" className="h-1 animate-pulse rounded-full bg-primary">
    ...
    status === 403
      ...
      : status === 404
        ...
        : status === 503
          ...
          : "Unable to load disaster recovery data";
  return (
    <PageShell>
      <section
        role="alert"
        className="mx-auto flex min-h-[28rem] max-w-2xl flex-col items-center justify-center rounded-xl border bg-card p-8 text-center"
      >
...
export const DomainEmptyState = ({
  ...
  <section className="flex min-h-72 flex-col items-center justify-center rounded-xl border border-dashed bg-card p-8 text-center">
    <span className="mb-4 rounded-full bg-muted p-4">
      <Icon aria-hidden="true" className="h-9 w-9 text-muted-foreground" />
    </span>
  ...
  <div className="overflow-x-auto rounded-xl border bg-card">
    <table className="w-full min-w-[48rem] text-left text-sm">
      ...
      <thead className="border-b bg-muted/60">
        <tr>
          {headers.map((header) => (
            <th key={header} scope="col" className="px-4 py-3 font-medium text-muted-foreground">
              {header}
            </th>
          ))}
        </tr>
  ...
  return (
    ...
    >
      ...
      {moduleError?.correlationId ? (
        <p className="mt-1 font-mono text-xs">Correlation ID: {moduleError.correlationId}</p>
      ) : null}
    </div>
  );
};
      ...
      <Card className="max-w-2xl">
        <CardContent className="space-y-4 pt-6">
          <FormField
            id="resource-id"
            label={label}
            hint="Tenant isolation and RBAC are enforced again when the record is opened."
          >
            <input
              id="resource-id"
          ...
          <div className="flex justify-end">
            <Button disabled={!id.trim()} onClick={() => navigate(destination(id.trim()))}>
              Open
            </Button>
          </div>
        </CardContent>
