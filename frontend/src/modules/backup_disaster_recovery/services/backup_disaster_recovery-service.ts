...
const isMessages = (
  ...
  Array.isArray(value) && value.every((item: object) => typeof item === "string");
...
export const backupDisasterRecoveryService = {
  requestBackup: (payload: BackupExecutionCreateRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<BackupExecutionReceipt>>(
        ENDPOINTS.BACKUP_EXECUTIONS.CREATE,
        payload
      )
    ),
  getBackupStatus: (backupJobId: UUID) =>
    request(() =>
      apiClient.get<ApiV2Envelope<BackupExecutionStatus>>(
        ENDPOINTS.BACKUP_EXECUTIONS.DETAIL(backupJobId)
      )
    ),
  ...
  getRecoveryPoint: (id: UUID) =>
    request(() =>
      apiClient.get<ApiV2Envelope<RecoveryPoint>>(ENDPOINTS.RECOVERY_POINTS.DETAIL(id))
    ),
  verifyRecoveryPoint: (id: UUID, payload: RecoveryPointVerifyRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<DurableJobReceipt>>(
        ENDPOINTS.RECOVERY_POINTS.VERIFY(id),
        payload
      )
    ),
  expireRecoveryPoint: (id: UUID, payload: RecoveryPointExpireRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<RecoveryPoint>>(ENDPOINTS.RECOVERY_POINTS.EXPIRE(id), payload)
    ),
  ...
  createRestoreRun: (payload: RestoreRunCreateRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<RestoreRun>>(ENDPOINTS.RESTORE_RUNS.CREATE, payload)
    ),
  executeRestoreRun: (id: UUID, payload: RestoreRunExecuteRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<DurableJobReceipt>>(ENDPOINTS.RESTORE_RUNS.EXECUTE(id), payload)
    ),
  cancelRestoreRun: (id: UUID, payload: RestoreRunCancelRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<RestoreRun>>(ENDPOINTS.RESTORE_RUNS.CANCEL(id), payload)
    ),
  ...
  updateRunbook: (id: UUID, payload: DRRunbookUpdateRequest) =>
    request(() =>
      apiClient.patch<ApiV2Envelope<DRRunbook>>(ENDPOINTS.RUNBOOKS.UPDATE(id), payload)
    ),
  ...
  publishRunbook: (id: UUID, payload: RunbookTransitionRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<DRRunbook>>(ENDPOINTS.RUNBOOKS.PUBLISH(id), payload)
    ),
  ...
  reorderRunbookSteps: (id: UUID, payload: RunbookStepReorderRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<readonly RunbookStep[]>>(
        ENDPOINTS.RUNBOOKS.REORDER_STEPS(id),
        payload
      )
    ),
  ...
  listRunbookSteps: (runbookId: UUID) => {
    ...
    return requestPage(() =>
      apiClient.get<ApiV2Page<RunbookStep>>(withQuery(ENDPOINTS.RUNBOOK_STEPS.LIST, query))
    );
  },
  ...
  createRunbookStep: (payload: RunbookStepCreateRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<RunbookStep>>(ENDPOINTS.RUNBOOK_STEPS.CREATE, payload)
    ),
  updateRunbookStep: (id: UUID, payload: RunbookStepUpdateRequest) =>
    request(() =>
      apiClient.patch<ApiV2Envelope<RunbookStep>>(ENDPOINTS.RUNBOOK_STEPS.UPDATE(id), payload)
    ),
  ...
  updateExercise: (id: UUID, payload: DRExerciseUpdateRequest) =>
    request(() =>
      apiClient.patch<ApiV2Envelope<DRExercise>>(ENDPOINTS.EXERCISES.UPDATE(id), payload)
    ),
  startExercise: (id: UUID, payload: DRExerciseStartRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<DurableJobReceipt>>(ENDPOINTS.EXERCISES.START(id), payload)
    ),
  cancelExercise: (id: UUID, payload: DRExerciseCancelRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<DRExercise>>(ENDPOINTS.EXERCISES.CANCEL(id), payload)
    ),
  ...
  listStepExecutions: (filters: StepExecutionFilters) => {
    ...
    append(query, "page_size", filters.page_size);
    return requestPage(() =>
      apiClient.get<ApiV2Page<DRStepExecution>>(withQuery(ENDPOINTS.STEP_EXECUTIONS.LIST, query))
    );
  },
  getStepExecution: (id: UUID) =>
    request(() =>
      apiClient.get<ApiV2Envelope<DRStepExecution>>(ENDPOINTS.STEP_EXECUTIONS.DETAIL(id))
    ),

  getReadiness: () =>
    request(() => apiClient.get<ApiV2Envelope<ReadinessSummary>>(ENDPOINTS.READINESS)),
  getObjectiveReport: (filters: ObjectiveReportFilters) => {
    ...
    return request(() =>
      apiClient.get<ApiV2Envelope<ObjectiveReport>>(withQuery(ENDPOINTS.REPORTS.OBJECTIVES, query))
    );
  },

  getConfiguration: () =>
    request(() =>
      apiClient.get<ApiV2Envelope<BackupDisasterRecoveryConfiguration>>(
        ENDPOINTS.CONFIGURATIONS.CURRENT
      )
    ),
  updateConfiguration: (payload: BDRConfigurationMutation) =>
    request(() =>
      apiClient.patch<ApiV2Envelope<BackupDisasterRecoveryConfiguration>>(
        ENDPOINTS.CONFIGURATIONS.CURRENT,
        payload
      )
    ),
  previewConfiguration: (payload: BDRConfigurationMutation) =>
    request(() =>
      apiClient.post<ApiV2Envelope<BDRConfigurationPreview>>(
  ...
  listConfigurationVersions: () =>
    request(() =>
      apiClient.get<ApiV2Envelope<readonly BDRConfigurationVersion[]>>(
        ENDPOINTS.CONFIGURATIONS.VERSIONS
      )
    ),
  rollbackConfiguration: (payload: BDRConfigurationRollbackRequest) =>
    request(() =>
      apiClient.post<ApiV2Envelope<BackupDisasterRecoveryConfiguration>>(
        ENDPOINTS.CONFIGURATIONS.ROLLBACK,
        payload
      )
    ),
  importConfiguration: (payload: BDRConfigurationMutation) =>
    request(() =>
      apiClient.post<ApiV2Envelope<BackupDisasterRecoveryConfiguration>>(
        ENDPOINTS.CONFIGURATIONS.IMPORT,
        payload
      )
    ),
  exportConfiguration: () =>
    request(() =>
      apiClient.get<ApiV2Envelope<BDRConfigurationExport>>(ENDPOINTS.CONFIGURATIONS.EXPORT)
    ),
};
