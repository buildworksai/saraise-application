...
describe("backup and disaster recovery contracts", () => {
  it("constructs every governed v2 resource and action endpoint", () => {
    ...
    expect(ENDPOINTS.RECOVERY_POINTS.VERIFY("point")).toBe(
      `${MODULE_API_PREFIX}/recovery-points/point/verify/`
    );
    expect(ENDPOINTS.RESTORE_RUNS.EXECUTE("restore")).toBe(
      `${MODULE_API_PREFIX}/restore-runs/restore/execute/`
    );
    expect(ENDPOINTS.RUNBOOKS.REORDER_STEPS("book")).toBe(
      `${MODULE_API_PREFIX}/runbooks/book/reorder-steps/`
    );
    expect(ENDPOINTS.EXERCISES.CANCEL("exercise")).toBe(
      `${MODULE_API_PREFIX}/exercises/exercise/cancel/`
    );
    expect(ENDPOINTS.STEP_EXECUTIONS.DETAIL("evidence")).toBe(
      `${MODULE_API_PREFIX}/step-executions/evidence/`
    );
    ...
    expect(ENDPOINTS.CONFIGURATIONS.IMPORT).toBe(
      `${MODULE_API_PREFIX}/configurations/import-document/`
    );
    expect(ENDPOINTS.CONFIGURATIONS.EXPORT).toBe(
      `${MODULE_API_PREFIX}/configurations/export-document/`
    );
  });
});
...
describe("tenant route registry", () => {
  ...
  it("resolves every contextual route to a registered sidebar parent", () => {
    ...
    const sidebarSourceFiles = new Set(
      tenantRoutes
