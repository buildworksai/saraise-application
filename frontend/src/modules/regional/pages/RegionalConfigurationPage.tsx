...
function validateDocument(document: RegionalConfigurationDocument): string[] {
  ...
  if (resource.name_max_length < resource.name_min_length || resource.name_max_length > 512) {
  ...
  if (
    resource.allowed_config_keys.some(
      (value) => !CONFIG_KEYS.includes(value as (typeof CONFIG_KEYS)[number])
    )
  ) {
  ...
  if (
    ...
    !resource.allowed_jurisdiction_types.includes(resource.default_config.jurisdiction_type)
  ) {
    errors.push("Default jurisdiction type must be enabled.");
  }
  if (
    resource.allowed_jurisdiction_types.some(
      (value) => !JURISDICTION_TYPES.includes(value as (typeof JURISDICTION_TYPES)[number])
    )
  ) {
  ...
  if (
    rollout.roles.some((value) => !ROLLOUT_ROLES.includes(value as (typeof ROLLOUT_ROLES)[number]))
  ) {
...
export const RegionalConfigurationPage = () => {
  ...
  const [environment, setEnvironment] = useState<RegionalConfigurationEnvironment>("development");
  ...
  const rollback = useMutation({
    mutationFn: (version: number) => regionalService.rollbackConfiguration(environment, version),
    onSuccess: async () => {
  ...
  const importDocument = async (file: File) => {
    ...
    try {
      ...
      const candidate =
        isRecord(parsed) && isConfigurationDocument(parsed.document) ? parsed.document : parsed;
    ...
    <main id="main-content" className="space-y-6 p-8">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            variant="outline"
            disabled={preview.isPending || validationErrors.length > 0}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Environment and rollout</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-2">
          <label className="text-sm font-medium">
            ...
            <select
              ...
              value={environment}
              onChange={(event) =>
                setEnvironment(event.target.value as RegionalConfigurationEnvironment)
              }
            >
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Resource defaults and safe limits</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <Input
              ...
              type="number"
              min="1"
              max={draft.resource.name_max_length}
              value={draft.resource.name_min_length}
              onChange={(event) => updateResource("name_min_length", Number(event.target.value))}
            />
            <Input
              label="Maximum name length"
              type="number"
              min={draft.resource.name_min_length}
              max="512"
              value={draft.resource.name_max_length}
              onChange={(event) => updateResource("name_max_length", Number(event.target.value))}
            />
            <Input
              label="Default name"
              value={draft.resource.name_default}
              minLength={draft.resource.name_min_length}
              maxLength={draft.resource.name_max_length}
              onChange={(event) => updateResource("name_default", event.target.value)}
            />
            <Input
              ...
              min="0"
              max="10000"
              value={draft.resource.description_max_length}
              onChange={(event) =>
                updateResource("description_max_length", Number(event.target.value))
              }
            />
            <Input
              label="Compliance tag limit"
              type="number"
              min="0"
              max="100"
              value={draft.resource.max_compliance_tags}
              onChange={(event) =>
                updateResource("max_compliance_tags", Number(event.target.value))
              }
            />
            <Input
              label="Configuration byte limit"
              type="number"
              min="128"
              max="65536"
              value={draft.resource.max_config_bytes}
              onChange={(event) => updateResource("max_config_bytes", Number(event.target.value))}
            />
            <label className="flex items-center gap-3 rounded-md border border-border p-3 text-sm">
              <input
                type="checkbox"
                checked={draft.resource.default_active}
                onChange={(event) => updateResource("default_active", event.target.checked)}
              />
              New resources active by default
            </label>
            <div className="sm:col-span-2">
              <label htmlFor="description-default" className="mb-1 block text-sm font-medium">
                Default description
              </label>
              <Textarea
                id="description-default"
                maxLength={draft.resource.description_max_length}
                value={draft.resource.description_default}
                onChange={(event) => updateResource("description_default", event.target.value)}
              />
            </div>
            <Input
              ...
              onChange={(event) => {
                ...
                void omitted;
                updateDefaultConfig(countryCode ? { ...rest, country_code: countryCode } : rest);
              }}
            />
            <label className="text-sm font-medium">
              ...
              <select
                ...
                onChange={(event) => {
                  const { jurisdiction_type: omitted, ...rest } = draft.resource.default_config;
                  void omitted;
                  updateDefaultConfig(
                    event.target.value ? { ...rest, jurisdiction_type: event.target.value } : rest
                  );
                }}
              >
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                const { compliance_tags: omitted, ...rest } = draft.resource.default_config;
                void omitted;
        ...
        <Card>
          <CardHeader>
            <CardTitle>Workflow and API policy</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <label className="flex items-center gap-3 rounded-md border border-border p-3 text-sm">
              <input
                type="checkbox"
                checked={draft.workflow.activation_state}
                onChange={(event) => updateWorkflow("activation_state", event.target.checked)}
              />
            ...
            <label className="flex items-center gap-3 rounded-md border border-border p-3 text-sm">
              <input
                type="checkbox"
                checked={draft.workflow.deactivation_state}
                onChange={(event) => updateWorkflow("deactivation_state", event.target.checked)}
              />
              Deactivation target state
            </label>
            <label className="flex items-center gap-3 rounded-md border border-border p-3 text-sm sm:col-span-2">
              <input
                type="checkbox"
                checked={draft.workflow.require_delete_confirmation}
                onChange={(event) =>
                  updateWorkflow("require_delete_confirmation", event.target.checked)
                }
              />
              Require accessible archive confirmation
            </label>
            <Input
              label="Default page size"
              type="number"
              min="1"
              max={draft.api.max_page_size}
              value={draft.api.default_page_size}
              onChange={(event) => updateApi("default_page_size", Number(event.target.value))}
            />
            <Input
              label="Maximum page size"
              type="number"
              min={draft.api.default_page_size}
              max="500"
              value={draft.api.max_page_size}
              onChange={(event) => updateApi("max_page_size", Number(event.target.value))}
            />
            <Input
              label="Cache probe TTL (seconds)"
              type="number"
              min="1"
              max="300"
              value={draft.health.cache_probe_ttl_seconds}
              onChange={(event) =>
                setDraft({
                  ...draft,
                  health: { cache_probe_ttl_seconds: Number(event.target.value) },
                })
              }
            />
          </CardContent>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <MultiChoice
          label="Resource configuration keys"
          guidance="Only selected typed keys can be persisted."
          options={CONFIG_KEYS}
          selected={draft.resource.allowed_config_keys}
          onChange={(values) => {
            const defaultConfig = Object.fromEntries(
              Object.entries(draft.resource.default_config).filter(([key]) => values.includes(key))
            );
            setDraft({
              ...draft,
              resource: {
        ...
        />
        <MultiChoice
          label="Jurisdiction types"
          guidance="Tenant subset of the platform jurisdiction allow-list."
          options={JURISDICTION_TYPES}
          selected={draft.resource.allowed_jurisdiction_types}
          onChange={(values) => {
        ...
        <MultiChoice
          label="Search fields"
          guidance="Server-side fields included in resource search."
          options={SEARCH_FIELDS}
          selected={draft.resource.search_fields}
          onChange={(values) =>
            updateResource(
              "search_fields",
              values as RegionalConfigurationDocument["resource"]["search_fields"]
            )
          }
        />
        <MultiChoice
          label="Filter fields"
          guidance="Server-side query filter allow-list."
          options={FILTER_FIELDS}
          selected={draft.api.allowed_filters}
          onChange={(values) =>
            updateApi(
              "allowed_filters",
              values as RegionalConfigurationDocument["api"]["allowed_filters"]
            )
          }
        />
        <div className="lg:col-span-2">
          <MultiChoice
            ...
            guidance="Deterministic server-side ordering allow-list."
            options={ORDER_FIELDS}
            selected={draft.api.allowed_ordering}
            onChange={(values) =>
              updateApi(
                "allowed_ordering",
                values as RegionalConfigurationDocument["api"]["allowed_ordering"]
              )
            }
          />
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Advanced JSON editor for review and configuration-as-code workflows. All fields remain
            server validated.
          </p>
          <Textarea
            aria-label="Complete configuration document"
            rows={20}
            value={documentText}
            onChange={(event) => setDocumentText(event.target.value)}
          />
          <Button variant="outline" onClick={applyDocumentText}>
            Validate and stage JSON
          </Button>
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Version history and immutable audit
          </CardTitle>
        </CardHeader>
        <CardContent>
          {history.isError ? (
            <ErrorState
              message="Configuration history could not be loaded."
            ...
            <ol className="divide-y divide-border">
              {history.data.map((version) => (
                <li
                  key={version.id}
                  className="flex flex-col justify-between gap-3 py-3 md:flex-row md:items-center"
                >
                  <div>
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    disabled={version.version === current.data?.version || rollback.isPending}
            ...
            <p className="text-sm text-muted-foreground">
              No prior versions exist for this environment.
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (rollbackVersion !== null) rollback.mutate(rollbackVersion);
        }}
      />
    </main>
  );
