...
export function LogExplorerPage() {
  ...
  const logs = useQuery({
    queryKey: ["performance-monitoring", "logs", deferredSearch, level],
    queryFn: () =>
      performanceMonitoringService.listLogs({
        page_size: 100,
        ordering: "-timestamp",
  ...
  return (
    <MonitoringPage
      title="Log explorer"
      description="Search structured tenant logs and pivot into correlated traces without exposing credentials or cross-tenant evidence."
    >
      <Card>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <label className="relative flex-1">
              ...
              <Input
                ...
                placeholder="Search messages, correlation IDs or attributes"
              />
            </label>
            <label>
      ...
      {logs.isPending ? (
        ...
        <EmptyTelemetry
          ...
          description={
            search || level
              ? "No persisted logs match these filters. Clear a filter or widen the time range."
              : "No logs have been ingested for this tenant."
          }
          action={
            search || level ? (
              <Button
  ...
  return (
    <>
      <tr className="border-b align-top hover:bg-muted/30">
        ...
        <td className={`px-4 py-3 font-mono text-xs uppercase ${levelTone}`}>{entry.level}</td>
