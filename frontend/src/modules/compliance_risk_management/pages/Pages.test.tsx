...
const result = (items: RiskAssessment[]): Paginated<RiskAssessment> => ({
  items,
...
function renderPage() {
  ...
  return render(
    <QueryClientProvider client={client}>
      <MemoryRouter initialEntries={["/compliance-risk-management/risks"]}>
...
describe("compliance risk pages", () => {
  ...
  it("renders geometry-matching loading state then a permission-aware empty state", async () => {
    vi.spyOn(service, "listRisks").mockResolvedValue(result([]));
    renderPage();
    expect(screen.getByLabelText("Loading compliance risk information")).toHaveAttribute(
      "aria-busy",
      "true"
    );
  ...
  it("renders governed successful data without client-derived scoring", async () => {
    vi.spyOn(service, "listRisks").mockResolvedValue(result([risk]));
    renderPage();
    expect(await screen.findByRole("button", { name: "RISK-001" })).toBeVisible();
    expect(screen.getByText("20.00")).toBeVisible();
    expect(screen.getAllByText("Critical").some((element) => element.tagName === "SPAN")).toBe(
      true
    );
  });

  it("shows retryable governed errors with correlation ID", async () => {
    vi.spyOn(service, "listRisks").mockRejectedValue(
      new ComplianceRiskApiError(
