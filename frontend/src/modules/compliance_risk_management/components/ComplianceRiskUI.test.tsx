...
describe("compliance risk shared UX", () => {
  it("renders dedicated 403 and 404 states without leaking resource existence", () => {
    const { rerender } = render(
      <GovernedProblem
        error={new ComplianceRiskApiError("denied", 403, "POLICY_DENIED", "corr-403")}
      />
    );
    ...
    rerender(
      <GovernedProblem
        error={new ComplianceRiskApiError("missing", 404, "NOT_FOUND", "corr-404")}
      />
    );
  ...
  it("offers retry only for general governed failures", async () => {
    ...
    render(
      <GovernedProblem
        error={new ComplianceRiskApiError("unavailable", 503, "UNAVAILABLE", "corr-503")}
        retry={retry}
      />
    );
  ...
  it("supports permission-aware empty actions and non-color-only status", async () => {
    ...
    render(
      <>
        <EmptyState
          title="No risks"
          message="Create one"
          action={{ label: "Create risk", onClick: action }}
        />
    ...
    expect(screen.getByText("Critical")).toHaveTextContent("▲");
  });
