...
export function validateZones(zones: readonly ExtractionTemplateZoneInput[]): readonly ZoneIssue[] {
  ...
  zones.forEach((zone, index) => {
    ...
    if (x < 0 || y < 0 || width <= 0 || height <= 0 || x + width > 1 || y + height > 1)
      issues.push({ index, message: "Zone must remain inside the normalized page." });
    zones.slice(index + 1).forEach((candidate, offset) => {
      if (overlap(zone, candidate)) {
        issues.push({ index, message: `Overlaps ${candidate.zone_name}.` });
        issues.push({ index: index + offset + 1, message: `Overlaps ${zone.zone_name}.` });
      }
    });
  });
  return issues;
