...
export function TemplateForm({
  ...
  configuration: DocumentIntelligenceConfigurationDocument;
  pending: boolean;
  serverError?: string;
  submitLabel: string;
  ...
  const defaults = initial ?? {
    ...
    description: "",
    document_category: "",
    engine: configuration.templates.default_engine,
    match_threshold: configuration.templates.default_match_threshold.toFixed(
  ...
  const submit = async (event: FormEvent) => {
    ...
    await onSubmit({
      ...
      engine,
      match_threshold: Number(threshold).toFixed(4),
      zones,
    });
    setSubmitted(true);
  };
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        />
        <div>
          <label htmlFor="template-engine" className="mb-1 block text-sm font-medium">
            OCR engine
          </label>
          <select
            id="template-engine"
          ...
          >
            {configuration.providers.allowed_ocr_engines.map((value) => (
        ...
        <Input
          ...
          min="0"
          max="1"
          step={configuration.editor.coordinate_snap}
          required
          value={threshold}
      ...
      <ZoneEditor zones={zones} configuration={configuration} onChange={setZones} />
      {serverError && (
      ...
      <div className="flex justify-end">
        <Button
          type="submit"
          disabled={pending || !name.trim() || issues.length > 0}
          aria-busy={pending}
        >
