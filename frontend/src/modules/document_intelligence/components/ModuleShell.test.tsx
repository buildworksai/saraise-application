...
describe("document intelligence shared states", () => {
  it("renders a layout-matched accessible skeleton", () => {
    render(<PageSkeleton />);
    expect(screen.getByLabelText("Loading document intelligence")).toHaveAttribute(
      "aria-busy",
      "true"
    );
  });
  ...
  it("renders a permission-aware empty action when supplied", () => {
    render(
      <EmptyPanel
        ...
        action={<button>Process document</button>}
      />
    );
  ...
  it("does not leak object existence for access errors and preserves correlation", () => {
    ...
    render(
      <ApiProblem
        error={
          new DocumentIntelligenceApiError("Hidden server detail", 404, "not_found", "corr-404", {})
        }
        onRetry={retry}
      />
    );
  ...
  it("shows quota remaining and reset evidence", () => {
    render(
      <ApiProblem
        error={
          new DocumentIntelligenceApiError("Quota", 429, "quota_exhausted", "corr-429", {
            quota: { resource: "pages", remaining: 0, reset_at: "2026-07-22T00:00:00Z" },
          })
        }
        onRetry={() => undefined}
      />
