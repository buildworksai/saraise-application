...
function TemplateCards({
  ...
  templates: readonly ExtractionTemplateListItem[];
  open: (id: string) => void;
}) {
  return (
      ...
      {templates.map((template) => (
        <Card key={template.id} className="flex flex-col p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Revision {template.version} · {template.engine}
              </p>
            </div>
          ...
          <div className="mt-5 flex items-center justify-between text-sm">
            <span>Match ≥ {formatConfidence(template.match_threshold)}</span>
            <Button variant="outline" size="sm" onClick={() => open(template.id)}>
              Open template
            </Button>
          </div>
        </Card>
...
export function TemplateListPage() {
  ...
  const query = useQuery({
    queryKey: ["document-intelligence", "templates", page, status, category],
    queryFn: () =>
      documentIntelligenceService.listTemplates({
        page,
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        ...
        description="Provider-neutral, versioned layouts with normalized zones and immediate geometric validation."
        actions={
          canManage ? (
            <Button onClick={() => navigate(DOCUMENT_INTELLIGENCE_PATHS.TEMPLATES.CREATE)}>
      ...
      <Card className="p-4">
        <div className="grid gap-3 sm:grid-cols-2">
          <Select value={status ?? "all"} onValueChange={(value) => update("status", value)}>
            <SelectTrigger aria-label="Template status">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All statuses</SelectItem>
              {["draft", "active", "inactive", "retired"].map((value) => (
      ...
      {query.data.items.length === 0 ? (
        <EmptyPanel
          ...
          description="Create a draft layout and define normalized fields without leaking provider-specific schemas."
          action={
            canManage ? (
              <Button onClick={() => navigate(DOCUMENT_INTELLIGENCE_PATHS.TEMPLATES.CREATE)}>
        ...
        <>
          <TemplateCards
            ...
            open={(id) => navigate(DOCUMENT_INTELLIGENCE_PATHS.TEMPLATES.DETAIL(id))}
          />
          <Card>
            <Pagination
