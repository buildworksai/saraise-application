...
import type {
  DocumentExtractionCreateRequest,
  ExtractionEngine,
  ExtractionType,
} from "../contracts";
...
export function CreateExtractionPage() {
  ...
  const mutation = useMutation({
    mutationFn: (request: DocumentExtractionCreateRequest) =>
      documentIntelligenceService.createExtraction(request),
    onSuccess: ({ extraction }) => {
      setSubmitted(true);
      navigate(DOCUMENT_INTELLIGENCE_PATHS.EXTRACTIONS.DETAIL(extraction.id));
    },
  });
  const fieldErrors =
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        ...
        description="Validate an immutable DMS version, reserve quota, and enqueue a durable evidence-producing extraction."
        actions={
          <Button
            variant="ghost"
        ...
        <form className="space-y-5" onSubmit={submit} noValidate>
          <div className="grid gap-4 sm:grid-cols-2">
            <Input
              ...
              required
              value={documentId}
              error={errorFor("document_id")}
              aria-describedby={errorFor("document_id") ? "document-id-error" : undefined}
            ...
            <Input
              ...
              required
              value={versionId}
              error={errorFor("document_version_id")}
          ...
          {requiresTemplate && (
            <Input
              id="template-id"
              label="Extraction template UUID"
              required
              value={templateId}
              error={errorFor("template_id")}
              onChange={(event) => setTemplateId(event.target.value)}
            />
          )}
          <div className="rounded-md border bg-muted/40 p-4 text-sm text-muted-foreground">
            The API verifies tenant ownership, MIME type, 50 MiB limit, provider readiness,
            concurrency, and quota before returning 202. No document bytes pass through this form.
          </div>
          <div className="flex justify-end">
            <Button
              ...
              disabled={
                ...
                !documentId.trim() ||
                !versionId.trim() ||
                (requiresTemplate && !templateId.trim())
              }
              aria-busy={mutation.isPending}
            >
              {mutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {mutation.isPending ? "Validating and queuing…" : "Queue extraction"}
            </Button>
          </div>
        </form>
      </Card>
