...
export function ExtractionDetailPage() {
  ...
  const extraction = useQuery({ queryKey: ['document-intelligence', 'extraction', id], queryFn: () => documentIntelligenceService.getExtraction(id), enabled: Boolean(id), refetchInterval: (state) => state.state.data && ACTIVE.has(state.state.data.status) ? 5_000 : false });
  const pages = useQuery({ queryKey: ['document-intelligence', 'extraction-pages', id], queryFn: () => documentIntelligenceService.listExtractionPages(id), enabled: Boolean(id), refetchInterval: extraction.data && ACTIVE.has(extraction.data.status) ? 5_000 : false });
  ...
  if (extraction.isLoading || pages.isLoading) return <PageSkeleton cards={2} />;
  if (extraction.error || !extraction.data) return <div className="p-4 sm:p-8"><ApiProblem error={extraction.error} onRetry={() => { void extraction.refetch(); }} /></div>;
