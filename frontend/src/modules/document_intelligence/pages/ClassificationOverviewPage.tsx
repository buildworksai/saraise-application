...
export function ClassificationOverviewPage() {
  ...
  const query = useQuery({ queryKey: ['document-intelligence', 'classifications', page, status, category, review], queryFn: () => documentIntelligenceService.listClassifications({ page, page_size: 25, status, category, needs_review: review || undefined, ordering: '-created_at' }), refetchInterval: (state) => state.state.data?.items.some((item) => ['queued', 'processing'].includes(item.status)) ? 5_000 : false });
  const create = useMutation({ mutationFn: () => documentIntelligenceService.createClassification({ document_id: documentId.trim(), document_version_id: versionId.trim(), idempotency_key: deterministicKey('classify', documentId, versionId) }), onSuccess: ({ classification }) => navigate(`/document-intelligence/classifications/${classification.id}`) });
