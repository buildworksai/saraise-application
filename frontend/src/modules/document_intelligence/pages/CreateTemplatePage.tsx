...
export function CreateTemplatePage() {
  ...
  const mutation = useMutation({
    mutationFn: (value: TemplateFormValue) => documentIntelligenceService.createTemplate(value),
    onSuccess: (template) => navigate(DOCUMENT_INTELLIGENCE_PATHS.TEMPLATES.DETAIL(template.id)),
  });
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        ...
        description="Define provider-neutral fields on normalized page coordinates, then validate before activation."
        actions={
          <Button
            variant="ghost"
      ...
      <Card className="p-6">
        <TemplateForm
          ...
          onSubmit={(value) => mutation.mutateAsync(value).then(() => undefined)}
        />
      </Card>
    </main>
  );
