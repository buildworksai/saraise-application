...
function ExtractionRows({
  ...
  items: readonly DocumentExtractionListItem[];
  open: (id: string) => void;
}) {
  return (
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.id} className="border-b hover:bg-muted/40">
              <td className="p-4">
                <button
                  className="font-mono text-xs font-medium text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                  onClick={() => open(item.id)}
                >
                  {item.document_id}
                </button>
                <p className="mt-1 text-xs text-muted-foreground">
                  {item.extraction_type} · v{item.document_version_id.slice(0, 8)}
                </p>
              </td>
              <td className="p-4">
                <StatusPill status={item.status} />
              </td>
              <td className="p-4">{item.engine}</td>
              <td className="p-4 font-semibold">{formatConfidence(item.confidence)}</td>
              <td className="p-4">
                {item.processing_time_ms === null ? "—" : `${item.processing_time_ms} ms`}
              </td>
              <td className="p-4 text-right">
                <Button variant="ghost" size="sm" onClick={() => open(item.id)}>
                  View evidence
                </Button>
              </td>
            </tr>
...
export function ExtractionDashboardPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      documentIntelligenceService.listExtractions({
        page,
        page_size: configuration.data?.document.ui.page_size,
        status,
        engine,
        confidence_min: confidence === "all" ? undefined : confidence,
        ordering: "-created_at",
      }),
  ...
  const avgLatency = measuredLatency.length
    ? `${Math.round(measuredLatency.reduce((sum, item) => sum + (item.processing_time_ms ?? 0), 0) / measuredLatency.length)} ms`
    : "—";
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        ...
        description="Trace OCR and structured extraction outcomes back to immutable DMS versions, providers, pages, and timing evidence."
        actions={
          canManage ? (
            <Button onClick={() => navigate(DOCUMENT_INTELLIGENCE_PATHS.EXTRACTIONS.CREATE)}>
      ...
      <StaleIndicator
        ...
        active={items.some((item) => ACTIVE.has(item.status))}
      />
      <section className="grid gap-4 sm:grid-cols-3" aria-label="Current page outcomes">
        <MetricCard label="Completed" value={completed} detail="Validated outcomes on this page" />
        <MetricCard
          label="Needs review"
          value={items.filter((item) => item.status === "needs_review").length}
          detail="Evidence kept visible"
        />
        <MetricCard
          label="Average latency"
          value={avgLatency}
          detail="Completed records on this page"
        />
      </section>
      <Card className="p-4">
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <Select value={engine ?? "all"} onValueChange={(value) => update("engine", value)}>
            <SelectTrigger aria-label="Filter by engine">
              <SelectValue placeholder="All engines" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All engines</SelectItem>
              {configuration.data.document.providers.allowed_ocr_engines.map((value) => (
                <SelectItem key={value} value={value}>
                  {value}
                </SelectItem>
              ))}
          ...
          <Select value={confidence} onValueChange={(value) => update("confidence", value)}>
            <SelectTrigger aria-label="Minimum confidence">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">Any confidence</SelectItem>
              {configuration.data.document.ui.confidence_filter_presets.map((value) => (
      ...
      {items.length === 0 ? (
        <EmptyPanel
          ...
          description="No extraction records match the selected server filters."
          action={
            canManage ? (
              <Button onClick={() => navigate(DOCUMENT_INTELLIGENCE_PATHS.EXTRACTIONS.CREATE)}>
        ...
        <Card className="overflow-hidden">
          <ExtractionRows
            items={items}
            open={(id) => navigate(DOCUMENT_INTELLIGENCE_PATHS.EXTRACTIONS.DETAIL(id))}
          />
          <Pagination
