...
export function ClassificationDetailPage() {
  ...
  const detail = useQuery({ queryKey: ['document-intelligence', 'classification', id], queryFn: () => documentIntelligenceService.getClassification(id), enabled: Boolean(id) && Boolean(configuration.data), refetchInterval: (state) => state.state.data && ['queued', 'processing'].includes(state.state.data.status) ? configuration.data?.document.ui.poll_interval_ms : false });
  ...
  if (detail.isLoading || scores.isLoading || configuration.isLoading) return <PageSkeleton cards={2} />;
  if (detail.error || configuration.error || !detail.data || !configuration.data) return <div className="p-4 sm:p-8"><ApiProblem error={detail.error ?? configuration.error} onRetry={() => { void detail.refetch(); void configuration.refetch(); }} /></div>;
