...
export function GovernedErrorState({ error, onRetry }: { error: Error; onRetry: () => void }) {
  ...
  if (apiError?.status === 403)
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center text-center">
        ...
        <h2 className="mt-4 text-xl font-semibold">Access required</h2>
        <p className="mt-2 max-w-md text-muted-foreground">
          Your role does not grant access to this metadata capability. Ask a tenant administrator
  ...
  if (apiError?.status === 404)
    return (
      <ErrorState
        title="Not found"
        message="This item does not exist in your tenant or is no longer available."
      />
    );
  if (apiError?.status === 401)
    return <ErrorState title="Session expired" message="Sign in again to continue." />;
...
export function fieldErrorsFrom(error: Error): Readonly<Record<string, string>> {
  ...
  const source =
    isRecord(error.details.error) && isRecord(error.details.error.fields)
      ? error.details.error.fields
      : isRecord(error.details.fields)
  ...
  for (const [field, value] of Object.entries(source)) {
    if (typeof value === "string") result[field] = value;
      ...
      else if (isRecord(first) && typeof first.message === "string") result[field] = first.message;
    }
  }
  return result;
}
