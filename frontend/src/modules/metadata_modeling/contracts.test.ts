...
describe("metadata modeling API contract", () => {
  it("keeps every endpoint on governed API v2 and encodes identifiers", () => {
    const paths = [
      ...
      ENDPOINTS.entityDefinition("id/unsafe"),
      ENDPOINTS.schemaVersions("entity"),
      ENDPOINTS.resources,
      ENDPOINTS.resource("record"),
      ENDPOINTS.namingSequences,
      ENDPOINTS.health,
      ENDPOINTS.configuration,
      ENDPOINTS.previewConfiguration,
      ENDPOINTS.configurationVersions,
      ENDPOINTS.rollbackConfiguration(3),
      ENDPOINTS.importConfiguration,
      ENDPOINTS.exportConfiguration,
    ];
  ...
  it("centralizes every frontend path", () => {
    ...
    expect(ROUTES.recordCreateFor("entity id")).toBe(
      "/metadata-modeling/records/new?entity=entity%20id"
    );
