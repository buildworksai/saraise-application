...
function field(field_type: FieldType, overrides: Partial<FieldDefinition> = {}): FieldDefinition {
  return {
    ...
    default_value: null,
    validation_rules: {},
    options: field_type === "select" ? ["open", "closed"] : [],
    reference_entity_code: field_type === "reference" ? "target" : null,
    help_text: "",
    placeholder: "",
    order: 1,
...
describe("dynamic form client validation", () => {
  it.each<[FieldType, JSONValue]>([
    ...
    ["json", { nested: true }],
  ])("accepts a valid %s value", (type, value) => {
  ...
  it("preserves false and zero while rejecting missing required values", () => {
    expect(
      validateDynamicValues([field("boolean", { is_required: true })], { boolean: false })
    ).toEqual({});
    expect(validateDynamicValues([field("number", { is_required: true })], { number: 0 })).toEqual(
      {}
    );
    expect(validateDynamicValues([field("text", { is_required: true })], {})).toEqual({
      text: "This field is required.",
    });
  });

  it("rejects malformed built-in values before server validation", () => {
    expect(validateDynamicValues([field("select")], { select: "missing" })).toHaveProperty(
      "select"
    );
