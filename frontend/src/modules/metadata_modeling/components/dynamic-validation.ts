  ...
  switch (field.field_type) {
    case "select":
      return typeof value === "string" && value !== "" && !field.options.includes(value)
        ...
        : null;
    case "number":
      return !isBlank(value) && (typeof value !== "number" || !Number.isFinite(value))
    ...
    case "date":
      return typeof value === "string" && value !== "" && !/^\d{4}-\d{2}-\d{2}$/u.test(value)
        ...
        : null;
    case "json":
      return typeof value === "string" ? "Enter valid JSON." : null;
    default:
      return null;
  }
}
function validateField(field: FieldDefinition, value: JSONValue | undefined): string | null {
  return field.is_required && isBlank(value)
