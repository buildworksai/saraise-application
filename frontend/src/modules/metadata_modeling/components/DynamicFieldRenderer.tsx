...
function renderControl(
  { field, value, error, disabled = false, onChange }: Props,
  id: string,
  describedBy: string
): ReactNode {
  const common = {
    ...
    case "reference":
      return (
        <Input
          {...common}
          value={textValue(value)}
          placeholder={field.placeholder || "Record UUID"}
          onChange={(event) => onChange(event.target.value)}
        />
      );
    case "text":
      return (
        <Input
          ...
          pattern={field.validation_rules.regex}
  ...
  return (
    <div className="space-y-1">
      ...
      <p id={`${id}-help`} className="text-xs text-muted-foreground">
        {field.help_text || `${field.field_type} field`}
      </p>
      {error && (
