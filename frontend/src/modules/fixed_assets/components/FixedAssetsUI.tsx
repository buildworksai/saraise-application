...
export function PageSkeleton({ table = false }: { table?: boolean }) {
  return (
    <main
      ...
      aria-label="Loading fixed asset information"
      aria-busy="true"
    >
...
function asModuleError(error: unknown): {
  status?: number;
  message: string;
  correlation?: string | null;
} {
  if (error instanceof FixedAssetsApiError)
    return { status: error.status, message: error.message, correlation: error.correlationId };
...
export function ProblemState({ error, onRetry }: { error: unknown; onRetry?: () => void }) {
  ...
  const message = forbidden
    ? "Your current policy does not allow this action. No resource existence has been disclosed."
    : missing
    ...
    >
      ...
      {problem.correlation && (
        <p className="mt-3 font-mono text-xs text-muted-foreground">
          Correlation ID: {problem.correlation}
        </p>
      )}
...
export function commandAffordance(
  commands: readonly ServerAllowedCommand[] | undefined,
  ...
  return {
    ...
    explanation: allowed
      ? undefined
      : denialCode
        ? `The server denied this command: ${denialCode}.`
        : "The server did not grant this command.",
  };
}
...
export function CommandButton({
  ...
  return (
    <span
      title={!affordance.allowed ? affordance.explanation ?? affordance.denial_code : undefined}
    >
      <Button
        variant={variant}
        disabled={!affordance.allowed}
        aria-disabled={!affordance.allowed}
        aria-describedby={!affordance.allowed ? `command-${affordance.command}-reason` : undefined}
        onClick={onClick}
      >
        {children}
      </Button>
      {!affordance.allowed && (
        <span id={`command-${affordance.command}-reason`} className="sr-only">
          {affordance.explanation ?? affordance.denial_code}
        </span>
      )}
    </span>
  );
...
export function StatusPill({ value }: { value: string }) {
  ...
  const success =
    value === "active" || value === "posted" || value === "completed" || value === "succeeded";
  return (
    <span
      className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-medium ${danger ? "border-destructive/30 bg-destructive/10 text-destructive" : success ? "border-green-500/30 bg-green-500/10 text-green-700 dark:text-green-300" : "border-border bg-muted text-muted-foreground"}`}
    >
      {titleCase(value)}
    </span>
  );
...
export function UnsavedChangesDialog({
  ...
  return (
    <Dialog
      ...
      description="Your edits have not been sent to the server and will be lost."
    >
