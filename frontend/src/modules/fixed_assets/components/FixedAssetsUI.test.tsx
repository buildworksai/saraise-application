...
describe("fixed asset shared UX states", () => {
  it("renders a distinct forbidden state without implying existence", () => {
    render(
      <ProblemState
        error={new ApiError("Denied", 403, undefined, "POLICY_DENIED", "corr-denied")}
      />
    );
  ...
  it("renders a distinct not-found state and a retryable general error", async () => {
    const retry = vi.fn();
    const { rerender } = render(<ProblemState error={new ApiError("Missing", 404)} />);
    expect(screen.getByRole("alert")).toHaveTextContent("Record not found");
    rerender(
      <ProblemState
        error={new ApiError("Unavailable", 503, undefined, "CAPABILITY_UNAVAILABLE", "corr-503")}
        onRetry={retry}
      />
    );
  ...
  it("offers the permitted next action in empty states", async () => {
    ...
    render(
      <EmptyPanel
        ...
        action={{ label: "Register asset", onClick: action }}
      />
    );
    ...
    render(
      <CommandButton
        affordance={{
          ...
          explanation: "Only active assets can be disposed.",
        }}
        onClick={vi.fn()}
      >
        Dispose
