...
const parseFilters = (params: URLSearchParams): AssetFilters => ({
  page: Number(params.get("page") ?? 1),
  page_size: 25,
  search: params.get("search") ?? undefined,
  status: (params.get("status") as AssetStatus | null) ?? undefined,
  method: (params.get("method") as DepreciationMethod | null) ?? undefined,
  currency: params.get("currency") ?? undefined,
  ordering: params.get("ordering") ?? "asset_code",
});
...
export const FixedAssetListPage = () => {
  ...
  const query = useQuery({
    queryKey: fixedAssetQueryKeys.assets(tenantId, filters),
    queryFn: () => fixedAssetsService.listAssets(filters),
  });
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        ...
        description="Search, filter, and reconcile tenant-owned financial assets."
        actions={
          <Button onClick={() => navigate("/fixed-assets/assets/new")}>
      ...
      <Card className="grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        >
          ...
          {["draft", "active", "fully_depreciated", "disposed"].map((value) => (
        ...
        >
          ...
          {["straight_line", "declining_balance", "units_of_production"].map((value) => (
            <option key={value} value={value}>
              {titleCase(value)}
            </option>
          ))}
        ...
        >
          ...
          <option value="-net_book_value">Highest book value</option>
      ...
      {!result || result.items.length === 0 ? (
        <EmptyPanel
          ...
          description="Clear filters or register a new asset if your policy permits."
          action={{ label: "Register asset", onClick: () => navigate("/fixed-assets/assets/new") }}
        />
      ) : (
                ...
                {result.items.map((asset) => (
                  <tr key={asset.id} className="border-t hover:bg-muted/50">
                    ...
                    <td className="px-4 py-3 font-medium">
                      {formatMoney(asset.net_book_value, asset.currency)}
                    </td>
                    <td className="px-4 py-3">{titleCase(asset.depreciation_method)}</td>
                    <td className="px-4 py-3">{formatDateOnly(asset.next_depreciation_date)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
