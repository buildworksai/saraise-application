...
export interface CommandDenialReasons {
...
export interface BalanceReconciliation {
  ...
  readonly accumulated_depreciation: string;
  readonly accumulated_impairment: string;
  readonly calculated_net_book_value: string;
...
export interface AssetCategoryCreateRequest {
  ...
  default_declining_balance_rate?: string | null;
...
export type AssetCategoryUpdateRequest = Partial<AssetCategoryCreateRequest> & {
...
export interface FixedAssetCreateRequest {
  ...
  expected_total_units?: string | null;
  location?: string;
  cost_center?: string;
}
export type FixedAssetUpdateRequest = Partial<FixedAssetCreateRequest> & {
  expected_version: number;
};
...
export interface ImpairmentRequest {
  effective_date: string;
...
export interface ScheduleCalculateRequest {
  units_by_period?: readonly { period_start: string; units_consumed: string }[];
}
export interface ScheduleTransitionRequest {
  reason?: string;
}
export interface LinePostingRequest {
  expected_asset_version?: number;
}
export interface DuePostingRequest {
  through_date: string;
}
...
export interface LifecyclePreview {
  ...
  readonly journal_effect: {
    readonly status: "ready" | "not_required" | "unavailable";
    readonly entries: readonly FinancialEffectEntry[];
  };
  readonly schedule_effect: {
    readonly status: "unchanged" | "created" | "superseded" | "completed" | "voided";
    readonly description: string;
  };
}
...
export interface StrategyParameterDescriptor {
  ...
  readonly input_type: "decimal" | "integer" | "date" | "select" | "text";
  readonly required: boolean;
  readonly help_text: string;
  readonly options?: readonly { readonly value: string; readonly label: string }[];
}
export interface DepreciationStrategyDescriptor {
  readonly id: string;
  ...
  readonly entitlement_code: string | null;
}
export interface StrategyParameterValue {
  readonly key: string;
  readonly value: string;
}
export interface AssetDetailContributionDescriptor {
  readonly id: string;
  readonly schema_version: string;
  readonly label: string;
...
export interface FixedAssetReportDescriptor {
  readonly id: string;
  readonly schema_version: string;
  readonly label: string;
  readonly route: string;
...
export interface FixedAssetActionDescriptor {
  readonly id: string;
  readonly schema_version: string;
  readonly label: string;
  readonly preview_route: string;
  readonly command_route: string;
  readonly entitlement_code: string | null;
}
export interface FixedAssetFrontendExtension {
  readonly namespace: string;
  readonly schema_version: string;
  readonly strategies: readonly DepreciationStrategyDescriptor[];
  readonly asset_detail_contributions: readonly AssetDetailContributionDescriptor[];
  readonly reports: readonly FixedAssetReportDescriptor[];
  readonly actions: readonly FixedAssetActionDescriptor[];
}
...
export interface FixedAssetDashboard {
  readonly asset_counts: {
    ...
    readonly fully_depreciated: number;
  ...
  readonly pending_postings: number;
  readonly failed_postings: number;
  readonly impairments: number;
...
export interface JobResult {
  readonly posted_line_ids?: readonly string[];
  readonly failed_line_ids?: readonly string[];
...
export interface GovernedErrorBody {
  readonly error: {
    ...
    readonly message: string;
    readonly correlation_id?: string;
    readonly field_errors?: readonly ValidationError[];
  };
}
...
export interface AssetFilters extends CommonFilters {
  status?: AssetStatus;
  category_id?: string;
  method?: DepreciationMethod;
  currency?: string;
  location?: string;
  cost_center?: string;
  capitalized_from?: string;
  capitalized_to?: string;
}
export interface CategoryFilters extends CommonFilters {
  is_active?: boolean;
  method?: DepreciationMethod;
}
...
export interface LineFilters extends CommonFilters {
  asset_id?: string;
  schedule_id?: string;
  status?: DepreciationLineStatus;
  period_from?: string;
  period_to?: string;
}
...
export const ENDPOINTS = {
  CATEGORIES: {
    LIST: `${MODULE_API_PREFIX}/categories/`,
    CREATE: `${MODULE_API_PREFIX}/categories/`,
    DETAIL: (id: string) => `${MODULE_API_PREFIX}/categories/${id}/` as const,
    UPDATE: (id: string) => `${MODULE_API_PREFIX}/categories/${id}/` as const,
    DELETE: (id: string) => `${MODULE_API_PREFIX}/categories/${id}/` as const,
  },
  ASSETS: {
    ...
    CAPITALIZE: (id: string) => `${MODULE_API_PREFIX}/assets/${id}/capitalize/` as const,
    TRANSFER: (id: string) => `${MODULE_API_PREFIX}/assets/${id}/transfer/` as const,
    IMPAIR: (id: string) => `${MODULE_API_PREFIX}/assets/${id}/impair/` as const,
    DISPOSE: (id: string) => `${MODULE_API_PREFIX}/assets/${id}/dispose/` as const,
    PREVIEW_CAPITALIZE: (id: string) =>
      `${MODULE_API_PREFIX}/assets/${id}/preview-capitalize/` as const,
    PREVIEW_TRANSFER: (id: string) =>
      `${MODULE_API_PREFIX}/assets/${id}/preview-transfer/` as const,
    PREVIEW_IMPAIR: (id: string) => `${MODULE_API_PREFIX}/assets/${id}/preview-impair/` as const,
    PREVIEW_DISPOSE: (id: string) => `${MODULE_API_PREFIX}/assets/${id}/preview-dispose/` as const,
    TRANSACTIONS: (id: string) => `${MODULE_API_PREFIX}/assets/${id}/transactions/` as const,
  },
  SCHEDULES: {
    LIST: `${MODULE_API_PREFIX}/depreciation-schedules/`,
    CREATE: `${MODULE_API_PREFIX}/depreciation-schedules/`,
    DETAIL: (id: string) => `${MODULE_API_PREFIX}/depreciation-schedules/${id}/` as const,
    UPDATE: (id: string) => `${MODULE_API_PREFIX}/depreciation-schedules/${id}/` as const,
    DELETE: (id: string) => `${MODULE_API_PREFIX}/depreciation-schedules/${id}/` as const,
    CALCULATE: (id: string) =>
      `${MODULE_API_PREFIX}/depreciation-schedules/${id}/calculate/` as const,
    ACTIVATE: (id: string) =>
      `${MODULE_API_PREFIX}/depreciation-schedules/${id}/activate/` as const,
    SUPERSEDE: (id: string) =>
      `${MODULE_API_PREFIX}/depreciation-schedules/${id}/supersede/` as const,
  },
  LINES: {
    LIST: `${MODULE_API_PREFIX}/depreciation-lines/`,
    DETAIL: (id: string) => `${MODULE_API_PREFIX}/depreciation-lines/${id}/` as const,
    POST: (id: string) => `${MODULE_API_PREFIX}/depreciation-lines/${id}/post/` as const,
    POST_DUE: `${MODULE_API_PREFIX}/depreciation-lines/post-due/`,
  },
  TRANSACTIONS: { DETAIL: (id: string) => `${MODULE_API_PREFIX}/transactions/${id}/` as const },
