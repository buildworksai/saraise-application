...
export interface ProcessEvent { id: UUID; process_name: string; source_module: string; source_event_id: string | null; case_id: string; activity: string; occurred_at: ISODateTime; resource: string | null; attributes?: Record<string, unknown>; ingested_at: ISODateTime; event_hash: string; created_by: UUID; created_at: ISODateTime }
...
export interface ProcessModelVersion { id: UUID; process_model: UUID; version: number; discovery_job: UUID | null; algorithm: MiningAlgorithm | null; parameters: Record<string, unknown>; model_data: ProcessGraph; event_count: number; case_count: number; activity_count: number; avg_case_duration_seconds: string | null; is_reference: boolean; published_at: ISODateTime; created_by: UUID; created_at: ISODateTime }
...
export interface FitnessEvidence { check: ConformanceCheck; cases: CaseFitness[] }
