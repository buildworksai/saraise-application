...
export function EditProcessModelPage() {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      processMiningService.updateModel(id, { name: name.trim(), description: description.trim() }),
    onSuccess: async () => {
      await client.invalidateQueries({ queryKey: ["process-mining", "model", id] });
      navigate(PROCESS_MINING_ROUTES.MODEL_MAP(id));
    },
  });
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        ...
        description="Only the logical model name and description are mutable. Published graph versions remain immutable."
      />
        ...
        >
          ...
          <div className="flex justify-end gap-2">
            <Button
              type="button"
              variant="outline"
              onClick={() => navigate(PROCESS_MINING_ROUTES.MODEL_MAP(id))}
            >
              Cancel
            </Button>
