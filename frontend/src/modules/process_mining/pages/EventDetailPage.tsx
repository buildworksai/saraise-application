...
export function EventDetailPage() {
  ...
  const query = useQuery({
    queryKey: ["process-mining", "event", id],
    queryFn: () => processMiningService.getEvent(id),
    enabled: Boolean(id),
  });
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        title={item.activity}
        description="Immutable canonical event evidence. Sensitive classified attributes are redacted at the API boundary."
        actions={
          <span className="flex items-center gap-2 rounded-full bg-muted px-3 py-2 text-xs">
      ...
      <Card className="p-6">
        <h2 className="font-semibold">Redacted attributes</h2>
