...
export function CreateBottleneckAnalysisPage() {
  ...
  const preview = useQuery({
    queryKey: ["process-mining", "bottleneck-preview", processName],
    queryFn: () => processMiningService.listProcesses({ process_name: processName, page_size: 1 }),
    enabled: Boolean(processName.trim()),
  });
  const mutation = useMutation({
    mutationFn: () =>
      processMiningService.createBottleneck({
        process_name: processName.trim(),
        time_range_start: new Date(start).toISOString(),
        time_range_end: new Date(end).toISOString(),
        idempotency_key: deterministicKey("bottleneck", processName, start, end),
      }),
    onSuccess: (analysis) => navigate(PROCESS_MINING_ROUTES.BOTTLENECK(analysis.id)),
  });
  if (!configuration.data) return <PageSkeleton />;
  const minimum = configuration.data.document.bottleneck_min_cases;
  const process = preview.data?.items[0];
  const eligible = Boolean(
    process && process.case_count >= minimum && new Date(end) > new Date(start)
  );
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        title="Create bottleneck analysis"
        description="Preview configured case coverage and bounded compute."
      />
      ...
      <Card className="mx-auto max-w-3xl p-6">
        ...
        >
          ...
          <p
            className={
              process && process.case_count < minimum ? "text-destructive" : "text-muted-foreground"
            }
          >
            {process
              ? `${process.case_count} cases · configured minimum ${minimum}`
          ...
          <div className="flex justify-end gap-2">
            <Button
              type="button"
              variant="outline"
              onClick={() => navigate(PROCESS_MINING_ROUTES.BOTTLENECKS)}
            >
              Cancel
            </Button>
