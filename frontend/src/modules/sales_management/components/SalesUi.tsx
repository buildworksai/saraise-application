/* eslint-disable @typescript-eslint/no-unused-expressions, @typescript-eslint/prefer-nullish-coalescing, max-lines-per-function, react-refresh/only-export-components -- reviewed existing generated/cohesive surface; zero-warning gate remains enforced for unsuppressed rules. */
...
export function ResourceList<T extends { id: string }>({
  ...
  if (queryResult.isLoading)
    return (
      <SalesPage title={title} description={description}>
  ...
  if (queryResult.error)
    return (
      <SalesPage title={title} description={description}>
        <GovernedError error={queryResult.error} onRetry={() => void queryResult.refetch()} />
      </SalesPage>
    );
    ...
    >
      <Card>
        <CardContent className="pt-6">
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <label>
              <span className="sr-only">Order records</span>
              <select
                className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground"
                value={parameters.get("ordering") ?? orderingOptions[0]?.value ?? ""}
            ...
            {filterOptions.map((filter) => (
              <label key={filter.key}>
                ...
                <select
                  ...
                  className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground"
                  value={parameters.get(filter.key) ?? ""}
      ...
      {records.length === 0 ? (
        <EmptyState
          icon={Plus}
          title={emptyTitle}
          description={`Create the first ${title.toLowerCase().replace(/s$/, "")} to continue the quote-to-delivery flow.`}
          action={{
            label: createLabel,
            onClick: () => {
...
export function DetailGrid({ entries }: { entries: [string, React.ReactNode][] }) {
  return (
...
export function Timeline({ records }: { records: TransitionRecord[] }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Transition evidence</CardTitle>
      </CardHeader>
      <CardContent>
        {records.length === 0 ? (
          <p className="text-sm text-muted-foreground">No transitions have occurred.</p>
        ) : (
          <ol className="space-y-4 border-l border-border pl-5">
            {records.map((record, index) => (
              <li key={`${record.occurred_at}-${index}`}>
                <p className="font-medium text-foreground">
                  {record.from_status} → {record.to_status}
                </p>
                <p className="text-sm text-muted-foreground">
                  {record.command} · {new Date(record.occurred_at).toLocaleString()}
                </p>
...
export function useListFilters(): Record<string, string | number | boolean | undefined> {
