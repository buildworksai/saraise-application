...
export function CustomerForm({ customer }: { customer?: Customer }) {
  ...
  if (!form.customer_code.trim()) errors.customer_code = "Customer code is required.";
  ...
  const mutation = useMutation({
    mutationFn: () =>
      customer
        ? salesService.updateCustomer(customer.id, {
            ...form,
            expected_version: customer.lock_version,
          })
        : salesService.createCustomer(form, newIdempotencyKey()),
    onSuccess: (saved) => {
      setSubmitted(true);
      void queryClient.invalidateQueries({ queryKey: salesQueryKeys.all });
      toast.success(customer ? "Customer updated" : "Customer created");
      navigate(`${SALES_PATHS.CUSTOMERS}/${encodeURIComponent(saved.id)}`);
    },
  });
  const change = <K extends keyof CustomerCreate>(key: K, value: CustomerCreate[K]) =>
    setForm((current) => ({ ...current, [key]: value }));
  return (
    <SalesPage
      title={customer ? "Edit customer" : "Create customer"}
      description="Maintain a tenant-owned sales profile. Server validation remains authoritative."
    >
        ...
        <CardContent>
          ...
          >
            <Input
              ...
              onChange={(e) => change("customer_code", e.target.value)}
              error={errors.customer_code}
              aria-describedby={errors.customer_code ? "customer-code-error" : "customer-code-help"}
              required
            />
            <p id="customer-code-help" className="sr-only">
              A unique code within this tenant.
            </p>
            <Input
              id="customer-name"
              label="Customer name"
            ...
            <Input
              ...
              required
            />
            <p id="currency-help" className="sr-only">
            ...
            <Input
              ...
              onChange={(e) => change("credit_limit", e.target.value || null)}
            ...
            <div role="status" aria-live="polite" className="sm:col-span-2 flex flex-wrap gap-3">
