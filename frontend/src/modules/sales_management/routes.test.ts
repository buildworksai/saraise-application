  ...
  it("keeps sidebar and route registry in parity", () => {
    expect(
      tenantRoutes
        ...
        .sort((left, right) =>
          left.navigation.type === "sidebar" && right.navigation.type === "sidebar"
    ...
    for (const route of tenantRoutes.filter((entry) => entry.navigation.type === "sidebar"))
      expect(route.path.includes(":")).toBe(false);
  });

  it("declares static routes before parameter routes", () => {
    expect(
      tenantRoutes.findIndex((route) => route.path === "/sales-management/configuration")
    ).toBeLessThan(tenantRoutes.findIndex((route) => route.path.includes(":")));
    for (const stem of ["customers", "quotations", "sales-orders", "deliveries"]) {
      expect(
        tenantRoutes.findIndex((route) => route.path === `/sales-management/${stem}/new`)
      ).toBeLessThan(
        tenantRoutes.findIndex((route) => route.path === `/sales-management/${stem}/:id`)
      );
    }
  });
});
