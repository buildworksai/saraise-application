/**
 * Customer Detail Page - Sales Management
 */
...
export const CustomerDetailPage = () => {
  ...
  const { data: customer, isLoading, error } = useQuery({
    ...
    queryFn: () => (id ? salesService.getCustomer(id) : Promise.reject(new Error('No ID'))),
    enabled: !!id,
  });

  const deleteMutation = useMutation({
    mutationFn: (customerId: string) => salesService.deleteCustomer(customerId),
    onSuccess: () => {
    ...
    return (
      <div className="p-8">
        <div className="text-center">
          ...
          <Button onClick={() => navigate(MODULE_PATH)}>Back to Customers</Button>
        </div>
      </div>
    );
  }
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => navigate(`${MODULE_PATH}/${customer.id}/edit`)}>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm font-medium">{customer.customer_code}</p>
            </div>
            <div>
          ...
          <div className="pt-4 border-t border-border text-sm text-muted-foreground">
            ...
            <span className="ml-4">Updated: {new Date(customer.updated_at).toLocaleDateString()}</span>
          </div>
        </CardContent>
      </Card>
    </div>
