...
export function EditCustomerPage() {
  ...
  if (query.isLoading)
    return (
      <SalesPage title="Edit customer" description="Loading customer…">
        <div className="h-64 animate-pulse rounded-lg bg-muted" />
      </SalesPage>
    );
  if (query.error || !query.data)
    return (
      <SalesPage title="Edit customer" description="Customer could not be loaded.">
        <GovernedError error={query.error} onRetry={() => void query.refetch()} />
      </SalesPage>
    );
  return <CustomerForm customer={query.data} />;
}
