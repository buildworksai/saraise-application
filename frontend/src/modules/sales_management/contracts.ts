...
export interface ApiV2Error {
  error: { code: string; message: string; detail: Record<string, unknown>; correlation_id: UUID };
}
...
export interface ResourceCapability {
  key: string;
  state: "available" | "degraded" | "not_configured" | "unavailable";
  reason_code: string;
}
...
export interface QuotationPreview
  extends Pick<Quotation, "subtotal_amount" | "discount_amount" | "tax_amount" | "total_amount"> {
  lines: Pick<
    QuotationLine,
    "line_number" | "gross_amount" | "discount_amount" | "tax_amount" | "line_total"
  >[];
  warnings?: string[];
}
...
export interface SalesOrderLine extends MutableEntity {
  sales_order: UUID;
  source_quotation_line_id: UUID | null;
  line_number: number;
  item_id: UUID | null;
  item_code: string;
  item_name: string;
...
export interface DeliveryNoteLine extends MutableEntity {
  delivery_note: UUID;
  sales_order_line: UUID;
  line_number: number;
  item_id: UUID | null;
...
export interface CustomerFilters extends PageFilters {
  search?: string;
  is_active?: boolean;
  currency?: string;
  ordering?: Ordering<"customer_code" | "customer_name" | "created_at">;
}
export interface QuotationFilters extends PageFilters {
  search?: string;
  customer_id?: UUID;
  status?: QuotationStatus;
  currency?: string;
  date_from?: ISODate;
  date_to?: ISODate;
  valid_until_from?: ISODate;
  valid_until_to?: ISODate;
  ordering?: Ordering<
    "quotation_number" | "quotation_date" | "valid_until" | "total_amount" | "created_at"
  >;
}
export interface SalesOrderFilters extends PageFilters {
  search?: string;
  customer_id?: UUID;
  quotation_id?: UUID;
  status?: SalesOrderStatus;
  ...
  date_from?: ISODate;
  date_to?: ISODate;
  delivery_from?: ISODate;
  delivery_to?: ISODate;
  ordering?: Ordering<
    "order_number" | "order_date" | "delivery_date" | "total_amount" | "created_at"
  >;
}
export interface DeliveryNoteFilters extends PageFilters {
  search?: string;
  sales_order_id?: UUID;
  status?: DeliveryNoteStatus;
  warehouse_id?: UUID;
  ...
  date_to?: ISODate;
  tracking_number?: string;
  ordering?: Ordering<"delivery_number" | "delivery_date" | "created_at">;
}

export type RoundingMode = "ROUND_HALF_UP" | "ROUND_HALF_EVEN";
export interface SalesConfiguration extends MutableEntity {
  environment: string;
  ...
  inventory_confirmation_required: boolean;
  manual_discount_enabled: boolean;
  maximum_manual_discount_percent: DecimalString;
  manual_tax_enabled: boolean;
...
export type SalesConfigurationValues = Pick<
  SalesConfiguration,
  | "default_currency"
...
export interface ConfigurationDiff {
  field: keyof SalesConfigurationValues;
...
export interface ConfigurationPreview {
  ...
  proposed?: SalesConfigurationValues;
}
export interface SalesConfigurationVersion {
  id: UUID;
  tenant_id: UUID;
  configuration_id: UUID;
  version: number;
  snapshot: SalesConfigurationValues;
...
export interface ConfigurationExport {
  ...
  values: SalesConfigurationValues;
}
...
export interface SalesHealth {
  ...
  outbox: CapabilityState;
  adapters: Record<string, "available" | "degraded" | "not_configured">;
}
export interface SalesDashboardDelivery {
  id: UUID;
  delivery_number: string;
  delivery_date: ISODate;
  status: DeliveryNoteStatus;
...
export interface SalesDashboardSummary {
  ...
  fulfillment_stages: Record<string, number>;
  recent_deliveries: SalesDashboardDelivery[];
}
export type SalesCapabilityStatus =
  | "available"
  | "not_installed"
  | "not_entitled"
  | "not_configured"
  | "temporarily_unavailable";
export interface SalesExtensionCapability {
  capability: string;
  status: SalesCapabilityStatus;
  reason_code: string;
...
export const ENDPOINTS = {
  ...
  CUSTOMERS: {
    ...
    DETAIL: (id: UUID) => `${MODULE_API_PREFIX}/customers/${encoded(id)}/`,
    UPDATE: (id: UUID) => `${MODULE_API_PREFIX}/customers/${encoded(id)}/`,
    DELETE: (id: UUID) => `${MODULE_API_PREFIX}/customers/${encoded(id)}/`,
  },
  QUOTATIONS: {
    LIST: `${MODULE_API_PREFIX}/quotations/`,
    CREATE: `${MODULE_API_PREFIX}/quotations/`,
    PREVIEW: `${MODULE_API_PREFIX}/quotations/preview/`,
    DETAIL: (id: UUID) => `${MODULE_API_PREFIX}/quotations/${encoded(id)}/`,
    UPDATE: (id: UUID) => `${MODULE_API_PREFIX}/quotations/${encoded(id)}/`,
    DELETE: (id: UUID) => `${MODULE_API_PREFIX}/quotations/${encoded(id)}/`,
    COMMAND: (id: UUID, command: "send" | "accept" | "reject" | "expire" | "revise" | "convert") =>
      `${MODULE_API_PREFIX}/quotations/${encoded(id)}/commands/${encoded(command)}/`,
  },
  SALES_ORDERS: {
    LIST: `${MODULE_API_PREFIX}/sales-orders/`,
    CREATE: `${MODULE_API_PREFIX}/sales-orders/`,
    DETAIL: (id: UUID) => `${MODULE_API_PREFIX}/sales-orders/${encoded(id)}/`,
    UPDATE: (id: UUID) => `${MODULE_API_PREFIX}/sales-orders/${encoded(id)}/`,
    DELETE: (id: UUID) => `${MODULE_API_PREFIX}/sales-orders/${encoded(id)}/`,
    COMMAND: (
      id: UUID,
      command:
        | "confirm"
        ...
        | "mark-invoiced"
        | "cancel"
    ) => `${MODULE_API_PREFIX}/sales-orders/${encoded(id)}/commands/${encoded(command)}/`,
  },
  DELIVERY_NOTES: {
    LIST: `${MODULE_API_PREFIX}/delivery-notes/`,
    CREATE: `${MODULE_API_PREFIX}/delivery-notes/`,
    DETAIL: (id: UUID) => `${MODULE_API_PREFIX}/delivery-notes/${encoded(id)}/`,
    UPDATE: (id: UUID) => `${MODULE_API_PREFIX}/delivery-notes/${encoded(id)}/`,
    DELETE: (id: UUID) => `${MODULE_API_PREFIX}/delivery-notes/${encoded(id)}/`,
    COMMAND: (id: UUID, command: "complete" | "cancel") =>
      `${MODULE_API_PREFIX}/delivery-notes/${encoded(id)}/commands/${encoded(command)}/`,
  },
  CONFIGURATION: {
    ...
    VERSION: (version: number) =>
      `${MODULE_API_PREFIX}/configuration/versions/${encoded(version)}/`,
...
export const SALES_PATHS = {
  ...
  CUSTOMERS: "/sales-management/customers",
  QUOTATIONS: "/sales-management/quotations",
  ORDERS: "/sales-management/sales-orders",
