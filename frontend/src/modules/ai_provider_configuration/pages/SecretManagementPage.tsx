...
export const SecretManagementPage = () => {
  ...
  useEffect(() => () => { setNewKey(''); }, []);
  const copyKey = async () => {
    try { await navigator.clipboard.writeText(newKey); setCopied(true); window.setTimeout(() => setCopied(false), 2000); }
    ...
    <div className="min-h-full bg-muted/20">
      ...
      {credentials.isLoading ? <ConsoleSkeleton /> : credentials.error ? <main className="p-4 sm:p-6 lg:p-8"><ApiProblem error={credentials.error} onRetry={() => { void credentials.refetch(); }} /></main> : (
        <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
          <Card className="border-amber-500/40 bg-amber-500/5 p-5"><div className="flex gap-3"><ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-amber-700 dark:text-amber-300" /><div><h2 className="font-medium">Plan key rotation as a controlled operation</h2><p className="mt-1 text-sm text-muted-foreground">Generate and store the replacement key in your approved secret manager before re-encrypting credentials. Losing either key during the transition can make provider credentials unavailable.</p></div></div></Card>
