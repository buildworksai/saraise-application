...
export const AiProviderConfigurationDetailPage = () => {
  ...
  const provider = useQuery({
    queryKey: ["ai-provider-configuration", "provider", id],
    queryFn: () => aiProviderConfigurationService.getProvider(id ?? ""),
    enabled: Boolean(id),
  });
  const models = useQuery({
    queryKey: ["ai-provider-configuration", "models", id],
    queryFn: () => aiProviderConfigurationService.listModels({ provider_id: id }),
    enabled: Boolean(id),
  });
  const credentials = useQuery({
    queryKey: ["ai-provider-configuration", "credentials", id],
    queryFn: () => aiProviderConfigurationService.listCredentials({ provider_id: id }),
    enabled: Boolean(id),
  });
  const allDeployments = useQuery({
    queryKey: ["ai-provider-configuration", "deployments", "provider", id],
  ...
  const loading =
    provider.isLoading || models.isLoading || credentials.isLoading || allDeployments.isLoading;
  const retry = () => {
    void Promise.all([
      provider.refetch(),
      models.refetch(),
      credentials.refetch(),
      allDeployments.refetch(),
    ]);
  };
  ...
  return (
    <div className="min-h-full bg-muted/20">
      <ConsoleHeader
        title={provider.data?.name ?? "Provider details"}
        description="Review the platform-approved model catalog and this tenant’s provider connection footprint."
        actions={
          <>
            <Button variant="outline" onClick={() => navigate(AI_PROVIDER_ROUTES.HOME)}>
              <ArrowLeft className="mr-2 h-4 w-4" />
              Back
            </Button>
            <Button onClick={() => navigate(AI_PROVIDER_ROUTES.CONNECT)}>
      ...
      {loading ? (
        ...
        <main className="p-4 sm:p-6 lg:p-8">
          <ApiProblem error={error ?? new Error("Provider not found")} onRetry={retry} />
        </main>
      ) : (
        <main className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 lg:p-8">
          <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <DetailMetric
              label="Provider type"
              value={provider.data.provider_type.replaceAll("_", " ")}
            />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Approved models available through {provider.data.name}.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {models.data?.length ? (
                ...
                <EmptyPanel
                  ...
                  description="This provider has no active catalog models."
                />
              )}
            </CardContent>
          </Card>
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Tenant credentials</CardTitle>
                <CardDescription>Secret metadata only.</CardDescription>
              </CardHeader>
              <CardContent>
                {credentials.data?.length ? (
                  ...
                  <EmptyPanel
                    ...
                    title="No credential"
                    description="This tenant is not connected to the provider."
                  />
                )}
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
                ...
                <CardDescription>Tenant runtimes backed by this provider.</CardDescription>
              </CardHeader>
              <CardContent>
                {deployments.length ? (
                  <ul className="divide-y">
                    {deployments.map((deployment) => (
                      <li key={deployment.id} className="flex items-center justify-between py-3">
                        <div className="flex items-center gap-3">
                          <Server className="h-4 w-4 text-primary" />
                          <div>
                            <p className="font-medium">{deployment.deployment_name}</p>
                            <p className="text-xs text-muted-foreground">{deployment.model_name}</p>
                          </div>
                        </div>
                  ...
                  <EmptyPanel
                    ...
                    description="No tenant deployment uses this provider."
                  />
                )}
              </CardContent>
            </Card>
