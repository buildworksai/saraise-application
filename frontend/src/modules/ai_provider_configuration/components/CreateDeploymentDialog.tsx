...
export function CreateDeploymentDialog({
  ...
  const deploymentPolicy =
    typeof values?.deployment_policy === "object" && values.deployment_policy !== null
      ? (values.deployment_policy as Record<string, unknown>)
      : {};
  const defaults =
    typeof deploymentPolicy.defaults === "object" && deploymentPolicy.defaults !== null
      ? (deploymentPolicy.defaults as Record<string, unknown>)
      : {};
  const limits =
    typeof deploymentPolicy.limits === "object" && deploymentPolicy.limits !== null
      ? (deploymentPolicy.limits as Record<string, unknown>)
      : {};
  ...
  const maxTokensMax =
    typeof limits.max_tokens_max === "number" ? limits.max_tokens_max : undefined;
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (
      ...
      return setError(`Temperature must be between ${temperatureMin} and ${temperatureMax}.`);
    if (
      !Number.isInteger(parsedMaxTokens) ||
      parsedMaxTokens < maxTokensMin ||
      (maxTokensMax !== undefined && parsedMaxTokens > maxTokensMax)
    )
      return setError("Maximum tokens violates the tenant runtime limits.");
    setError("");
    await onSubmit({
      model,
      credential: credential || null,
      deployment_name: name.trim(),
      config: { temperature: parsedTemperature, max_tokens: parsedMaxTokens },
    });
  };
  ...
  return (
    <Dialog
      ...
      title="Create model deployment"
      description="Deploy a catalog model with tenant-specific inference defaults."
      size="lg"
    >
      <form
        className="space-y-4"
      ...
      >
        <Input
          ...
          placeholder="Customer support primary"
          value={name}
          onChange={(event) => setName(event.target.value)}
          autoComplete="off"
        />
        <div>
          <label htmlFor="deployment-model" className="mb-1 block text-sm font-medium">
            Model
          </label>
          <select
            id="deployment-model"
        ...
        <div>
          <label htmlFor="deployment-credential" className="mb-1 block text-sm font-medium">
            Credential
          </label>
          <select
            ...
            {credentials
              .filter(
                (item) =>
                  !model ||
                  item.provider ===
                    models.find((catalogModel) => catalogModel.id === model)?.provider
              )
              .map((item) => (
                <option key={item.id} value={item.id}>
                  {item.label}
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Only credentials for the selected model provider are available.
          </p>
        </div>
        <div className="grid gap-4 sm:grid-cols-2">
          <Input
            id="deployment-temperature"
            label="Temperature"
            type="number"
            min={temperatureMin}
            max={temperatureMax}
