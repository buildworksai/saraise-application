...
import { QUERY_KEYS, ROUTES, type DeletionReasonInput, type Role, type RoleCreateInput, type RolePermissionDecision, type RoleType } from "../contracts";
...
function updateSearch(params: URLSearchParams, setParams: (next: URLSearchParams) => void, key: string, value: string): void { const next = new URLSearchParams(params); if (value)
    next.set(key, value);
else
...
export function RoleDetailPage() {
    ...
    const remove = useMutation({ mutationFn: (input: DeletionReasonInput) => securityService.roles.delete(id, input), onSuccess: () => navigate(ROUTES.ROLES) });
    const decision = useMutation({ mutationFn: async ({ permissionId, granted, reason }: {
            permissionId: string;
            ...
            await securityService.roles.setPermission(id, { permission_id: permissionId, is_granted: granted }); }, onSuccess: () => void client.invalidateQueries({ queryKey: QUERY_KEYS.role(id) }) });
    if (query.isLoading)
        return <PageSkeleton />;
    if (query.error)
        return <GovernedError error={query.error} retry={() => void query.refetch()}/>;
    if (!query.data)
        return <GovernedError error={new Error("Role not found.")}/>;
...
function RoleForm({ initial }: {
    ...
    const dirty = name !== (initial?.name ?? "") || code !== (initial?.code ?? "") || description !== (initial?.description ?? "") || parent !== (initial?.parent_role_id ?? "") || active !== (initial?.is_active ?? true);
    ...
    const submit = (event: React.FormEvent) => { event.preventDefault(); const parsed = roleSchema.safeParse({ name, code, description, role_type: roleType, parent_role_id: parent || null }); if (!parsed.success) {
 ...
 onClick={() => { if (!dirty || window.confirm("Discard unsaved role changes?"))
        navigate(initial ? ROUTES.ROLE_DETAIL(initial.id) : ROUTES.ROLES); }}>Cancel</Button><Button type="submit" disabled={mutation.isPending || parents.isLoading}>{mutation.isPending ? "Saving…" : "Save role"}</Button></div></form></main>;
}
export function RoleCreatePage() { return <RoleForm />; }
export function RoleEditPage() { const { id = "" } = useParams(); const query = useQuery({ queryKey: QUERY_KEYS.role(id), queryFn: () => securityService.roles.get(id), enabled: Boolean(id) }); if (query.isLoading)
    return <PageSkeleton />; if (query.error)
    return <GovernedError error={query.error} retry={() => void query.refetch()}/>; return query.data ? <RoleForm key={query.data.data.updated_at} initial={query.data.data}/> : <GovernedError error={new Error("Role not found.")}/>; }
