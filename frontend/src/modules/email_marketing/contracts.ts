...
export interface ApiV2Meta {
  ...
  readonly pagination?: ApiV2PaginationMeta;
}
...
export interface ApiV2Page<T> extends ApiV2Envelope<readonly T[]> {
  readonly meta: ApiV2Meta & { readonly pagination: ApiV2PaginationMeta };
}
...
export interface EmailCampaignSummary {
  ...
  readonly scheduled_at: ISODateTime | null;
  readonly timezone: string;
  readonly resolved_recipient_count: number;
...
export interface EmailCampaignDetail extends EmailCampaignSummary, AuditFields {
  ...
  readonly audience_definition: Readonly<Record<string, unknown>>;
  readonly audience_snapshot_at: ISODateTime | null;
  readonly queue_started_at: ISODateTime | null;
  readonly send_started_at: ISODateTime | null;
  readonly completed_at: ISODateTime | null;
  readonly content_snapshot_subject: string;
  readonly content_snapshot_html: string;
  readonly content_snapshot_text: string;
  readonly template_version_snapshot: number | null;
  readonly unique_opened_count: number;
  readonly unique_clicked_count: number;
...
export interface SendCampaignInput extends IdempotentActionInput {
  readonly preflight_receipt: string;
}
...
export interface EmailTemplateDetail extends EmailTemplateSummary, AuditFields {
  readonly description: string;
  readonly preview_text: string;
  readonly body_html: string;
  readonly body_text: string;
  readonly design_json: Readonly<Record<string, unknown>>;
  readonly last_used_at: ISODateTime | null;
...
export interface CampaignPreflight {
  readonly content_valid: boolean;
  readonly receipt: string;
  readonly rendered: boolean;
  readonly resolved_count: number;
  readonly eligible_count: number;
  readonly suppressed_count: number;
  readonly consent_failure_count: number;
  readonly suppression_failure_count: number;
  readonly sender_healthy: boolean;
  readonly sender_detail: string;
  readonly quota_required: number;
  readonly quota_remaining: number | null;
  readonly scheduled_at: ISODateTime | null;
  readonly timezone: string;
  readonly blocking_reasons: readonly string[];
}
export interface CampaignAnalytics {
  readonly campaign_id: UUID;
  readonly resolved: number;
  readonly eligible: number;
  readonly sent: number;
  readonly delivered: number;
  readonly unique_opened: number;
  readonly unique_clicked: number;
  readonly bounced: number;
  readonly failed: number;
  readonly unsubscribed: number;
  readonly complained: number;
  readonly delivery_rate: number;
  readonly unique_open_rate: number;
  readonly unique_click_rate: number;
  readonly bounce_rate: number;
  readonly counter_drift: Readonly<Record<string, number>>;
  readonly preflight: CampaignPreflight;
}
...
export interface DeliveryAttemptDetail extends DeliveryAttemptSummary {
  readonly updated_at: ISODateTime;
  readonly events: readonly DeliveryEvent[];
}
export interface DeliveryEvent {
  readonly id: UUID;
  readonly recipient_id: UUID;
  readonly attempt_id: UUID | null;
  readonly gateway_key: string;
  readonly provider_event_id: string;
  readonly event_type: DeliveryEventType;
  readonly occurred_at: ISODateTime;
  readonly link_url_hash: string;
  readonly bounce_class: "" | "hard" | "soft" | "block";
...
export interface SuppressionEntrySummary {
  ...
  readonly suppressed_at: ISODateTime;
  readonly expires_at: ISODateTime | null;
}
export interface SuppressionEntryDetail extends SuppressionEntrySummary, AuditFields {
  readonly evidence_event_id: UUID | null;
  readonly notes: string;
  readonly deactivated_at: ISODateTime | null;
  readonly deactivated_by: UUID | null;
}
...
export interface SuppressionDeactivateInput {
  readonly reason: string;
}
...
export interface ConsentRecordSummary {
  ...
  readonly lawful_basis: ConsentLawfulBasis;
  readonly source: ConsentSource;
  readonly notice_version: string;
  readonly captured_at: ISODateTime;
  readonly created_at: ISODateTime;
}
export interface ConsentRecordDetail extends ConsentRecordSummary {
  readonly actor_id: UUID | null;
...
export interface PublicUnsubscribeInput {
  readonly token: string;
  readonly occurred_at: ISODateTime;
}
export interface PublicUnsubscribeResponse {
  readonly suppression_id: UUID;
...
export interface ConfigurationPreviewInput {
  readonly document: EmailMarketingConfigurationDocument;
}
...
export interface CampaignFilters extends PageFilters {
  ...
  readonly ordering?: Ordering<CampaignOrderingField>;
}
...
export interface TemplateFilters extends PageFilters {
  ...
  readonly status?: TemplateStatus;
  readonly category?: string;
  readonly ordering?: Ordering<TemplateOrderingField>;
}
export interface RecipientFilters extends PageFilters {
  readonly campaign_id?: UUID;
  readonly status?: RecipientStatus;
  readonly email?: string;
  readonly ordering?: Ordering<"created_at" | "status">;
}
export interface DeliveryFilters extends PageFilters {
  readonly campaign_id?: UUID;
  readonly recipient_id?: UUID;
  readonly status?: DeliveryAttemptStatus;
  readonly gateway_key?: string;
...
export interface SuppressionFilters extends PageFilters {
  readonly active?: boolean;
  readonly scope?: SuppressionScope;
  readonly reason?: SuppressionReason;
  readonly email?: string;
  readonly ordering?: Ordering<"suppressed_at" | "email">;
}
export interface ConsentFilters extends PageFilters {
  readonly status?: ConsentStatus;
  readonly purpose?: string;
  readonly source?: ConsentSource;
  readonly email?: string;
  ...
  readonly ordering?: Ordering<"captured_at" | "email">;
}
...
export const ENDPOINTS = {
  CAMPAIGNS: {
    ...
    DETAIL: (id: UUID) => `${resource("campaigns")}${id}/` as const,
    UPDATE: (id: UUID) => `${resource("campaigns")}${id}/` as const,
    DELETE: (id: UUID) => `${resource("campaigns")}${id}/` as const,
    RESOLVE_AUDIENCE: (id: UUID) => `${resource("campaigns")}${id}/resolve-audience/` as const,
    SCHEDULE: (id: UUID) => `${resource("campaigns")}${id}/schedule/` as const,
    UNSCHEDULE: (id: UUID) => `${resource("campaigns")}${id}/unschedule/` as const,
    SEND: (id: UUID) => `${resource("campaigns")}${id}/send/` as const,
    PAUSE: (id: UUID) => `${resource("campaigns")}${id}/pause/` as const,
    RESUME: (id: UUID) => `${resource("campaigns")}${id}/resume/` as const,
    CANCEL: (id: UUID) => `${resource("campaigns")}${id}/cancel/` as const,
    ANALYTICS: (id: UUID) => `${resource("campaigns")}${id}/analytics/` as const,
  },
  TEMPLATES: {
    LIST: resource("templates"),
    CREATE: resource("templates"),
    DETAIL: (id: UUID) => `${resource("templates")}${id}/` as const,
    UPDATE: (id: UUID) => `${resource("templates")}${id}/` as const,
    DELETE: (id: UUID) => `${resource("templates")}${id}/` as const,
    ACTIVATE: (id: UUID) => `${resource("templates")}${id}/activate/` as const,
    ARCHIVE: (id: UUID) => `${resource("templates")}${id}/archive/` as const,
    CLONE: (id: UUID) => `${resource("templates")}${id}/clone/` as const,
    PREVIEW: (id: UUID) => `${resource("templates")}${id}/preview/` as const,
  },
  RECIPIENTS: {
    LIST: resource("recipients"),
    DETAIL: (id: UUID) => `${resource("recipients")}${id}/` as const,
    RETRY: (id: UUID) => `${resource("recipients")}${id}/retry/` as const,
  },
  DELIVERIES: {
    LIST: resource("deliveries"),
    DETAIL: (id: UUID) => `${resource("deliveries")}${id}/` as const,
  },
  SUPPRESSIONS: {
    LIST: resource("suppressions"),
    CREATE: resource("suppressions"),
    DETAIL: (id: UUID) => `${resource("suppressions")}${id}/` as const,
    DEACTIVATE: (id: UUID) => `${resource("suppressions")}${id}/deactivate/` as const,
  },
  CONSENTS: {
    LIST: resource("consents"),
    CREATE: resource("consents"),
    DETAIL: (id: UUID) => `${resource("consents")}${id}/` as const,
    REVOKE: `${resource("consents")}revoke/`,
  },
  CONFIGURATION: {
  ...
  PUBLIC_UNSUBSCRIBE: `${MODULE_API_PREFIX}/public/unsubscribe/`,
  TRACK_OPEN: (token: string) =>
    `${MODULE_API_PREFIX}/t/${encodeURIComponent(token)}/open.gif` as const,
  TRACK_CLICK: (token: string) =>
    `${MODULE_API_PREFIX}/t/${encodeURIComponent(token)}/click/` as const,
  HEALTH: resource("health"),
} as const;

export const ROUTES = {
  CAMPAIGNS: "/email-marketing/campaigns",
  CAMPAIGN_CREATE: "/email-marketing/campaigns/new",
  CAMPAIGN_DETAIL: (id: UUID) => `/email-marketing/campaigns/${id}` as const,
  CAMPAIGN_EDIT: (id: UUID) => `/email-marketing/campaigns/${id}/edit` as const,
  TEMPLATES: "/email-marketing/templates",
  TEMPLATE_CREATE: "/email-marketing/templates/new",
  TEMPLATE_DETAIL: (id: UUID) => `/email-marketing/templates/${id}` as const,
  TEMPLATE_EDIT: (id: UUID) => `/email-marketing/templates/${id}/edit` as const,
  DELIVERY: "/email-marketing/delivery",
  RECIPIENT_DETAIL: (id: UUID) => `/email-marketing/delivery/recipients/${id}` as const,
  DELIVERY_DETAIL: (id: UUID) => `/email-marketing/delivery/attempts/${id}` as const,
  SUPPRESSIONS: "/email-marketing/suppressions",
  SUPPRESSION_CREATE: "/email-marketing/suppressions/new",
  SUPPRESSION_DETAIL: (id: UUID) => `/email-marketing/suppressions/${id}` as const,
  CONSENTS: "/email-marketing/consents",
  CONSENT_CREATE: "/email-marketing/consents/new",
  CONSENT_DETAIL: (id: UUID) => `/email-marketing/consents/${id}` as const,
  CONFIGURATION: "/email-marketing/configuration",
} as const;

export const isRecord = (value: unknown): value is Record<string, unknown> =>
  value !== null && typeof value === "object" && !Array.isArray(value);
export function isPaginationMeta(value: unknown): value is ApiV2PaginationMeta {
  return (
    isRecord(value) &&
    ["count", "page", "page_size", "total_pages"].every((key) => Number.isInteger(value[key])) &&
    typeof value.has_next === "boolean" &&
    typeof value.has_previous === "boolean"
  );
}
export function isApiV2Envelope(value: unknown): value is ApiV2Envelope<unknown> {
  return (
    isRecord(value) &&
    "data" in value &&
    isRecord(value.meta) &&
...
export function isApiV2Page(value: unknown): value is ApiV2Page<unknown> {
  return (
    isApiV2Envelope(value) && Array.isArray(value.data) && isPaginationMeta(value.meta.pagination)
  );
}

const hasStrings = (value: Record<string, unknown>, keys: readonly string[]) =>
  keys.every((key) => typeof value[key] === "string");
export const isCampaignSummary = (value: unknown): value is EmailCampaignSummary =>
  isRecord(value) &&
  hasStrings(value, [
    "id",
    "campaign_code",
  ...
  typeof value.resolved_recipient_count === "number";
export const isCampaignDetail = (value: unknown): value is EmailCampaignDetail =>
  isRecord(value) &&
  isCampaignSummary(value) &&
  hasStrings(value, ["from_name", "from_email", "description"]) &&
  Array.isArray(value.transition_history);
export const isTemplateSummary = (value: unknown): value is EmailTemplateSummary =>
  isRecord(value) &&
  hasStrings(value, [
    "id",
    "template_code",
...
export const isTemplateDetail = (value: unknown): value is EmailTemplateDetail =>
  isRecord(value) &&
  isTemplateSummary(value) &&
  hasStrings(value, ["body_html", "body_text", "description"]) &&
  isRecord(value.design_json);
export const isRecipientSummary = (value: unknown): value is CampaignRecipientSummary =>
  isRecord(value) &&
  hasStrings(value, ["id", "campaign_id", "email", "display_name", "status", "created_at"]);
export const isRecipientDetail = (value: unknown): value is CampaignRecipientDetail =>
  isRecord(value) &&
  isRecipientSummary(value) &&
  Array.isArray(value.delivery_attempts) &&
  Array.isArray(value.events);
export const isDeliverySummary = (value: unknown): value is DeliveryAttemptSummary =>
  isRecord(value) &&
  hasStrings(value, ["id", "recipient_id", "status", "error_code", "created_at"]) &&
  typeof value.attempt_number === "number";
export const isDeliveryDetail = (value: unknown): value is DeliveryAttemptDetail =>
  isRecord(value) &&
  isDeliverySummary(value) &&
  typeof value.updated_at === "string" &&
  Array.isArray(value.events);
export const isSuppressionSummary = (value: unknown): value is SuppressionEntrySummary =>
  isRecord(value) &&
  hasStrings(value, ["id", "email", "scope", "reason", "source", "suppressed_at"]) &&
  typeof value.active === "boolean";
export const isSuppressionDetail = (value: unknown): value is SuppressionEntryDetail =>
  isRecord(value) &&
  isSuppressionSummary(value) &&
  hasStrings(value, ["notes", "created_at", "updated_at"]);
export const isConsentSummary = (value: unknown): value is ConsentRecordSummary =>
  isRecord(value) &&
  hasStrings(value, [
    "id",
    "email",
...
export const isConsentDetail = (value: unknown): value is ConsentRecordDetail =>
  isRecord(value) &&
  isConsentSummary(value) &&
  (value.actor_id === null || typeof value.actor_id === "string") &&
  (value.supersedes_id === null || typeof value.supersedes_id === "string");
export const isAsyncJob = (value: unknown): value is AsyncJobSummary =>
  isRecord(value) &&
  hasStrings(value, [
    "id",
    "job_type",
...
export const isCampaignAnalytics = (value: unknown): value is CampaignAnalytics =>
  isRecord(value) && typeof value.delivery_rate === "number" && isRecord(value.preflight);
export const isRenderedEmail = (value: unknown): value is RenderedEmail =>
  isRecord(value) &&
  hasStrings(value, ["subject", "html", "text"]) &&
  Array.isArray(value.warnings);
export const isHealth = (value: unknown): value is ModuleHealth =>
  isRecord(value) &&
  typeof value.ready === "boolean" &&
  ["ready", "degraded", "unhealthy"].includes(String(value.status)) &&
  Array.isArray(value.checks);
export const isPublicUnsubscribeResponse = (value: unknown): value is PublicUnsubscribeResponse =>
  isRecord(value) && typeof value.suppression_id === "string" && value.status === "unsubscribed";
export const isEmailMarketingConfigurationDocument = (
  value: unknown
): value is EmailMarketingConfigurationDocument =>
  isRecord(value) &&
...
export const isEmailMarketingConfiguration = (
  value: unknown
): value is EmailMarketingConfiguration =>
  isRecord(value) &&
  hasStrings(value, ["id", "environment", "updated_at"]) &&
  Number.isInteger(value.version) &&
  isEmailMarketingConfigurationDocument(value.document);
export const isEmailMarketingConfigurationPreview = (
  value: unknown
): value is EmailMarketingConfigurationPreview =>
  isRecord(value) &&
  value.valid === true &&
  isEmailMarketingConfigurationDocument(value.normalized_document) &&
  Array.isArray(value.changes) &&
  Array.isArray(value.warnings);
export const isEmailMarketingConfigurationVersion = (
  value: unknown
): value is EmailMarketingConfigurationVersion =>
  isRecord(value) &&
  hasStrings(value, ["id", "change_type", "correlation_id", "created_at"]) &&
  Number.isInteger(value.version) &&
  isEmailMarketingConfigurationDocument(value.document);
