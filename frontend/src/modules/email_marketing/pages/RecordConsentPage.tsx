...
export function RecordConsentPage() {
  ...
  if (configuration.error)
    return (
      <Page title="Record consent evidence" description="Append trusted consent evidence.">
  ...
  if (!configuration.data)
    return (
      <Page title="Record consent evidence" description="Append trusted consent evidence.">
        <GovernedError
          error={new Error("No governed consent configuration was received.")}
    ...
    return (
      <Page title="Record consent evidence" description="Append trusted consent evidence.">
        <GovernedError
          error={
            new Error(
              "Consent allow-lists are empty; configuration must be corrected before recording evidence."
            )
          }
        />
      </Page>
...
function ConfiguredRecordConsentPage({
  configuration,
}: {
  ...
  const [status, setStatus] = useState<ConsentStatus>(
    configuration.compliance.consent_required_status
  );
  const [basis, setBasis] = useState<ConsentLawfulBasis>(
    configuration.compliance.consent_lawful_bases[0]!
  );
  ...
  const mutation = useMutation({
    mutationFn: () =>
      emailMarketingService.consents.create({
        email,
        purpose,
        status,
        lawful_basis: basis,
  ...
  return (
    <Page
      ...
      description="Append a grant or revocation. Capture time and trusted request evidence are derived by the server."
    ...
    >
      ...
      >
        <Surface
          title="Immutable consent event"
          description="Network evidence, actor, and capture time cannot be supplied or spoofed by the browser."
        >
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="text-sm font-medium">
              Email
              <input
                aria-label="Consent email"
                type="email"
                required
                className={cls}
                value={email}
            ...
            <label className="text-sm font-medium">
              ...
              <select
                ...
                className={cls}
                value={basis}
                onChange={(event) => setBasis(event.target.value as ConsentLawfulBasis)}
              >
                {configuration.compliance.consent_lawful_bases.map((value) => (
            ...
            <label className="text-sm font-medium">
              ...
              >
                {configuration.compliance.consent_sources.map((value) => (
                  <option key={value}>{value}</option>
                ))}
              </select>
            </label>
        ...
        <p className="sr-only" aria-live="polite">
          {mutation.isPending ? "Appending consent evidence" : ""}
        </p>
        <div className="flex justify-end gap-2">
          <Button
            ...
            variant="outline"
            onClick={() => {
              if (!dirty || window.confirm("Discard this consent event?"))
          ...
          <Button disabled={mutation.isPending} type="submit">
            {mutation.isPending ? "Appending…" : "Append consent event"}
          </Button>
        </div>
      </form>
    </Page>
