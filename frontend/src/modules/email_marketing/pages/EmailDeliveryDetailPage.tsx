...
export function EmailDeliveryDetailPage() {
  ...
  if (query.error)
    return (
      <Page title="Delivery attempt" description="Sanitized operational evidence.">
  ...
  return (
    <Page
      title={`Delivery attempt ${item.attempt_number}`}
      description="Accepted means a real gateway acknowledgement; delivered requires verified provider evidence. Internal provider and job identifiers are redacted."
      back={{ label: "Audience & delivery", to: `${ROUTES.DELIVERY}?view=deliveries` }}
    >
      <Surface title="Operational evidence">
        <DetailGrid>
          <Detail label="Status">
          ...
          <Detail label="Recipient">
            <Link
              ...
              to={ROUTES.RECIPIENT_DETAIL(item.recipient_id)}
            >
              {item.recipient_id}
            </Link>
          </Detail>
          ...
          <ol className="space-y-3">
            {item.events.map((event) => (
              <li key={event.id} className="rounded border p-3 text-sm">
                ...
                <p className="mt-1 font-mono text-xs">correlation {event.correlation_id}</p>
              </li>
            ))}
          </ol>
        ) : (
          <p className="text-sm text-muted-foreground">
            No verified provider event has been appended.
          </p>
        )}
      </Surface>
    </Page>
