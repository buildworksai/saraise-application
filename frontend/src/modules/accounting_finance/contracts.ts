...
export interface PostingPeriod extends MutableRecord {
  ...
  readonly status: PostingPeriodStatus;
...
export interface PostingPeriodCreateRequest {
  period_name: string;
  start_date: string;
  end_date: string;
  fiscal_year: number;
...
export interface JournalLine {
  ...
  readonly base_debit_amount: string;
  readonly base_credit_amount: string;
  readonly description: string;
...
export interface JournalEntry extends MutableRecord {
  ...
  readonly posting_period: string;
  ...
  readonly reversed_entry: string | null;
...
export interface JournalEntryCreateRequest {
  ...
  description?: string;
  currency?: string;
  lines: readonly JournalLineWriteRequest[];
}
export type JournalEntryUpdateRequest = Partial<Omit<JournalEntryCreateRequest, "lines">> & {
  version: number;
  lines?: readonly JournalLineWriteRequest[];
};
...
export interface InvoiceBase extends MutableRecord {
  ...
  readonly exchange_rate: string;
  readonly description: string;
  readonly journal_entry: string | null;
  readonly legacy_without_lines: boolean;
  readonly transition_history: readonly TransitionHistoryItem[];
...
export interface APInvoiceCreateRequest {
  ...
  exchange_rate?: string;
  description?: string;
  tax_provenance?: Readonly<Record<string, string>>;
  lines: readonly InvoiceLineWriteRequest[];
}
export type APInvoiceUpdateRequest = Partial<Omit<APInvoiceCreateRequest, "lines">> & {
  version: number;
  lines?: readonly InvoiceLineWriteRequest[];
};
...
export type ARInvoiceUpdateRequest = Partial<Omit<ARInvoiceCreateRequest, "lines">> & {
  version: number;
  lines?: readonly InvoiceLineWriteRequest[];
};
...
export interface Payment {
  ...
  readonly ap_invoice: string | null;
  readonly ar_invoice: string | null;
  readonly description: string;
...
export interface PaymentVoidCommand {
  transition_key: string;
  reason?: string;
}
export interface ApprovalCommand extends TransitionCommand {
  comments?: string;
}
export interface ReversalCommand extends TransitionCommand {
  posting_date: string;
  reason: string;
}
export interface BatchImportCommand {
...
export interface ReportMeta {
  readonly period: { readonly start_date?: string; readonly end_date: string };
  readonly currency: string;
...
export interface ReportAmountRow {
  ...
  readonly amount: string;
  readonly comparison_amount?: string;
}
export interface TrialBalanceRow {
  ...
  readonly balance: string;
  readonly journal_entry_ids: readonly string[];
}
...
export interface LedgerRow {
  ...
  readonly journal_line_id: string;
  readonly posting_date: string;
  readonly entry_number: string;
  readonly description: string;
  readonly debit: string;
...
export interface GeneralLedger {
  readonly meta: ReportMeta;
  readonly account: Pick<Account, "id" | "code" | "name" | "currency">;
...
export interface FinancialStatement {
  ...
  readonly sections: readonly {
    readonly name: string;
    readonly rows: readonly ReportAmountRow[];
...
export interface CashFlowStatement extends FinancialStatement {
  ...
  readonly unclassified_account_ids: readonly string[];
}
export interface AgingBucket {
  readonly key: "current" | "1_30" | "31_60" | "61_90" | "over_90";
  readonly label: string;
  readonly amount: string;
...
export type ReportType =
  ...
  | "income-statement"
  | "cash-flow";
export interface AsOfDateQuery {
  as_of_date: string;
}
...
export interface AccountingHealth {
  ...
  readonly checks: readonly AccountingHealthCheck[];
}
...
export interface AccountListQuery extends CommonListQuery {
  account_type?: AccountType;
  parent?: string;
  is_group?: boolean;
  is_active?: boolean;
}
export interface PostingPeriodListQuery extends CommonListQuery {
  status?: PostingPeriodStatus;
  fiscal_year?: number;
  date?: string;
}
export interface JournalEntryListQuery extends CommonListQuery {
...
export interface APInvoiceListQuery extends CommonListQuery {
...
export interface ARInvoiceListQuery extends CommonListQuery {
...
export interface PaymentListQuery extends CommonListQuery {
...
export const postingPeriodCreateSchema = z
  .object({
...
export const postingPeriodUpdateSchema = z.object({
  period_name: z.string().trim().min(1).max(50).optional(),
  start_date: date.optional(),
  end_date: date.optional(),
  fiscal_year: z.number().int().min(1900).max(9999).optional(),
  version,
});
export const journalLineWriteSchema = z
  .object({
    ...
    dimension_values: dimensions.optional(),
  })
  .refine((line) => Number(line.debit_amount) > 0 !== Number(line.credit_amount) > 0, {
    message: "Exactly one of debit or credit must be greater than zero.",
  });
export const journalEntryCreateSchema = z.object({
  ...
  description: z.string().max(5000).optional(),
  currency: currency.optional(),
  lines: z.array(journalLineWriteSchema).min(2),
});
export const journalEntryUpdateSchema = journalEntryCreateSchema.partial().extend({ version });
export const invoiceLineWriteSchema = z.object({
  line_number: z.number().int().positive(),
  description: z.string().trim().min(1).max(500),
  account: uuid,
  quantity: decimal.refine((value) => Number(value) > 0, "Quantity must be greater than zero."),
  unit_price: money,
...
const invoiceBaseSchema = z
  .object({
    ...
    exchange_rate: decimal.optional(),
    description: z.string().max(5000).optional(),
    tax_provenance: dimensions.optional(),
    lines: z.array(invoiceLineWriteSchema).min(1),
  })
  .refine((value) => value.invoice_date <= value.due_date, {
    path: ["due_date"],
...
export const apInvoiceUpdateSchema = z.object({
  ...
  description: z.string().max(5000).optional(),
  tax_provenance: dimensions.optional(),
  lines: z.array(invoiceLineWriteSchema).min(1).optional(),
  version,
});
...
export const paymentCreateSchema = z
  .object({
    ...
    payment_method: z.enum(["cash", "check", "wire_transfer", "ach", "credit_card", "other"]),
    currency: currency.optional(),
    reference_number: z.string().max(100).optional(),
    ap_invoice: uuid.nullable().optional(),
    ar_invoice: uuid.nullable().optional(),
    description: z.string().max(5000).optional(),
  })
  .refine((value) => Boolean(value.ap_invoice) !== Boolean(value.ar_invoice), {
...
export const paymentUpdateSchema = z
  ...
  .refine(
    (value) => value.reference_number !== undefined || value.description !== undefined,
    "Provide a reference or description change."
  );
export const transitionCommandSchema = z.object({
  transition_key: z.string().min(8),
  version,
  reason: z.string().min(1).max(2000).optional(),
});
export const approvalCommandSchema = transitionCommandSchema.extend({
  comments: z.string().max(2000).optional(),
});
export const reversalCommandSchema = transitionCommandSchema.extend({
  posting_date: date,
  reason: z.string().min(1).max(2000),
});
export const batchImportCommandSchema = z.object({
  file_reference: z.string().trim().min(1).max(2048),
});
...
export const ENDPOINTS = {
  ACCOUNTS: {
    ...
    HIERARCHY: `${MODULE_API_PREFIX}/accounts/hierarchy/`,
  },
  POSTING_PERIODS: {
    ...
    CLOSE: (id: string) => `${MODULE_API_PREFIX}/posting-periods/${id}/close/` as const,
    REOPEN: (id: string) => `${MODULE_API_PREFIX}/posting-periods/${id}/reopen/` as const,
    LOCK: (id: string) => `${MODULE_API_PREFIX}/posting-periods/${id}/lock/` as const,
  },
  JOURNAL_ENTRIES: {
    ...
    POST: (id: string) => `${MODULE_API_PREFIX}/journal-entries/${id}/post/` as const,
    REVERSE: (id: string) => `${MODULE_API_PREFIX}/journal-entries/${id}/reverse/` as const,
    BATCH_IMPORT: `${MODULE_API_PREFIX}/journal-entries/batch-import/`,
  },
  AP_INVOICES: {
    ...
    SUBMIT: (id: string) => `${MODULE_API_PREFIX}/ap-invoices/${id}/submit/` as const,
    APPROVE: (id: string) => `${MODULE_API_PREFIX}/ap-invoices/${id}/approve/` as const,
    REJECT: (id: string) => `${MODULE_API_PREFIX}/ap-invoices/${id}/reject/` as const,
    POST: (id: string) => `${MODULE_API_PREFIX}/ap-invoices/${id}/post/` as const,
    CANCEL: (id: string) => `${MODULE_API_PREFIX}/ap-invoices/${id}/cancel/` as const,
    AGING: `${MODULE_API_PREFIX}/ap-invoices/aging/`,
  },
  AR_INVOICES: {
    ...
    POST: (id: string) => `${MODULE_API_PREFIX}/ar-invoices/${id}/post/` as const,
    CANCEL: (id: string) => `${MODULE_API_PREFIX}/ar-invoices/${id}/cancel/` as const,
    AGING: `${MODULE_API_PREFIX}/ar-invoices/aging/`,
  },
  PAYMENTS: {
    ...
    VOID: (id: string) => `${MODULE_API_PREFIX}/payments/${id}/void/` as const,
  },
  REPORTS: {
    TRIAL_BALANCE: `${MODULE_API_PREFIX}/reports/trial-balance/`,
    GENERAL_LEDGER: `${MODULE_API_PREFIX}/reports/general-ledger/`,
    BALANCE_SHEET: `${MODULE_API_PREFIX}/reports/balance-sheet/`,
    INCOME_STATEMENT: `${MODULE_API_PREFIX}/reports/income-statement/`,
    CASH_FLOW: `${MODULE_API_PREFIX}/reports/cash-flow/`,
    GENERATE: `${MODULE_API_PREFIX}/reports/generate/`,
  },
...
export const accountingQueryKeys = {
  ...
  accounts: (query: AccountListQuery = {}) =>
    [...accountingQueryKeys.root, "accounts", query] as const,
  account: (id: string) => [...accountingQueryKeys.root, "account", id] as const,
  hierarchy: () => [...accountingQueryKeys.root, "hierarchy"] as const,
  periods: (query: PostingPeriodListQuery = {}) =>
    [...accountingQueryKeys.root, "periods", query] as const,
  period: (id: string) => [...accountingQueryKeys.root, "period", id] as const,
  journals: (query: JournalEntryListQuery = {}) =>
    [...accountingQueryKeys.root, "journals", query] as const,
  journal: (id: string) => [...accountingQueryKeys.root, "journal", id] as const,
  apInvoices: (query: APInvoiceListQuery = {}) =>
    [...accountingQueryKeys.root, "ap-invoices", query] as const,
  apInvoice: (id: string) => [...accountingQueryKeys.root, "ap-invoice", id] as const,
  arInvoices: (query: ARInvoiceListQuery = {}) =>
    [...accountingQueryKeys.root, "ar-invoices", query] as const,
  arInvoice: (id: string) => [...accountingQueryKeys.root, "ar-invoice", id] as const,
  payments: (query: PaymentListQuery = {}) =>
    [...accountingQueryKeys.root, "payments", query] as const,
  payment: (id: string) => [...accountingQueryKeys.root, "payment", id] as const,
  report: (type: ReportType, query: object) =>
    [...accountingQueryKeys.root, "report", type, query] as const,
  aging: (kind: "ap" | "ar", asOfDate: string) =>
    [...accountingQueryKeys.root, `${kind}-aging`, asOfDate] as const,
