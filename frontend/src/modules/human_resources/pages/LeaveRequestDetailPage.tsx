...
export function LeaveRequestDetailPage() {
  ...
  const transition = useMutation({
    mutationFn: async (next: RequestAction) => {
      ...
      if (next === "reject") {
        return hrService.rejectLeaveRequest(id, {
          transition_key,
  ...
  if (detail.error || !detail.data) {
    return (
      <PageShell title="Leave request" description="Leave request detail.">
        <GovernedError
          error={detail.error}
  ...
  const cancellable =
    pending || (item.status === "approved" && new Date(item.start_date) > new Date());
  return (
    <PageShell
      ...
      actions={
        <>
          {pending && can(capabilities, "hr.leave_request:update") ? (
            <Button variant="outline" onClick={() => navigate(ROUTES.LEAVE_REQUEST_EDIT(id))}>
              <Edit className="mr-2 h-4 w-4" />
              Edit
            </Button>
          ) : null}
          {pending && can(capabilities, "hr.leave_request:approve") ? (
            <Button onClick={() => setAction("approve")}>
              <Check className="mr-2 h-4 w-4" />
              Approve
            </Button>
          ) : null}
          {pending && can(capabilities, "hr.leave_request:reject") ? (
            <Button variant="danger" onClick={() => setAction("reject")}>
              <X className="mr-2 h-4 w-4" />
              Reject
            </Button>
          ) : null}
          {cancellable && can(capabilities, "hr.leave_request:cancel") ? (
    ...
    >
      {!capabilities.length ? (
        <p className="rounded-lg border bg-muted/40 p-3 text-sm text-muted-foreground">
          Action permissions are unavailable; privileged actions remain hidden for safety.
        </p>
      ) : null}
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <DetailGrid>
            ...
            <Detail label="Balance">
              <Link
                className="text-primary hover:underline"
                to={ROUTES.LEAVE_BALANCE_DETAIL(item.leave_balance)}
              >
                View allocation
              </Link>
            </Detail>
            ...
            <Detail label="Approved">
              {item.approved_at
                ? `${formatInstant(item.approved_at)} by ${item.approved_by}`
                : "Not approved"}
            </Detail>
            <Detail label="Cancelled">
              {item.cancelled_at
                ? `${formatInstant(item.cancelled_at)} by ${item.cancelled_by}`
                : "Not cancelled"}
            </Detail>
            {item.rejection_reason ? (
              <Detail label="Rejection reason">{item.rejection_reason}</Detail>
            ) : null}
          </DetailGrid>
        </CardContent>
      </Card>
          ...
          {item.transition_history.length ? (
            <ol className="border-l pl-5">
              {item.transition_history.map((record) => (
                <li key={record.transition_key} className="mb-4">
                  <strong className="capitalize">{record.command}</strong>
                  <p className="text-xs text-muted-foreground">
                    {record.from_state} → {record.to_state} · {formatInstant(record.occurred_at)}
                  </p>
                </li>
              ))}
            </ol>
            ...
            <p className="text-sm text-muted-foreground">
              The request is pending its first state transition.
            </p>
          )}
        </CardContent>
      </Card>
      {action === "reject" ? (
        <Dialog
          ...
          description="A reason is required and will be visible in the request audit trail."
        >
          ...
          <div className="mt-4 flex justify-end gap-3">
            ...
            <Button
              variant="danger"
              disabled={!rejectionReason.trim() || transition.isPending}
              onClick={() => transition.mutate("reject")}
            >
              {transition.isPending ? "Rejecting…" : "Reject request"}
            </Button>
          </div>
        </Dialog>
      ) : null}
      <ConfirmAction
        ...
        description={
          action === "approve"
            ? "Approval atomically converts reserved days into used leave."
            : "Cancellation releases reserved or future used days according to the request state."
        }
        confirmLabel={action === "approve" ? "Approve request" : "Cancel request"}
        danger={action === "cancel"}
        pending={transition.isPending}
        onConfirm={() => {
          if (action) transition.mutate(action);
        }}
      />
