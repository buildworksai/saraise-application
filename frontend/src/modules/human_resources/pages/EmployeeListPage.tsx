...
export function EmployeeListPage() {
  ...
  const configuration = useHumanResourcesConfiguration(); const document = configuration.data?.data.document;
  const filters: EmployeeFilters = { page: Number(params.get('page') ?? 1), page_size: document?.limits.list_page_size,
  ...
  const query = useQuery({ queryKey: hrKeys.employees(filters), queryFn: () => hrService.listEmployees(filters), enabled: Boolean(document) });
  ...
  if (configuration.error || query.error) return <PageShell title="Employees" description="Tenant-safe employee lifecycle records."><GovernedError error={configuration.error ?? query.error} retry={() => { void configuration.refetch(); void query.refetch(); }} resource="Employees" /></PageShell>;
  ...
  return <PageShell title="Employees" description="Search staff, inspect reporting relationships, and govern lifecycle changes."
    ...
    <section aria-label="Employee filters" className="grid gap-3 rounded-xl border bg-card p-4 sm:grid-cols-3">
      ...
      <select aria-label="Employment status" className="min-h-11 rounded-md border bg-background px-3" value={filters.employment_status ?? ''} onChange={(event) => update('status', event.target.value)}><option value="">All statuses</option>{document!.allowed_values.employment_statuses.map((item) => <option key={item}>{item}</option>)}</select>
