...
export function PageSkeleton({ cards = 3, rows = 5 }: { cards?: number; rows?: number }) {
  return (
    <div
      ...
      className="mx-auto max-w-[1500px] animate-pulse space-y-6 p-4 sm:p-8"
    >
      ...
      <div className="rounded-xl border p-4">
        {Array.from({ length: rows }, (_, index) => (
          <div key={index} className="mb-3 h-12 rounded bg-muted last:mb-0" />
        ))}
      </div>
...
export function GovernedError({
  ...
  const title = notFound
    ...
    : denied
      ? "Access denied"
      : "Human Resources is unavailable";
  const message = notFound
    ? `This ${resource.toLowerCase()} may have been archived or is outside your tenant.`
    : denied
      ? `Your access policy does not allow this ${resource.toLowerCase()}. Ask an administrator if you need access.`
    ...
    <Card>
      ...
      >
        ...
        {normalized?.correlationId ? (
          <p className="mt-3 rounded bg-muted px-3 py-2 font-mono text-xs">
            Correlation ID: {normalized.correlationId}
          </p>
        ) : null}
        {retry && !notFound && !denied ? (
          <Button type="button" variant="outline" onClick={retry} className="mt-5 min-h-11">
  ...
  return (
    <Card>
      <CardContent className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
        ...
        {action ? (
          <Button type="button" className="mt-5 min-h-11" onClick={action.onClick}>
            {action.label}
          </Button>
        ) : null}
      </CardContent>
...
export function StatusChip({ status }: { status: string }) {
  const tone = ["active", "present", "approved", "ready", "healthy"].includes(status)
    ? "border-primary/30 bg-primary/10 text-primary"
    : ["pending", "late", "on_leave", "half_day"].includes(status)
      ? "border-accent bg-accent/50 text-accent-foreground"
      : ["rejected", "terminated", "not_ready", "unhealthy"].includes(status)
...
export function Pagination({
  ...
  return (
    <nav aria-label="Pagination" className="flex items-center justify-between border-t px-4 py-4">
      <Button
        variant="outline"
        disabled={page <= 1}
...
export function DetailGrid({ children }: { children: ReactNode }) {
  return <dl className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{children}</dl>;
}
