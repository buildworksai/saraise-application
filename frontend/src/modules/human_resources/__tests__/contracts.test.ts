...
describe("Human Resources contracts", () => {
  it("constructs every action beneath the canonical v2 prefix", () => {
    ...
    const paths = [
      ...
      ENDPOINTS.EMPLOYEES.REPORTING_TREE(id),
      ENDPOINTS.EMPLOYEES.ACTIVATE(id),
      ENDPOINTS.EMPLOYEES.DEACTIVATE(id),
      ENDPOINTS.EMPLOYEES.PLACE_ON_LEAVE(id),
      ENDPOINTS.EMPLOYEES.RETURN_FROM_LEAVE(id),
      ENDPOINTS.EMPLOYEES.TERMINATE(id),
      ENDPOINTS.ATTENDANCES.CLOCK_IN,
      ENDPOINTS.ATTENDANCES.CLOCK_OUT(id),
      ENDPOINTS.LEAVE_BALANCES.DETAIL(id),
  ...
  it("serializes only defined query values", () => {
    expect(
      withQuery(ENDPOINTS.EMPLOYEES.LIST, {
        page: 2,
        search: "Ada Lovelace",
    ...
    ).toContain("search=Ada+Lovelace");
    expect(withQuery(ENDPOINTS.EMPLOYEES.LIST, { department: undefined })).toBe(
  ...
  it("strictly recognizes governed envelopes", () => {
    ...
    expect(isApiPageSuccess({ results: [] })).toBe(false);
  });
