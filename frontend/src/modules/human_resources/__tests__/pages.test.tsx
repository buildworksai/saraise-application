...
import type { Employee, HumanResourcesConfiguration } from "../contracts";
...
describe("Human Resources pages", () => {
  ...
  it("renders a layout-preserving skeleton then a permission-aware empty state", async () => {
    vi.spyOn(hrService, "listEmployees").mockResolvedValue({
      items: [],
      pagination,
      correlationId: "corr-1",
      capabilities: [],
    });
    renderAt(<EmployeeListPage />, ROUTES.EMPLOYEES);
  ...
  it("shows a create action only with an explicit capability", async () => {
    vi.spyOn(hrService, "listEmployees").mockResolvedValue({
      ...
      pagination,
      correlationId: "corr-1",
      capabilities: ["hr.employee:create"],
    });
    renderAt(<EmployeeListPage />, ROUTES.EMPLOYEES);
  ...
  it("shows governed correlation IDs and supports retry", async () => {
    const list = vi
      ...
      .mockRejectedValueOnce(
        new HrApiError("Unavailable", "unavailable", 503, "capability_unavailable", "corr-recovery")
      )
      .mockResolvedValueOnce({ items: [], pagination, correlationId: "corr-2", capabilities: [] });
    renderAt(<EmployeeListPage />, ROUTES.EMPLOYEES);
    expect(await screen.findByText(/corr-recovery/)).toBeInTheDocument();
  ...
  it("offers lifecycle actions only when the transition capability is returned", async () => {
    vi.spyOn(hrService, "getEmployee").mockResolvedValue({
      data: employee,
      correlationId: "corr-1",
      capabilities: ["hr.employee:transition"],
    });
    vi.spyOn(hrService, "getReportingTree").mockResolvedValue({
      data: {
        id: employee.id,
    ...
    vi.spyOn(hrService, "listLeaveBalances").mockResolvedValue({
      items: [],
      pagination,
      correlationId: "corr-1",
      capabilities: [],
    ...
    renderAt(
      <EmployeeDetailPage />,
      ROUTES.EMPLOYEE_DETAIL(employee.id),
      `${ROUTES.EMPLOYEES}/:id`
    );
    expect(await screen.findByRole("button", { name: "place on leave" })).toBeInTheDocument();
    await userEvent.click(screen.getByRole("button", { name: "place on leave" }));
    expect(screen.getByRole("dialog")).toHaveTextContent("place on leave employee");
  });
});
