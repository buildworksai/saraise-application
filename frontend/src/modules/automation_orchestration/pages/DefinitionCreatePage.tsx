...
export function DefinitionCreatePage() {
  ...
  function submit(event: React.FormEvent) {
    ...
    const schema = z.object({
      ...
      maxParallel: z.coerce
        .number()
        .int()
        .min(limits.parallel_tasks_min)
        .max(limits.parallel_tasks_max),
      timeout: z.coerce
        .number()
        .int()
        .min(limits.timeout_seconds_min)
        .max(limits.timeout_seconds_max),
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        description="Start with safe defaults, then compose and validate the graph in the visual builder."
      />
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <Textarea
              ...
              placeholder="What business-safe technical outcome does this DAG produce?"
            />
            <p className="text-xs text-muted-foreground">
              Input starts as a closed JSON object schema. Add fields in the graph builder before
              publication.
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Input
              ...
              type="number"
              min={limits.attempts_min}
              max={limits.attempts_max}
              value={attempts}
        ...
        {mutation.error ? (
          <div
            role="alert"
            className="rounded-lg border border-destructive/50 bg-destructive/5 p-4 text-sm text-destructive lg:col-span-2"
          >
            Creation failed: {mutation.error.message}. Review the fields and try again.
          </div>
        ) : null}
        <div className="flex flex-wrap justify-end gap-3 lg:col-span-2">
          <Button
            ...
            variant="ghost"
            onClick={() => {
              if (!dirty || window.confirm("Discard this unsaved definition?"))
                navigate(ROUTE_PATHS.DEFINITIONS);
            }}
          >
