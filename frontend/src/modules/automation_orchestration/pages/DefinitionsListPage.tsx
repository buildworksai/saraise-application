...
export function DefinitionsListPage() {
  ...
  if (!result)
    return (
      <LoadError
        error={new Error("No definition response was received.")}
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button onClick={() => navigate(ROUTE_PATHS.DEFINITION_CREATE)}>
      ...
      />
      <section
        aria-label="Definition filters"
        className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-[minmax(220px,1fr)_180px_140px_190px]"
      >
        ...
        <select
          ...
          value={status}
          onChange={(event) => {
            setStatus(event.target.value as DefinitionStatus | "");
            setPage(1);
          }}
        ...
        >
          ...
          <option value="version">Lowest version</option>
        </select>
      </section>
      {result.items.length === 0 ? (
        hasFilters ? (
          <EmptyPanel
            ...
            description="Adjust or clear the filters to see your orchestration catalog."
            action={
              <Button
                variant="outline"
                onClick={() => {
          ...
          <EmptyPanel
            title="Build your first reliable automation"
            description="Orchestrations connect machine-executed steps into observable, retry-safe dependency graphs."
            action={
              <Button onClick={() => navigate(ROUTE_PATHS.DEFINITION_CREATE)}>
                Create orchestration
              </Button>
                  ...
                  <tr key={definition.id} className="hover:bg-muted/30">
                    <td className="px-4 py-4">
                      <Link
                        ...
                        to={ROUTE_PATHS.DEFINITION_DETAIL(definition.id)}
                      >
                        {definition.name}
                      </Link>
                    ...
                    <td className="px-4 py-4">{definition.node_count}</td>
                    <td className="px-4 py-4">{definition.schedule_count}</td>
                    <td className="px-4 py-4">{formatDate(definition.last_run_at)}</td>
                    <td className="px-4 py-4">
                      {definition.success_rate === null
                        ? "No runs"
                        : `${Math.round(definition.success_rate * 100)}%`}
                    </td>
                    <td className="px-4 py-4">
                      <Button
                        variant="ghost"
                        ...
                        onClick={() =>
                          navigate(
                            definition.status === "draft"
                              ? ROUTE_PATHS.DEFINITION_EDIT(definition.id)
                              : ROUTE_PATHS.DEFINITION_DETAIL(definition.id)
                          )
                        }
                      >
                        {definition.status === "draft" ? "Edit graph" : "View"}
                      </Button>
                    </td>
                  </tr>
                ))}
