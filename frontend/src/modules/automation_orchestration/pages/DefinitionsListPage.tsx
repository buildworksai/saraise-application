...
import { useDefinitions } from "../hooks/use-orchestration";
import type { DefinitionStatus } from "../contracts";
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => navigate("/automation-orchestration/definitions/new")}><Plus className="mr-2 h-4 w-4" />Create orchestration</Button>}
      />
      ...
      {result.items.length === 0 ? (
        hasFilters ? <EmptyPanel title="No definitions match" description="Adjust or clear the filters to see your orchestration catalog." action={<Button variant="outline" onClick={() => { setSearch(""); setStatus(""); setVersion(""); }}>Clear filters</Button>} /> : <EmptyPanel title="Build your first reliable automation" description="Orchestrations connect machine-executed steps into observable, retry-safe dependency graphs." action={<Button onClick={() => navigate("/automation-orchestration/definitions/new")}>Create orchestration</Button>} />
      ) : (
              ...
              <tbody className="divide-y">
                {result.items.map((definition) => (
                  <tr key={definition.id} className="hover:bg-muted/30">
                    <td className="px-4 py-4"><Link className="font-medium text-primary hover:underline" to={`/automation-orchestration/definitions/${definition.id}`}>{definition.name}</Link><p className="mt-1 text-xs text-muted-foreground">{definition.key}</p></td>
                    ...
                    <td className="px-4 py-4"><Button variant="ghost" size="sm" onClick={() => navigate(definition.status === "draft" ? `/automation-orchestration/definitions/${definition.id}/edit` : `/automation-orchestration/definitions/${definition.id}`)}>{definition.status === "draft" ? "Edit graph" : "View"}</Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
