...
export function DefinitionEditPage() {
  ...
  const invalidate = () =>
    void queryClient.invalidateQueries({ queryKey: orchestrationKeys.definition(id) });
  const addNode = useMutation({
    mutationFn: (descriptor: NodeDescriptorDTO) =>
      service.createNode(id, {
        key: `${descriptor.key.replace(/[^a-z0-9]+/g, "-")}-${Date.now().toString().slice(-5)}`,
        name: descriptor.display_name,
        node_type:
          descriptor.source_module === "automation_orchestration" ? "internal" : "extension",
  ...
  const removeNode = useMutation({
    mutationFn: (nodeId: string) => service.deleteNode(nodeId),
    onSuccess: () => {
      setSelectedNode("");
      invalidate();
    },
  ...
  const updateNode = useMutation({
    mutationFn: () => {
      ...
      return service.updateNode(selected.id, {
        timeout_seconds: Number(nodeTimeout),
        max_attempts: Number(nodeAttempts),
  ...
  const validate = useMutation({
    mutationFn: () => service.validateDefinition(id),
  ...
  if (configurationQuery.error || !configurationQuery.data)
    return (
      <LoadError
        error={configurationQuery.error ?? new Error("Runtime configuration unavailable.")}
  ...
  return (
    <main className="space-y-5">
      <PageHeader
        eyebrow={`${definition.key} · draft v${definition.version}`}
        title="Graph builder"
        description="Compose the DAG with the palette or keyboard controls, then validate against the authoritative server contract."
        actions={
          <>
            <Button
              variant="outline"
            ...
            <Button onClick={() => navigate(ROUTE_PATHS.DEFINITION_DETAIL(id))}>
      ...
      <div className="grid min-h-[620px] gap-4 xl:grid-cols-[280px_minmax(0,1fr)_300px]">
        <aside aria-label="Node palette" className="rounded-xl border bg-card p-4">
          <h2 className="font-semibold">Node palette</h2>
          <p className="mt-1 text-xs text-muted-foreground">
            Drag to the canvas or choose Add for keyboard access.
          </p>
          {catalogQuery.isLoading ? (
            <p className="mt-5 text-sm">Loading accessible node types…</p>
          ) : catalogQuery.error ? (
            ...
            <p className="mt-5 text-sm text-muted-foreground">
              No entitled node types are currently available.
            </p>
          ) : (
                ...
                <section key={category}>
                  ...
                  <div className="space-y-2">
                    {descriptors.map((descriptor) => (
                      <div
                        key={descriptor.key}
                        draggable={descriptor.availability === "available"}
                        onDragStart={(event) =>
                      ...
                      >
                        <div className="flex justify-between gap-2">
                          ...
                          <Button
                            aria-label={`Add ${descriptor.display_name}`}
                            size="icon"
                            variant="ghost"
                            disabled={descriptor.availability !== "available" || addNode.isPending}
                            onClick={() => addNode.mutate(descriptor)}
                          >
                        ...
                        {descriptor.availability !== "available" ? (
                          <p className="mt-2 text-xs text-secondary-foreground">
                            {descriptor.availability_reason ?? descriptor.availability}
                          </p>
                        ) : null}
                      </div>
                    ))}
        ...
        <section
          ...
          onDrop={(event) => {
            ...
            const descriptor = catalogQuery.data?.find((item) => item.key === key);
            if (descriptor?.availability === "available") addNode.mutate(descriptor);
          }}
          className="space-y-3"
        >
          <div className="flex items-center justify-between rounded-lg border bg-card px-3 py-2">
            <span className="text-xs text-muted-foreground">
              {pending ? "Saving graph change…" : "All server changes saved"}
            </span>
            <div className="flex items-center gap-1">
              <Button
                ...
                aria-label="Zoom out"
                onClick={() =>
                  setZoom((value) => Math.max(visual.zoom_min, value - visual.zoom_step))
                }
              >
              ...
              <Button
                ...
                aria-label="Zoom in"
                onClick={() =>
                  setZoom((value) => Math.min(visual.zoom_max, value + visual.zoom_step))
                }
              >
          ...
          <div
            style={{
              transform: `scale(${zoom / visual.zoom_default})`,
              transformOrigin: "top left",
              width: `${(visual.zoom_default * visual.zoom_default) / zoom}%`,
            }}
          >
            <Topology
        ...
        <aside aria-label="Node configuration" className="rounded-xl border bg-card p-4">
          <h2 className="font-semibold">Configuration</h2>
          {selected ? (
            <div className="mt-5 space-y-4">
              <div>
                <p className="font-medium">{selected.name}</p>
                <p className="text-xs text-muted-foreground">{selected.handler_key}</p>
              </div>
              <Input label="Node key" value={selected.key} readOnly />
              <Input
                label="Timeout"
                title="Execution deadline constrained by tenant policy."
                type="number"
                min={configurationQuery.data.document.limits.timeout_seconds_min}
                max={configurationQuery.data.document.limits.timeout_seconds_max}
                value={nodeTimeout}
                onChange={(event) => setNodeTimeout(event.target.value)}
              />
              <Input
                label="Maximum attempts"
                title="Includes the initial attempt and all automatic retries."
                type="number"
                min={configurationQuery.data.document.limits.attempts_min}
                max={configurationQuery.data.document.limits.attempts_max}
                value={nodeAttempts}
                onChange={(event) => setNodeAttempts(event.target.value)}
              />
              <Textarea
                aria-label="Node configuration JSON"
                value={nodeConfig}
                onChange={(event) => setNodeConfig(event.target.value)}
              />
              <pre className="max-h-32 overflow-auto rounded bg-muted p-2 text-xs">
                {JSON.stringify(
                  catalogQuery.data?.find((item) => item.key === selected.handler_key)
              ...
              <Button
                ...
                onClick={() => {
                  if (window.confirm(`Remove ${selected.name}? Connected edges are protected.`))
                    removeNode.mutate(selected.id);
                }}
              >
            ...
            <p className="mt-5 text-sm text-muted-foreground">
              Select a node to inspect its retry, timeout, mapping, and schema configuration.
            </p>
          )}
        </aside>
      </div>
      {validationOpen && validate.data ? (
        <Card className={validate.data.valid ? "border-primary/40" : "border-destructive/40"}>
          <CardHeader className="flex-row items-center justify-between">
            <CardTitle>
              {validate.data.valid ? "Graph is publishable" : "Graph needs attention"}
            </CardTitle>
            <Button
              variant="ghost"
              size="icon"
          ...
          <CardContent>
            {validate.data.issues.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                All handlers, dependencies, schemas, and limits passed validation.
              </p>
            ) : (
              <ul className="space-y-2">
                {validate.data.issues.map((issue) => (
                  ...
                  >
                    ...
                    <strong className="ml-2">{issue.code}</strong>
                    <p className="mt-1">{issue.message}</p>
                    {issue.remediation ? (
                      <p className="mt-1 text-muted-foreground">{issue.remediation}</p>
                    ) : null}
                  </li>
                ))}
              </ul>
