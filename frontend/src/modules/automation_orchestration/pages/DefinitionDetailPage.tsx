...
import { useDefinition, useRuns, useRuntimeConfiguration, useSchedules, orchestrationKeys } from "../hooks/use-orchestration";
...
export function DefinitionDetailPage() {
  ...
  const schedulesQuery = useSchedules({ definition_id: id, page_size: configuration?.ui.definition_detail_page_size });
  const runsQuery = useRuns({ definition_id: id, page_size: configuration?.ui.definition_detail_page_size, ordering: "-created_at" });
  ...
  const clone = useMutation({ mutationFn: () => service.cloneDefinition(id), onSuccess: (next) => navigate(ROUTE_PATHS.DEFINITION_EDIT(next.id)) });
  const retire = useMutation({ mutationFn: () => service.retireDefinition(id, crypto.randomUUID()), onSuccess: refresh });
  const run = useMutation({ mutationFn: () => service.startRun({ definition_id: id, input: {}, idempotency_key: idempotencyKey, trigger_type: "manual" }), onSuccess: (created) => navigate(ROUTE_PATHS.RUN_DETAIL(created.id)) });

  if (configurationQuery.isLoading || definitionQuery.isLoading) return <PageSkeleton rows={configuration?.ui.skeleton_rows} />;
  if (configurationQuery.error || definitionQuery.error || !configuration) return <LoadError error={configurationQuery.error ?? definitionQuery.error ?? new Error("Runtime configuration is unavailable.")} retry={() => { void configurationQuery.refetch(); void definitionQuery.refetch(); }} />;
