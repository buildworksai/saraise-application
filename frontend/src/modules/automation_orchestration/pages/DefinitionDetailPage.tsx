...
export function DefinitionDetailPage() {
  ...
  const publish = useMutation({
    mutationFn: () => service.publishDefinition(id, crypto.randomUUID()),
    onSuccess: refresh,
  });
  const clone = useMutation({
    mutationFn: () => service.cloneDefinition(id),
    onSuccess: (next) => navigate(ROUTE_PATHS.DEFINITION_EDIT(next.id)),
  });
  const retire = useMutation({
    mutationFn: () => service.retireDefinition(id, crypto.randomUUID()),
    onSuccess: refresh,
  });
    ...
    return (
      <LoadError
        error={
          ...
          new Error("Runtime configuration is unavailable.")
        }
        retry={() => {
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        eyebrow={`${definition.key} · version ${definition.version}`}
        ...
        actions={
          <>
            ...
            {definition.status === "draft" ? (
              <Button
                variant="outline"
                onClick={() => navigate(`/automation-orchestration/definitions/${id}/edit`)}
              >
            ...
            {definition.status === "draft" ? (
              <Button
                onClick={() => {
                  if (window.confirm("Publish this immutable version?")) publish.mutate();
                }}
              ...
              <>
                ...
                <Button
                  ...
                  onClick={() => {
                    if (
                      window.confirm(
                        "Retire this published version? Existing history remains available."
                      )
                    )
                      retire.mutate();
                  }}
      ...
      <section aria-label="Definition metrics" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ["Nodes", definition.nodes.length],
          ["Schedules", schedulesQuery.data?.pagination.count ?? "—"],
          [
            "Recent successes",
            runsQuery.data?.items.filter((item) => item.status === "succeeded").length ?? "—",
          ],
          ["Last run", formatDate(runsQuery.data?.items[0]?.created_at ?? null)],
        ].map(([label, value]) => (
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between">
          ...
          {validation ? (
            <span
              className={validation.valid ? "text-sm text-primary" : "text-sm text-destructive"}
            >
              {validation.valid ? "Validated" : `${validation.issues.length} validation issue(s)`}
            </span>
          ) : (
            <span className="text-sm text-muted-foreground">Not validated in this session</span>
          )}
        </CardHeader>
        <CardContent>
          <Topology nodes={definition.nodes} edges={definition.edges} />
          {validation && !validation.valid ? (
            <ul className="mt-4 space-y-2" aria-label="Validation issues">
        ...
        <Card>
          ...
          <CardContent>
            {schedulesQuery.isLoading ? (
              ...
              <p role="alert">Schedules unavailable.</p>
            ) : schedulesQuery.data?.items.length ? (
              <ul className="divide-y">
                {schedulesQuery.data.items.map((schedule) => (
                  <li key={schedule.id} className="flex justify-between py-3">
                    <span>
                      {schedule.name}
                      <small className="block text-muted-foreground">
                        {schedule.timezone} · {schedule.cron_expression}
                      </small>
                    </span>
              ...
              <p className="text-sm text-muted-foreground">No schedules target this version.</p>
            )}
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            {runsQuery.isLoading ? (
              ...
              <p role="alert">Runs unavailable.</p>
            ) : runsQuery.data?.items.length ? (
              <ul className="divide-y">
                {runsQuery.data.items.map((item) => (
                  <li key={item.id} className="flex justify-between py-3">
                    <Link
                      to={`/automation-orchestration/runs/${item.id}`}
              ...
              <p className="text-sm text-muted-foreground">This version has not run yet.</p>
            )}
          </CardContent>
        </Card>
      </div>
      <Card>
        ...
        <CardContent>
          {definition.transition_history.length ? (
            <ol className="border-l pl-5">
              {definition.transition_history.map((entry) => (
                <li key={`${entry.occurred_at}-${entry.transition}`} className="mb-4">
                  <p className="font-medium">{entry.transition}</p>
                  <p className="text-xs text-muted-foreground">
                    {entry.from} → {entry.to} · {formatDate(entry.occurred_at)}
                  </p>
                </li>
              ))}
            </ol>
      ...
      {showRunDialog ? (
        ...
        >
          <Card className="w-full max-w-lg">
            <CardHeader>
              <CardTitle id="run-title">Execute {definition.name}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                This run uses an empty input object. The backend validates it against the published
                input schema.
              </p>
              <Input
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={() => run.mutate()} disabled={!idempotencyKey || run.isPending}>
                  {run.isPending ? "Starting…" : "Start durable run"}
                </Button>
              </div>
            </CardContent>
          </Card>
