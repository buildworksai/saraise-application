...
export function EdgeEditor({
  ...
  const create = useMutation({
    mutationFn: () =>
      service.createEdge(definitionId, {
        upstream_node_id: selectedNodeId,
        downstream_node_id: downstreamId,
        condition,
      }),
    onSuccess: () => {
      setDownstreamId("");
  ...
  const remove = useMutation({
    mutationFn: (edgeId: string) => service.deleteEdge(edgeId),
    onSuccess: onChanged,
  });
      ...
      <CardContent className="grid gap-5 lg:grid-cols-2">
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Create an explicit conditional dependency from the selected node.
          </p>
          {!selected ? (
            <p className="rounded border border-secondary p-3 text-sm text-secondary-foreground">
              Select an upstream node on the canvas first.
            </p>
          ) : (
            <div className="grid gap-3 sm:grid-cols-[1fr_180px_auto]">
              <select
                aria-label="Downstream node"
                value={downstreamId}
                onChange={(event) => setDownstreamId(event.target.value)}
                className="h-10 rounded-md border bg-background px-3 text-sm"
              >
                <option value="">Choose downstream node</option>
                {nodes
              ...
              <select
                ...
                value={condition}
                onChange={(event) => setCondition(event.target.value as EdgeCondition)}
                className="h-10 rounded-md border bg-background px-3 text-sm"
              >
              ...
              <Button disabled={!downstreamId || create.isPending} onClick={() => create.mutate()}>
        ...
        <div>
          <p className="text-sm font-medium">Outgoing from {selected?.name ?? "selected node"}</p>
          {outgoing.length === 0 ? (
            <p className="mt-2 text-sm text-muted-foreground">No outgoing edges.</p>
          ) : (
            <ul className="mt-2 divide-y">
              {outgoing.map((edge) => {
                const downstream = nodes.find((node) => node.id === edge.downstream_node_id);
                return (
                  <li key={edge.id} className="flex items-center justify-between py-2 text-sm">
                    <span>
                      {downstream?.name ?? edge.downstream_node_id}
