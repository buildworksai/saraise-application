...
export function nextCronRuns(
  ...
  for (let checked = 0; checked < searchHorizonMinutes && runs.length < count; checked += 1) {
    ...
    if (
      dow !== undefined &&
      matchesPart(minute, Number(find("minute")), 0, 59) &&
      matchesPart(hour, Number(find("hour")), 0, 23) &&
      matchesPart(day, Number(find("day")), 1, 31) &&
      matchesPart(month, Number(find("month")), 1, 12) &&
      matchesPart(weekday, dow, 0, 6)
    )
...
export function CronPreview({
  ...
  const preview = useMemo(() => {
    try {
      return nextCronRuns(expression, timezone, count, searchHorizonDays);
    } catch {
      return [];
    }
