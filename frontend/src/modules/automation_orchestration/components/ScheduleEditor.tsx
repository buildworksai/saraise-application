...
export function ScheduleEditor({ scheduleId }: { scheduleId?: string }) {
  ...
  useEffect(() => {
    ...
    if (!value) return;
    setName(value.name);
    setDefinitionId(value.definition_id);
    setCron(value.cron_expression);
    setTimezone(value.timezone);
    setMisfire(value.misfire_policy);
    setConcurrency(value.concurrency_policy);
  }, [existingQuery.data]);
  ...
  if (error || !configuration)
    return (
      <LoadError
        error={error ?? new Error("Runtime configuration is unavailable.")}
        retry={() => {
          void configurationQuery.refetch();
          void definitionsQuery.refetch();
          if (editing) void existingQuery.refetch();
        }}
      />
    );
  ...
  function submit(event: React.FormEvent) {
    ...
    const formSchema = z.object({
      name: z
        .string()
        .trim()
        .min(configuration.limits.schedule_name_min)
        .max(configuration.limits.schedule_name_max),
      definitionId: z.string().uuid("Choose a published definition"),
      cron: z
        .string()
        .trim()
        .refine(
          (value) => value.split(/\s+/).length === configuration.scheduler.cron_fields,
          `Use a ${configuration.scheduler.cron_fields}-field cron expression`
        ),
    ...
    const previewValid = (() => {
      try {
        return (
          nextCronRuns(cron, timezone, 1, configuration.scheduler.search_horizon_days).length === 1
        );
      } catch {
        return false;
      }
    ...
    if (!result.success || !previewValid) {
      ...
      if (!previewValid)
        next.cron = "Cron expression or timezone cannot produce a valid upcoming run";
      setErrors(next);
      return;
    }
    setErrors({});
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        description="Preview the exact cadence before attaching it to an immutable published definition version."
      />
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
        <Card>
          ...
          <CardContent className="space-y-5">
            ...
            <label className="block text-sm font-medium">
              ...
              <select
                aria-label="Published definition"
                disabled={editing}
                value={definitionId}
              ...
              {errors.definitionId ? (
                <span className="mt-1 block text-sm text-destructive">{errors.definitionId}</span>
              ) : null}
            </label>
            {definitions.length === 0 ? (
              <p
                role="status"
                className="rounded border border-secondary p-3 text-sm text-secondary-foreground"
              >
                Publish a validated definition before creating a schedule.
              </p>
            ) : null}
            <Input
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="text-sm font-medium">
                ...
                <select
                  value={misfire}
                  onChange={(event) => setMisfire(event.target.value as MisfirePolicy)}
                  className="mt-1 block h-10 w-full rounded-md border bg-background px-3"
                >
              ...
              <label className="text-sm font-medium">
                ...
                <select
                  value={concurrency}
                  onChange={(event) => setConcurrency(event.target.value as ConcurrencyPolicy)}
                  className="mt-1 block h-10 w-full rounded-md border bg-background px-3"
                >
                  <option value="forbid">Forbid overlap</option>
        ...
        <div className="space-y-5">
          <CronPreview
            expression={cron}
            timezone={timezone}
            count={configuration.ui.cron_preview_count}
            searchHorizonDays={configuration.scheduler.search_horizon_days}
          />
          <Card>
            <CardContent className="space-y-2 p-5 text-sm">
              <p className="font-medium">Execution safeguards</p>
              <p className="text-muted-foreground">
                Misfires are applied deterministically. “Forbid overlap” will not enqueue while this
                schedule already has a nonterminal run.
              </p>
            </CardContent>
          </Card>
        </div>
        {mutation.error ? (
          <p
            role="alert"
            className="rounded border border-destructive/40 p-3 text-sm text-destructive lg:col-span-2"
          >
            {mutation.error.message}
          </p>
        ) : null}
        <div className="flex justify-end gap-3 lg:col-span-2">
          <Button type="button" variant="ghost" onClick={() => navigate(ROUTE_PATHS.SCHEDULES)}>
            Cancel
          </Button>
          <Button type="submit" disabled={mutation.isPending || definitions.length === 0}>
            {mutation.isPending ? "Saving…" : editing ? "Save schedule" : "Create schedule"}
          </Button>
        </div>
      </form>
    </main>
