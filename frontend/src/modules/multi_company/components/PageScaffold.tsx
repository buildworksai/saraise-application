...
export function PageScaffold({
  ...
  return (
    <main className="mx-auto w-full max-w-screen-2xl space-y-6 p-4 sm:p-6 lg:p-8">
...
export function LoadingPage({ title = "Loading records" }: { title?: string }) {
  return (
    <div aria-busy="true" aria-label={title} className="p-4 sm:p-8">
  ...
  if (apiError?.status === 403)
    return (
      <StateCard
        ...
        message="Your current company scope does not allow this operation. Ask an administrator for the required grant."
      />
    );
  if (apiError?.status === 404)
    return (
      <StateCard
        ...
        message="This record does not exist in your tenant, or is no longer available."
      />
    );
  const support = apiError?.correlationId ? ` Support reference: ${apiError.correlationId}.` : "";
  return (
    <ErrorState
      message={`${apiError?.message ?? "The request could not be completed."}${support}`}
      onRetry={onRetry}
    />
  );
}
  ...
  return (
    <Card>
      <CardContent className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
        ...
        <p className="mt-2 max-w-lg text-muted-foreground">{message}</p>
      </CardContent>
    </Card>
  );
}
...
export function StatusPill({ value }: { value: string }) {
  return (
    <span className="inline-flex rounded-full border border-border bg-muted px-2.5 py-1 text-xs font-medium capitalize text-foreground">
      {value.replaceAll("_", " ")}
    </span>
  );
}
