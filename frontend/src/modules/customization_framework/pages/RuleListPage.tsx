...
export function RuleListPage() {
  ...
  const ordering = (params.get("ordering") ??
    configuration.data?.document.list_preferences.rule_ordering ??
    "") as RuleOrdering | "";
  ...
  if (!query.data || !configuration.data)
    return <GovernedError error={new Error("No governed rule response was received.")} />;
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        description="Build bounded, deterministic validation and presentation rules—never tenant-authored code."
        actions={
      ...
      <section
        ...
        className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-[1fr_170px_190px_190px]"
      >
        ...
        >
          ...
          {["draft", "published", "paused", "retired"].map((item) => (
            <option key={item}>{item}</option>
          ))}
        </select>
        <select
          aria-label="Filter rule trigger"
          className="rounded-md border bg-background px-3"
          value={trigger}
          onChange={(event) => update("trigger", event.target.value)}
        >
          <option value="">All triggers</option>
          {runtime.policies.rule_triggers.map((item) => (
            <option key={item}>{item}</option>
          ))}
        </select>
        <select
          aria-label="Order rules"
          className="rounded-md border bg-background px-3"
          value={ordering}
      ...
      {query.data.data.length === 0 ? (
        <EmptyPanel filtered={filtered} noun="rules" create={() => navigate(ROUTES.RULE_CREATE)} />
      ) : (
                  ...
                  <tr key={rule.id}>
                    <td className="px-4 py-4">
                      <button
                        ...
                        onClick={() => navigate(ROUTES.RULE_DETAIL(rule.id))}
                      >
                        {rule.name}
                      </button>
                    ...
                    <td className="px-4 py-4">
                      {rule.owner_module} · {rule.target_resource}
                    </td>
                    <td className="px-4 py-4">{rule.trigger.replaceAll("_", " ")}</td>
                    <td className="px-4 py-4">{rule.priority}</td>
                    <td className="px-4 py-4">
                      <StatusChip status={rule.status} />
                    </td>
                    <td className="px-4 py-4">{rule.diagnostic_count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
