...
import { ROUTES, type FormOrdering, type FormStatus } from "../contracts";
...
export function FormListPage() {
  ...
  const status = (params.get("status") ?? "") as FormStatus | "";
  const ordering = (params.get("ordering") ??
    configuration.data?.document.list_preferences.form_ordering ??
    "") as FormOrdering | "";
  ...
  const query = useQuery({
    queryKey: ["customization", "forms", search, status, ordering, page, pageSize],
    queryFn: () =>
      service.listForms({
  ...
  if (!query.data)
    return <GovernedError error={new Error("No governed form response was received.")} />;
  const filtered = Boolean(search || status);
  return (
    <main className="space-y-6">
      <PageHeader
        title="Forms"
        description="Design accessible, versioned layouts over registered resource contracts."
        actions={
          <Button onClick={() => navigate(ROUTES.FORM_CREATE)}>
      ...
      />
      <section
        aria-label="Form filters"
        className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-[1fr_180px_200px]"
      >
        ...
        >
          ...
          <option value="status">Publication state</option>
        </select>
      </section>
      {query.data.data.length === 0 ? (
        <EmptyPanel filtered={filtered} noun="forms" create={() => navigate(ROUTES.FORM_CREATE)} />
      ) : (
        <section className="overflow-hidden rounded-xl border bg-card">
          <div className="grid gap-4 p-4 md:grid-cols-2 xl:grid-cols-3">
            {query.data.data.map((form) => (
              <button
                key={form.id}
                className="rounded-lg border p-4 text-left transition hover:border-primary hover:shadow-sm"
                onClick={() => navigate(ROUTES.FORM_DETAIL(form.id))}
              >
                <div className="flex justify-between gap-3">
                  <h2 className="font-semibold">{form.name}</h2>
                  <StatusChip status={form.status} />
                </div>
                <p className="mt-2 font-mono text-xs text-muted-foreground">{form.key}</p>
                <p className="mt-4 text-sm">
                  {form.owner_module} · {form.target_resource}
                </p>
                <p className="mt-3 text-xs text-muted-foreground">
                  {form.published_version
                    ? `Published v${form.published_version}`
                    : "No published version"}{" "}
                  · {form.dependency_count} dependencies
                </p>
              </button>
            ))}
          </div>
