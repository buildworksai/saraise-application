...
const ACTIONS: readonly RuleActionType[] = [
  "reject-with-message",
  "set-derived-value",
  "set-required",
  "set-visible",
...
export function EditRulePage() {
  ...
  const save = useMutation({
    mutationFn: () =>
      service.createRuleVersion(id, {
        condition_ast: condition,
        action_ast: actions,
        change_summary: summary,
      }),
    onSuccess: (response) => {
      setPublishId(response.data.id);
      void client.invalidateQueries({ queryKey: ["customization", "rule-versions", id] });
    },
  });
  const publish = useMutation({
    mutationFn: () =>
      service.publishRule(id, { version_id: publishId, transition_key: crypto.randomUUID() }),
    onSuccess: () => navigate(ROUTES.RULE_DETAIL(id)),
  });
  const evaluate = useMutation({
    mutationFn: () =>
      service.evaluateRule(id, {
        record: { [field]: parsePrimitive(value) },
  ...
  function addAction() {
    ...
    setActions([
      ...actions,
      {
        ...
        value: actionType === "set-derived-value" ? parsePrimitive(actionMessage) : undefined,
      },
    ]);
    setActionMessage("");
  }
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title={`${rule.data.data.name} builder`}
        description="Candidate versions use bounded operators and actions. Free-form code is never accepted."
      />
      <div className="grid gap-6 lg:grid-cols-2">
        <Surface>
          <h2 className="font-semibold">When this condition matches</h2>
        ...
        <Surface>
          <h2 className="font-semibold">Apply allowlisted actions</h2>
          <div className="mt-4 grid gap-3">
            <select
              aria-label="Action type"
              className="h-10 rounded-md border bg-background px-3"
              value={actionType}
              onChange={(event) => setActionType(event.target.value as RuleActionType)}
            >
              {ACTIONS.map((item) => (
                <option key={item}>{item}</option>
              ))}
            ...
            />
            <Input
              aria-label="Action value or message"
              value={actionMessage}
          ...
          <ul className="mt-4 space-y-2">
            {actions.map((action, index) => (
              ...
              >
                <span>
                  {action.type} · {action.field ?? action.message}
                </span>
                <Button
                  size="sm"
                  variant="ghost"
      ...
      <Surface>
        <h2 className="font-semibold">Deterministic test console</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          The published rule is evaluated against a redacted sample; immutable evidence records the
          outcome.
        </p>
        <Button
          className="mt-4"
          variant="outline"
          disabled={!field || evaluate.isPending}
          onClick={() => evaluate.mutate()}
        >
          <Play className="mr-2 h-4 w-4" />
          {evaluate.isPending ? "Evaluating…" : "Evaluate sample"}
        </Button>
        {result ? (
          <div className="mt-4 rounded-lg border p-3 text-sm">
            <StatusChip status={result.status} />
            <span className="ml-3">
              {result.duration_ms} ms · correlation {result.correlation_id}
            </span>
            <ul className="mt-3">
      ...
      <Surface>
        <h2 className="font-semibold">Version diff and impact</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          Compared with{" "}
          {versions.data?.data[0] ? `version ${versions.data.data[0].version}` : "an empty rule"}.
          New condition: {field || "not configured"}; {actions.length} actions.
        </p>
        <Input
          className="mt-4"
        ...
        <div className="mt-4 flex justify-end gap-3">
          <Button variant="outline" onClick={() => navigate(ROUTES.RULE_DETAIL(id))}>
            Cancel
          </Button>
          <Button
            disabled={!field || !actions.length || !summary.trim() || save.isPending}
            onClick={() => save.mutate()}
          >
            {save.isPending ? "Validating…" : "Save candidate version"}
          </Button>
        </div>
      </Surface>
      {save.error ? <GovernedError error={save.error} /> : null}
      {evaluate.error ? <GovernedError error={evaluate.error} /> : null}
      <ConfirmAction
        open={Boolean(publishId)}
        title="Publish validated rule?"
        description="Publication atomically supersedes the prior version. Evidence remains immutable."
