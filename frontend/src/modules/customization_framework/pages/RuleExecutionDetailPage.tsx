...
export function RuleExecutionDetailPage() {
  ...
  const query = useQuery({
    queryKey: ["customization", "execution", id],
    queryFn: () => service.getExecution(id),
    enabled: Boolean(id),
  });
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title={`Execution ${item.id.slice(0, 8)}`}
        description="Redacted evaluation trace and immutable correlation evidence."
      />
      <Surface>
        <DetailGrid>
          ...
          <Detail label="Rule version">
            <span className="font-mono text-xs">{item.rule_version_id}</span>
          </Detail>
          ...
          <Detail label="Target">
            <span className="font-mono text-xs">{item.target_record_id ?? "Not persisted"}</span>
          </Detail>
        </DetailGrid>
      </Surface>
      <div className="grid gap-6 lg:grid-cols-2">
        <Surface>
          <h2 className="font-semibold">Returned actions and mutations</h2>
          <pre className="mt-3 max-h-96 overflow-auto rounded bg-muted p-3 text-xs">
            {JSON.stringify(item.result, null, 2)}
          </pre>
        </Surface>
        <Surface>
          <h2 className="font-semibold">Matched conditions and diagnostics</h2>
          {item.diagnostics.length ? (
            ...
            <p className="mt-3 text-sm text-muted-foreground">No diagnostics were returned.</p>
          )}
        </Surface>
      </div>
      <p className="text-xs text-muted-foreground">
        Raw input, replay keys, actor identifiers, and secret-bearing payloads are deliberately
        omitted from this least-privilege response.
      </p>
    </main>
  );
}
