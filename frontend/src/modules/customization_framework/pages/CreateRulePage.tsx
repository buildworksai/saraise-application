...
export function CreateRulePage() {
  ...
  const mutation = useMutation({
    mutationFn: (request: RuleCreateRequest) => service.createRule(request),
    onSuccess: (result) => navigate(ROUTES.RULE_EDIT(result.data.id)),
  });
  function submit(event: React.FormEvent) {
    ...
    if (
      ...
      !version ||
      !trigger ||
      !document.policies.rule_triggers.includes(trigger) ||
      !Number.isInteger(numeric) ||
      numeric < document.limits.rule_priority_min ||
      numeric > document.limits.rule_priority_max
    ) {
    ...
    mutation.mutate({
      ...
      trigger,
      priority: numeric,
      stop_on_match: document.defaults.rule_stop_on_match,
    });
  }
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        description="Define rule identity and deterministic order, then compose a candidate version with allowlisted operators and actions."
      />
      <form onSubmit={submit} className="space-y-6">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <TargetContractSelector
              ...
              onSelect={(contract) => {
                ...
                const allowed = contract.rule_triggers.filter((item) =>
                  runtime.policies.rule_triggers.includes(item)
                );
                setTrigger(allowed.includes(trigger as RuleTrigger) ? trigger : allowed[0] ?? "");
              }}
            />
            <Input
              id="rule-name"
            ...
            <label className="grid gap-1 text-sm font-medium">
              ...
              >
                {runtime.policies.rule_triggers.map((value) => (
            ...
            <Input
              ...
              label="Priority"
              type="number"
              min={runtime.limits.rule_priority_min}
              max={runtime.limits.rule_priority_max}
              value={priority}
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => navigate(ROUTES.RULES)}>
            Cancel
          </Button>
          <Button type="submit" disabled={mutation.isPending}>
            {mutation.isPending ? "Creating…" : "Create and build"}
          </Button>
        </div>
      </form>
    </main>
