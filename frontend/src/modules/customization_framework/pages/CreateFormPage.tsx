...
export function CreateFormPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => navigate(ROUTES.FORM_EDIT(result.data.id)),
  });
  function submit(event: React.FormEvent) {
    ...
    if (
      !runtime ||
      !name.trim() ||
      !new RegExp(runtime.policies.slug_pattern, "u").test(key) ||
      !module ||
      !resource ||
      !version
    ) {
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title="Create form"
        description="Start with a registered target contract, then compose the initial layout in the accessible designer."
      />
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" disabled={mutation.isPending}>
            {mutation.isPending ? "Creating…" : "Create and design"}
          </Button>
        </div>
      </form>
    </main>
