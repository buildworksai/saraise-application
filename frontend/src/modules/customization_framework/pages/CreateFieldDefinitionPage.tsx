...
export function CreateFieldDefinitionPage() {
  ...
  const mutation = useMutation({
    mutationFn: (request: FieldDefinitionCreateRequest) => service.createField(request),
    onSuccess: (result) => navigate(ROUTES.FIELD_DETAIL(result.data.id)),
  });
  function request(): FieldDefinitionCreateRequest | null {
    ...
    if (
      !document ||
      !label.trim() ||
      !new RegExp(document.policies.slug_pattern, "u").test(key) ||
      !module ||
      !resource ||
      !version ||
      !dataType ||
      !document.policies.field_types.includes(dataType)
    ) {
    ...
    try {
      ...
      return {
        ...
        data_type: dataType,
        required: document.defaults.field_required,
        searchable: document.defaults.field_searchable,
        validation_schema,
        presentation_schema: { label },
      };
    } catch (caught) {
  ...
  if (!configuration.data)
    return <GovernedError error={new Error("Runtime configuration was not returned.")} />;
  const runtime = configuration.data.document;
  return (
    <main className="space-y-6">
      <PageHeader
        title="Create custom field"
        description="Choose a registered target, define a safe field contract, validate it, then review before creation."
      />
      <nav aria-label="Creation steps" className="flex gap-2">
        {["Target", "Definition", "Review"].map((item, index) => (
          <Button
            key={item}
            type="button"
            variant={step === index + 1 ? "primary" : "outline"}
      ...
      <Surface>
        {step === 1 ? (
          <div className="grid gap-5 sm:grid-cols-2">
            <TargetContractSelector
              value={module ? `${module}/${resource}@${version}` : ""}
              onSelect={(contract) => {
                setModule(contract.module);
                setResource(contract.resource);
                setVersion(contract.version);
                if (!contract.custom_field_types.includes(dataType as FieldDataType))
                  setDataType(contract.custom_field_types[0] ?? "");
              }}
            />
            <p className="sm:col-span-2 text-sm text-muted-foreground">
              Targets are explicitly registered and versioned. Unavailable capabilities remain
              visible without fabricated enablement.
            </p>
          </div>
        ) : null}
        {step === 2 ? (
          <div className="space-y-5">
            <div className="grid gap-5 sm:grid-cols-2">
              <Input
                ...
                label="Label"
                required
                maxLength={runtime.limits.field_label_length}
                value={label}
                onChange={(event) => {
              ...
              <Input
                ...
                label="Stable key"
                required
                maxLength={runtime.limits.field_key_length}
                value={key}
              ...
              <label className="grid gap-1 text-sm font-medium">
                ...
                >
                  {runtime.policies.field_types.map((type) => (
            ...
            <Textarea
              ...
              placeholder="How administrators and users should understand this field"
            />
            <Textarea
            ...
            <p className="text-xs text-muted-foreground">
              Declarative JSON Schema only. Scripts, functions, network calls, and executable
        ...
        {step === 3 ? (
          <div>
            <h2 className="font-semibold">Review contract</h2>
            <pre className="mt-4 overflow-auto rounded-lg bg-muted p-4 text-xs">
              {JSON.stringify({ module, resource, version, key, label, dataType, schema }, null, 2)}
            </pre>
            <p className="mt-4 text-sm text-muted-foreground">
              The lifecycle default is governed by tenant configuration:{" "}
              {runtime.defaults.field_status}.
            </p>
          </div>
        ) : null}
        {error ? (
          <p id="field-form-error" role="alert" className="mt-4 text-sm text-destructive">
            {error}
          </p>
        ) : null}
      </Surface>
      ...
      <div className="flex justify-between">
        <Button variant="ghost" onClick={() => navigate(ROUTES.FIELDS)}>
          Cancel
        </Button>
