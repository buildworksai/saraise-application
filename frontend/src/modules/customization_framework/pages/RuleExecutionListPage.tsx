...
export function RuleExecutionListPage() {
  ...
  const ordering = (params.get("ordering") ??
    configuration.data?.document.list_preferences.execution_ordering ??
    "") as ExecutionOrdering | "";
  ...
  if (!query.data)
    return <GovernedError error={new Error("No governed execution response was received.")} />;
  const filtered = Boolean(status || correlation);
  return (
    <main className="space-y-6">
      <PageHeader
        title="Rule executions"
        description="Immutable, redacted evidence for deterministic rule outcomes. Executions cannot be created, edited, or deleted here."
      />
      <section
      ...
      >
        ...
        >
          ...
          {["matched", "not_matched", "rejected", "failed"].map((item) => (
        ...
        >
          <option value="-executed_at">Newest first</option>
          <option value="executed_at">Oldest first</option>
          <option value="-duration_ms">Slowest first</option>
          <option value="status">Outcome</option>
        </select>
      </section>
      {query.data.data.length === 0 ? (
        <EmptyPanel filtered={filtered} noun="executions" />
      ) : (
                  ...
                  <tr key={item.id} className="hover:bg-muted/30">
                    <td className="px-4 py-4">
                      <button
                        ...
                        onClick={() => navigate(ROUTES.EXECUTION_DETAIL(item.id))}
                      >
                        {item.rule_name}
                      </button>
                    ...
                    <td className="px-4 py-4 font-mono text-xs">
                      {item.target_record_id ?? "No persisted target"}
                    </td>
                    <td className="px-4 py-4">
                      <StatusChip status={item.status} />
                    </td>
                    <td className="px-4 py-4">{item.duration_ms} ms</td>
                    <td className="px-4 py-4">{formatDate(item.executed_at)}</td>
                    <td className="px-4 py-4 font-mono text-xs">{item.correlation_id}</td>
                  </tr>
                ))}
              </tbody>
            </table>
