  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        description="Inspect and maintain tenant-isolated values through their active field contracts."
        actions={
          <Button onClick={() => navigate(ROUTES.FIELD_VALUE_CREATE)}>
      ...
      {query.data.data.length === 0 ? (
        <EmptyPanel
          filtered={false}
          noun="field values"
          create={() => navigate(ROUTES.FIELD_VALUE_CREATE)}
        />
      ) : (
                  ...
                  <tr key={item.id}>
                    <td className="px-4 py-4">
                      <button
                        ...
                        onClick={() => navigate(ROUTES.FIELD_VALUE_DETAIL(item.id))}
                      >
                        {item.definition_key}
                      </button>
                    </td>
                    <td className="px-4 py-4 font-mono text-xs">{item.target_record_id}</td>
...
export function CreateFieldValuePage() {
  ...
  const definitions = useQuery({
    queryKey: ["customization", "active-fields-for-values", pageSize],
    queryFn: () => service.listFields({ status: "active", page_size: pageSize }),
    enabled: pageSize !== undefined,
  });
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => navigate(ROUTES.FIELD_VALUE_DETAIL(result.data.id)),
  });
  function submit(event: React.FormEvent) {
    event.preventDefault();
    try {
      const parsed = parseJSON(value);
      if (!definitionId || !targetRecordId || !source)
        throw new Error("Field, target record, and governed source are required.");
      setError("");
      mutation.mutate({
  ...
  if (definitions.error)
    return <GovernedError error={definitions.error} retry={() => void definitions.refetch()} />;
  if (!configuration.data || !definitions.data)
    return <GovernedError error={new Error("Field-value dependencies were not returned.")} />;
  return (
    <main className="space-y-6">
      <PageHeader
        title="Create custom field value"
        description="Persist a value only against an active tenant field definition."
      />
      <form className="space-y-6" onSubmit={submit}>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <label className="grid gap-1 text-sm font-medium">
              ...
              >
                ...
                {definitions.data.data.map((field) => (
            ...
            <Input
              id="field-value-target"
              label="Target record UUID"
              required
            ...
            <label className="grid gap-1 text-sm font-medium">
              ...
              <select
                ...
                value={source}
                onChange={(event) =>
                  setSource(event.target.value as Exclude<FieldValueSource, "rule">)
                }
              >
                {configuration.data.document.policies.value_sources.map((item) => (
        ...
        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => navigate(ROUTES.FIELD_VALUES)}>
            Cancel
          </Button>
          <Button type="submit" disabled={mutation.isPending}>
            {mutation.isPending ? "Creating…" : "Create value"}
          </Button>
        </div>
      </form>
    </main>
...
export function FieldValueDetailPage() {
  ...
  const query = useQuery({
    queryKey: ["customization", "field-value", id],
    queryFn: () => service.getValue(id),
    enabled: Boolean(id),
  });
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title={item.definition_key}
        description="Tenant-isolated field value and optimistic-lock evidence."
        actions={
          <>
            <Button variant="outline" onClick={() => navigate(ROUTES.FIELD_VALUE_EDIT(id))}>
      ...
      <Surface>
        <DetailGrid>
          <Detail label="Target record">{item.target_record_id}</Detail>
          <Detail label="Source">{item.source}</Detail>
          <Detail label="Definition revision">{item.definition_revision}</Detail>
          <Detail label="Lock version">{item.lock_version}</Detail>
          <Detail label="Updated">{formatDate(item.updated_at)}</Detail>
        </DetailGrid>
        <pre className="mt-5 overflow-auto rounded bg-muted p-4 text-xs">
          {JSON.stringify(item.value, null, 2)}
        </pre>
      </Surface>
      <ConfirmAction
        open={confirmDelete}
        title="Delete this field value?"
        description="Deletion uses the displayed optimistic-lock revision and is audit logged."
        confirmLabel="Delete value"
        pending={remove.isPending}
        onOpenChange={setConfirmDelete}
        onConfirm={() => remove.mutate()}
      />
      {remove.error ? <GovernedError error={remove.error} /> : null}
    </main>
  );
}
...
export function EditFieldValuePage() {
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title={`Edit ${query.data.data.definition_key}`}
        description="Update the value through its server-validated field contract."
      />
      <form className="space-y-6" onSubmit={submit}>
        <Surface>
          <Textarea
        ...
        <div className="flex justify-end gap-3">
          <Button
            type="button"
            variant="outline"
            onClick={() => navigate(ROUTES.FIELD_VALUE_DETAIL(id))}
          >
            Cancel
          </Button>
          <Button type="submit" disabled={mutation.isPending}>
