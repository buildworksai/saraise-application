...
export function FieldDefinitionListPage() {
  ...
  const status = (params.get("status") ?? "") as FieldStatus | "";
  const dataType = (params.get("data_type") ?? "") as FieldDataType | "";
  const configuration = useRuntimeConfiguration();
  const ordering = (params.get("ordering") ??
    configuration.data?.document.list_preferences.field_ordering ??
    "") as FieldOrdering | "";
  const pageSize = configuration.data?.document.list_preferences.page_size;
  const query = useQuery({
    queryKey: ["customization", "fields", page, search, status, dataType, ordering, pageSize],
    queryFn: () =>
      service.listFields({
        page,
        page_size: pageSize,
  ...
  if (!envelope)
    return (
      <GovernedError
        error={new Error("No governed field response was received.")}
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title="Custom fields"
        description="Extend registered resources with tenant-owned, schema-validated fields—without executable code."
        actions={
          <Button onClick={() => navigate(ROUTES.FIELD_CREATE)}>
      ...
      <section
        ...
        className="grid gap-3 rounded-xl border bg-card p-4 md:grid-cols-[minmax(220px,1fr)_180px_180px_200px]"
      >
        ...
        >
          ...
          {["draft", "active", "deprecated", "retired"].map((item) => (
      ...
      {envelope.data.length === 0 ? (
        <EmptyPanel
          filtered={filtered}
          noun="fields"
          create={() => navigate(ROUTES.FIELD_CREATE)}
        />
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-sm">
              ...
              <tbody className="divide-y">
                {envelope.data.map((field) => (
                  <tr key={field.id} className="hover:bg-muted/30">
                    <td className="px-4 py-4">
                      <button
                        className="font-medium text-primary hover:underline"
                        onClick={() => navigate(ROUTES.FIELD_DETAIL(field.id))}
                      >
                        {field.label}
                      </button>
                    ...
                    <td className="px-4 py-4">
                      {field.owner_module} · {field.target_resource}
                    </td>
                    <td className="px-4 py-4">{field.data_type.replaceAll("_", " ")}</td>
                    <td className="px-4 py-4">
                      <StatusChip status={field.status} />
                    </td>
                    <td className="px-4 py-4">{field.dependency_count}</td>
                    <td className="px-4 py-4">{new Date(field.updated_at).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
