...
function portableConfiguration(value: unknown): ConfigurationExportDocument {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new Error("The import must be a configuration object.");
  const object = value as Record<string, unknown>;
  if (
    typeof object.schema !== "string" ||
    typeof object.tenant_id !== "string" ||
    ...
    throw new Error(
      "The import must contain the governed export schema, tenant, version, environment, and document."
    );
  return {
    ...
    document: object.document as RuntimeConfigurationDocument,
  };
}
...
export function RuntimeConfigurationPage() {
  ...
  const versions = useQuery({
    queryKey: ["customization", "configuration-versions"],
    queryFn: service.listConfigurationVersions,
  });
  const audit = useQuery({
    queryKey: ["customization", "configuration-audit"],
    queryFn: service.listConfigurationAudit,
  });
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return imported
        ? service.importConfiguration({
            payload: { ...imported, environment, document: document() },
            expected_version,
          })
    ...
    onSuccess: () => {
      setImported(null);
      setPreviewedDraft("");
      void client.invalidateQueries({ queryKey: ["customization"] });
    },
  });
  const rollback = useMutation({
    mutationFn: (version: RuntimeConfigurationVersion) => {
      if (!current.data) throw new Error("Current configuration is unavailable.");
      return service.rollbackConfiguration({
  ...
  if (!current.data || !versions.data || !audit.data)
    return (
      <GovernedError error={new Error("The governed configuration response is incomplete.")} />
    );
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        description="Tenant-scoped, versioned runtime policy. Every change is previewed, audited with its correlation ID, and reversible."
        actions={
          <>
            <input
              ref={fileInput}
      ...
      <Surface>
        <DetailGrid>
          <Detail label="Tenant">{current.data.tenant_id}</Detail>
          <Detail label="Version">{current.data.version}</Detail>
          <Detail label="Environment">{current.data.environment}</Detail>
          <Detail label="Updated">{formatDate(current.data.updated_at)}</Detail>
          <Detail label="Updated by">{current.data.updated_by ?? "—"}</Detail>
          <Detail label="Rollout">
            {current.data.document.rollout.enabled ? "Enabled" : "Disabled"}
          </Detail>
        </DetailGrid>
      </Surface>
      <Surface>
        <div className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Limits, allow-lists, lifecycle transitions, list preferences, navigation, rollout, and
            RBAC are validated server-side. Save remains unavailable until this exact document
            passes preview.
          </p>
          {localError ? (
          ...
          <div className="flex justify-end gap-3">
            <Button
              variant="outline"
              disabled={!previewDocument || preview.isPending}
              onClick={() => preview.mutate()}
            >
              <Eye className="mr-2 h-4 w-4" />
              {preview.isPending ? "Simulating…" : "Preview changes"}
            </Button>
            <Button disabled={!previewCurrent || save.isPending} onClick={() => save.mutate()}>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
                ...
                <li key={version.id} className="flex items-center justify-between gap-3 py-3">
                  <div>
                    ...
                    <p className="font-mono text-xs text-muted-foreground">
                      {version.correlation_id} · {formatDate(version.created_at)}
                    </p>
                  </div>
                  <Button
                    variant="outline"
        ...
        <Surface>
          ...
          {audit.data.data.length ? (
            <ul className="mt-3 divide-y">
              {audit.data.data.map((record) => (
                <li key={record.id} className="py-3">
                  <p className="font-medium">
                    {record.action} · version {record.version}
                  </p>
                  <p className="font-mono text-xs text-muted-foreground">{record.correlation_id}</p>
                  <p className="text-xs text-muted-foreground">
                    {record.actor_id} · {formatDate(record.created_at)}
                  </p>
                </li>
              ))}
            </ul>
          ) : (
            <p className="mt-3 text-sm text-muted-foreground">No audit records were returned.</p>
          )}
        </Surface>
      </div>
      <ConfirmAction
        ...
        description="Rollback creates a new immutable version; it never rewrites history."
