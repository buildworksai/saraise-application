...
export function PageSkeleton({ rows = 5 }: { readonly rows?: number }) {
  return (
    <main aria-busy="true" aria-label="Loading customization data" className="space-y-6">
...
function errorPresentation(error: Error): {
  ...
  if (error instanceof ApiError && error.status === 403)
    return {
      title: "Access denied",
      message:
        "Your current role cannot access this customization. Its existence has not been disclosed.",
      icon: LockKeyhole,
  ...
  if (
    ...
    return {
      ...
      message:
        "The target module is currently unavailable. Existing configuration remains safe and readable.",
      icon: ServerOff,
      correlation: error.correlationId,
    };
  if (error instanceof ApiError && error.status === 409)
    return {
      ...
      message:
        "Someone changed this item after you opened it. Reload the latest revision, review the differences, and apply your change again.",
      icon: RefreshCw,
      correlation: error.correlationId,
    };
  return {
    title: "We could not load this view",
    message: error.message || "A governed request failed safely.",
    icon: AlertCircle,
...
export function GovernedError({
  ...
  return (
    <section role="alert" className="rounded-xl border bg-card p-8 text-center">
      ...
      <p className="mx-auto mt-2 max-w-lg text-sm text-muted-foreground">{view.message}</p>
      {view.correlation ? (
  ...
  return (
    <section className="rounded-xl border border-dashed bg-card p-10 text-center">
      ...
      <h2 className="mt-4 text-lg font-semibold">
        {filtered ? `No ${noun} match` : `No ${noun} yet`}
      </h2>
      <p className="mt-2 text-sm text-muted-foreground">
        {filtered
          ? "Adjust or clear the server-side filters."
          : "Create the first governed customization for this tenant."}
      </p>
      {!filtered && create ? (
        <Button className="mt-5" onClick={create}>
          Create {noun.replace(/s$/u, "")}
        </Button>
      ) : null}
    </section>
  );
...
export function StatusChip({ status }: { readonly status: string }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${positive ? "bg-primary/10 text-primary" : warning ? "bg-accent text-accent-foreground" : "bg-muted text-muted-foreground"}`}
    >
      {status.replaceAll("_", " ")}
    </span>
  );
...
export function CapabilityNotice({ state }: { readonly state?: CapabilityState }) {
  return state === "capability_unavailable" ? (
    <div
      role="status"
      className="flex gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-4 text-sm text-foreground"
    >
      <Ban className="h-5 w-5 shrink-0 text-destructive" />
      <p>
        The owning module is unavailable. You can inspect this configuration, but publishing and
        evaluation are disabled until capability is restored.
      </p>
    </div>
  ) : null;
}
...
export function Pagination({
  ...
  return (
    ...
    >
      <span>
        Page {meta.page} of {meta.total_pages} · {meta.count} results
      </span>
