  ...
  if (query.isPending)
    return (
      <div
        ...
        aria-label="Loading target contracts"
      />
    );
  if (query.error)
    return (
      <p role="alert" className="text-sm text-destructive">
        Target contracts are unavailable. Retry before creating this customization.
      </p>
    );
  ...
  return (
    <label className="grid gap-1 text-sm font-medium sm:col-span-2">
      ...
      <select
        ...
        value={value}
        onChange={(event) => {
          const selected = contracts.find((contract) => identity(contract) === event.target.value);
      ...
      >
        ...
        {contracts.map((contract) => (
          ...
          >
            {identity(contract)}
            {contract.available ? "" : " — capability unavailable"}
          </option>
        ))}
      </select>
      {contracts.length === 0 ? (
        <span className="text-xs text-muted-foreground">
          No module has registered a customization contract. Install or enable a compatible module.
        </span>
      ) : null}
    </label>
  );
