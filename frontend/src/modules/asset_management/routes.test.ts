...
describe("asset-management tenant routes", () => {
  ...
  it("is discovered by the application registry consumed by TenantSidebar", () => {
    expect(
      registeredTenantRoutes.filter((route) => route.module === "asset_management")
    ).toHaveLength(5);
    const sidebarBranch = getTenantSidebarTreeForMode("development").find(
      (branch) => branch.module === "asset_management"
    );
