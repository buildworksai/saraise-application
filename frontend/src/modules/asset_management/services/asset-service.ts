...
function extractFieldErrors(details: unknown): FieldErrors {
  ...
  const source =
    isRecord(details.error) && isRecord(details.error.field_errors)
...
function unwrapList<T>(response: unknown, guard: (value: unknown) => value is T): ListResult<T> {
  ...
  if (Array.isArray(source)) {
    if (!source.every(guard))
      return malformed("The server returned invalid records in a list response.");
...
export const assetService = {
  listAssets: async (filters: AssetFilters = {}): Promise<ListResult<Asset>> => {
    const response = await translate(
      apiClient.get<PaginatedResponse<Asset> | Asset[] | ApiEnvelope<PaginatedResponse<Asset>>>(
  ...
  getAsset: async (id: string): Promise<Asset> => {
    const response = await translate(
      apiClient.get<Asset | ApiEnvelope<Asset>>(ENDPOINTS.ASSETS.DETAIL(id))
    );
  ...
  createAsset: async (data: AssetCreate): Promise<Asset> => {
    const response = await translate(
      apiClient.post<Asset | ApiEnvelope<Asset>>(
  ...
  updateAsset: async (id: string, data: AssetUpdate): Promise<Asset> => {
    const response = await translate(
      apiClient.patch<Asset | ApiEnvelope<Asset>>(
  ...
  deleteAsset: async (id: string): Promise<void> =>
    translate(
      apiClient.delete<void>(ENDPOINTS.ASSETS.DELETE(id), idempotencyHeaders(`asset-delete-${id}`))
    ),
  ...
  listDepreciationEntries: async (
    ...
    const response = await translate(
      apiClient.get<
        | PaginatedResponse<DepreciationEntry>
        | DepreciationEntry[]
        | ApiEnvelope<PaginatedResponse<DepreciationEntry>>
  ...
  calculateDepreciation: async (
    ...
    const response = await translate(
      apiClient.post<DepreciationEntry | ApiEnvelope<DepreciationEntry>>(
  ...
  getConfiguration: async (): Promise<AssetConfiguration> => {
    const response = await translate(
      apiClient.get<AssetConfiguration | ApiEnvelope<AssetConfiguration>>(
  ...
  updateConfiguration: async (
    ...
    const response = await translate(
      apiClient.patch<AssetConfiguration | ApiEnvelope<AssetConfiguration>>(
  ...
  previewConfiguration: async (
    document: AssetConfigurationDocument
  ): Promise<AssetConfigurationPreview> => {
    const response = await translate(
      apiClient.post<AssetConfigurationPreview>(ENDPOINTS.CONFIGURATION.PREVIEW, { document })
    );
  ...
  listConfigurationHistory: async (): Promise<ListResult<AssetConfigurationVersion>> => {
    const response = await translate(
      apiClient.get<PaginatedResponse<AssetConfigurationVersion> | AssetConfigurationVersion[]>(
  ...
  rollbackConfiguration: async (version: number): Promise<AssetConfiguration> => {
    const response = await translate(
      apiClient.post<AssetConfiguration>(ENDPOINTS.CONFIGURATION.ROLLBACK, { version })
    );
    return unwrapDetail(response, isConfiguration);
  },

  importConfiguration: async (
    configuration: AssetConfigurationExport
  ): Promise<AssetConfiguration> => {
    const response = await translate(
      apiClient.post<AssetConfiguration>(ENDPOINTS.CONFIGURATION.IMPORT, { configuration })
    );
    return unwrapDetail(response, isConfiguration);
  },
  ...
  exportConfiguration: async (): Promise<AssetConfigurationExport> => {
    const response = await translate(
      apiClient.get<AssetConfigurationExport>(ENDPOINTS.CONFIGURATION.EXPORT)
    );
