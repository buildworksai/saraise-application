...
export function BackgroundRefresh({ active }: { active: boolean }) {
  return (
    <div className="h-5 text-xs text-muted-foreground" role="status" aria-live="polite">
      {active && (
...
export function GovernedError({ error, retry }: { error: Error | null; retry: () => void }) {
  ...
  const title = forbidden
    ...
    : missing
      ? "Record not found"
      : "Integration Platform is unavailable";
  const message = forbidden
    ? "Your current tenant role does not grant this governed action."
    : missing
      ? "This record does not exist or belongs to another tenant."
      : apiError?.message ?? error?.message ?? "The governed API could not complete this request.";
  return (
    <Card className="p-8 text-center" role="alert">
      ...
      {apiError?.correlationId && (
        <p className="mt-3 font-mono text-xs">Correlation ID: {apiError.correlationId}</p>
      )}
...
export function EmptyPanel({
  ...
  return (
    <Card className="p-10 text-center">
      ...
      <h2 className="mt-4 text-lg font-semibold">{filtered ? `No matching ${title}` : title}</h2>
      <p className="mx-auto mt-2 max-w-lg text-sm text-muted-foreground">
        {filtered ? "No records match the selected server-side filters." : description}
      </p>
...
export function StatusBadge({ status }: { status: string }) {
  ...
  const positive =
    configuration.data?.document.navigation.status_positive.includes(status) === true;
  const danger = configuration.data?.document.navigation.status_danger.includes(status) === true;
  return (
    <span
      className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-medium capitalize ${positive ? "border-primary/30 bg-primary/10 text-primary" : danger ? "border-destructive/30 bg-destructive/10 text-destructive" : "border-border bg-muted text-muted-foreground"}`}
    >
      {status.replaceAll("_", " ")}
    </span>
  );
...
export function Pagination({
  ...
  return (
    ...
    >
      <p>
        {meta.count} results · Page {meta.page} of {Math.max(meta.total_pages, 1)}
      </p>
...
export function RedactedJsonViewer({ value, label }: { value: JsonValue; label: string }) {
  return (
    <div>
      <p className="mb-2 text-sm font-medium">{label}</p>
      <pre
        className="max-h-96 overflow-auto rounded-lg border bg-muted/40 p-4 text-xs"
        aria-label={`${label}, redacted evidence`}
      >
        {JSON.stringify(value, null, 2)}
      </pre>
      <p className="mt-2 text-xs text-muted-foreground">
        Sensitive fields are redacted by the server before persistence.
      </p>
    </div>
  );
}
...
export function SuccessEvidence({
  ...
  return (
    ...
    >
      ...
      {correlationId && <p className="mt-2 font-mono text-xs">Correlation ID: {correlationId}</p>}
    </div>
  );
}
...
export function FilterForm({
  ...
  return (
    <Card className="p-4">
      <form className="grid gap-3 md:grid-cols-4" role="search" onSubmit={onSubmit}>
        {children}
      </form>
    </Card>
  );
