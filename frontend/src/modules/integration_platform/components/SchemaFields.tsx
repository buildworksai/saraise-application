    ...
    <div className="grid gap-5 md:grid-cols-2">
      {Object.entries(schema.properties).map(([key, property]) => {
        ...
        return (
          <div key={key}>
            {property.enum ? (
              <label className="block text-sm font-medium" htmlFor={`schema-${key}`}>
                {property.title ?? key}
                {required ? " *" : ""}
                <select
                  id={`schema-${key}`}
                  ...
                  disabled={disabled}
                  value={stringValue}
                  onChange={(event) => update(key, event)}
                >
              ...
              <Input
                id={`schema-${key}`}
                label={`${property.title ?? key}${required ? " *" : ""}`}
                required={required}
                disabled={disabled}
                type={
                  property.format === "password" || property.secret
                    ? "password"
