...
function isSavedView(value: unknown): value is SavedView {
  return Boolean(value && typeof value === "object" && "name" in value && "query" in value);
}
...
export function useSavedViews(scope: string, currentQuery: string) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) setViews(parsed.filter(isSavedView));
    } catch {
      localStorage.removeItem(storageKey);
    }
  }, [storageKey]);
  const save = (name: string) => {
    const next = [...views.filter((view) => view.name !== name), { name, query: currentQuery }];
