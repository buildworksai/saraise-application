...
export function RuleGovernancePanel({
  ...
  const historyKey =
    kind === "quality"
      ? QUERY_KEYS.qualityRuleHistory(ruleId)
      : QUERY_KEYS.matchingRuleHistory(ruleId);
  const detailKey =
    kind === "quality" ? QUERY_KEYS.qualityRule(ruleId) : QUERY_KEYS.matchingRule(ruleId);
  ...
  const rollback = useMutation<
    ItemResult<DataQualityRule> | ItemResult<MatchingRule>,
    Error,
    number
  >({
    mutationFn: (versionNumber: number) =>
      kind === "quality"
        ? masterDataService.qualityRules.rollback(ruleId, {
            version_number: versionNumber,
            reason,
            idempotency_key: rollbackKey,
          })
        : masterDataService.matchingRules.rollback(ruleId, {
            version_number: versionNumber,
            reason,
            idempotency_key: rollbackKey,
          }),
  ...
  const imported = useMutation<ItemResult<DataQualityRule> | ItemResult<MatchingRule>, Error, void>(
    {
      mutationFn: () => {
        ...
        return kind === "quality"
          ? masterDataService.qualityRules.importDocument(ruleId, {
              document,
              reason,
              idempotency_key: importKey,
            })
          : masterDataService.matchingRules.importDocument(ruleId, {
              document,
              reason,
              idempotency_key: importKey,
            });
  ...
  if (history.error)
    return <GovernedError error={history.error} retry={() => void history.refetch()} />;
  return (
    <Surface title="Versioning and portability">
      <p className="text-sm text-muted-foreground">
        Every change is immutable and correlated. Supply a reason before rollback or import.
      </p>
      <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto]">
        ...
        />
        <Button variant="outline" disabled={exported.isPending} onClick={() => exported.mutate()}>
      ...
      <Button
        className="mt-3"
        disabled={!document || !reason.trim() || imported.isPending}
      ...
      <div className="mt-5 space-y-2">
        {history.isLoading ? (
          <p className="text-sm text-muted-foreground">Loading immutable history…</p>
        ) : (
