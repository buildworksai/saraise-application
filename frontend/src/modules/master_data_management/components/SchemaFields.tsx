...
function setPath(object: JsonObject, path: readonly string[], next: JsonValue): JsonObject {
  ...
  return {
    ...object,
    [head]: rest.length
      ? setPath(previous !== undefined && isJsonObject(previous) ? previous : {}, rest, next)
      : next,
  };
}
  ...
  if (!entries.length)
    return (
      <p className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
        This entity type has no custom fields. Core identity fields remain available above.
      </p>
    );
  return (
    <div className="grid gap-5 sm:grid-cols-2">
      {entries.map(([name, field]) => {
        ...
        const id = `mdm-field-${pathKey.replaceAll(".", "-")}`;
        const current = atPath(value, path);
        const required = schema.required?.includes(name) ?? false;
        if (field.type === "object")
          return (
            <fieldset key={pathKey} className="col-span-full rounded-lg border p-4">
              <legend className="px-2 text-sm font-semibold">{field.title ?? name}</legend>
              {field.description ? (
                <p className="mb-4 text-xs text-muted-foreground">{field.description}</p>
              ) : null}
              <SchemaFields
                schema={field}
                value={value}
        ...
        if (field.type === "boolean")
          return (
            <label key={pathKey} className="flex items-center gap-3 rounded-md border p-3 text-sm">
              <input
                id={id}
                type="checkbox"
                disabled={disabled}
                checked={current === true}
                onChange={(event) => onChange(setPath(value, path, event.target.checked))}
              />
              <span>
                <strong>{field.title ?? name}</strong>
                {field.description ? (
                  <small className="block text-muted-foreground">{field.description}</small>
                ) : null}
              </span>
            </label>
          );
        if (field.enum)
          return (
            <label key={pathKey} className="text-sm font-medium" htmlFor={id}>
              {field.title ?? name}
              {required ? " *" : ""}
              <select
                id={id}
                ...
                onChange={(event) => onChange(setPath(value, path, event.target.value))}
              >
              ...
              onChange={(event) =>
                onChange(
                  setPath(
                    ...
                    path,
                    numeric
                      ? parseNumber(event.target.value, field.type === "integer")
                      : event.target.value
                  )
                )
              }
