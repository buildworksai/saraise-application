...
function errorPresentation(error: Error) {
  if (error instanceof ApiError && error.status === 403)
    return {
      title: "Access denied",
      message: "Your current policy does not permit this master-data action.",
      icon: LockKeyhole,
    };
  if (error instanceof ApiError && error.status === 404)
    return {
      title: "Record not found",
      message: "The record does not exist or is outside your tenant.",
      icon: SearchX,
    };
  if (error instanceof ApiError && (error.status === 501 || error.status === 503))
    return {
      title: "Capability unavailable",
      message: "The authoritative master-data service is not ready. No success has been assumed.",
      icon: ServerOff,
    };
  if (error instanceof ApiError && error.status === 409)
    return {
      title: "Concurrent change detected",
      message: "Reload the latest version and review the conflict before trying again.",
      icon: RefreshCw,
    };
  return {
    title: "Master-data request failed",
    message: error.message || "The request failed safely.",
    icon: AlertTriangle,
  };
}
...
export function formatScore(value: string | number | null | undefined): string {
  return value === null || value === undefined ? "Not evaluated" : `${Number(value).toFixed(1)}%`;
}
export function successMessage(state: unknown): string | undefined {
  if (!state || typeof state !== "object" || !("success" in state)) return undefined;
...
export function StatusPill({ value }: { readonly value: string }) {
  const configuration = useMasterDataConfiguration();
  const positive = ["active", "resolved", "confirmed", "applied", "succeeded", "ready"].includes(
    value
  );
  const danger = ["critical", "failed", "timed_out", "rejected"].includes(value);
  const configuredToken = danger
    ? configuration.data?.data.document.ui.status_tokens.danger
    : positive
      ? configuration.data?.data.document.ui.status_tokens.success
      : configuration.data?.data.document.ui.status_tokens.warning;
  const tokenClass =
    configuredToken === "destructive"
      ? "bg-destructive/10 text-destructive"
      : configuredToken === "success"
        ? "bg-primary/10 text-primary"
        : configuredToken === "warning"
          ? "bg-muted text-muted-foreground"
          : "bg-muted text-muted-foreground";
  return (
    <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${tokenClass}`}>
      {value.replaceAll("_", " ")}
    </span>
  );
}

export function SeverityPill({ value }: { readonly value: IssueSeverity }) {
...
export function ConfirmAction({
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen} title={title} description={description} size="sm">
...
export function fieldErrors(error: Error | null, field: string): string | undefined {
  ...
  return governedDetails(error)?.error.field_errors?.find((entry) => entry.field === field)
    ?.message;
}
...
export function MutationNotice({
  ...
  return success ? (
    <p role="status" aria-live="polite" className={`rounded-md border p-3 text-sm ${tokenClass}`}>
      {success}
    </p>
  ) : null;
}
