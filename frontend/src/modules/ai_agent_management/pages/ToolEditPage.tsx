...
export const ToolEditPage = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      void client.invalidateQueries({ queryKey: ["ai-tool", id] });
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-6">
      <PageHeader
        ...
        description="Update schemas and governance metadata; immutable invocation evidence keeps its recorded descriptor version."
      />
      <ToolForm
        initial={query.data}
        pending={mutation.isPending}
        error={mutation.error}
        submit={(request) => mutation.mutate(request as ToolUpdateRequest)}
      />
    </main>
  );
};
