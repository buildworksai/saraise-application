  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        description={value.correlation_id}
        actions={<StatusPill status={value.final_outcome ?? "pending"} />}
      />
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <p>{formatDate(value.request_timestamp)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <small>Completed</small>
            <p>{formatDate(value.completed_timestamp)}</p>
          </CardContent>
        </Card>
      </section>
      <Card>
        <CardHeader>
          <CardTitle>Sanitized summary</CardTitle>
        </CardHeader>
        <CardContent>
          <JsonEvidence label="Audit trail summary" value={value.summary} />
        </CardContent>
      </Card>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          {value.events?.length ? (
              ...
              {value.events.map((item) => (
                <li key={item.id} className="pb-5">
                  <p className="font-medium">
                    {item.event_type} · <StatusPill status={item.outcome} />
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {formatDate(item.event_timestamp)} · {item.correlation_id}
                  </p>
                </li>
              ))}
            </ol>
            ...
            <EmptyState
              title="No event projection"
              description="The trail summary exists, but event detail was not included in this response."
            />
          )}
        </CardContent>
      </Card>
