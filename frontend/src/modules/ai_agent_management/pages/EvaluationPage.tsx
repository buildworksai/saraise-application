...
function jobEvidence(value: unknown): {
  readonly result?: JSONValue | null;
  readonly error_message?: string | null;
} {
  return value !== null && typeof value === "object"
    ? (value as { readonly result?: JSONValue | null; readonly error_message?: string | null })
    : {};
}
...
export const EvaluationPage = () => {
  ...
  const start = useMutation({
    mutationFn: () =>
      aiAgentService.evaluateAgent(id, {
        suite_key: suite,
        ...
        <Card>
          <CardContent className="space-y-4 p-5">
            <div className="flex items-center justify-between gap-4">
              <div>
                <strong>Durable evaluation job</strong>
                <p className="font-mono text-xs text-muted-foreground">{durableJob.id}</p>
              </div>
              <StatusPill status={durableJob.status} />
            </div>
            <p className="text-sm text-muted-foreground">
              Attempt {durableJob.attempts} · correlation {durableJob.correlation_id}
            </p>
            {evidence.error_message ? (
              <Unavailable title="Evaluation did not complete" detail={evidence.error_message} />
            ) : null}
            {evidence.result !== undefined && evidence.result !== null ? (
              <JsonEvidence
                label="Evaluation metric and regression evidence"
                value={evidence.result}
              />
            ) : (
              <p role="status" className="text-sm text-muted-foreground">
                Waiting for the registered suite to publish metric evidence…
              </p>
            )}
            {evaluationStatus ? (
              <p className="text-sm">
                Suite outcome: <StatusPill status={evaluationStatus} />
              </p>
            ) : null}
          </CardContent>
        </Card>
