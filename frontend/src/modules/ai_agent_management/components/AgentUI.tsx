/* eslint-disable react-refresh/only-export-components, @typescript-eslint/consistent-indexed-object-style -- visual primitives intentionally expose formatting helpers. */
...
export function PageSkeleton({ rows = 6 }: { readonly rows?: number }) {
  return (
    <main aria-busy="true" aria-label="Loading AI agent data" className="space-y-4">
...
function errorPresentation(error: Error): { title: string; message: string; correlation?: string } {
  if (!(error instanceof ApiError))
    return { title: "AI agent data is unavailable", message: error.message };
  if (error.status === 403)
    return {
      title: "Permission required",
      message: "Your role does not include the governed permission for this operation.",
      correlation: error.correlationId,
    };
  if (error.status === 404)
    return {
      title: "Record not found",
      message: "This record does not exist or is outside your tenant boundary.",
      correlation: error.correlationId,
    };
  if (error.status === 409)
    return {
      title: "The record changed",
      message: "Reload the latest state before retrying this transition.",
      correlation: error.correlationId,
    };
  if (error.status === 503)
    return {
      ...
      message:
        "A required runner, provider, or governance dependency is unavailable. No success was recorded.",
      correlation: error.correlationId,
    };
  return {
    title: "AI agent data is unavailable",
  ...
  return (
    <Card role="alert" className="border-destructive/40">
      <CardContent className="flex min-h-64 flex-col items-center justify-center text-center">
        ...
        <p className="mt-2 max-w-xl text-sm text-muted-foreground">{value.message}</p>
        {value.correlation ? (
...
export function MutationError({ error }: { readonly error: Error }) {
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 text-muted-foreground">{value.message}</p>
      {value.correlation ? (
        <p className="mt-2 font-mono text-xs">Correlation: {value.correlation}</p>
      ) : null}
    </div>
  );
}
...
export function Unavailable({
  ...
  return (
    <div role="status" className="rounded-xl border border-accent bg-accent/20 p-5">
      <div className="flex gap-3">
        <LockKeyhole className="mt-0.5 h-5 w-5 text-accent-foreground" />
        <div>
...
export function JsonEvidence({
  ...
  return value === undefined ? (
    <Unavailable
      title="Sensitive evidence withheld"
      detail="This projection deliberately omits opaque task, tool-input, prompt, and provider payloads. Follow the correlated audit metadata instead."
    />
  ) : (
    <pre
      aria-label={label}
      tabIndex={0}
      className="max-h-80 overflow-auto rounded-lg border bg-muted/30 p-4 text-xs leading-5"
    >
      {JSON.stringify(value, null, 2)}
    </pre>
  );
}
export function can(actions: readonly string[] | undefined, action: string): boolean {
...
export function Pagination({
  ...
  return (
    <nav aria-label="Pagination" className="flex items-center justify-end gap-3 border-t p-4">
