...
export type OpportunityTransitionCommand =
...
export interface CrmFieldLimits {
  ...
  lead_status: number;
  account_name: number;
  account_industry: number;
  account_postal_code: number;
  account_country: number;
  contact_name: number;
  contact_email: number;
  contact_phone: number;
  opportunity_name: number;
  opportunity_amount_digits: number;
  opportunity_amount_decimals: number;
  opportunity_currency: number;
  opportunity_stage: number;
  opportunity_status: number;
  activity_subject: number;
  activity_outcome: number;
  activity_external_id: number;
  actor_id: number;
  correlation_id: number;
  async_idempotency_key: number;
  domain_override_reason: number;
  transition_reason: number;
  loss_reason: number;
  provider_id: number;
  provider_evidence_string: number;
}
export interface CrmLeadTransitionConfiguration {
  readonly from: readonly LeadStatus[];
  readonly to: LeadStatus;
}
export interface CrmLeadConfiguration {
  default_score: number;
  default_grade: LeadGrade;
  default_score_source: ScoreSource;
  default_status: LeadStatus;
  score_min: number;
  score_max: number;
  grade_thresholds: Record<LeadGrade, number>;
  qualification_threshold: number;
  field_score_weights: Record<string, number>;
  source_score_weights: Record<string, number>;
  terminal_states: readonly LeadStatus[];
  transitions: Readonly<Record<string, CrmLeadTransitionConfiguration>>;
}
export interface CrmAccountConfiguration {
  default_type: AccountType;
  allowed_types: readonly AccountType[];
  hierarchy_max_depth: number;
}
export interface CrmContactConfiguration {
  default_engagement_score: number;
  engagement_score_min: number;
  engagement_score_max: number;
  enforce_account_email_domain: boolean;
  engagement_lookback_days: number;
  engagement_points_per_interaction: number;
}
export interface CrmOpportunityStageConfiguration {
  name: OpportunityStage;
  probability: number;
  semantic_token: CrmSemanticToken;
}
export interface CrmOpportunityTransitionConfiguration {
...
export interface CrmOpportunityConfiguration {
  ...
  default_stage: OpportunityStage;
  default_status: OpportunityStatus;
  probability_min: number;
  probability_max: number;
  minimum_amount: string;
  closed_won_probability: number;
  closed_lost_probability: number;
  terminal_states: readonly OpportunityStage[];
  transitions: Readonly<Record<string, CrmOpportunityTransitionConfiguration>>;
  stages: readonly CrmOpportunityStageConfiguration[];
}
export interface CrmActivityConfiguration {
  default_type: ActivityType;
  default_related_type: RelatedEntityType;
  require_future_task_due_date: boolean;
}
export interface CrmHierarchyConfiguration {
  max_nodes: number;
  max_children: number;
  page_size: number;
}
export interface CrmForecastConfiguration {
...
export interface CrmProviderConfiguration {
  ...
  revenue_prediction: string | null;
  score_min: number;
  score_max: number;
  confidence_min: string;
  confidence_max: string;
  maximum_evidence_factors: number;
  extension_schema_version: string;
  extension_priority_default: number;
  extension_priority_min: number;
  extension_priority_max: number;
  retry_attempts: number;
  backoff_base_seconds: string;
  backoff_max_seconds: string;
  backoff_jitter_seconds: string;
}
export interface CrmJobsConfiguration {
  stale_deal_days: number;
  stale_deal_min_days: number;
  stale_deal_max_days: number;
  iterator_chunk_size: number;
}
export interface CrmPaginationConfiguration {
  default_page_size: number;
  maximum_page_size: number;
}
export interface CrmApiConfiguration {
  quota_cost: number;
}
export interface CrmConversionConfiguration {
  create_account_by_default: boolean;
  close_date_offset_days: number;
  use_current_version: boolean;
  transition_key_prefix: string;
}
export interface CrmHealthConfiguration {
  cache_timeout_seconds: number;
}
export type CrmSemanticToken =
  | "success"
  | "info"
  | "warning"
  | "danger"
  ...
  | "accent"
  | "positive";
export interface CrmScoreBand {
  minimum: number;
  grade: LeadGrade;
  semantic_token: CrmSemanticToken;
}
export interface CrmUiConfiguration {
  score_bands: readonly CrmScoreBand[];
  hierarchy_auto_expand_levels: number;
  hierarchy_indentation_pixels: number;
  minimum_pipeline_bar_percent: number;
  saved_page_size: number;
  dashboard_forecast_period_days: number;
  prediction_retry_enabled: boolean;
  stale_deal_page_size: number;
  pipeline_fetch_limit: number;
}
export interface CrmConfigurationDocument {
  field_limits: CrmFieldLimits;
  lead: CrmLeadConfiguration;
  account: CrmAccountConfiguration;
  contact: CrmContactConfiguration;
  opportunity: CrmOpportunityConfiguration;
  activity: CrmActivityConfiguration;
  hierarchy: CrmHierarchyConfiguration;
  forecast: CrmForecastConfiguration;
  providers: CrmProviderConfiguration;
  jobs: CrmJobsConfiguration;
  pagination: CrmPaginationConfiguration;
  api: CrmApiConfiguration;
  conversion: CrmConversionConfiguration;
  health: CrmHealthConfiguration;
  ui: CrmUiConfiguration;
}
export interface CrmRolloutConfiguration {
  enabled: boolean;
  percentage: number;
  roles: string[];
  cohorts: string[];
...
export interface CrmConfiguration {
  ...
  readonly document: CrmConfigurationDocument;
  readonly feature_flags: Readonly<Record<string, boolean>>;
  readonly rollout: CrmRolloutConfiguration;
  readonly updated_at: string | null;
}
export interface CrmConfigurationWrite {
  environment: string;
  document: CrmConfigurationDocument;
  feature_flags: Record<string, boolean>;
  rollout: CrmRolloutConfiguration;
}
export interface CrmConfigurationPreview {
  readonly valid: boolean;
  readonly diff: MetadataValue;
  readonly errors: Readonly<Record<string, readonly string[]>>;
  readonly effective: CrmConfiguration;
}
export interface CrmConfigurationVersion {
  ...
  readonly rollback_of_version: number | null;
  readonly created_at: string;
}
export interface CrmConfigurationExport {
  readonly schema_version: 1;
  readonly module: "crm";
  readonly configuration: CrmConfiguration;
}
...
export interface TransitionRecord {
  ...
  readonly metadata: {
    readonly actor_id?: string;
    readonly correlation_id?: string;
    readonly reason_supplied?: boolean;
  };
...
export interface DuplicateAccountResult {
  ...
  readonly external_matches: readonly {
    readonly id: string;
    readonly name: string;
    readonly website?: string;
  }[];
...
export interface CurrencyForecast {
  readonly currency: string;
  readonly total_pipeline_value: string;
  readonly weighted_pipeline_value: string;
...
export interface WinRate {
  readonly win_rate: string | null;
  readonly won_count: number;
  readonly lost_count: number;
  readonly total_closed: number;
  readonly period_days: number;
}
...
export interface AsyncJob {
  ...
  readonly result: MetadataValue | null;
...
export interface ApiError {
  ...
  readonly detail: MetadataValue | null;
  readonly correlation_id: string;
}
...
export interface V2PageEnvelope<T> {
  readonly data: readonly T[];
  readonly meta: ResponseMeta & { readonly pagination: PaginationMeta };
}
...
export interface LeadFilters extends PageFilters {
  status?: LeadStatus;
  owner_id?: string;
  score_min?: number;
  score_max?: number;
...
export interface AccountFilters extends PageFilters {
  account_type?: AccountType;
  owner_id?: string;
...
export interface ContactFilters extends PageFilters {
  account_id?: string;
  owner_id?: string;
  engagement_min?: number;
  search?: string;
}
export interface OpportunityFilters extends PageFilters {
...
export interface ActivityFilters extends PageFilters {
  related_to_type?: RelatedEntityType;
  related_to_id?: string;
  activity_type?: ActivityType;
  owner_id?: string;
...
export interface AsyncLeadScoreRequest {
...
export const ENDPOINTS = {
  LEADS: {
    ...
    TRANSITION: (id: string) => `${MODULE_API_PREFIX}/leads/${id}/transition/` as const,
    CONVERT: (id: string) => `${MODULE_API_PREFIX}/leads/${id}/convert/` as const,
    SCORE: (id: string) => `${MODULE_API_PREFIX}/leads/${id}/score/` as const,
  },
  ACCOUNTS: {
    ...
    DUPLICATES: `${MODULE_API_PREFIX}/accounts/duplicates/`,
  },
  CONTACTS: {
  ...
  OPPORTUNITIES: {
    ...
    TRANSITION: (id: string) => `${MODULE_API_PREFIX}/opportunities/${id}/transition/` as const,
  ...
  FORECASTING: {
    ...
    BY_STAGE: `${MODULE_API_PREFIX}/forecasting/by-stage/`,
    PREDICT: `${MODULE_API_PREFIX}/forecasting/predict/`,
  },
...
function entity(value: unknown): value is MutableEntityRead {
  return (
    object(value) &&
...
export function isV2Envelope(value: unknown): value is V2Envelope<unknown> {
  return (
    object(value) &&
    "data" in value &&
    object(value.meta) &&
...
export function isV2PageEnvelope(value: unknown): value is V2PageEnvelope<unknown> {
  if (!isV2Envelope(value) || !Array.isArray(value.data)) return false;
...
export function isLead(value: unknown): value is Lead {
  return (
    entity(value) &&
    typeof (value as Lead).last_name === "string" &&
    typeof (value as Lead).score === "number"
  );
}
export function isAccount(value: unknown): value is Account {
  return (
    ...
    typeof (value as Account).account_type === "string"
  );
}
export function isContact(value: unknown): value is Contact {
  return (
    entity(value) &&
    typeof (value as Contact).account_id === "string" &&
    typeof (value as Contact).last_name === "string"
  );
}
export function isOpportunity(value: unknown): value is Opportunity {
  return (
    entity(value) &&
    typeof (value as Opportunity).account_id === "string" &&
    typeof (value as Opportunity).stage === "string"
  );
}
export function isActivity(value: unknown): value is Activity {
  return (
    entity(value) &&
    typeof (value as Activity).subject === "string" &&
    typeof (value as Activity).related_to_id === "string"
  );
}
