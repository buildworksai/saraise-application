...
import type { Contact, ContactCreate } from "../contracts";
import { Field, SubmitButton, fieldClass } from "../components/CrmPage";
import { useDirtyGuard, validEmail, value, type FormErrors } from "./form-utils";
export function ContactForm({
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const email = value(d, "email");
    if (!value(d, "account_id")) next.account_id = "An account is required.";
    if (!value(d, "last_name")) next.last_name = "Last name is required.";
    if (email && !validEmail(email)) next.email = "Enter a valid email address.";
    setErrors(next);
    if (Object.keys(next).length) return;
    await onSubmit({
      account_id: value(d, "account_id"),
      ...
      domain_override_reason: allowDomainOverride
        ? value(d, "domain_override_reason") || undefined
        : undefined,
    });
    guard.markClean();
  }
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field id="twitter" label="Social handle">
          <input
            id="twitter"
            name="twitter"
        ...
        {allowDomainOverride ? (
          <Field
            id="domain_override_reason"
            label="Domain override reason"
            hint="Required only when the email domain differs from the account domain."
          >
            <input
              id="domain_override_reason"
              name="domain_override_reason"
              className={fieldClass}
            />
          </Field>
        ) : null}
      ...
      <SubmitButton pending={pending} label="Save contact" />
    </form>
  );
}
