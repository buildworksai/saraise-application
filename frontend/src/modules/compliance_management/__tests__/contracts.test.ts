...
describe("compliance contracts", () => {
  it("uses only the governed v2 module prefix", () => {
    ...
    expect(ENDPOINTS.POLICIES.PUBLISH("policy-1")).toBe(
      `${MODULE_API_PREFIX}/policies/policy-1/publish/`
    );
    expect(ENDPOINTS.CONFIGURATION.ROLLBACK("revision-1")).toBe(
      `${MODULE_API_PREFIX}/configuration/revision-1/rollback/`
    );
    expect(ENDPOINTS.EVIDENCE_LINKS.DELETE("link-1")).toBe(
      `${MODULE_API_PREFIX}/evidence-links/link-1/`
    );
  });
