...
function unwrapPage<T>(value: PaginatedEnvelope<T>): InventoryPage<T> {
  ...
  if (
    ![pagination.page, pagination.page_size, pagination.total_count, pagination.total_pages].every(
      Number.isInteger
    )
  ) {
    throw new ApiError(
...
const get = async <T>(url: string): Promise<InventoryResult<T>> =>
  unwrap(await apiClient.get<ApiEnvelope<T>>(url));
const create = async <T, B>(url: string, body: B, key: string): Promise<InventoryResult<T>> =>
  unwrap(await apiClient.post<ApiEnvelope<T>>(url, body, idempotencyHeaders(key)));
const update = async <T, B>(url: string, body: B, version: number): Promise<InventoryResult<T>> =>
  unwrap(await apiClient.patch<ApiEnvelope<T>>(url, body, versionHeaders(version)));
...
export const inventoryQueryKeys = {
  ...
  resource: (tenantId: string, resource: string) =>
    [...inventoryQueryKeys.root(tenantId), resource] as const,
  list: (tenantId: string, resource: string, filters: object) =>
    [...inventoryQueryKeys.resource(tenantId, resource), "list", filters] as const,
  detail: (tenantId: string, resource: string, id: string) =>
    [...inventoryQueryKeys.resource(tenantId, resource), "detail", id] as const,
...
export const inventoryService = {
  listWarehouses: (filters: WarehouseFilters = {}) =>
    list<Warehouse>(ENDPOINTS.WAREHOUSES.LIST, filters),
  getWarehouse: (id: string) => get<Warehouse>(ENDPOINTS.WAREHOUSES.DETAIL(id)),
  createWarehouse: (body: WarehouseCreate, key: string) =>
    create<Warehouse, WarehouseCreate>(ENDPOINTS.WAREHOUSES.CREATE, body, key),
  updateWarehouse: (id: string, body: WarehouseUpdate, expectedVersion: number) =>
    update<Warehouse, WarehouseUpdate>(ENDPOINTS.WAREHOUSES.UPDATE(id), body, expectedVersion),
  archiveWarehouse: async (id: string, expectedVersion: number) =>
    unwrap(
  ...
  setDefaultWarehouse: (id: string, key: string) =>
    command<Warehouse>(ENDPOINTS.WAREHOUSES.SET_DEFAULT(id), { transition_key: key }, key),

  listLocations: (filters: LocationFilters = {}) =>
    list<StorageLocation>(ENDPOINTS.LOCATIONS.LIST, filters),
  ...
  createLocation: (body: StorageLocationCreate, key: string) =>
    create<StorageLocation, StorageLocationCreate>(ENDPOINTS.LOCATIONS.CREATE, body, key),
  updateLocation: (id: string, body: StorageLocationUpdate, expectedVersion: number) =>
    update<StorageLocation, StorageLocationUpdate>(
      ENDPOINTS.LOCATIONS.UPDATE(id),
      body,
      expectedVersion
    ),
  archiveLocation: async (id: string, expectedVersion: number) =>
    unwrap(
  ...
  createItem: (body: ItemCreate, key: string) =>
    create<Item, ItemCreate>(ENDPOINTS.ITEMS.CREATE, body, key),
  updateItem: (id: string, body: ItemUpdate, expectedVersion: number) =>
    update<Item, ItemUpdate>(ENDPOINTS.ITEMS.UPDATE(id), body, expectedVersion),
  archiveItem: async (id: string, expectedVersion: number) =>
    unwrap(
  ...
  createBatch: (body: BatchCreate, key: string) =>
    create<Batch, BatchCreate>(ENDPOINTS.BATCHES.CREATE, body, key),
  updateBatch: (id: string, body: BatchUpdate, expectedVersion: number) =>
    update<Batch, BatchUpdate>(ENDPOINTS.BATCHES.UPDATE(id), body, expectedVersion),
  commandBatch: (
    id: string,
    name: "activate" | "quarantine" | "release" | "recall",
    body: InventoryCommandRequest,
    key: string
  ) => command<Batch>(ENDPOINTS.BATCHES.COMMAND(id, name), body, key),
  ...
  createSerial: (body: SerialNumberCreate, key: string) =>
    create<SerialNumber, SerialNumberCreate>(ENDPOINTS.SERIALS.CREATE, body, key),
  updateSerial: (id: string, body: SerialNumberUpdate, expectedVersion: number) =>
    update<SerialNumber, SerialNumberUpdate>(ENDPOINTS.SERIALS.UPDATE(id), body, expectedVersion),
  traceSerial: (id: string) => get<StockLedgerEntry[]>(ENDPOINTS.SERIALS.TRACE(id)),

  listStockEntries: (filters: StockEntryFilters = {}) =>
    list<StockEntry>(ENDPOINTS.STOCK_ENTRIES.LIST, filters),
  ...
  createStockEntry: (body: StockEntryCreate, key: string) =>
    create<StockEntry, StockEntryCreate>(ENDPOINTS.STOCK_ENTRIES.CREATE, body, key),
  updateStockEntry: (id: string, body: StockEntryUpdate, expectedVersion: number) =>
    update<StockEntry, StockEntryUpdate>(ENDPOINTS.STOCK_ENTRIES.UPDATE(id), body, expectedVersion),
  deleteStockEntryDraft: async (id: string, expectedVersion: number) =>
    unwrap(
  ...
  listReservations: (filters: ReservationFilters = {}) =>
    list<StockReservation>(ENDPOINTS.RESERVATIONS.LIST, filters),
  ...
  createReservation: (body: ReservationCreate, key: string) =>
    create<StockReservation, ReservationCreate>(ENDPOINTS.RESERVATIONS.CREATE, body, key),
  updateReservation: (id: string, body: ReservationUpdate, expectedVersion: number) =>
    update<StockReservation, ReservationUpdate>(
  ...
  commandReservation: (
    ...
    body: InventoryCommandRequest,
    key: string
  ) => command<StockReservation>(ENDPOINTS.RESERVATIONS.COMMAND(id, name), body, key),

  listCycleCounts: (filters: CycleCountFilters = {}) =>
    list<CycleCount>(ENDPOINTS.CYCLE_COUNTS.LIST, filters),
  getCycleCount: (id: string) => get<CycleCount>(ENDPOINTS.CYCLE_COUNTS.DETAIL(id)),
  createCycleCount: (body: CycleCountCreate, key: string) =>
    create<CycleCount, CycleCountCreate>(ENDPOINTS.CYCLE_COUNTS.CREATE, body, key),
  updateCycleCount: (id: string, body: CycleCountUpdate, expectedVersion: number) =>
    update<CycleCount, CycleCountUpdate>(ENDPOINTS.CYCLE_COUNTS.UPDATE(id), body, expectedVersion),
  commandCycleCount: (
    id: string,
    name: "start" | "submit" | "approve" | "reject" | "post" | "cancel",
    body: InventoryCommandRequest,
    key: string
  ) => command<CycleCount>(ENDPOINTS.CYCLE_COUNTS.COMMAND(id, name), body, key),

  listConfigurations: () => list<InventoryConfiguration>(ENDPOINTS.CONFIGURATIONS.LIST, {}),
  getConfiguration: (environment: Environment) =>
    get<InventoryConfiguration>(ENDPOINTS.CONFIGURATIONS.DETAIL(environment)),
  createConfigurationRevision: (
    environment: Environment,
    body: ConfigurationUpdate,
    expectedVersion: number
  ) =>
    update<InventoryConfiguration, ConfigurationUpdate>(
      ENDPOINTS.CONFIGURATIONS.UPDATE(environment),
      body,
      expectedVersion
    ),
  previewConfiguration: async (environment: Environment, body: ConfigurationUpdate) =>
    unwrap(
      await apiClient.post<ApiEnvelope<ConfigurationPreview>>(
        ENDPOINTS.CONFIGURATIONS.PREVIEW(environment),
        body
      )
    ),
  activateConfiguration: (
    environment: Environment,
    body: ConfigurationActivateRequest,
    key: string
  ) =>
    create<InventoryConfiguration, ConfigurationActivateRequest>(
  ...
  rollbackConfiguration: (
    environment: Environment,
    body: ConfigurationRollbackRequest,
    key: string
  ) =>
    create<InventoryConfiguration, ConfigurationRollbackRequest>(
  ...
  importConfiguration: (environment: Environment, body: ConfigurationImportRequest, key: string) =>
    create<InventoryConfiguration, ConfigurationImportRequest>(
      ENDPOINTS.CONFIGURATIONS.IMPORT(environment),
      body,
      key
    ),
  exportConfiguration: (environment: Environment) =>
    get<ConfigurationExportDocument>(ENDPOINTS.CONFIGURATIONS.EXPORT(environment)),
  configurationHistory: (environment: Environment) =>
    list<InventoryConfigurationRevision>(ENDPOINTS.CONFIGURATIONS.HISTORY(environment), {}),

  dashboard: () => get<InventoryDashboard>(ENDPOINTS.DASHBOARD),
  enqueueImport: (body: ImportRequest, key: string) =>
    create<InventoryJob, ImportRequest>(ENDPOINTS.IMPORTS, body, key),
