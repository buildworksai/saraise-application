...
export function InventorySkeleton({ label = "Loading inventory" }: { label?: string }) {
  return (
    <div className="space-y-4 p-4 sm:p-8" role="status" aria-label={label}>
...
export function InventoryErrorState({ error, onRetry }: { error: unknown; onRetry?: () => void }) {
  ...
  const detail = denied
    ? "Your account does not have the required inventory permission. Access is denied by default; ask an administrator to review your role."
    : missing
      ? "The record does not exist or is not visible in this tenant."
      : apiError?.message ?? "An unexpected failure occurred. No data was assumed or fabricated.";
  return (
    <Card className="m-4 border-destructive/40 p-6 sm:m-8" role="alert">
      <div className="flex gap-3">
        {denied ? (
          <LockKeyhole className="mt-1 h-5 w-5 text-destructive" />
        ) : (
