...
describe("DMS v2 contracts", () => {
  it("owns only governed v2 API endpoints", () => {
    ...
    expect(ENDPOINTS.FOLDERS.CONTENTS("folder id")).toBe(
      "/api/v2/dms/folders/folder%20id/contents/"
    );
    expect(ENDPOINTS.VERSIONS.RESTORE("version")).toBe(
      "/api/v2/dms/document-versions/version/restore/"
    );
  ...
  it("uses a browser File as upload input and exposes the seven canonical UI paths", () => {
    const request: DocumentUpload = {
      file: new File(["evidence"], "evidence.txt"),
      name: "Evidence",
    };
