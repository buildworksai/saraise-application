...
export function PermissionDialog({ open, onOpenChange, documentId, directory, minimumQueryLength, maximumQueryLength, onGrant }: { readonly open: boolean; readonly onOpenChange: (open: boolean) => void; readonly documentId: UUID; readonly directory?: PrincipalDirectoryPort; readonly minimumQueryLength: number; readonly maximumQueryLength: number; readonly onGrant: (request: DocumentPermissionCreate) => void }) {
  ...
  const results = useQuery({ queryKey: DMS_QUERY_KEYS.principals(search, type), queryFn: () => directory!.search(search, type), enabled: Boolean(directory) && open && search.trim().length >= minimumQueryLength });
