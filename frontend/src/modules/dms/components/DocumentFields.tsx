  ...
  return (
    <label className="block text-sm font-medium">
      ...
      >
        ...
        {folders
          .filter((folder) => folder.id !== exclude)
          .map((folder) => (
...
export function TagEditor({
  ...
  return (
    <Input
      id="document-tags"
      label="Tags"
      title={`At most ${maxTags} tags of ${maxTagLength} characters each.`}
      value={value.join(", ")}
      placeholder="contract, finance, signed"
      onChange={(event) =>
        onChange(
          [
            ...new Set(
              event.target.value
                ...
                .filter((tag) => Boolean(tag) && tag.length <= maxTagLength)
            ),
          ].slice(0, maxTags)
        )
      }
...
export function MetadataEditor({
  ...
  readonly value: DocumentMetadata;
  readonly onChange: (value: DocumentMetadata) => void;
  readonly maxKeyLength: number;
}) {
  const rows = toRows(value);
  const update = (index: number, field: keyof MetadataRow, next: string) =>
    onChange(
      toMetadata(
        rows.map((row, rowIndex) => (rowIndex === index ? { ...row, [field]: next } : row))
      )
    );
  return (
    <fieldset className="space-y-2">
      ...
      <p className="text-xs text-muted-foreground">
        Add searchable business labels. Provider-owned metadata remains isolated in extension
        namespaces.
      </p>
      {rows.map((row, index) => (
        <div key={`${index}:${row.key}`} className="grid grid-cols-[1fr_1fr_auto] gap-2">
          ...
          <Button
            ...
            variant="ghost"
            type="button"
            onClick={() => onChange(toMetadata(rows.filter((_, rowIndex) => rowIndex !== index)))}
          >
