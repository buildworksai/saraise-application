...
export function TagEditor({ value, onChange, maxTags, maxTagLength }: { readonly value: readonly string[]; readonly onChange: (value: readonly string[]) => void; readonly maxTags: number; readonly maxTagLength: number }) {
  return <Input id="document-tags" label="Tags" title={`At most ${maxTags} tags of ${maxTagLength} characters each.`} value={value.join(', ')} placeholder="contract, finance, signed" onChange={(event) => onChange([...new Set(event.target.value.split(',').map((tag) => tag.trim().toLowerCase()).filter((tag) => Boolean(tag) && tag.length <= maxTagLength))].slice(0, maxTags))}/>;
}
...
export function MetadataEditor({ value, onChange, maxKeyLength }: { readonly value: DocumentMetadata; readonly onChange: (value: DocumentMetadata) => void; readonly maxKeyLength: number }) {
  ...
  return <fieldset className="space-y-2"><legend className="text-sm font-medium">Metadata</legend><p className="text-xs text-muted-foreground">Add searchable business labels. Provider-owned metadata remains isolated in extension namespaces.</p>{rows.map((row, index) => <div key={`${index}:${row.key}`} className="grid grid-cols-[1fr_1fr_auto] gap-2"><Input aria-label={`Metadata key ${index + 1}`} maxLength={maxKeyLength} value={row.key} placeholder="Property" onChange={(event) => update(index, 'key', event.target.value)}/><Input aria-label={`Metadata value ${index + 1}`} value={row.value} placeholder="Value" onChange={(event) => update(index, 'value', event.target.value)}/><Button aria-label={`Remove metadata row ${index + 1}`} size="icon" variant="ghost" type="button" onClick={() => onChange(toMetadata(rows.filter((_, rowIndex) => rowIndex !== index)))}><Trash2 className="h-4 w-4"/></Button></div>)}<Button type="button" size="sm" variant="outline" onClick={() => onChange({ ...value, '': '' })}><Plus className="mr-2 h-4 w-4"/>Add property</Button></fieldset>;
}
