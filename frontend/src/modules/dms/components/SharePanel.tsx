...
import type { DmsConfigurationValues, DocumentShare, DocumentShareCreate, UUID } from '../contracts';
...
export function CreateShareDialog({ open, onOpenChange, documentId, pending, configuration, onCreate }: { readonly open: boolean; readonly onOpenChange: (open: boolean) => void; readonly documentId: UUID; readonly pending: boolean; readonly configuration: DmsConfigurationValues; readonly onCreate: (request: DocumentShareCreate) => void }) {
  const defaultExpiry = new Date(Date.now() + configuration.default_share_expiry_hours * 60 * 60 * 1000).toISOString().slice(0, 16);
  const [expiresAt, setExpiresAt] = useState(defaultExpiry);
  const [limited, setLimited] = useState(true);
  const [limit, setLimit] = useState(configuration.default_share_access_count);
  const maximumExpiry = new Date(Date.now() + configuration.max_share_lifetime_days * 24 * 60 * 60 * 1000).toISOString().slice(0, 16);
  return <Dialog open={open} onOpenChange={onOpenChange} title="Create secure share" description="The bearer URL is displayed once. Expiry and access policy cannot be omitted." size="md"><div className="space-y-4"><Input id="share-expiry" label="Expires at" type="datetime-local" required min={new Date().toISOString().slice(0, 16)} max={maximumExpiry} value={expiresAt} onChange={(event) => setExpiresAt(event.target.value)}/><label className="block text-sm font-medium">Access policy<select className="mt-1 block w-full rounded-md border bg-background px-3 py-2" value={limited ? 'limited' : 'unlimited'} onChange={(event) => setLimited(event.target.value === 'limited')}><option value="limited">Limit number of downloads</option><option value="unlimited">Unlimited until expiry</option></select></label>{limited ? <Input id="share-limit" label="Maximum downloads" type="number" min={1} max={configuration.max_share_access_count} value={limit} onChange={(event) => setLimit(Number(event.target.value))}/> : null}<div className="rounded-lg border border-accent bg-accent/40 p-3 text-sm text-accent-foreground">Anyone holding the URL can download this document until the selected boundary is reached. Send it only through a trusted channel.</div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button disabled={pending || !expiresAt || (limited && (limit < 1 || limit > configuration.max_share_access_count))} onClick={() => onCreate({ document_id: documentId, expires_at: new Date(expiresAt).toISOString(), max_access_count: limited ? limit : null })}>{pending ? 'Creating…' : 'Create share'}</Button></div></div></Dialog>;
}
...
export function SharePanel({ documentId, canShare }: { readonly documentId: UUID; readonly canShare: boolean }) {
  ...
  if (query.isLoading || configuration.isLoading) return <Card className="space-y-3 p-5"><Skeleton className="h-10"/><Skeleton className="h-20"/></Card>;
