...
export function DocumentDetailPage() {
  ...
  return <main className="space-y-6 p-4 sm:p-8"><PageHeader title={document.name} description={`${document.folder_name ?? 'Documents'} · ${document.version_count} immutable version${document.version_count === 1 ? '' : 's'}`} actions={<>{can(actions, 'download') ? <Button variant="outline" disabled={download.isPending} onClick={() => download.mutate()}><Download className="mr-2 h-4 w-4"/>Download</Button> : null}{can(actions, 'create_version') ? <Button variant="outline" onClick={() => setTab('versions')}><Upload className="mr-2 h-4 w-4"/>New version</Button> : null}{can(actions, 'update') ? <Button onClick={() => navigate(ROUTES.DOCUMENT_EDIT(document.id))}><Pencil className="mr-2 h-4 w-4"/>Edit metadata</Button> : null}{can(actions, 'delete') ? <Button variant="danger" disabled={remove.isPending} onClick={() => { if (window.confirm('Soft-delete this document and revoke its active shares? Immutable versions remain retained.')) remove.mutate(); }}><Trash2 className="mr-2 h-4 w-4"/>Delete</Button> : null}</>}/>{download.error || remove.error ? <ApiProblem error={download.error ?? remove.error ?? new Error('Document operation failed')}/> : null}<nav role="tablist" aria-label="Document details" className="flex overflow-x-auto border-b">{tabs.map((item) => <button role="tab" aria-selected={tab === item.id} key={item.id} className={`border-b-2 px-4 py-3 text-sm font-medium ${tab === item.id ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`} onClick={() => setTab(item.id)}>{item.label}</button>)}</nav>{tab === 'overview' ? <Overview document={document}/> : null}{tab === 'versions' ? <VersionTimeline documentId={document.id} canCreate={can(actions, 'create_version')} canRestore={can(actions, 'restore_version')} canDownload={can(actions, 'download')}/> : null}{tab === 'access' ? <PermissionPanel documentId={document.id} canManage={can(actions, 'manage_permissions')}/> : null}{tab === 'shares' ? <SharePanel documentId={document.id} canShare={can(actions, 'share')}/> : null}</main>;
}
