...
export function EditDocumentPage() {
  ...
  if (document.isLoading || folders.isLoading) return <PageSkeleton/>;
  if (document.error || folders.error) return <main className="p-4 sm:p-8"><ApiProblem error={document.error ?? folders.error ?? new Error('Document failed to load')} onRetry={() => { void document.refetch(); void folders.refetch(); }}/></main>;
  if (!document.data) return <main className="p-4 sm:p-8"><ApiProblem error={new Error('Document response was empty.')}/></main>;
  const conflict = save.error instanceof DmsApiError && save.error.problem.kind === 'conflict';
  return <main className="space-y-6 p-4 sm:p-8"><PageHeader title={`Edit ${document.data.name}`} description="Metadata and folder placement only. File content changes through an immutable version." actions={<Button variant="outline" type="button" onClick={() => navigate(`${ROUTES.DOCUMENT_DETAIL(id)}?tab=versions`)}><Upload className="mr-2 h-4 w-4"/>Upload new version</Button>}/><form className="mx-auto max-w-3xl space-y-5" onSubmit={submit} onChange={() => setDirty(true)}><Card className="space-y-4 p-5"><Input label="Document name" required maxLength={255} value={name} onChange={(event) => setName(event.target.value)}/><Textarea label="Description" value={description} onChange={(event) => setDescription(event.target.value)}/><FolderSelector folders={folders.data?.items ?? []} value={folderId} onChange={(value) => { setFolderId(value); setDirty(true); }}/><TagEditor value={tags} onChange={(value) => { setTags(value); setDirty(true); }}/><MetadataEditor value={metadata} onChange={(value) => { setMetadata(value); setDirty(true); }}/><p className="text-xs text-muted-foreground">Loaded revision {new Date(loadedRevision).toLocaleString()}. Saving sends an optimistic concurrency condition.</p></Card>{save.error ? <MutationProblem error={save.error}/> : null}{conflict ? <Button type="button" variant="outline" onClick={() => { setDirty(false); save.reset(); void document.refetch(); }}>Reload current values</Button> : null}<div className="flex justify-end gap-2"><Button type="button" variant="outline" disabled={save.isPending} onClick={() => navigate(ROUTES.DOCUMENT_DETAIL(id))}>Cancel</Button><Button type="submit" disabled={save.isPending || !name.trim()}>{save.isPending ? 'Saving…' : 'Save changes'}</Button></div></form></main>;
}
