...
describe("WorkflowListPage states", () => {
  ...
  it("renders the accessible loading skeleton", () => {
    vi.spyOn(workflowService.workflows, "list").mockReturnValue(new Promise(() => undefined));
    renderPage();
    expect(screen.getByLabelText("Loading workflow definitions")).toHaveAttribute(
      "aria-busy",
      "true"
    );
  });
  ...
  it("renders a governed correlation ID and retry", async () => {
    vi.spyOn(workflowService.workflows, "list").mockRejectedValue(
