...
export function WorkflowInstanceDetailPage() {
  ...
  const query = useQuery({ queryKey: ["workflow-instance", id], queryFn: () => workflowService.instances.get(id), enabled: Boolean(id && policy), refetchInterval: (state) => state.state.data && !terminal.has(state.state.data.state) && document.visibilityState === "visible" ? policy?.operational.execution_detail_poll_interval_ms : false });
  useEffect(() => { if (query.data) document.title = `${query.data.workflow_name} execution · SARAISE`; }, [query.data]);
