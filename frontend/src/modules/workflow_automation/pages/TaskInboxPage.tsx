...
export function TaskInboxPage() {
  ...
  const [ordering, setOrdering] = useState<TaskOrdering | null>(null);
  const [selected, setSelected] = useState<WorkflowTaskListDTO | null>(null);
  ...
  const filters = {
    page,
    page_size: policy?.limits.workflow_page_size,
    scope: policy?.defaults.task_scope,
  ...
  const query = useQuery({
    ...
    queryFn: () => workflowService.tasks.list(filters),
    enabled: Boolean(policy),
  });
  const mutation = useMutation({
    mutationFn: ({ task, reason }: { task: WorkflowTaskListDTO; reason: string }) =>
      decision === "complete"
        ? workflowService.tasks.complete(task.id, {
            meta_data: {},
            transition_key: newTransitionKey("complete"),
          })
        : workflowService.tasks.reject(task.id, {
            reason,
            meta_data: {},
            transition_key: newTransitionKey("reject"),
          }),
    onSuccess: (task) => {
      setSuccess(`${task.step_name} is now ${task.status}.`);
      setSelected(null);
      void cache.invalidateQueries({ queryKey: ["workflow-tasks"] });
    },
  });
  const choose = (task: WorkflowTaskListDTO, next: "complete" | "reject"): void => {
    setSelected(task);
    setDecision(next);
    mutation.reset();
  };
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title="My tasks"
        description="Decide assigned work with deadline, workflow, subject, and immutable evidence in view."
      />
      {success ? (
        <div
          role="status"
          className="rounded border border-primary/40 bg-primary/5 p-3 text-sm text-primary"
        >
          {success}
        </div>
      ) : null}
      {stale ? (
        ...
        >
          This task changed before your decision was recorded. No duplicate decision was applied.{" "}
          <Button
            variant="ghost"
            onClick={() => {
              setSelected(null);
      ...
      <Card className="flex flex-wrap gap-3 p-4">
        ...
        >
          ...
          {["pending", "completed", "rejected", "cancelled", "expired"].map((value) => (
        ...
        <select
          ...
          className={selectClass}
          value={resolvedOrdering ?? ""}
          onChange={(event) => setOrdering(event.target.value as TaskOrdering)}
        >
          <option value="due_date">Due soonest</option>
          <option value="-due_date">Due latest</option>
          <option value="created_at">Oldest assigned</option>
        </select>
      </Card>
      {!query.data?.items.length ? (
        <EmptyPanel
          title={status || overdue ? "No tasks match these filters" : "All caught up"}
          description={
            status || overdue
              ? "Change the status or overdue filter to view other decisions."
              : "You have no workflow decisions waiting."
          }
          action={
            status || overdue ? (
              <Button
              ...
              <Card key={task.id} className="p-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <button
                      className="mt-2 text-lg font-semibold hover:text-primary"
                      onClick={() => navigate(ROUTES.TASK_DETAIL(task.id))}
                    >
                      {task.step_name}
                    </button>
                    <p className="text-sm text-muted-foreground">
                      {task.workflow_name} · v{task.workflow_version}
                    </p>
                    <p className="mt-2 text-sm">{task.subject ?? "Workflow decision"}</p>
                    <p className="mt-2 text-xs text-muted-foreground">
                      Due {formatDate(task.due_date)} · assigned to {task.assignment_label}
                    </p>
                  </div>
      ...
      <TaskDecisionDialog
        ...
        decision={decision}
        taskName={selected?.step_name ?? "task"}
        ...
        onSubmit={(reason) => {
          if (selected) mutation.mutate({ task: selected, reason });
        }}
      />
    </main>
  );
