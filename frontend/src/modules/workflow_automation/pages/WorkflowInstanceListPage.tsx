...
export function WorkflowInstanceListPage() {
  ...
  const [ordering, setOrdering] = useState<InstanceOrdering>("-created_at");
  ...
  const query = useQuery({
    ...
    queryFn: () => workflowService.instances.list(filters),
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        title="Executions"
        description="Monitor durable workflow progress, waiting tasks, failures, and immutable evidence."
      />
      <Card className="grid gap-3 p-4 md:grid-cols-4">
        <Input
          aria-label="Search executions"
          placeholder="Workflow, entity, correlation ID"
          value={search}
          onChange={(event) => {
        ...
        <select
          ...
          value={state}
          onChange={(event) => setState(event.target.value as InstanceState | "")}
        >
          <option value="">All states</option>
          {["pending", "running", "waiting", "completed", "failed", "cancelled"].map((value) => (
        ...
        />
        <select
          aria-label="Order executions"
          className={selectClass}
          value={ordering}
          onChange={(event) => setOrdering(event.target.value as InstanceOrdering)}
        >
          <option value="-created_at">Newest</option>
          <option value="-priority">Highest priority</option>
          <option value="completed_at">Completion time</option>
        </select>
      </Card>
      {!query.data?.items.length ? (
        <EmptyPanel
          title="No executions found"
          description="Published manual workflows appear here after a real durable start is accepted."
        />
      ) : (
          ...
          <table className="w-full min-w-[880px]">
            <thead>
              <tr className="border-b text-left text-xs uppercase text-muted-foreground">
                <th className="p-4">Workflow / subject</th>
                <th>State</th>
                <th>Current step</th>
                <th>Priority</th>
                <th>Elapsed</th>
                ...
                <tr key={instance.id} className="border-b last:border-0">
                  <td className="p-4">
                    <button
                      ...
                      onClick={() => navigate(ROUTES.INSTANCE_DETAIL(instance.id))}
                    >
                      {instance.workflow_name} · v{instance.workflow_version}
                    </button>
                    <p className="text-xs text-muted-foreground">
                      {instance.subject ?? (instance.entity_type || "Manual execution")}
                    </p>
                  </td>
                  <td>
                  ...
                  <td>
                    {policy
                      ? formatDuration(
                          ...
                          policy.ui.duration_display_threshold_ms
                        )
                      : "—"}
                  </td>
                  <td className="max-w-48 truncate text-sm text-destructive">
                  ...
                  <td className="p-4 text-right">
                    <Button
                      size="icon"
                      variant="ghost"
                      aria-label={`View ${instance.workflow_name} execution`}
                      onClick={() => navigate(ROUTES.INSTANCE_DETAIL(instance.id))}
                    >
