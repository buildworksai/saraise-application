...
export function WorkflowInstanceListPage() {
  ...
  const filters = { page, page_size: 20, search: deferredSearch || undefined, state: state || undefined, entity_type: entityType || undefined, ordering };
  const query = useQuery({ queryKey: ["workflow-instances", filters], queryFn: () => workflowService.instances.list(filters), refetchInterval: document.visibilityState === "visible" ? 15000 : false });
