...
function isExportDocument(value: unknown): value is WorkflowConfigurationExportDTO {
  ...
  return (
    candidate.schema === "saraise.workflow-automation.configuration/v1" &&
...
export function WorkflowConfigurationPage() {
  ...
  if (!current)
    return (
      <WorkflowProblem
        error={new Error("Configuration response is empty.")}
  ...
  const flagHasAdapter = (key: string): boolean => {
    ...
    if (key === "scheduled_triggers")
      return draft.allowed_values.trigger_types.includes("scheduled");
    if (key === "parallel_workflows")
      return draft.allowed_values.workflow_types.includes("parallel");
    if (key === "timeout_notifications")
      return draft.allowed_values.timeout_actions.includes("notify");
    return false;
  };
  const applyText = () => {
    setProblem("");
  ...
  const importDocument = async (file: File) => {
    ...
    try {
      ...
      if (!isExportDocument(value))
        throw new Error("This is not a workflow automation configuration export.");
  ...
  return (
    <main className="space-y-6">
      <PageHeader
        ...
        description="Versioned tenant policy for workflow defaults, safe limits, retries, health, rollout, and operator presentation."
      />
      <Card>
        <CardHeader>
          <CardTitle>Scope and guarded defaults</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-4 md:grid-cols-3">
          ...
          <Input
            ...
            type="number"
            min={draft.limits.execution_priority_min}
            max={draft.limits.execution_priority_max}
            value={draft.defaults.execution_priority}
            readOnly
            title="Change defaults in the full policy document; limits below constrain all saves."
          />
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <Input
              ...
              min="1"
              max={draft.limits.execution_priority_max}
              value={draft.limits.execution_priority_min}
              onChange={(event) => setLimit("execution_priority_min", Number(event.target.value))}
            />
            <Input
              label="Priority maximum"
              type="number"
              min={draft.limits.execution_priority_min}
              max="9"
              value={draft.limits.execution_priority_max}
              onChange={(event) => setLimit("execution_priority_max", Number(event.target.value))}
            />
            <Input
              label="Catalog default limit"
              type="number"
              min="1"
              max={draft.limits.catalog_max_limit}
              value={draft.limits.catalog_default_limit}
              onChange={(event) => setLimit("catalog_default_limit", Number(event.target.value))}
            />
            <Input
              label="Catalog maximum"
              type="number"
              min={draft.limits.catalog_default_limit}
              max="500"
              value={draft.limits.catalog_max_limit}
              onChange={(event) => setLimit("catalog_max_limit", Number(event.target.value))}
            />
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle>Feature flags and phased rollout</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {Object.entries(draft.feature_flags).map(([key, rollout]) => {
              const available = flagHasAdapter(key);
              return (
                <div key={key} className="space-y-2 rounded border p-3 text-sm">
                  <label className="flex items-center justify-between">
                    <span>
                      <strong>{key.replaceAll("_", " ")}</strong>
                      <small className="block text-muted-foreground">
                        {available
                          ? "Available for phased rollout."
                          : "Unavailable until a complete backend adapter is installed."}
                      </small>
                    </span>
                    <input
                      type="checkbox"
                      ...
                      title={
                        available
                          ? "Enable this capability for the selected rollout."
                          : "No end-to-end adapter is installed."
                      }
                  ...
                  <div className="grid gap-2 sm:grid-cols-2">
                    <Input
                      ...
                      title="Comma-separated role identifiers; blank targets every role."
                      onChange={(event) => setRolloutList(key, "roles", event.target.value)}
                    />
                    <Input
                      label="Cohorts"
                      value={rollout.cohorts.join(", ")}
                      title="Comma-separated tenant cohort keys; blank targets every cohort."
                      onChange={(event) => setRolloutList(key, "cohorts", event.target.value)}
                    />
                  </div>
                </div>
              );
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <p className="text-sm text-muted-foreground">
            Every audited default, threshold, allow-list, handler mapping, lifecycle policy,
            operational value, and visual setting is portable here. The server rejects unknown
            sections and unsafe combinations.
          </p>
          <Textarea
            aria-label="Complete workflow configuration document"
            rows={24}
          ...
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={applyText}>
          ...
          {save.error ? <WorkflowProblem error={save.error} retry={() => save.mutate()} /> : null}
          {preview.data ? (
            <p role="status" className="rounded border p-3 text-sm">
              Changed sections: {preview.data.changed_sections.join(", ") || "none"}. Restart
              required: {preview.data.restart_required ? "yes" : "no"}.
            </p>
          ) : null}
        </CardContent>
      </Card>
              ...
              {history.data.map((revision) => (
                ...
                >
                  <span>
                    ...
                    <small className="block text-muted-foreground">
                      {revision.change_reason} · actor {revision.actor_id ?? "bootstrap"} ·{" "}
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    disabled={revision.version === current.version || rollback.isPending}
                    onClick={() => rollback.mutate(revision.version)}
                  >
