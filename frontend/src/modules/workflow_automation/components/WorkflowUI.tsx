...
export function WorkflowProblem({ error, retry }: { error: Error; retry: () => void }) {
  ...
  const title = denied
    ...
    : missing
      ...
      : degraded
        ? "Workflow capability unavailable"
        : "Workflow data is unavailable";
  const message = denied
    ? "Your role is not allowed to use this workflow surface."
    : missing
      ? "The record may have been removed or belongs to another tenant."
      : degraded
        ? "A required workflow handler or delivery dependency is unavailable. No success has been assumed."
        : error.message;
  return (
    <Card role="alert" className="border-destructive/30">
      <CardContent className="flex min-h-72 flex-col items-center justify-center text-center">
        <Icon className="mb-4 h-10 w-10 text-destructive" aria-hidden="true" />
        <h2 className="text-xl font-semibold">{title}</h2>
        <p className="mt-2 max-w-lg text-sm text-muted-foreground">{message}</p>
        {api?.correlationId ? (
          <div className="mt-4 flex items-center gap-2 rounded bg-muted px-3 py-2 font-mono text-xs">
            <span>Correlation: {api.correlationId}</span>
            <Button
              size="icon"
              variant="ghost"
              aria-label="Copy correlation ID"
              onClick={() => void navigator.clipboard.writeText(api.correlationId ?? "")}
            >
...
const colors: Readonly<Record<string, string>> = {
  ...
  setup_required: "bg-amber-100 text-amber-800",
  degraded: "bg-red-100 text-red-800",
};
export function StatusPill({ status }: { status: string }) {
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${colors[status] ?? "bg-muted text-muted-foreground"}`}
    >
      {status.replaceAll("_", " ")}
    </span>
  );
