...
export function TaskDecisionDialog({
  open,
  decision,
  taskName,
  pending,
  ...
  return (
    <Dialog
      ...
      title={rejecting ? `Reject ${taskName}` : `Complete ${taskName}`}
      description={
        rejecting
          ? "A reason is required and becomes part of the immutable decision evidence."
          : "Confirm that the requested human action is complete."
      }
    >
      <div className="space-y-4">
        {rejecting ? (
          <label htmlFor="decision-reason" className="block text-sm font-medium">
            Reason
            <Textarea
              id="decision-reason"
              autoFocus
        ...
        <div className="flex justify-end gap-2">
          ...
          >
            {pending ? "Recording decision…" : rejecting ? "Reject task" : "Complete task"}
          </Button>
        </div>
      </div>
    </Dialog>
