...
describe("blockchain traceability route contract", () => {
  it("publishes every page as a titled, discoverable sidebar NavItem", () => {
    ...
    expect(
      tenantRoutes.every(
        (route) => route.navigation.type !== "sidebar" || Boolean(route.navigation.label.trim())
      )
    ).toBe(true);
  ...
  it("passes registry parity and ownership validation", () => {
    ...
    expect(
      tenantRoutes.every((route) =>
        route.sourceFile.startsWith("modules/blockchain_traceability/pages/")
      )
    ).toBe(true);
    expect(
      tenantRoutes
        .filter((route) => route.navigation.type === "sidebar")
        .every((route) => !(route.navigation.path ?? route.path).includes(":"))
    ).toBe(true);
    expect(
      tenantRoutes
        .filter((route) => route.path.includes(":"))
        .every(
          (route) => route.navigation.type === "sidebar" && !route.navigation.path?.includes(":")
        )
    ).toBe(true);
  });
});
