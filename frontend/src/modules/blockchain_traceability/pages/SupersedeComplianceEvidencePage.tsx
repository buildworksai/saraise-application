...
const FIELDS = [
  {
    name: "superseded_id",
    label: "Finalized evidence ID to supersede",
    required: true,
    help: "The server verifies tenant ownership and that the source evidence is finalized.",
  },
...
function result(value: string): ComplianceResult {
  if (value === "pass" || value === "fail" || value === "warning" || value === "not_applicable")
    return value;
...
export function SupersedeComplianceEvidencePage() {
        ...
        <ApiProblem
          error={
            new BlockchainTraceabilityApiError(
              "The governed capability response does not permit compliance supersession.",
              403,
              "permission_denied",
              {},
              null
  ...
  if (
    !capabilities.data.document.features.enabled ||
    !capabilities.data.document.features.enable_supersede
  )
    return (
      <main className="p-4 sm:p-8">
        <PageHeader
          title="Supersession disabled"
          description="The active tenant feature policy has disabled compliance supersession."
        />
        <Card className="p-6">
          <p className="text-sm text-muted-foreground">
            An administrator can review the rollout policy in traceability configuration. No
  ...
  if (created)
    return (
      <main className="space-y-6 p-4 sm:p-8">
        <PageHeader
          title="Superseding evidence created"
          description="The correction was appended without rewriting or deleting the original evidence."
        />
        <Card className="p-6" aria-live="polite">
          <p className="text-sm">
            Created <strong>{created.evidence_key}</strong> with status {created.status}.
          </p>
          <Button
            className="mt-4"
            onClick={() => navigate(ROUTE_PATHS.COMPLIANCE_DETAIL(created.id))}
          >
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        ...
        description="Create an append-only correction linked to finalized evidence. The original record remains immutable."
        breadcrumbs={
          <Breadcrumbs
            items={[
      ...
      <MutationForm
        fields={FIELDS}
        submitLabel="Create superseding evidence"
        caution="Supersession is auditable and cannot erase the prior evidence. Review every identifier and correction detail before continuing."
        onCancel={() => navigate(ROUTE_PATHS.COMPLIANCE)}
        onSubmit={async (data) => {
          const supersededId = formString(data, "superseded_id");
          setCreated(
            await blockchainTraceabilityService.supersedeComplianceEvidence(supersededId, {
