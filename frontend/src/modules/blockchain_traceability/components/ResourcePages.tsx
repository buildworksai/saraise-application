...
import { ApiProblem, Breadcrumbs, EmptyPanel, PageHeader, PageSkeleton, Pagination, StatusPill, useCanMutateTraceability } from './ModuleShell';
...
export interface ListPageConfig<T extends { id: string }, F extends PageFilters> {
  ...
  filters: (params: URLSearchParams) => F;
...
export function defaultPageFilters(params: URLSearchParams): PageFilters {
  return { page: positivePage(params.get('page')), page_size: 25, search: params.get('search') ?? undefined, ordering: params.get('ordering') ?? undefined };
}
...
function ResourceCard<T extends { id: string }>({ item, config, open }: { item: T; config: DisplayConfig<T>; open: () => void }) {
  return <Card className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><button className="truncate text-left font-semibold text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={open}>{config.titleFor(item)}</button><p className="mt-1 truncate text-xs text-muted-foreground">{config.subtitleFor(item)}</p></div><StatusPill status={config.statusFor(item)} /></div><dl className="mt-4 grid grid-cols-2 gap-3">{config.facts(item).slice(0, 4).map((fact) => <div key={fact.label}><dt className="text-xs text-muted-foreground">{fact.label}</dt><dd className="mt-1 truncate text-sm">{fact.value}</dd></div>)}</dl></Card>;
}
...
export function ResourceListPage<T extends { id: string }, F extends PageFilters>({ config }: { config: ListPageConfig<T, F> }) {
  ...
  const query = useQuery({ queryKey: ['blockchain-traceability', config.queryKey, params.toString()], queryFn: () => config.list(filters) });
  ...
  if (query.isLoading) return <PageSkeleton label={`Loading ${config.noun}`} />;
