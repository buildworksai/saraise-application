...
export function defaultPageFilters(params: URLSearchParams, pageSize?: number): PageFilters {
  ...
  if (!Number.isInteger(effectivePageSize) || effectivePageSize < 1)
    throw new Error("The governed API did not provide a valid list page size.");
  return {
...
type DisplayConfig<T extends { id: string }> = Pick<
  ListPageConfig<T, PageFilters>,
  "titleFor" | "subtitleFor" | "statusFor" | "facts"
>;
    ...
    <Card className="p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <button
            className="truncate text-left font-semibold text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            onClick={open}
          >
            {config.titleFor(item)}
          </button>
          <p className="mt-1 truncate text-xs text-muted-foreground">{config.subtitleFor(item)}</p>
        </div>
        <ConfiguredStatusPill status={config.statusFor(item)} />
      </div>
...
export function ResourceListPage<T extends { id: string }, F extends PageFilters>({
  config,
}: {
  config: ListPageConfig<T, F>;
}) {
  ...
  const query = useQuery({
    queryKey: ["blockchain-traceability", config.queryKey, params.toString(), pageSize],
  ...
  if (capabilities.isLoading || query.isLoading)
    return <PageSkeleton label={`Loading ${config.noun}`} />;
  if (capabilities.error || query.error || !query.data)
    return (
      <main className="p-4 sm:p-8">
        <ApiProblem
          error={capabilities.error ?? query.error}
          onRetry={() => {
            void Promise.all([capabilities.refetch(), query.refetch()]);
          }}
        />
      </main>
    );
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        ...
        breadcrumbs={
          <Breadcrumbs
            items={[{ label: "Traceability", to: ROUTE_PATHS.ASSETS }, { label: config.title }]}
          />
        }
        actions={
          canMutate && config.createPath ? (
            <Button onClick={() => navigate(config.createPath!)}>
              <Plus className="mr-2 h-4 w-4" />
              {config.createLabel ?? `Create ${config.noun}`}
            </Button>
          ) : undefined
        }
      />
      <Card className="p-4">
        <form
          className="grid gap-3 md:grid-cols-[minmax(0,1fr)_12rem_12rem_auto]"
          role="search"
          onSubmit={submitSearch}
        >
          <Input
            id={`${config.queryKey}-search`}
            name="search"
            defaultValue={params.get("search") ?? ""}
            aria-label={`Search ${config.noun}`}
          ...
          <select
            ...
            value={params.get("ordering") ?? config.orderingOptions[0]?.value ?? ""}
      ...
      {query.data.items.length === 0 ? (
        <EmptyPanel
          title={`No ${config.noun} found`}
          description={
            params.toString()
              ? "No records match the selected server-side filters. Clear or adjust the filters and retry."
          ...
          <div className="grid gap-3 p-3 md:hidden">
            {query.data.items.map((item) => (
              <ResourceCard
                ...
                item={item}
                config={config}
                open={() => navigate(config.detailPath(item.id))}
              />
            ))}
          </div>
          <div className="hidden overflow-x-auto md:block">
            <table className="w-full text-left text-sm">
              <thead className="border-b bg-muted/50 text-xs uppercase text-muted-foreground">
                <tr>
                  <th className="p-4">{config.noun}</th>
                  <th className="p-4">Status</th>
                  {(query.data.items[0] ? config.facts(query.data.items[0]) : [])
                    .slice(0, 3)
                    .map((fact) => (
                      <th key={fact.label} className="p-4">
                        {fact.label}
                      </th>
                    ))}
                  <th className="p-4">
                ...
                {query.data.items.map((item) => (
                  <tr key={item.id} className="border-b hover:bg-muted/40">
                    ...
                    {config
                      ...
                      .slice(0, 3)
                      .map((fact) => (
                        <td key={fact.label} className="max-w-xs truncate p-4">
                          {fact.value}
                        </td>
                      ))}
...
export function ResourceDetailPage<T>({ config }: { config: DetailPageConfig<T> }) {
  const query = useQuery({
    queryKey: ["blockchain-traceability", config.queryKey, config.id],
    queryFn: () => config.get(config.id),
  ...
  return (
    <main className="space-y-6 p-4 sm:p-8">
      <PageHeader
        title={config.title(query.data)}
        description={config.description(query.data)}
        breadcrumbs={
          <Breadcrumbs
            items={[
              { label: "Traceability", to: ROUTE_PATHS.ASSETS },
              { label: config.listLabel, to: config.listPath },
              { label: config.title(query.data) },
            ]}
          />
        }
        actions={config.actions?.(query.data)}
      />
      {config.render(query.data)}
    </main>
  );
}
...
export function MutationForm({
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (failure instanceof BlockchainTraceabilityApiError) {
        const apiFailure: BlockchainTraceabilityApiError = failure;
        setFieldErrors(apiFailure.fieldErrors);
      }
    } finally {
      setPending(false);
    }
  ...
  return (
    ...
    >
      {caution && (
        <div
          className="rounded-lg border text-accent-foreground bg-accent p-4 text-sm text-accent-foreground"
          role="note"
        >
          {caution}
        </div>
      )}
      {error !== null && <ApiProblem error={error} mutation />}
      <Card className="grid gap-5 p-6 md:grid-cols-2">
        {fields.map((field) => {
          const errorMessage = fieldErrors.get(field.name);
          const common = {
            id: field.name,
            name: field.name,
            label: field.label,
          ...
          return (
            <div
              key={field.name}
              className={
                field.type === "textarea" || field.type === "json" ? "md:col-span-2" : undefined
              }
            >
              ...
              {field.help && (
                <p id={`${field.name}-help`} className="mt-1 text-xs text-muted-foreground">
                  {field.help}
                </p>
              )}
            </div>
