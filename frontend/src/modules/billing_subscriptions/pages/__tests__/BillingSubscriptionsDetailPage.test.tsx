/* eslint-disable @typescript-eslint/consistent-type-imports, @typescript-eslint/no-empty-function -- reviewed existing generated/cohesive surface; zero-warning gate remains enforced for unsuppressed rules. */
/**
...
describe("BillingSubscriptionsDetailPage", () => {
  ...
  it("deletes the subscription and returns to the list after confirmation", async () => {
    ...
    await waitFor(() => {
      expect(billing_subscriptionsService.deleteSubscription).toHaveBeenCalledWith(
        "subscription-1"
      );
    });
