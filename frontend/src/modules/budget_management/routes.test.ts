...
describe("budget management tenant routes", () => {
  ...
  it("publishes unique valid paths and parents", () => {
    expect(new Set(budgetRoutes.map((route) => route.path)).size).toBe(budgetRoutes.length);
    expect(getTenantRouteValidationIssues(budgetRoutes)).toEqual([]);
  });
  it("keeps sidebar leaves in parity with sidebar descriptors", () => {
    const tree = buildTenantSidebarTree(budgetRoutes);
    expect(tree).toHaveLength(1);
    expect(tree[0]?.children.map((leaf) => leaf.routeId)).toEqual(
      budgetRoutes.filter((route) => route.navigation.type === "sidebar").map((route) => route.id)
    );
  });
});
