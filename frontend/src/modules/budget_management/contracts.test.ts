...
describe("budget management v2 contracts", () => {
  it("publishes the single governed v2 authority", () => {
    ...
    expect(ENDPOINTS.BUDGETS.ALLOCATIONS("b1")).toBe(
      "/api/v2/budget-management/budgets/b1/allocations/"
    );
    ...
    expect(ENDPOINTS.BUDGETS.SYNC_ACTUALS("b1")).toBe(
      "/api/v2/budget-management/budgets/b1/sync-actuals/"
    );
    expect(ENDPOINTS.APPROVALS.LIST).toBe("/api/v2/budget-management/approvals/");
    expect(ENDPOINTS.VARIANCE_ALERTS.GENERATE).toBe(
  ...
  it("serializes only supplied filter values", () => {
    expect(withQuery(ENDPOINTS.BUDGETS.LIST, { page: 2, search: "ops", status: undefined })).toBe(
      `${ENDPOINTS.BUDGETS.LIST}?page=2&search=ops`
    );
  });
});
