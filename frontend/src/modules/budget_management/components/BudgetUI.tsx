...
import type { BudgetStatus, DecimalString, PaginationMeta } from "../contracts";
...
export function PageSkeleton({ rows = 6 }: { readonly rows?: number }) {
  return (
    <main className="space-y-5 p-4 sm:p-8" aria-label="Loading budget data" aria-busy="true">
...
export function RefreshIndicator({ active }: { readonly active: boolean }) {
  return active ? (
...
export function GovernedError({
  ...
  const governed = error instanceof BudgetManagementApiError ? error : undefined;
  const title =
    governed?.status === 403
      ? "Permission required"
      : governed?.status === 404
  ...
  const message =
    governed?.status === 404
      ? "This record does not exist or is outside your tenant boundary."
      ...
      <CardContent className="flex min-h-64 flex-col items-center justify-center p-8 text-center">
        ...
        {governed?.correlationId ? (
          <p className="mt-2 font-mono text-xs text-muted-foreground">
            Correlation: {governed.correlationId}
          </p>
        ) : null}
        {onRetry ? (
...
const statusColors: Record<BudgetStatus, string> = {
  ...
  pending_approval: "bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-100",
  ...
  revision: "bg-blue-100 text-blue-900 dark:bg-blue-950 dark:text-blue-100",
  closed: "bg-zinc-200 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100",
};
export function StatusPill({ status }: { readonly status: BudgetStatus }) {
  return (
    <span
...
export function Pagination({
  ...
  return (
    <nav
      ...
      className="flex flex-col items-center justify-between gap-3 border-t p-4 sm:flex-row"
    >
      <span className="text-sm text-muted-foreground">
        Page {value.page} of {Math.max(value.total_pages, 1)} · {value.count} budgets
      </span>
