...
export function ApiManagementListPage() {
  ...
  useEffect(() => {
    document.title = "API Management resources · SARAISE";
  }, []);
  ...
  const lifecycle = useMutation({
    mutationFn: (resource: ApiManagementResource) =>
      resource.is_active
  ...
  const archive = useMutation({
    mutationFn: (resource: ApiManagementResource) =>
      api_managementService.deleteResource(resource.id),
    onSuccess: async (_result, resource) => {
      setPendingDelete(null);
      await refresh();
      toast.success("Resource archived", {
        action: { label: "Restore", onClick: () => restore.mutate(resource.id) },
      });
    },
    ...
    return (
      <div className="p-8">
        <ErrorState
          ...
          message="Resource operations are disabled because the tenant policy could not be loaded."
          onRetry={() => {
    ...
    return (
      <div className="p-8">
        <ErrorState
          ...
          message="The tenant configuration currently disables this capability."
        />
      </div>
    );
  }
  if (resources.isLoading)
    return (
      <div className="p-8" role="status" aria-label="Loading resources">
        <TableSkeleton rows={policy.table_skeleton_rows} columns={resourceColumns.length} />
      </div>
    );
  if (resources.error) {
    return (
    ...
    <main className="space-y-6 p-8">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Tenant-isolated API resources governed by versioned policy.
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => navigate(ROUTES.RESOURCE_CREATE)}>
      ...
      <Card className="p-6">
        <div className="mb-4 grid gap-3 md:grid-cols-[1fr_180px]">
          <div className="relative">
            ...
            <Input
              ...
              placeholder={
                canSearch
                  ? `Search ${policy.search_fields.join(" or ")}…`
                  : "Search disabled by tenant policy"
              }
              value={search}
              disabled={!canSearch}
              onChange={(event) => {
          ...
          <select
            ...
            disabled={!policy.filter_fields.includes("is_active")}
        ...
        {items.length === 0 ? (
          <EmptyState
            icon={Plus}
            title={deferredSearch ? "No matching resources" : "No resources yet"}
            description={
              deferredSearch
                ? "Change the server-side search term."
                : "Create the first governed API resource."
            }
            action={
              deferredSearch
                ? undefined
                : { label: "Create resource", onClick: () => navigate(ROUTES.RESOURCE_CREATE) }
            }
          />
        ) : (
              ...
              <thead>
                <tr className="border-b text-left">
                  {resourceColumns.map((column) => (
                    <th key={column} className={column === "Actions" ? "p-2 text-right" : "p-2"}>
                      {column}
                    </th>
                  ))}
                </tr>
                ...
                {items.map((resource) => (
                  <tr key={resource.id} className="border-b hover:bg-muted/40">
                    ...
                    <td className="p-2 text-right">
                      ...
                      {resource.is_active && policy.deactivation_enabled ? (
                        <Button
                          size="sm"
                          variant="ghost"
                          disabled={lifecycle.isPending}
                          onClick={() => lifecycle.mutate(resource)}
                        >
                          Deactivate
                        </Button>
                      ) : null}
                      {!resource.is_active && policy.activation_enabled ? (
                        <Button
                          size="sm"
                          variant="ghost"
                          disabled={lifecycle.isPending}
        ...
        {resources.data && resources.data.count > policy.page_size ? (
          <nav aria-label="Resource pages" className="mt-4 flex items-center justify-end gap-2">
            <Button
              variant="outline"
              size="sm"
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open && !archive.isPending) setPendingDelete(null);
        }}
      ...
      >
        <div className="flex justify-end gap-2">
          ...
          <Button
            variant="danger"
            disabled={!pendingDelete || archive.isPending}
            onClick={() => {
