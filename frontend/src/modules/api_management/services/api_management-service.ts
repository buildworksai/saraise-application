...
async function getRuntimeConfiguration(): Promise<ApiManagementConfiguration> {
  const schema = await apiClient.get<ApiManagementConfigurationSchema>(
...
export const api_managementService = {
  ...
  ): Promise<PaginatedResponse<ApiManagementResource>> => {
    const response = await apiClient.get<
      PaginatedResponse<ApiManagementResource> | readonly ApiManagementResource[]
    >(ENDPOINTS.RESOURCES.LIST(filters));
    return asPage(response);
  },
  getResource: (id: string): Promise<ApiManagementResource> =>
    apiClient.get(ENDPOINTS.RESOURCES.DETAIL(id)),
  createResource: (data: ApiManagementResourceCreate): Promise<ApiManagementResource> => {
    const { idempotency_key, ...body } = data;
    return apiClient.post(ENDPOINTS.RESOURCES.CREATE, body, {
      headers: { "Idempotency-Key": idempotency_key },
    });
  },
  updateResource: (
    id: string,
    data: ApiManagementResourceUpdate
  ): Promise<ApiManagementResource> => {
    const { idempotency_key, ...body } = data;
    return apiClient.put(ENDPOINTS.RESOURCES.UPDATE(id), body, {
      headers: { "Idempotency-Key": idempotency_key },
    });
  },
  deleteResource: (id: string): Promise<void> =>
    apiClient.delete(ENDPOINTS.RESOURCES.DELETE(id), {
      headers: { "Idempotency-Key": crypto.randomUUID() },
    }),
  restoreResource: (id: string): Promise<ApiManagementResource> =>
    apiClient.post(ENDPOINTS.RESOURCES.RESTORE(id), undefined, {
      headers: { "Idempotency-Key": crypto.randomUUID() },
    }),
  activateResource: (id: string): Promise<ApiManagementResource> =>
    apiClient.post(ENDPOINTS.RESOURCES.ACTIVATE(id), undefined, {
      headers: { "Idempotency-Key": crypto.randomUUID() },
    }),
  deactivateResource: (id: string): Promise<ApiManagementResource> =>
    apiClient.post(ENDPOINTS.RESOURCES.DEACTIVATE(id), undefined, {
      headers: { "Idempotency-Key": crypto.randomUUID() },
    }),
  ...
  ): Promise<ApiManagementConfigurationSchema> =>
    apiClient.get(ENDPOINTS.CONFIGURATION.SCHEMA(environment)),
  getRuntimeConfiguration,
  getConfiguration: (environment: DeploymentEnvironment): Promise<ApiManagementConfiguration> =>
    apiClient.get(ENDPOINTS.CONFIGURATION.CURRENT(environment)),
  updateConfiguration: (
    environment: DeploymentEnvironment,
    request: ConfigurationWriteRequest
  ): Promise<ApiManagementConfiguration> =>
    apiClient.put(ENDPOINTS.CONFIGURATION.CURRENT(environment), request),
  previewConfiguration: (
    environment: DeploymentEnvironment,
    request: ConfigurationPreviewRequest
  ): Promise<ConfigurationPreview> =>
    apiClient.post(ENDPOINTS.CONFIGURATION.PREVIEW(environment), request),
  listConfigurationHistory: (
    environment: DeploymentEnvironment,
    filters: ConfigurationHistoryFilters = {}
  ): Promise<PaginatedResponse<ConfigurationVersion>> =>
    apiClient.get(ENDPOINTS.CONFIGURATION.HISTORY(environment, filters)),
  rollbackConfiguration: (
    environment: DeploymentEnvironment,
    request: ConfigurationRollbackRequest
  ): Promise<ApiManagementConfiguration> =>
    apiClient.post(ENDPOINTS.CONFIGURATION.ROLLBACK(environment), request),
  importConfiguration: (
    environment: DeploymentEnvironment,
    request: ConfigurationImportRequest
  ): Promise<ApiManagementConfiguration> =>
    apiClient.post(ENDPOINTS.CONFIGURATION.IMPORT(environment), request),
  exportConfiguration: (
    environment: DeploymentEnvironment
  ): Promise<PortableApiManagementConfiguration> =>
    apiClient.get(ENDPOINTS.CONFIGURATION.EXPORT(environment)),
};
