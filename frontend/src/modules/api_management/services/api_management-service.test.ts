...
describe("api_managementService", () => {
  ...
  it("normalizes a transitional list response without fabricating items", async () => {
    ...
    await expect(
      api_managementService.listResources({ search: "Resource", page: 1 })
    ).resolves.toEqual({ count: 1, next: null, previous: null, results: [resource] });
  ...
  it("sends resource idempotency in the required header, never the body", async () => {
    ...
    await api_managementService.createResource({
      name: resource.name,
      config: {},
  ...
  it("adds fresh idempotency headers to lifecycle operations", async () => {
    ...
    expect(postMock).toHaveBeenCalledWith(ENDPOINTS.RESOURCES.ACTIVATE(resource.id), undefined, {
      headers: { "Idempotency-Key": idempotencyKey },
    });
    await api_managementService.restoreResource(resource.id);
    expect(postMock).toHaveBeenLastCalledWith(ENDPOINTS.RESOURCES.RESTORE(resource.id), undefined, {
      headers: { "Idempotency-Key": idempotencyKey },
    });
  });
  ...
  it("uses only the configuration endpoint registry", async () => {
    ...
    expect(postMock).toHaveBeenCalledWith(ENDPOINTS.CONFIGURATION.PREVIEW("staging"), {
      document: { page_size: 25 },
    });
    getMock.mockResolvedValue({ count: 0, next: null, previous: null, results: [] });
    await api_managementService.listConfigurationHistory("staging", { page: 2, page_size: 25 });
    expect(getMock).toHaveBeenCalledWith(
      ENDPOINTS.CONFIGURATION.HISTORY("staging", { page: 2, page_size: 25 })
    );
  });
