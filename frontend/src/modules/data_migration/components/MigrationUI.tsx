...
export function EmptyPanel({
  ...
  return (
    <Card className="p-10 text-center">
      <DatabaseZap className="mx-auto h-10 w-10 text-muted-foreground" aria-hidden="true" />
...
const stateStyles: Record<JobStatus | RunStatus | RollbackStatus, string> = {
  ...
  partial: "bg-destructive/10 text-destructive",
...
export function StateBadge({ state }: { state: JobStatus | RunStatus | RollbackStatus }) {
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${stateStyles[state]}`}
    >
      {state.replace("_", " ")}
    </span>
  );
...
export function SuccessBanner({ children }: { children: ReactNode }) {
  return (
    <div
      className="flex gap-3 rounded-lg border border-primary/20 bg-primary/5 p-4 text-sm text-foreground"
      role="status"
    >
