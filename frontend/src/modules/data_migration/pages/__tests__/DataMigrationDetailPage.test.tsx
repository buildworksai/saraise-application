...
vi.mock("../../services/data-migration-service", async (load) => {
  ...
  return {
    ...
    dataMigrationService: {
      ...
      jobs: { ...actual.dataMigrationService.jobs, get: vi.fn() },
    },
  };
});
...
describe("DataMigrationDetailPage", () => {
  it("renders an accessible skeleton while durable detail is pending", () => {
    ...
    render(
      ...
      >
        <MemoryRouter
          initialEntries={["/data-migration/jobs/00000000-0000-4000-8000-000000000001"]}
        >
          <Routes>
            <Route path="/data-migration/jobs/:id" element={<DataMigrationDetailPage />} />
          </Routes>
        </MemoryRouter>
      </QueryClientProvider>
    );
    expect(
      screen.getByRole("status", { name: "Loading migration definition" })
    ).toBeInTheDocument();
  });
});
