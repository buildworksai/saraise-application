/**
 * Create DataMigration Resource Page
 * 
 * Form for creating a new resource with validation.
 */
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import { useNavigate } from 'react-router-dom';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { toast } from 'sonner';
import { migrationService } from '../services/migration-service';
import type { MigrationJobCreate } from '../contracts';
import { Button } from '@/components/ui/Button';
import { Input } from '@/components/ui/Input';
import { Textarea } from '@/components/ui/Textarea';
import { Card } from '@/components/ui/Card';

const resourceSchema = z.object({
  name: z.string().min(1, 'Name is required'),
  source_type: z.string().min(1, 'Source type is required'),
  source_config: z.record(z.unknown()).optional(),
});

type ResourceFormData = z.infer<typeof resourceSchema>;

export const CreateDataMigrationResourcePage = () => {
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const form = useForm<ResourceFormData>({
    resolver: zodResolver(resourceSchema),
    defaultValues: {
      name: '',
      source_type: '',
      source_config: {},
    },
  });

  const createMutation = useMutation({
    mutationFn: (data: MigrationJobCreate) => migrationService.jobs.create(data),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['data_migration-jobs'] });
      toast.success('Migration job created successfully');
      navigate('/data-migration');
    },
    onError: () => {
      toast.error('Failed to create migration job. Please try again.');
    },
  });

  const onSubmit = async (data: ResourceFormData) => {
    try {
      await createMutation.mutateAsync({
        name: data.name,
        source_type: data.source_type as 'csv' | 'excel' | 'json' | 'database' | 'api',
        source_config: data.source_config ?? {},
      });
    } catch (err) {
      console.error('Failed to create migration job:', err);
    }
  };

  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">Create DataMigration Resource</h1>
      </div>

      <Card className="p-6">
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-1">
              Name *
            </label>
            <Input
              id="name"
              {...form.register('name')}
              error={form.formState.errors.name?.message}
            />
          </div>

          <div>
            <label htmlFor="source_type" className="block text-sm font-medium text-gray-700 mb-1">
              Source Type *
            </label>
            <Input
              id="source_type"
              {...form.register('source_type')}
              error={form.formState.errors.source_type?.message}
              placeholder="e.g., csv, excel, api"
            />
          </div>

          <div className="flex gap-4 pt-4">
            <Button type="submit" disabled={createMutation.isPending}>
              {createMutation.isPending ? 'Creating...' : 'Create Resource'}
            </Button>
            <Button type="button" variant="outline" onClick={() => navigate('/data-migration')}>
              Cancel
            </Button>
          </div>
        </form>
      </Card>
    </div>
  );
};
