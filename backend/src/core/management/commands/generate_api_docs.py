...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--module", type=str, help="Module name in snake_case (e.g., workflow_automation)")
        parser.add_argument("--all-modules", action="store_true", help="Generate docs for all modules")
        parser.add_argument(
            "--output-dir",
            type=str,
            default=None,
    ...
    def _find_viewset_classes(self, module):
        ...
        for name, obj in inspect.getmembers(module):
            if inspect.isclass(obj) and issubclass(obj, viewsets.ViewSet) and obj is not viewsets.ViewSet:
    ...
    def _document_viewset(self, viewset_name, viewset_class, module_prefix):
        ...
        if hasattr(viewset_class, "retrieve"):
            content += self._document_endpoint(
                "GET", f"/api/v1/{module_prefix}/resources/{{id}}/", "Get resource by ID"
            )

        if hasattr(viewset_class, "update"):
            content += self._document_endpoint(
                "PUT", f"/api/v1/{module_prefix}/resources/{{id}}/", "Update resource (full)"
            )

        if hasattr(viewset_class, "partial_update"):
            content += self._document_endpoint(
                "PATCH", f"/api/v1/{module_prefix}/resources/{{id}}/", "Update resource (partial)"
            )

        if hasattr(viewset_class, "destroy"):
            content += self._document_endpoint(
                "DELETE", f"/api/v1/{module_prefix}/resources/{{id}}/", "Delete resource"
            )
                ...
                if hasattr(action, "detail") and hasattr(action, "methods"):
                    ...
                    for http_method in methods:
                        if http_method.upper() != "OPTIONS":
                            content += self._document_endpoint(
                                http_method.upper(), url, f"Custom action: {method_name}"
                            )
