...
def enable(apps, schema_editor):
    ...
    with schema_editor.connection.cursor() as cursor:
        for table in TABLES:
            ...
            cursor.execute(
                f"CREATE POLICY {q(policy)} ON {q(table)} USING (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid) WITH CHECK (tenant_id = NULLIF(current_setting('app.tenant_id', true), '')::uuid)"
            )
