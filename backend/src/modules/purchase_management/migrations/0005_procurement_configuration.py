...
def validate_configuration_rows(apps, schema_editor):
    ...
    invalid = list(
        Config.objects.exclude(receipt_tolerance_percent__gte=0, receipt_tolerance_percent__lte=100).values_list(
            "id", flat=True
        )
    )
    if invalid:
        raise RuntimeError(f"Invalid procurement configuration rows: {invalid}")
