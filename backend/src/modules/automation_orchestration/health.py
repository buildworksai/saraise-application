"""
AutomationOrchestration Health Checks
...
def health_check(request):
    """
    Health check endpoint for AutomationOrchestration module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "automation-orchestration", "checks": {}}
    ...
    try:
        cache.set("health_check_automation_orchestration", "ok", 10)
        result = cache.get("health_check_automation_orchestration")
        if result == "ok":
    ...
    try:
        ...
        count = AutomationOrchestrationResource.objects.count()
