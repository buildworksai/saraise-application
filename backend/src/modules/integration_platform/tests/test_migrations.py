...
def test_forward_reverse_forward_cycle_and_rls(settings):
    ...
    connector = old.get_model("integration_platform", "Connector").objects.create(  # noqa: F841
        id=str(uuid.uuid4()), name="Legacy", connector_type="api", schema={}, config={}, is_active=True
    )
    integration = old.get_model("integration_platform", "Integration").objects.create(  # noqa: F841
        id=str(uuid.uuid4()),
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "SELECT relrowsecurity, relforcerowsecurity FROM pg_class WHERE relname = 'integration_platform_integrations'"  # noqa: E501
        )
        assert cursor.fetchone() == (True, True)
        cursor.execute(
            "SELECT COUNT(*) FROM pg_policies WHERE tablename = 'integration_platform_integrations' AND qual IS NOT NULL AND with_check IS NOT NULL"  # noqa: E501
        )
