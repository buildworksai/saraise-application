...
from ..models import ImmutableRecordError, IntegrationPlatformConfiguration, IntegrationPlatformConfigurationAudit
...
def test_configuration_is_tenant_scoped_versioned_audited_and_reversible() -> None:
    ...
    saved = service.save(tenant_a, actor, first, environment="test", correlation_id="config-create")
    second = default_configuration()
    second["environment"] = "test"
    second["mapping"]["preview_record_limit"] = 17
    updated = service.save(tenant_a, actor, second, environment="test", correlation_id="config-update")
    ...
    rolled_back = service.rollback(tenant_a, actor, 1, environment="test", correlation_id="config-rollback")
...
def test_configuration_rejects_unsafe_limits_and_audit_tampering() -> None:
    ...
    saved = service.save(tenant, actor, default_configuration(), correlation_id="immutable")
    audit = IntegrationPlatformConfigurationAudit.objects.get(configuration_id=saved["id"])
