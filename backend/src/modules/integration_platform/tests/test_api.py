"""
API Integration Tests for IntegrationPlatform module.
...
class TestIntegrationPlatformResourceViewSet:
    """Test IntegrationPlatformResourceViewSet CRUD operations."""

    def test_list_resources_requires_authentication(self, api_client):
        """Test that listing resources requires authentication."""
        response = api_client.get(f"/api/v1/integration-platform/resources/")
    ...
    def test_list_resources(self, authenticated_client, tenant_user):
        ...
        IntegrationPlatformResource.objects.create(
            tenant_id=tenant_id,
        ...
        response = authenticated_client.get(f"/api/v1/integration-platform/resources/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_create_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.post(
            f"/api/v1/integration-platform/resources/",
            data,
            format="json"
        )
    ...
    def test_get_resource_detail(self, authenticated_client, tenant_user):
        ...
        resource = IntegrationPlatformResource.objects.create(
            tenant_id=tenant_id,
            name="Test Resource",
        ...
        response = authenticated_client.get(f"/api/v1/integration-platform/resources/{resource.id}/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.put(
            f"/api/v1/integration-platform/resources/{resource.id}/",
            data,
            format="json"
        )
        assert response.status_code == status.HTTP_200_OK
