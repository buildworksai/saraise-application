"""
IntegrationPlatform Health Checks
...
def health_check(request):
    """
    Health check endpoint for IntegrationPlatform module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "integration-platform", "checks": {}}
    ...
    try:
        cache.set("health_check_integration_platform", "ok", 10)
        result = cache.get("health_check_integration_platform")
        if result == "ok":
