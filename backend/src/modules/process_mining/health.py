...
def health_check(request):
    """
    Health check endpoint for ProcessMining module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "process-mining", "checks": {}}
    ...
    try:
        cache.set("health_check_process_mining", "ok", 10)
        result = cache.get("health_check_process_mining")
        if result == "ok":
    ...
    try:
        ...
        count = ProcessMiningResource.objects.count()
