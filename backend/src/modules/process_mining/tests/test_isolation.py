...
@pytest.mark.parametrize(
    "factory,path",
    [
        (ExportFactory, "exports"),
        (DiscoveryFactory, "discoveries"),
        (ModelFactory, "models"),
        (ConformanceFactory, "conformance-checks"),
        (AnalysisFactory, "bottleneck-analyses"),
    ],
)
...
def test_event_list_and_detail_hide_other_tenant(authenticated_tenant_a_client, tenant_a, tenant_b):
    ...
    response = authenticated_tenant_a_client.get(
        f"{BASE}/events/", {"process_name": "orders", "start": start.isoformat(), "end": end.isoformat()}
    )
...
def test_create_payload_cannot_spoof_tenant(authenticated_tenant_a_client, tenant_a, tenant_b):
    ...
    response = authenticated_tenant_a_client.post(
        f"{BASE}/models/",
        {"name": "Reference", "process_name": "orders", "description": "", "model_data": graph()},
        format="json",
    )
...
def test_cross_tenant_update_delete_and_actions_are_404(authenticated_tenant_a_client, tenant_b):
    ...
    assert (
        authenticated_tenant_a_client.patch(f"{BASE}/models/{model.id}/", {"name": "stolen"}, format="json").status_code
        == 404
    )
    assert authenticated_tenant_a_client.delete(f"{BASE}/models/{model.id}/").status_code == 404
    assert (
        authenticated_tenant_a_client.post(
            f"{BASE}/models/{model.id}/set-reference/", {"version_id": model.id, "transition_key": "x"}, format="json"
        ).status_code
        == 404
    )
    assert (
        authenticated_tenant_a_client.post(
            f"{BASE}/exports/{export.id}/cancel/", {"transition_key": "x"}, format="json"
        ).status_code
        == 404
    )
...
def test_complete_cross_tenant_verb_matrix(authenticated_tenant_a_client, tenant_a, tenant_b, resource, factory):
    ...
    assert authenticated_tenant_a_client.patch(
        f"{BASE}/{resource}/{record.id}/", {"status": "completed"}, format="json"
    ).status_code in {404, 405}
    assert authenticated_tenant_a_client.delete(f"{BASE}/{resource}/{record.id}/").status_code == 404
    spoofed = authenticated_tenant_a_client.post(f"{BASE}/{resource}/", {"tenant_id": str(tenant_b.id)}, format="json")
