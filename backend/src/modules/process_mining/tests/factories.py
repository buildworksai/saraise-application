...
def graph() -> dict[str, object]:
    return {
        "schema_version": "1.0",
        "nodes": [
            {"id": "start", "label": "Start", "type": "start", "frequency": 10},
            {"id": "a", "label": "Approve", "type": "activity", "frequency": 10},
            {"id": "end", "label": "End", "type": "end", "frequency": 10},
        ],
        "edges": [
            {"id": "sa", "source": "start", "target": "a", "frequency": 10, "duration_seconds": 0},
            {"id": "ae", "source": "a", "target": "end", "frequency": 10, "duration_seconds": 0},
        ],
        "extensions": {},
    }
...
class EventFactory(factory.django.DjangoModelFactory):
    ...
    source_event_id = factory.Sequence(lambda n: f"event-{n}")
    ...
    event_hash = factory.LazyFunction(lambda: hashlib.sha256(uuid.uuid4().bytes).hexdigest())
...
class ExportFactory(factory.django.DjangoModelFactory):
    ...
    process_name = "order_to_cash"
    format = "json"
    event_filter = factory.LazyFunction(dict)
...
class DiscoveryFactory(factory.django.DjangoModelFactory):
    ...
    tenant_id = factory.LazyFunction(uuid.uuid4)
    created_by = factory.LazyFunction(uuid.uuid4)
    process_name = factory.Sequence(lambda n: f"order_to_cash_{n}")
...
class VersionFactory(factory.django.DjangoModelFactory):
    ...
    process_model = factory.SubFactory(ModelFactory)
    tenant_id = factory.SelfAttribute("process_model.tenant_id")
    created_by = factory.SelfAttribute("process_model.created_by")
    version = factory.Sequence(lambda n: n + 1)
    parameters = factory.LazyFunction(dict)
    model_data = factory.LazyFunction(graph)
    event_count = 100
    case_count = 10
    activity_count = 1
    avg_case_duration_seconds = Decimal("60.00")
...
class ConformanceFactory(factory.django.DjangoModelFactory):
    ...
    process_model_version = factory.SubFactory(VersionFactory)
    tenant_id = factory.SelfAttribute("process_model_version.tenant_id")
    created_by = factory.SelfAttribute("process_model_version.created_by")
    event_filter = factory.LazyFunction(dict)
    idempotency_key = factory.LazyFunction(lambda: str(uuid.uuid4()))
...
class DeviationFactory(factory.django.DjangoModelFactory):
    ...
    conformance_check = factory.SubFactory(ConformanceFactory)
    tenant_id = factory.SelfAttribute("conformance_check.tenant_id")
    created_by = factory.SelfAttribute("conformance_check.created_by")
    case_id = "case-1"
...
class AnalysisFactory(factory.django.DjangoModelFactory):
    ...
    created_by = factory.LazyFunction(uuid.uuid4)
    process_name = "order_to_cash"
    time_range_start = factory.LazyFunction(lambda: timezone.now() - timedelta(days=1))
    time_range_end = factory.LazyFunction(timezone.now)
    idempotency_key = factory.LazyFunction(lambda: str(uuid.uuid4()))
...
class FindingFactory(factory.django.DjangoModelFactory):
    ...
    tenant_id = factory.SelfAttribute("analysis.tenant_id")
    created_by = factory.SelfAttribute("analysis.created_by")
    from_activity = "Create"
    to_activity = "Approve"
    avg_duration_seconds = Decimal("20")
    median_duration_seconds = Decimal("10")
    p95_duration_seconds = Decimal("100")
    case_count = 20
    severity = "high"
    rank = 1
...
class VariantFactory(factory.django.DjangoModelFactory):
    ...
    tenant_id = factory.SelfAttribute("analysis.tenant_id")
    created_by = factory.SelfAttribute("analysis.created_by")
    variant_key = factory.LazyFunction(lambda: hashlib.sha256(uuid.uuid4().bytes).hexdigest())
    activities = factory.LazyFunction(lambda: ["Create", "Approve"])
    case_count = 20
