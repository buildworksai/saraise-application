...
class ProcessMiningConfigurationService:
    ...
    def validate_document(document: object) -> dict[str, object]:
        ...
        if not (
            normalized["bottleneck_critical_ratio"]
            > normalized["bottleneck_high_ratio"]
        ...
        if (
            ...
            or not keys
            or len(keys) > 100
            or not all(isinstance(item, str) and item.strip() and len(item) <= 64 for item in keys)
        ):
    ...
    def get_configuration(
        self, tenant_id: UUID, actor_id: UUID | None = None, correlation_id: str | None = None
    ) -> ProcessMiningConfiguration:
    ...
    def history(self, tenant_id: UUID) -> QuerySet[ProcessMiningConfigurationVersion]:
        ...
        return (
            ProcessMiningConfigurationVersion.objects.for_tenant(configuration.tenant_id)
            .filter(configuration=configuration)
            .order_by("-version", "-created_at")
        )

    def rollback(
        self, tenant_id: UUID, actor_id: UUID, correlation_id: str, version: int
    ) -> ProcessMiningConfiguration:
    ...
    def import_document(
        self, tenant_id: UUID, actor_id: UUID, correlation_id: str, payload: object
    ) -> ProcessMiningConfiguration:
        if (
            not isinstance(payload, dict)
            or payload.get("schema_version") != "1.0"
            or payload.get("module") != "process_mining"
        ):
            raise ValidationError(
                {"configuration": "Expected a process_mining configuration document with schema_version 1.0."}
            )
        return self.update(
    ...
    def _record(
        ...
        ProcessMiningConfigurationAudit.objects.create(
...
def _enqueue_or_unavailable(
    ...
    try:
        ...
        logger.exception(
            "process_mining.queue_unavailable",
            extra={
                "event_name": "process_mining.queue_unavailable",
        ...
        raise CapabilityUnavailable(
            capability="durable_job_dispatch", message="Durable processing is temporarily unavailable."
        ) from exc
...
class EventLogService:
    def ingest_events(
        ...
        for index, raw in enumerate(events):
            try:
                ...
                event_hash = _canonical_hash(
                    tenant_id, process_name, case_id, activity, occurred_at, source_module, source_event_id
                )
                if event_hash in seen_hashes or (source_event_id and source_event_id in seen_sources):
                    evidence.append(
                        IngestRowEvidence(
                            ...
                            message="Duplicate event identity in this batch.",
                        )
                    )
                    continue
        ...
        for index, event in valid:
            if event.event_hash in existing_hashes or (
                event.source_event_id and event.source_event_id in existing_sources
            ):
                evidence.append(
                    IngestRowEvidence(
                        index, "duplicate", code="ALREADY_INGESTED", message="Event identity already exists."
                    )
                )
            else:
        ...
        with transaction.atomic():
            ...
            for event in inserts:
                persisted = (
                    ProcessEvent.objects.for_tenant(tenant_id).filter(event_hash=event.event_hash).only("id").first()
                )
                evidence.append(
                    IngestRowEvidence(
                        next(index for index, candidate in valid if candidate is event),
                        "accepted",
            ...
            aggregate_id = (
                inserts[0].id
                if inserts
                else uuid.uuid5(uuid.NAMESPACE_URL, f"process-events:{tenant_id}:{process_name}")
            )
            _publish(
                tenant_id,
                "process_event_batch",
                aggregate_id,
                "process.events.ingested",
                {
                    "process_name": process_name,
                    "source_module": source_module,
                    "accepted": len(inserts),
                    "rejected": sum(item.status == "rejected" for item in evidence),
                    "duplicates": sum(item.status == "duplicate" for item in evidence),
                },
            )
        ordered = tuple(sorted(evidence, key=lambda item: item.index))
        result = IngestResult(
            sum(item.status == "accepted" for item in ordered),
            sum(item.status == "rejected" for item in ordered),
            sum(item.status == "duplicate" for item in ordered),
            ordered,
        )
        logger.info(
            "process.events.ingested",
            extra={
                "event_name": "process.events.ingested",
                "tenant_id": str(tenant_id),
                "actor_id": str(actor_id),
    ...
    def purge_expired_events(
        self, tenant_id: UUID, retention_days: int | None = None, actor_id: UUID | None = None
    ) -> int:
        tenant_id = _tenant(tenant_id)
        ...
        with transaction.atomic():
            ...
            tombstone, _created = ProcessEventRetentionTombstone.objects.get_or_create(
                ...
                defaults={
                    "created_by": actor_id,
                    "event_count": count,
                    "reason": "configured retention authorization",
                    "correlation_id": _correlation_id(),
                },
            )
            _publish(
                tenant_id,
                "process_event_retention",
                tombstone.id,
                "process.events.retention_authorized",
                {"count": count, "cutoff": cutoff.isoformat(), "actor_id": str(actor_id)},
            )
        return count
...
class ProcessMiningQueryService:
    ...
    def models(tenant_id: UUID, filters: Mapping[str, object]) -> QuerySet[ProcessModel]:
        ...
        if filters.get("has_reference") is not None:
            ...
            referenced = ProcessModelReferenceAssignment.objects.for_tenant(_tenant(tenant_id)).values(
                "process_model_id"
            )
...
class ExportService:
    def request_export(
        ...
        if count > _config_int(configuration, "max_export_events") or projected > _config_int(
            ...
            raise OperationFailed(
                ...
                detail={"projected_rows": count, "projected_bytes": projected},
                http_status=413,
            )
        with transaction.atomic():
            record = EventExportJob.objects.create(
                tenant_id=tenant_id,
                created_by=actor_id,
                process_name=process_name,
                format=format,
                event_filter=dict(event_filter),
                idempotency_key=idempotency_key,
            )
            job = _enqueue_or_unavailable(
                tenant_id,
                actor_id,
                "process_mining.export_event_log",
                {"export_id": str(record.id)},
                f"process_mining.export:{idempotency_key}",
            )
            ...
            _publish(
                ...
                "event_export",
                record.id,
                "process.export.requested",
                {"export_id": str(record.id), "format": format, "projected_rows": count},
            )
        return record
    ...
    def run_export(self, tenant_id: UUID, export_id: UUID, async_job_id: UUID) -> EventExportJob:
        ...
        try:
            ...
            now = timezone.now()
            with transaction.atomic():
                EventExportJob.objects.for_tenant(tenant_id).filter(pk=record.id).update(
            ...
            if stored_key and default_storage.exists(stored_key):
                default_storage.delete(stored_key)
            EventExportJob.objects.for_tenant(tenant_id).filter(pk=record.id).update(
                error_code="EXPORT_STORAGE_FAILED", error_message="Export storage is unavailable."
            )
            ...
            if fresh.status == ExportStatus.RUNNING:
                _workflow_machine(tenant_id, "export").apply(
                    ...
                    "fail",
                    tenant_id=tenant_id,
                    transition_key=f"worker:{async_job_id}:fail",
                    metadata=_audit(actor_id, "Export failed"),
                )
            _publish(
                tenant_id,
                "event_export",
                ...
                {"export_id": str(record.id), "error_code": "EXPORT_STORAGE_FAILED"},
            )
    ...
    def open_download(self, tenant_id: UUID, export_id: UUID) -> tuple[EventExportJob, Any]:
        ...
        if (
            ...
            or not default_storage.exists(record.artifact_key)
        ):
        ...
        if digest.hexdigest() != record.sha256:
            stream.close()
            raise OperationFailed(
                error_code="ARTIFACT_INTEGRITY_FAILED", message="Export verification failed.", http_status=503
            )
        stream.seek(0)
        return record, stream

    def _transition(
        self, tenant_id: UUID, export_id: UUID, actor_id: UUID, command: str, transition_key: str, reason: str = ""
    ) -> EventExportJob:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
        record = _get(EventExportJob, tenant_id, export_id, active=True)
        updated = cast(
    ...
    def cancel_export(
        self, tenant_id: UUID, export_id: UUID, actor_id: UUID, transition_key: str, reason: str = ""
    ) -> EventExportJob:
        return self._transition(tenant_id, export_id, actor_id, "cancel", transition_key, reason)

    def retry_export(
        self, tenant_id: UUID, export_id: UUID, actor_id: UUID, transition_key: str, idempotency_key: str
    ) -> EventExportJob:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
        ...
        with transaction.atomic():
            ...
            if artifact_key:
                ...
                _publish(
                    ...
                    deletion.id,
                    "process.export.artifact_deletion_requested",
                    {"export_id": str(record.id), "artifact_key": artifact_key},
                )
        if artifact_key:
            try:
                ...
                logger.exception(
                    "process.export.artifact_deletion_deferred",
                    extra={
                        "event_name": "process.export.artifact_deletion_deferred",
                        "tenant_id": str(tenant_id),
                        "export_id": str(record.id),
                        "correlation_id": _correlation_id(),
                    },
                ...
                raise CapabilityUnavailable(
                    capability="export_artifact_deletion", message="Deletion is durably queued for reconciliation."
                ) from exc
...
class ProcessModelService:
    def create_imported_model(
        ...
        with transaction.atomic():
            model = ProcessModel.objects.create(
                tenant_id=tenant_id,
                created_by=actor_id,
                name=_required_text(name, "name", limit),
                process_name=_required_text(process_name, "process_name", limit),
                description=str(description or "").strip(),
                source_kind=ModelSourceKind.IMPORTED,
            )
            self.create_version(
                tenant_id, model.id, actor_id, model_data, algorithm=None, parameters={}, discovery_job=None
            )
        return model
    ...
    def create_version(
        ...
        *,
        ...
        discovery_job: ProcessDiscoveryJob | None,
        event_count: int = 0,
        case_count: int = 0,
        activity_count: int = 0,
        avg_case_duration_seconds: Decimal | None = None,
    ) -> ProcessModelVersion:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
        validate_graph(model_data)
        with transaction.atomic():
            model = (
                ...
                .select_for_update()
                .filter(pk=model_id, is_deleted=False)
                .first()
            )
            if model is None:
                raise NotFound()
    ...
    def update_model_metadata(
        self, tenant_id: UUID, model_id: UUID, actor_id: UUID, name: str, description: str
    ) -> ProcessModel:
        tenant_id = _tenant(tenant_id)
        _actor(actor_id)
        limit = _config_int(_configuration(tenant_id), "text_max_length")
    ...
    def soft_delete_model(self, tenant_id: UUID, model_id: UUID, actor_id: UUID) -> None:
        ...
        if (
            ...
            .filter(
                process_model_version__process_model=model,
                status__in=[AnalysisStatus.QUEUED, AnalysisStatus.RUNNING],
                is_deleted=False,
            )
            .exists()
        ):
            raise ValidationError({"model": "Active analyses still reference this model."})
        ProcessModel.objects.for_tenant(tenant_id).filter(pk=model.id).update(
            is_deleted=True, deleted_at=timezone.now()
        )
    ...
    def set_reference_version(
        ...
        with transaction.atomic():
            ...
            version = (
                ProcessModelVersion.objects.for_tenant(tenant_id)
                .filter(pk=version_id, process_model_id=model_id)
                .first()
            )
            ...
            _publish(
                ...
                "process_model",
                model.id,
                "process.model.reference_changed",
                {
    ...
    def get_process_overview(self, tenant_id: UUID, filters: Mapping[str, object]) -> list[dict[str, object]]:
        ...
        for row in rows:
            model = (
                ProcessModel.objects.for_tenant(tenant_id)
                .filter(process_name=row["process_name"], is_deleted=False)
                .order_by("-created_at")
                .first()
            )
            discovery = (
                ProcessDiscoveryJob.objects.for_tenant(tenant_id)
                .filter(process_name=row["process_name"], is_deleted=False)
                .order_by("-created_at")
                .first()
            )
            has_reference = bool(
                model
                and ProcessModelReferenceAssignment.objects.for_tenant(tenant_id).filter(process_model=model).exists()
            )
            row.update(
                {
                    "has_reference": has_reference,
                    "model_id": model.id if model else None,
                    "last_discovery": discovery.created_at if discovery else None,
                }
            )
        if filters.get("has_reference") is not None:
...
class ProcessDiscoveryService:
    def request_discovery(
        ...
        threshold_name, default = (
            ("dependency_threshold", configuration["heuristic_default_threshold"])
            if algorithm == MiningAlgorithmName.HEURISTIC
        ...
        with transaction.atomic():
            ...
            job = _enqueue_or_unavailable(
                ...
                f"process_mining.discovery:{idempotency_key}",
            )
            record.async_job_id = job.id
            record.save(update_fields=["async_job_id", "updated_at"])
            _publish(
                ...
                "process_discovery",
                record.id,
                "process.discovery.requested",
                {"discovery_id": str(record.id), "algorithm": algorithm},
            )
        return record
    ...
    def run_discovery(self, tenant_id: UUID, discovery_id: UUID, async_job_id: UUID) -> ProcessDiscoveryJob:
        ...
        _publish(
            ...
            "process_discovery",
            record.id,
            "process.discovery.started",
            {"discovery_id": str(record.id), "algorithm": record.algorithm},
        )
        try:
            rows = list(
                ProcessEvent.objects.for_tenant(tenant_id)
                .filter(process_name=record.process_name)
                .order_by("case_id", "occurred_at", "source_event_id", "activity", "id")
            )
            ...
            with transaction.atomic():
                model = (
                    ProcessModel.objects.for_tenant(tenant_id)
                    .filter(process_name=record.process_name, source_kind=ModelSourceKind.DISCOVERED, is_deleted=False)
                    .order_by("created_at")
                    .first()
                )
                if model is None:
                ...
                model_service.create_version(
                    ...
                    discovery_job=record,
                    event_count=len(rows),
                    case_count=len(traces),
                    activity_count=len({row.activity for row in rows}),
                    avg_case_duration_seconds=(
                        Decimal(str(round(sum(durations) / len(durations), 2))) if durations else None
                    ),
                )
            ...
            ProcessDiscoveryJob.objects.for_tenant(tenant_id).filter(pk=record.id).update(
                ...
                error_message="The discovery algorithm could not complete.",
                completed_at=timezone.now(),
            )
            ...
            if fresh.status == AnalysisStatus.RUNNING:
                _workflow_machine(tenant_id, "discovery").apply(
                    ...
                    tenant_id=tenant_id,
                    transition_key=f"worker:{async_job_id}:fail",
                    metadata=_audit(fresh.created_by, "Discovery failed"),
                )
            _publish(
                tenant_id,
                "process_discovery",
                ...
                {"discovery_id": str(record.id), "error_code": "DISCOVERY_FAILED"},
            )
    ...
    def _transition(
        self, tenant_id: UUID, discovery_id: UUID, actor_id: UUID, command: str, transition_key: str, reason: str = ""
    ) -> ProcessDiscoveryJob:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
        record = _get(ProcessDiscoveryJob, tenant_id, discovery_id, active=True)
        return cast(
    ...
    def cancel_discovery(
        self, tenant_id: UUID, discovery_id: UUID, actor_id: UUID, transition_key: str, reason: str = ""
    ) -> ProcessDiscoveryJob:
        return self._transition(tenant_id, discovery_id, actor_id, "cancel", transition_key, reason)

    def retry_discovery(
        self, tenant_id: UUID, discovery_id: UUID, actor_id: UUID, transition_key: str, idempotency_key: str
    ) -> ProcessDiscoveryJob:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
...
class ConformanceService:
    def request_check(
        ...
        with transaction.atomic():
            record = ConformanceCheck.objects.create(
                tenant_id=tenant_id,
                created_by=actor_id,
                process_model_version=version,
                event_filter=dict(event_filter),
            ...
            job = _enqueue_or_unavailable(
                ...
                f"process_mining.conformance:{idempotency_key}",
            )
            record.async_job_id = job.id
            record.save(update_fields=["async_job_id", "updated_at"])
        return record
    ...
    def run_check(self, tenant_id: UUID, check_id: UUID, async_job_id: UUID) -> ConformanceCheck:
        ...
        try:
            ...
            with transaction.atomic():
                ConformanceCaseMetric.objects.bulk_create(
                    [
                        ConformanceCaseMetric(
                            tenant_id=tenant_id,
                            created_by=record.created_by,
                            conformance_check=record,
                            case_id=item.case_id,
                            fitness=item.fitness,
                            is_conformant=item.is_conformant,
                ...
                ConformanceDeviation.objects.bulk_create(
                    [
                        ConformanceDeviation(
                            tenant_id=tenant_id,
                            created_by=record.created_by,
                ...
                ConformanceCheck.objects.for_tenant(tenant_id).filter(pk=record.id).update(
                    ...
                    deviating_cases=len(result.cases) - conformant,
                    completed_at=timezone.now(),
                    error_code="",
                    error_message="",
                )
                ...
                _publish(
                    ...
                    "conformance_check",
                    record.id,
                    "process.conformance.completed",
                    {"check_id": str(record.id), "fitness": str(record.fitness), "total_cases": record.total_cases},
                )
                if record.fitness is not None and record.fitness < Decimal(str(configuration["low_fitness_threshold"])):
                    _publish(
                        ...
                        "conformance_check",
                        record.id,
                        "process.conformance.low_fitness",
                        {
                            "check_id": str(record.id),
                            "fitness": str(record.fitness),
                            "notification_contract": "notification.requested.v1",
                        },
                    )
            return record
        except Exception as exc:
            ConformanceCheck.objects.for_tenant(tenant_id).filter(pk=record.id).update(
                error_code="CONFORMANCE_FAILED",
                error_message="Conformance evaluation could not complete.",
                completed_at=timezone.now(),
            )
            ...
            if fresh.status == AnalysisStatus.RUNNING:
                _workflow_machine(tenant_id, "conformance").apply(
                    ...
                    tenant_id=tenant_id,
                    transition_key=f"worker:{async_job_id}:fail",
                    metadata=_audit(fresh.created_by, "Conformance failed"),
                )
            raise OperationFailed(error_code="CONFORMANCE_FAILED", message="Conformance evaluation failed.") from exc

    def list_deviations(
        self, tenant_id: UUID, check_id: UUID, filters: Mapping[str, object]
    ) -> QuerySet[ConformanceDeviation]:
    ...
    def get_fitness(self, tenant_id: UUID, check_id: UUID) -> tuple[ConformanceCheck, QuerySet[ConformanceCaseMetric]]:
        check = _get(ConformanceCheck, _tenant(tenant_id), check_id, active=True)
        return check, ConformanceCaseMetric.objects.for_tenant(check.tenant_id).filter(
            conformance_check=check
        ).order_by("fitness", "case_id")

    def _transition(
        self, tenant_id: UUID, check_id: UUID, actor_id: UUID, command: str, transition_key: str, reason: str = ""
    ) -> ConformanceCheck:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
        record = _get(ConformanceCheck, tenant_id, check_id, active=True)
        return cast(
    ...
    def cancel_check(
        self, tenant_id: UUID, check_id: UUID, actor_id: UUID, transition_key: str, reason: str = ""
    ) -> ConformanceCheck:
        return self._transition(tenant_id, check_id, actor_id, "cancel", transition_key, reason)

    def retry_check(
        self, tenant_id: UUID, check_id: UUID, actor_id: UUID, transition_key: str, idempotency_key: str
    ) -> ConformanceCheck:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
...
class BottleneckService:
    def request_analysis(
        ...
        start, end = (
            _aware_datetime(time_range[0], "time_range_start"),
            _aware_datetime(time_range[1], "time_range_end"),
        )
        if end <= start:
            raise ValidationError({"time_range_end": "End must follow start."})
        exact = (
            BottleneckAnalysis.objects.for_tenant(tenant_id)
            .filter(
                process_name=process_name,
        ...
        with transaction.atomic():
            record = BottleneckAnalysis.objects.create(
                tenant_id=tenant_id,
                created_by=actor_id,
                process_name=process_name,
                time_range_start=start,
            ...
            job = _enqueue_or_unavailable(
                ...
                f"process_mining.bottleneck:{idempotency_key}",
            )
            record.async_job_id = job.id
            record.save(update_fields=["async_job_id", "updated_at"])
        return record
    ...
    def run_analysis(self, tenant_id: UUID, analysis_id: UUID, async_job_id: UUID) -> BottleneckAnalysis:
        ...
        try:
            rows = (
                ...
                .filter(
                    process_name=record.process_name,
                    occurred_at__gte=record.time_range_start,
                    occurred_at__lte=record.time_range_end,
                )
                .order_by("case_id", "occurred_at", "source_event_id", "activity", "id")
            )
            ...
            with transaction.atomic():
                BottleneckFinding.objects.bulk_create(
                    [
                        BottleneckFinding(
                            tenant_id=tenant_id, created_by=record.created_by, analysis=record, **dict(item)
                        )
                        for item in result.findings
                    ],
                    ignore_conflicts=True,
                )
                ProcessVariant.objects.bulk_create(
                    [
                        ProcessVariant(tenant_id=tenant_id, created_by=record.created_by, analysis=record, **dict(item))
                ...
                BottleneckAnalysis.objects.for_tenant(tenant_id).filter(pk=record.id).update(
                    ...
                    avg_case_duration_seconds=result.average_case_duration_seconds,
                    completed_at=timezone.now(),
                    error_code="",
                    error_message="",
                )
                ...
                _publish(
                    ...
                    "bottleneck_analysis",
                    record.id,
                    "process.variant.analysis_completed",
                    {"analysis_id": str(record.id), "variant_count": len(result.variants)},
                )
            return record
        except Exception as exc:
            BottleneckAnalysis.objects.for_tenant(tenant_id).filter(pk=record.id).update(
                error_code="BOTTLENECK_FAILED",
                error_message="Bottleneck analysis could not complete.",
                completed_at=timezone.now(),
            )
            ...
            if fresh.status == AnalysisStatus.RUNNING:
                _workflow_machine(tenant_id, "bottleneck").apply(
                    ...
                    tenant_id=tenant_id,
                    transition_key=f"worker:{async_job_id}:fail",
                    metadata=_audit(fresh.created_by, "Bottleneck analysis failed"),
                )
            raise OperationFailed(error_code="BOTTLENECK_FAILED", message="Bottleneck analysis failed.") from exc

    def get_variants(
        self, tenant_id: UUID, analysis_id: UUID, filters: Mapping[str, object]
    ) -> QuerySet[ProcessVariant]:
    ...
    def _transition(
        self, tenant_id: UUID, analysis_id: UUID, actor_id: UUID, command: str, transition_key: str, reason: str = ""
    ) -> BottleneckAnalysis:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
        record = _get(BottleneckAnalysis, tenant_id, analysis_id, active=True)
        return cast(
    ...
    def cancel_analysis(
        self, tenant_id: UUID, analysis_id: UUID, actor_id: UUID, transition_key: str, reason: str = ""
    ) -> BottleneckAnalysis:
        return self._transition(tenant_id, analysis_id, actor_id, "cancel", transition_key, reason)

    def retry_analysis(
        self, tenant_id: UUID, analysis_id: UUID, actor_id: UUID, transition_key: str, idempotency_key: str
    ) -> BottleneckAnalysis:
        tenant_id, actor_id = _tenant(tenant_id), _actor(actor_id)
