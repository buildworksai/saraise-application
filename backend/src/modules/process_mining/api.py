...
from .serializers import (
    BottleneckAnalysisDetailSerializer,
    BottleneckAnalysisListSerializer,
    BottleneckCreateSerializer,
    BottleneckFindingSerializer,
    CaseFitnessSerializer,
    ConfigurationDocumentSerializer,
    ConfigurationImportSerializer,
    ConfigurationRollbackSerializer,
    ConformanceCreateSerializer,
    ConformanceDetailSerializer,
    ConformanceListSerializer,
    DeviationListSerializer,
    DiscoveryCreateSerializer,
    DiscoveryDetailSerializer,
    DiscoveryListSerializer,
    EventBatchIngestSerializer,
    EventExportCreateSerializer,
    EventExportDetailSerializer,
    EventExportListSerializer,
    IngestResultSerializer,
    ModuleHealthSerializer,
    ProcessEventDetailSerializer,
    ProcessEventListSerializer,
    ProcessMiningConfigurationSerializer,
    ProcessMiningConfigurationVersionSerializer,
    ProcessModelCreateSerializer,
    ProcessModelDetailSerializer,
    ProcessModelListSerializer,
    ProcessModelUpdateSerializer,
    ProcessModelVersionDetailSerializer,
    ProcessModelVersionListSerializer,
...
class ProcessEventViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "process_mining.event:read",
        "retrieve": "process_mining.event:read",
        "create": "process_mining.event:ingest",
    }
    ...
    def get_queryset(self) -> Any:
        ...
        return self.service_class().query_events(
            self.tenant_id(),
            {
    ...
    def retrieve(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        return Response(
            ProcessEventDetailSerializer(self.service_class().get_event(self.tenant_id(), self.kwargs["pk"])).data
        )
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        result = self.service_class().ingest_events(
            self.tenant_id(), self.actor_id(), data["source_module"], data["process_name"], data["events"]
        )
...
class EventExportViewSet(TransitionActionsMixin, TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "process_mining.export:read",
        "retrieve": "process_mining.export:read",
        "create": "process_mining.export:create",
        "download": "process_mining.export:download",
        "cancel": "process_mining.export:cancel",
        "retry": "process_mining.export:create",
        "destroy": "process_mining.export:delete",
    }
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = EventExportCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        value = self.service_class().request_export(
            self.tenant_id(),
            self.actor_id(),
            data["process_name"],
            data["format"],
            data.get("event_filter", {}),
    ...
    def download(self, request: object, pk: str | None = None) -> FileResponse:
        ...
        return FileResponse(
            ...
            as_attachment=True,
            filename=f"{record.process_name}.{record.format}",
        )

    def _transition(self, command: str) -> Response:
        serializer = TransitionActionSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        if command == "cancel":
            value = self.service_class().cancel_export(
                self.tenant_id(), self.kwargs["pk"], self.actor_id(), data["transition_key"], data.get("reason", "")
            )
        else:
            if not data.get("idempotency_key"):
                raise ValidationError({"idempotency_key": "This field is required for retry."})
            value = self.service_class().retry_export(
                self.tenant_id(), self.kwargs["pk"], self.actor_id(), data["transition_key"], data["idempotency_key"]
            )
        return Response(
            EventExportDetailSerializer(value).data, status=status.HTTP_202_ACCEPTED if command == "retry" else 200
        )
...
class DiscoveryViewSet(TransitionActionsMixin, TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "process_mining.discovery:read",
        "retrieve": "process_mining.discovery:read",
        "create": "process_mining.discovery:create",
        "model": "process_mining.discovery:read",
        "cancel": "process_mining.discovery:cancel",
        "retry": "process_mining.discovery:retry",
        "destroy": "process_mining.discovery:delete",
    }
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = DiscoveryCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        value = self.service_class().request_discovery(
            self.tenant_id(),
            self.actor_id(),
            data["process_name"],
            data["algorithm"],
    ...
    def destroy(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        if not ProcessMiningQueryService.exists(self.get_queryset(), self.kwargs["pk"]):
            raise NotFound()
        self.service_class().delete_discovery(self.tenant_id(), self.kwargs["pk"], self.actor_id())
    ...
    def model(self, request: object, pk: str | None = None) -> Response:
        ...
        return Response(
            ProcessModelVersionDetailSerializer(
                self.service_class().get_discovered_model(self.tenant_id(), self.kwargs["pk"])
            ).data
        )
    ...
    def _transition(self, command: str) -> Response:
        ...
        data = serializer.validated_data
        if command == "cancel":
            value = self.service_class().cancel_discovery(
                self.tenant_id(), self.kwargs["pk"], self.actor_id(), data["transition_key"], data.get("reason", "")
            )
        else:
            if not data.get("idempotency_key"):
                raise ValidationError({"idempotency_key": "This field is required for retry."})
            value = self.service_class().retry_discovery(
                self.tenant_id(), self.kwargs["pk"], self.actor_id(), data["transition_key"], data["idempotency_key"]
            )
...
class ProcessModelViewSet(TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "process_mining.model:read",
        "retrieve": "process_mining.model:read",
        "create": "process_mining.model:create",
        "partial_update": "process_mining.model:update",
        "destroy": "process_mining.model:delete",
        "versions": "process_mining.model:read",
        "set_reference": "process_mining.model:set_reference",
    }
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = ProcessModelCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        value = self.service_class().create_imported_model(
            self.tenant_id(),
            self.actor_id(),
            data["name"],
            data["process_name"],
    ...
    def partial_update(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        serializer = ProcessModelUpdateSerializer(data=payload)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        return Response(
            ProcessModelDetailSerializer(
                self.service_class().update_model_metadata(
                    self.tenant_id(), current.id, self.actor_id(), data["name"], data.get("description", "")
                )
            ).data
        )
    ...
    def destroy(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        if not ProcessMiningQueryService.exists(self.get_queryset(), self.kwargs["pk"]):
            raise NotFound()
        self.service_class().soft_delete_model(self.tenant_id(), self.kwargs["pk"], self.actor_id())
        return Response(status=204)
    ...
    def versions(self, request: object, pk: str | None = None) -> Response:
        ...
        return self.paginated(
            ProcessMiningQueryService.model_versions(self.tenant_id(), model.id), ProcessModelVersionListSerializer
        )
    ...
    def set_reference(self, request: object, pk: str | None = None) -> Response:
        ...
        if not ProcessMiningQueryService.exists(self.get_queryset(), self.kwargs["pk"]):
            raise NotFound()
        serializer = SetReferenceSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        value = self.service_class().set_reference_version(
            self.tenant_id(), self.kwargs["pk"], data["version_id"], self.actor_id(), data["transition_key"]
        )
...
class ConformanceViewSet(TransitionActionsMixin, TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "process_mining.conformance:read",
        "retrieve": "process_mining.conformance:read",
        "create": "process_mining.conformance:create",
        "deviations": "process_mining.conformance:read",
        "fitness": "process_mining.conformance:read",
        "cancel": "process_mining.conformance:cancel",
        "retry": "process_mining.conformance:retry",
        "destroy": "process_mining.conformance:delete",
    }
    action_quotas = {"create": "process_mining.analysis_compute", "retry": "process_mining.analysis_compute"}
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = ConformanceCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        value = self.service_class().request_check(
            self.tenant_id(),
            self.actor_id(),
            data["process_model_version_id"],
            data.get("event_filter", {}),
            data["idempotency_key"],
        )
    ...
    def destroy(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        if not ProcessMiningQueryService.exists(self.get_queryset(), self.kwargs["pk"]):
            raise NotFound()
        self.service_class().delete_check(self.tenant_id(), self.kwargs["pk"], self.actor_id())
        return Response(status=204)
    ...
    def deviations(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self.paginated(
            self.service_class().list_deviations(self.tenant_id(), self.kwargs["pk"], self.request.query_params),
            DeviationListSerializer,
        )
    ...
    def _transition(self, command: str) -> Response:
        ...
        data = serializer.validated_data
        if command == "cancel":
            value = self.service_class().cancel_check(
                self.tenant_id(), self.kwargs["pk"], self.actor_id(), data["transition_key"], data.get("reason", "")
            )
        else:
            if not data.get("idempotency_key"):
                raise ValidationError({"idempotency_key": "This field is required for retry."})
            value = self.service_class().retry_check(
                self.tenant_id(), self.kwargs["pk"], self.actor_id(), data["transition_key"], data["idempotency_key"]
            )
...
class BottleneckViewSet(TransitionActionsMixin, TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "process_mining.bottleneck:read",
        "retrieve": "process_mining.bottleneck:read",
        "create": "process_mining.bottleneck:create",
        "findings": "process_mining.bottleneck:read",
        "variants": "process_mining.bottleneck:read",
        "cancel": "process_mining.bottleneck:cancel",
        "retry": "process_mining.bottleneck:retry",
        "destroy": "process_mining.bottleneck:delete",
    }
    action_quotas = {"create": "process_mining.analysis_compute", "retry": "process_mining.analysis_compute"}
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = BottleneckCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        data = serializer.validated_data
        value = self.service_class().request_analysis(
            self.tenant_id(),
            self.actor_id(),
            data["process_name"],
    ...
    def destroy(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        if not ProcessMiningQueryService.exists(self.get_queryset(), self.kwargs["pk"]):
            raise NotFound()
        self.service_class().delete_analysis(self.tenant_id(), self.kwargs["pk"], self.actor_id())
        return Response(status=204)
    ...
    def findings(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self.paginated(
            self.service_class().get_findings(self.tenant_id(), self.kwargs["pk"], self.request.query_params),
            BottleneckFindingSerializer,
        )
    ...
    def variants(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self.paginated(
            self.service_class().get_variants(self.tenant_id(), self.kwargs["pk"], self.request.query_params),
            ProcessVariantSerializer,
        )
    ...
    def _transition(self, command: str) -> Response:
        ...
        data = serializer.validated_data
        if command == "cancel":
            value = self.service_class().cancel_analysis(
                self.tenant_id(), self.kwargs["pk"], self.actor_id(), data["transition_key"], data.get("reason", "")
            )
        else:
            if not data.get("idempotency_key"):
                raise ValidationError({"idempotency_key": "This field is required for retry."})
            value = self.service_class().retry_analysis(
                self.tenant_id(), self.kwargs["pk"], self.actor_id(), data["transition_key"], data["idempotency_key"]
            )
...
class ProcessMiningConfigurationViewSet(TenantGovernedViewSet):
    ...
    def history(self, request: object) -> Response:
        del request
        return self.paginated(
            self.service_class().history(self.tenant_id()), ProcessMiningConfigurationVersionSerializer
        )
