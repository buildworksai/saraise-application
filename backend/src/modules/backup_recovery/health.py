"""
Backup & Recovery (Extended) Health Checks
...
def health_check(request):
    """
    Health check endpoint for Backup & Recovery (Extended) module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "backup-recovery", "checks": {}}
    ...
    try:
        cache.set("health_check_backup_recovery", "ok", 10)
        result = cache.get("health_check_backup_recovery")
        if result == "ok":
