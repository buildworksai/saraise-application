"""
API Integration Tests for Localization module.
...
from ..models import LocalizationResource
...
class TestLocalizationResourceViewSet:
    """Test LocalizationResourceViewSet CRUD operations."""

    def test_list_resources_requires_authentication(self, api_client):
        """Test that listing resources requires authentication."""
        response = api_client.get(f"/api/v1/localization/resources/")
    ...
    def test_list_resources(self, authenticated_client, tenant_user):
        ...
        LocalizationResource.objects.create(
            tenant_id=tenant_id,
        ...
        response = authenticated_client.get(f"/api/v1/localization/resources/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_create_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.post(
            f"/api/v1/localization/resources/",
            data,
            format="json"
        )
    ...
    def test_get_resource_detail(self, authenticated_client, tenant_user):
        ...
        resource = LocalizationResource.objects.create(
            tenant_id=tenant_id,
            name="Test Resource",
        ...
        response = authenticated_client.get(f"/api/v1/localization/resources/{resource.id}/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.put(
            f"/api/v1/localization/resources/{resource.id}/",
            data,
            format="json"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_delete_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.delete(f"/api/v1/localization/resources/{resource.id}/")
        assert response.status_code == status.HTTP_204_NO_CONTENT
        
        # Verify resource is deleted
        assert not LocalizationResource.objects.filter(id=resource.id).exists()
