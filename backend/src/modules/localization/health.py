...
def health_check(request):
    """
    Health check endpoint for Localization module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "localization", "checks": {}}
    ...
    try:
        cache.set("health_check_localization", "ok", 10)
        result = cache.get("health_check_localization")
        if result == "ok":
