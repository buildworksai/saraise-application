...
from .integrations import AccountingPostingRequest, FixedAssetIntegrationError, JournalLeg, extension_registry
from .models import (
    ASSET_STATE_MACHINE,
    LINE_STATE_MACHINE,
    PRIMARY_BOOK,
...
def _transition(
    ...
    if existing is not None:
        if existing.command != command:
            raise IdempotencyConflictError(f"Transition key {key!r} already belongs to command {existing.command!r}.")
        return False
    ...
    def deactivate_category(cls, tenant_id: UUID | str, category_id: UUID | str, actor_id: str) -> AssetCategory:
        ...
        with transaction.atomic():
            ...
            if FixedAsset.objects.for_tenant(tenant).filter(category=category, status=AssetStatus.DRAFT).exists():
                raise FixedAssetServiceError(
                    "Draft assets require an active category.", code="CATEGORY_IN_USE_BY_DRAFT"
                )
            if category.is_active:
...
class AssetTransactionService:
    ...
    def append_transaction(cls, *, _authority: object | None = None, **values: object) -> AssetTransaction:
        if _authority is not _LEDGER_AUTHORITY:
            raise FixedAssetServiceError(
                "Transactions can only be appended by lifecycle services.", code="LEDGER_INTERNAL_ONLY"
            )
...
class FixedAssetService:
    ...
    def create_asset(
        ...
        values.setdefault(
            "residual_value",
            money(Decimal(values["purchase_cost"]) * category.default_residual_value_percent / Decimal("100")),
        )
        ...
        with transaction.atomic():
            ...
            asset = FixedAsset(
                tenant_id=tenant,
                category=category,
                net_book_value=money(values["purchase_cost"]),
                created_by=actor,
                updated_by=actor,
                creation_idempotency_key=key,
                creation_request_fingerprint=fingerprint,
        ...
        if category is None:
            category = AssetCategoryService.create_category(
                ...
                actor,
                {
                    "code": category_code,
                    "name": category_code,
                    "default_depreciation_method": kwargs.get("depreciation_method", "straight_line"),
                    "default_useful_life_months": int(kwargs.pop("useful_life_years", 1) or 1) * 12,
                    "is_active": False,
                },
                f"{key}:category",
            )
        ...
        with transaction.atomic():
            ...
            for name, value in data.items():
                ...
                if name == "category_id":
                    ...
                    if category is None:
                        raise FixedAssetServiceError(
                            "An active same-tenant category is required.", code="CATEGORY_UNAVAILABLE"
                        )
        ...
        *,
        ...
        with transaction.atomic():
            ...
            if start < effective_date:
                raise FixedAssetServiceError(
                    "Depreciation cannot precede capitalization.", code="INVALID_EFFECTIVE_DATE"
                )
            ...
            AssetTransactionService.append_transaction(
                ...
                tenant_id=tenant,
                asset=asset,
                book_code=asset.primary_book_code,
                transaction_type=TransactionType.CAPITALIZATION,
                effective_date=effective_date,
                amount=asset.purchase_cost,
                currency=asset.currency,
                opening_net_book_value=Decimal("0.00"),
                closing_net_book_value=asset.net_book_value,
                source_type="asset",
                source_id=asset.id,
                idempotency_key=key,
                actor_id=actor,
            ...
            _event(
                tenant,
                "fixed_assets.asset.capitalized",
                asset.id,
                actor,
                correlation,
                {"asset_id": asset.id, "version": asset.version},
    ...
    def transfer(
        ...
        with transaction.atomic():
            ...
            AssetTransactionService.append_transaction(
                ...
                amount=Decimal("0.00"),
                currency=asset.currency,
                opening_net_book_value=asset.net_book_value,
                closing_net_book_value=asset.net_book_value,
                from_location=before_location,
                to_location=asset.location,
                from_cost_center=before_cost_center,
            ...
            _event(
                tenant,
                "fixed_assets.asset.transferred",
                asset.id,
                actor,
                correlation,
                {"asset_id": asset.id, "version": asset.version},
    ...
    def record_impairment(
        ...
        with transaction.atomic():
            ...
            if recoverable < asset.residual_value or recoverable >= asset.net_book_value:
                raise FixedAssetServiceError(
                    "Recoverable amount must be below book value and not below residual value.",
                    code="INVALID_RECOVERABLE_AMOUNT",
                )
            opening = asset.net_book_value
            loss = money(opening - recoverable)
            ...
            AssetTransactionService.append_transaction(
                ...
                asset=asset,
                book_code=asset.primary_book_code,
                transaction_type=TransactionType.IMPAIRMENT,
                effective_date=effective_date,
                amount=loss,
                currency=asset.currency,
                opening_net_book_value=opening,
                closing_net_book_value=recoverable,
                source_type="asset",
                source_id=asset.id,
                idempotency_key=key,
                actor_id=actor,
                correlation_id=correlation,
                metadata={"reason": _required(reason, "reason", 2000), "resulting_asset_version": asset.version},
            )
            for schedule in DepreciationSchedule.objects.select_for_update().filter(
                tenant_id=tenant, asset=asset, status=ScheduleStatus.ACTIVE
            ):
                _transition(
                    SCHEDULE_STATE_MACHINE,
                    schedule,
                    "supersede",
                    f"{key}:schedule:{schedule.id}",
                    actor,
                    correlation,
                    {"reason": "impairment"},
                )
            ...
            _event(
                tenant,
                "fixed_assets.asset.impaired",
                asset.id,
                actor,
                correlation,
                {"asset_id": asset.id, "amount": loss, "version": asset.version},
            )
            return asset
    ...
    def dispose(
        ...
        with transaction.atomic():
            ...
            if existing:
                ...
                return asset
            opening = asset.net_book_value
            normalized_proceeds = money(proceeds)
            ...
            asset.disposal_proceeds = normalized_proceeds
            asset.disposal_gain_loss = money(normalized_proceeds - opening)
            asset.net_book_value = Decimal("0.00")
            asset.updated_by = actor
            asset.version += 1
            asset.save()
            for line in DepreciationLine.objects.select_for_update().filter(
                tenant_id=tenant, asset=asset, status__in=(LineStatus.PLANNED, LineStatus.FAILED)
            ):
                _transition(
                    LINE_STATE_MACHINE,
                    line,
            ...
            AssetTransactionService.append_transaction(
                ...
                correlation_id=correlation,
                metadata={
                    "reason": _required(reason, "reason", 2000),
                    "gain_loss": str(asset.disposal_gain_loss),
                    "resulting_asset_version": asset.version,
                },
            )
            _event(
                tenant,
                "fixed_assets.asset.disposed",
                asset.id,
                actor,
                correlation,
                {"asset_id": asset.id, "gain_loss": asset.disposal_gain_loss, "version": asset.version},
            )
            return asset

    @classmethod
    def mark_fully_depreciated(
        cls, tenant_id: UUID | str, asset_id: UUID | str, actor_id: str, transition_key: str
    ) -> FixedAsset:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        correlation = _correlation(transition_key)
        with transaction.atomic():
            asset = FixedAsset.objects.select_for_update().get(pk=asset_id, tenant_id=tenant)
    ...
    def _preview(
        ...
        return {
            ...
            "as_of": as_of,
            "opening_net_book_value": asset.net_book_value,
            "closing_net_book_value": money(closing),
            "currency": asset.currency,
            "warnings": [],
            "blockers": blockers,
            "journal_effect": {"status": journal_status, "entries": entries},
            "schedule_effect": {"status": schedule_status, "description": schedule_description},
        }

    @classmethod
    def preview_capitalization(
        cls, tenant_id: UUID | str, asset_id: UUID | str, effective_date: date, **kwargs: object
    ) -> dict[str, object]:
        del kwargs
        ...
        blockers = (
            []
            if asset.status == AssetStatus.DRAFT
            else [{"code": "ASSET_NOT_DRAFT", "message": "Only draft assets can be capitalized."}]
        )
        return cls._preview(
            tenant_id,
            asset_id,
            ...
            blockers,
            "created",
            "A draft depreciation schedule may be created.",
        )
    ...
    def preview_transfer(
        ...
        asset = FixedAsset.objects.for_tenant(_tenant(tenant_id)).get(pk=asset_id)
        changed = (
            str(kwargs.get("to_location", "")).strip() != asset.location
            or str(kwargs.get("to_cost_center", "")).strip() != asset.cost_center
        )
        ...
        return cls._preview(
            ...
            blockers,
            "unchanged",
            "Depreciation assumptions do not change.",
        )

    @classmethod
    def preview_impairment(
        cls,
        tenant_id: UUID | str,
        asset_id: UUID | str,
        effective_date: date,
        ...
        **kwargs: object,
        ...
        blockers = (
            []
            if asset.residual_value <= recoverable < asset.net_book_value
            else [{"code": "INVALID_RECOVERABLE_AMOUNT", "message": "Recoverable amount is outside the allowed range."}]
        )
        return cls._preview(
            tenant_id,
            asset_id,
            ...
            blockers,
            "superseded",
            "The active schedule will require revision.",
        )
    ...
    def preview_disposal(
        ...
        blockers = (
            []
            if asset.status in (AssetStatus.ACTIVE, AssetStatus.FULLY_DEPRECIATED)
            else [{"code": "DISPOSAL_NOT_ALLOWED", "message": "Asset is not disposable."}]
        )
        return cls._preview(
            tenant_id,
            asset_id,
            ...
            blockers,
            "voided",
            "Unposted depreciation lines will be voided.",
        )
...
class DepreciationService:
    ...
    def create_schedule_draft(
        ...
        values.setdefault("method", asset.depreciation_method)
        values.setdefault("frequency", "monthly")
        ...
        values.setdefault("cost_basis", asset.net_book_value)
        values.setdefault("residual_value", asset.residual_value)
        values.setdefault("declining_balance_rate", asset.declining_balance_rate)
        values.setdefault("expected_total_units", asset.expected_total_units)
        ...
        with transaction.atomic():
            ...
            if existing:
                ...
                return existing
            revision = (
                DepreciationSchedule.objects.for_tenant(tenant)
                .filter(asset=asset, book_code=values["book_code"])
                .order_by("-revision")
            ...
            schedule = DepreciationSchedule(
                tenant_id=tenant,
                asset=asset,
                schedule_number=f"{asset.asset_code}-{values['book_code']}-R{revision}",
                revision=revision,
                depreciable_amount=money(Decimal(values["cost_basis"]) - Decimal(values["residual_value"])),
                total_planned_depreciation=Decimal("0.00"),
                created_by=actor,
                updated_by=actor,
                creation_idempotency_key=key,
                creation_request_fingerprint=fingerprint,
    ...
    def delete_schedule_draft(tenant_id: UUID | str, schedule_id: UUID | str, actor_id: str) -> None:
        ...
        with transaction.atomic():
            ...
            if schedule.status != ScheduleStatus.DRAFT or schedule.lines.exists():
                raise FixedAssetServiceError(
                    "Only an empty draft schedule can be deleted.", code="SCHEDULE_NOT_DELETABLE"
                )
            schedule.delete()
    ...
    def calculate_schedule(
        ...
        schedule_id: UUID | str,
        actor_id: str,
        units_by_period: Mapping[str, Decimal],
        transition_key: str,
    ) -> DepreciationSchedule:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        ...
        with transaction.atomic():
            schedule = (
                ...
                .get(pk=schedule_id, tenant_id=tenant)
            )
            ...
            if schedule.method == DepreciationMethod.UNITS_OF_PRODUCTION:
                ...
                if expected_units is None or total_units != expected_units:
                    raise FixedAssetServiceError(
                        "Period units must reconcile to expected total units.", code="UNITS_NOT_RECONCILED"
                    )
            opening = money(schedule.cost_basis)
            accumulated = Decimal("0.00")
            raw_amounts: list[Decimal] = []
            for period_start, period_end in periods:
                fraction = Decimal((period_end - period_start).days + 1) / Decimal(
                ...
                if schedule.method == DepreciationMethod.STRAIGHT_LINE:
                    ...
                    raw = (
                        ...
                        * Decimal(units_by_period[period_start.isoformat()])
                    )
                amount = min(money(raw), money(opening - schedule.residual_value))
                raw_amounts.append(max(amount, Decimal("0.00")))
                opening = money(opening - max(amount, Decimal("0.00")))
            ...
            for sequence, ((period_start, period_end), amount) in enumerate(zip(periods, raw_amounts), 1):
                ...
                lines.append(
                    DepreciationLine(
                        ...
                        book_code=schedule.book_code,
                        sequence=sequence,
                        period_start=period_start,
                        period_end=period_end,
                        opening_net_book_value=opening,
                        units_consumed=(
                            units_by_period.get(period_start.isoformat())
                            if schedule.method == DepreciationMethod.UNITS_OF_PRODUCTION
                            else None
                        ),
                        depreciation_amount=amount,
                        accumulated_depreciation=accumulated,
                        closing_net_book_value=closing,
                        asset_version_snapshot=schedule.asset.version,
                    )
                )
                opening = closing
            DepreciationLine.objects.bulk_create(lines)
            schedule.total_planned_depreciation = money(sum(raw_amounts, Decimal("0.00")))
            schedule.calculated_at = timezone.now()
            schedule.updated_by = actor
            schedule.version += 1
            _transition(
                SCHEDULE_STATE_MACHINE, schedule, "calculate", key, actor, correlation, {"line_count": len(lines)}
            )
            schedule.save()
            _event(
                tenant,
                "fixed_assets.schedule.calculated",
                schedule.id,
                actor,
                correlation,
                {"schedule_id": schedule.id, "asset_id": schedule.asset_id, "revision": schedule.revision},
            )
            return schedule

    @classmethod
    def activate_schedule(
        cls, tenant_id: UUID | str, schedule_id: UUID | str, actor_id: str, transition_key: str
    ) -> DepreciationSchedule:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        correlation = _correlation(transition_key)
        with transaction.atomic():
            schedule = DepreciationSchedule.objects.select_for_update().get(pk=schedule_id, tenant_id=tenant)
            if (
                schedule.lines.count() == 0
                or money(sum(schedule.lines.values_list("depreciation_amount", flat=True), Decimal("0.00")))
                != schedule.total_planned_depreciation
            ):
                raise FixedAssetServiceError("Schedule lines do not reconcile.", code="SCHEDULE_NOT_RECONCILED")
            if (
                DepreciationSchedule.objects.for_tenant(tenant)
                .filter(asset=schedule.asset, book_code=schedule.book_code, status=ScheduleStatus.ACTIVE)
            ...
            if _transition(SCHEDULE_STATE_MACHINE, schedule, "activate", transition_key, actor, correlation):
                ...
                _event(
                    tenant,
                    "fixed_assets.schedule.activated",
                    schedule.id,
                    actor,
                    correlation,
                    {"schedule_id": schedule.id, "asset_id": schedule.asset_id},
                )
            return schedule

    @classmethod
    def supersede_schedule(
        cls, tenant_id: UUID | str, schedule_id: UUID | str, actor_id: str, reason: str, transition_key: str
    ) -> DepreciationSchedule:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        correlation = _correlation(transition_key)
    ...
    @classmethod
    def enqueue_line_posting(
        cls, tenant_id: UUID | str, line_id: UUID | str, actor_id: str, idempotency_key: str, correlation_id: str
    ) -> AsyncJob:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        key = _required(idempotency_key, "idempotency_key")
        with transaction.atomic():
            line = (
                DepreciationLine.objects.select_for_update().select_related("asset").get(pk=line_id, tenant_id=tenant)
            )
            ...
            line.posting_idempotency_key = key
            line.posting_request_fingerprint = fingerprint
            line.posting_job_id = job.id
            line.asset_version_snapshot = line.asset.version
            line.save()
            _event(
                tenant,
                "fixed_assets.depreciation.posting_requested",
                line.id,
                actor,
                correlation_id,
                {"line_id": line.id, "job_id": job.id},
            )
            return job

    @classmethod
    def enqueue_due_posting(
        cls, tenant_id: UUID | str, through_date: date, actor_id: str, idempotency_key: str, correlation_id: str
    ) -> AsyncJob:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        key = _required(idempotency_key, "idempotency_key")
    ...
    @classmethod
    def post_line(
        cls, tenant_id: UUID | str, line_id: UUID | str, actor_id: str, job_id: UUID | str, transition_key: str
    ) -> DepreciationLine:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        correlation = _correlation(transition_key)
        failure: FixedAssetIntegrationError | None = None
        with transaction.atomic():
            line = (
                DepreciationLine.objects.select_for_update()
                .select_related("asset__category", "schedule")
                .get(pk=line_id, tenant_id=tenant)
            )
            ...
            if earlier.exists():
                raise FixedAssetServiceError(
                    "Earlier depreciation periods must be resolved first.", code="POSTING_OUT_OF_ORDER"
                )
            ...
            _transition(
                ...
                line,
                command,
                f"{transition_key}:begin",
                actor,
                correlation,
                {"job_id": str(job_id)},
            )
            line.posting_job_id = job_id
            line.posting_error_code = ""
            line.save()
            try:
                accounts = _category_accounts(asset.category)
                port = extension_registry.accounting_port()
                port.validate_accounts(tenant, accounts)
                result = port.post_journal(
                    AccountingPostingRequest(
                        ...
                        metadata={"schedule_id": str(line.schedule_id), "book_code": line.book_code},
                    )
                )
                ...
                line.posting_error_code = exc.code
                line.save()
                _event(
                    tenant,
                    "fixed_assets.depreciation.failed",
                    line.id,
                    actor,
                    correlation,
                    {"line_id": line.id, "job_id": str(job_id), "error_code": exc.code},
                )
            else:
                opening = asset.net_book_value
                if opening != line.opening_net_book_value:
                    raise FixedAssetServiceError(
                        "Line opening value does not match the asset.", code="OPENING_VALUE_MISMATCH"
                    )
                asset.accumulated_depreciation = money(asset.accumulated_depreciation + line.depreciation_amount)
                asset.net_book_value = money(
                    asset.purchase_cost - asset.accumulated_depreciation - asset.accumulated_impairment
                )
                asset.updated_by = actor
                asset.version += 1
                asset.save()
                _transition(
                    ...
                    line,
                    "confirm",
                    f"{transition_key}:confirm",
                    actor,
                    correlation,
                    {"journal_entry_id": str(result.journal_entry_id)},
                )
                line.journal_entry_id = result.journal_entry_id
                ...
                AssetTransactionService.append_transaction(
                    ...
                    effective_date=line.period_end,
                    ...
                    source_id=line.id,
                    idempotency_key=f"depreciation:{line.id}",
                    actor_id=actor,
                    correlation_id=correlation,
                    metadata={"schedule_id": str(line.schedule_id), "resulting_asset_version": asset.version},
                )
                next_line = (
                    DepreciationLine.objects.select_for_update()
                    .filter(
                ...
                if next_line:
                    next_line.asset_version_snapshot = asset.version
                    next_line.save(update_fields={"asset_version_snapshot", "updated_at"})
                if asset.net_book_value == asset.residual_value and asset.status == AssetStatus.ACTIVE:
                    _transition(
                        ASSET_STATE_MACHINE,
                        asset,
                        "fully_depreciate",
                        f"{transition_key}:asset-complete",
                        actor,
                        correlation,
                    )
                    asset.version += 1
                ...
                if not DepreciationLine.objects.filter(
                    ...
                    schedule = DepreciationSchedule.objects.select_for_update().get(
                        pk=line.schedule_id, tenant_id=tenant
                    )
                    if schedule.status == ScheduleStatus.ACTIVE:
                        _transition(
                            ...
                            schedule,
                            "complete",
                            f"{transition_key}:schedule-complete",
                            actor,
                            correlation,
                        )
                ...
                _event(
                    tenant,
                    "fixed_assets.depreciation.posted",
                    line.id,
                    actor,
                    correlation,
                    {
    ...
    @classmethod
    def void_line(
        cls, tenant_id: UUID | str, line_id: UUID | str, actor_id: str, transition_key: str, reason: str = ""
    ) -> DepreciationLine:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        correlation = _correlation(transition_key)
    ...
    @classmethod
    def post_due_lines(
        cls, tenant_id: UUID | str, through_date: date, actor_id: str, job_id: UUID | str
    ) -> dict[str, list[str]]:
        tenant = _tenant(tenant_id)
        actor = _required(actor_id, "actor_id")
        ids = list(
