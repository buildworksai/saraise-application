...
from .models import ComplianceConfigurationRevision, RequirementPolicyMapping
...
from .serializers import (
    ...
    RequirementBulkImportSerializer,
...
def _call(operation: Callable[..., Any], *args: object, **kwargs: object):
    try:
        ...
        raise OperationFailed(
            error_code="CONFLICT", message="A conflicting record already exists.", http_status=409
        ) from exc
...
def _translate(exc: Exception) -> Exception:
    if isinstance(exc, ComplianceNotFound):
        return NotFound()
    if isinstance(exc, ComplianceConflict):
        return OperationFailed(error_code="CONFLICT", message=str(exc), http_status=409)
    if isinstance(exc, ComplianceDependencyUnavailable):
        return OperationFailed(error_code="DEPENDENCY_UNAVAILABLE", message=str(exc), http_status=503)
    if isinstance(exc, ComplianceValidationError):
    ...
    if isinstance(exc, IntegrityError):
        return OperationFailed(error_code="CONFLICT", message="A conflicting record already exists.", http_status=409)
    return exc
...
class FrameworkViewSet(GovernedTenantViewSet):
    ...
    def create(self, request):
        ...
        obj = _call(
            self.service.create_framework, self.tenant_id, self.actor, serializer.validated_data, self.correlation_id
        )
    ...
    def status(self, request, pk=None):
        ...
        return Response(
            {
                "framework": FrameworkDetailSerializer(obj).data,
                "readiness": AssessmentService.scorecard(self.tenant_id, obj.id),
                "gaps": MappingService.gap_analysis(self.tenant_id, obj.id),
            }
        )
...
class RequirementViewSet(GovernedTenantViewSet):
    ...
    def get_queryset(self):
        ...
        return self.service.list_requirements(
    ...
    def create(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        obj = _call(
            self.service.create_requirement, self.tenant_id, self.actor, serializer.validated_data, self.correlation_id
        )
    ...
    def import_rows(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        rows = _call(
            self.service.bulk_import,
            self.tenant_id,
            self.actor,
            serializer.validated_data["framework_id"],
...
class PolicyViewSet(GovernedTenantViewSet):
    ...
    def create(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        obj = _call(
            self.service.create_policy, self.tenant_id, self.actor, serializer.validated_data, self.correlation_id
        )
    ...
    def partial_update(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        obj = _call(
            self.service.update_policy, self.tenant_id, self.actor, pk, serializer.validated_data, self.correlation_id
        )
    ...
    def versions(self, request, pk=None):
        ...
        if request.method == "GET":
            return self.paginated(
                policy.versions.filter(tenant_id=self.tenant_id).order_by("-version"), PolicyVersionSerializer
            )
        ...
        obj = _call(
            ...
            serializer.validated_data["change_summary"],
            _idempotency(request),
            self.correlation_id,
        )
    ...
    def _transition(self, request, pk, command):
        ...
        if command == "request_changes":
            ...
            obj = _call(getattr(self.service, command), self.tenant_id, self.actor, pk, key, self.correlation_id)
        return Response(PolicyDetailSerializer(obj).data)
...
class MappingViewSet(GovernedTenantViewSet):
    ...
    def get_queryset(self):
        ...
        return self.service.list_mappings(
    ...
    def partial_update(self, request, pk=None):
        ...
        obj = _call(
            ...
            self.tenant_id,
            self.actor,
            data.pop("requirement_id", existing.requirement_id),
            data.pop("policy_id", existing.policy_id),
            data,
            _idempotency(request),
            self.correlation_id,
        )
    ...
    def bulk(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        rows = _call(
            self.service.bulk_set_mappings,
            self.tenant_id,
            self.actor,
            serializer.validated_data["rows"],
            _idempotency(request),
...
def _get_mapping(tenant_id, mapping_id):
    try:
        ...
        raise ComplianceNotFound("Mapping was not found.") from exc
...
class GapViewSet(GovernedTenantViewSet):
    ...
    def list(self, request):
        ...
        return Response(
            GapSerializer(
                _call(
                    MappingService.gap_analysis, self.tenant_id, framework_id, _as_of(request.query_params.get("as_of"))
                )
            ).data
        )
...
class AssessmentViewSet(GovernedTenantViewSet):
    ...
    def get_queryset(self):
        ...
        return self.service.list_assessments(
    ...
    def scorecard(self, request):
        ...
        return Response(
            ScorecardSerializer(
                _call(self.service.scorecard, self.tenant_id, framework_id, _as_of(request.query_params.get("as_of")))
            ).data
        )
...
class EvidenceViewSet(GovernedTenantViewSet):
    ...
    def create(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        obj = _call(
            self.service.register_evidence, self.tenant_id, self.actor, serializer.validated_data, self.correlation_id
        )
    ...
    def partial_update(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        obj = _call(
            self.service.update_evidence, self.tenant_id, self.actor, pk, serializer.validated_data, self.correlation_id
        )
...
class ConfigurationViewSet(GovernedTenantViewSet):
    ...
    def partial_update(self, request, pk=None):
        ...
        obj = _call(self.service.update_draft, self.tenant_id, self.actor, pk, data, self.correlation_id)
    ...
    def list(self, request):
        ...
        return self.paginated(
            ActivityService.list_activity(
                self.tenant_id, filters, request.query_params.get("ordering", "-occurred_at")
            ),
            ActivitySerializer,
        )
...
class DashboardViewSet(GovernedTenantViewSet):
    ...
    def list(self, request):
        result = _call(
            ...
            self.tenant_id,
            request.query_params.get("framework_id"),
            _as_of(request.query_params.get("as_of")),
        )
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            job = AsyncJob.objects.filter(
                tenant_id=self.tenant_id, pk=pk, command__startswith="compliance_management."
            ).get()
