...
def _validate_model(instance: object) -> None:
    try:
        ...
        raise ComplianceValidationError(getattr(exc, "message_dict", {"non_field_errors": exc.messages})) from exc
...
class ConfigurationService:
    ...
    def get_effective(cls, tenant_id: UUID | str, environment: str):
        ...
        revision = (
            ComplianceConfigurationRevision.objects.for_tenant(tenant)
    ...
    def create_revision(cls, tenant_id, actor, environment: str, data: Mapping[str, object], correlation_id):
        ...
        environment = cls._environment(environment)
        latest = (
            ComplianceConfigurationRevision.objects.select_for_update()
            .for_tenant(tenant)
        ...
        ActivityService.record(
            tenant, actor, revision, "configuration.created", {}, _state(revision), "", correlation_id
        )
        return revision
    ...
    def update_draft(cls, tenant_id, actor, revision_id, data: Mapping[str, object], correlation_id):
        ...
        revision.save()
        ActivityService.record(
            tenant_id, actor, revision, "configuration.updated", before, _state(revision), "", correlation_id
        )
        return revision
    ...
    def preview(cls, tenant_id, revision_id):
        ...
        active = (
            ...
            .filter(environment=revision.environment, status="active")
            .first()
        )
        ...
        diff = [
            ...
            for key in fields
            if active is None or getattr(active, key) != getattr(revision, key)
        ]
        return {
            ...
            "diff": diff,
            "affected": {
                "frameworks": ComplianceFramework.objects.for_tenant(_uuid(tenant_id))
                .filter(deleted_at__isnull=True)
                .count(),
                "policies": CompliancePolicy.objects.for_tenant(_uuid(tenant_id))
                .filter(deleted_at__isnull=True)
                .count(),
                "evidence": ComplianceEvidence.objects.for_tenant(_uuid(tenant_id))
                .filter(deleted_at__isnull=True)
                .count(),
            },
        }
    ...
    def activate(cls, tenant_id, actor, revision_id, transition_key: str, correlation_id):
        ...
        if revision.status == "active":
            if any(
                item.get("key") == transition_key and item.get("command") == "activate"
                for item in revision.transition_history
            ):
                return revision
        ...
        revision.save()
        ActivityService.record(
            tenant, actor, revision, "configuration.activated", before, _state(revision), "", correlation_id
        )
        return revision
...
class FrameworkService:
    ...
    def create_framework(tenant_id, actor, data, correlation_id):
        ...
        values = _only(
            data,
            {"code", "name", "version", "category", "description", "source_kind", "source_package", "source_version"},
        )
        ...
        _validate_model(framework)
        framework.save(force_insert=True)
    ...
    def update_framework(cls, tenant_id, actor, framework_id, data, correlation_id):
        ...
        if framework.status == "archived":
            raise ComplianceConflict("Archived frameworks are immutable.")
        values = _only(
            data,
            {"code", "name", "version", "category", "description", "source_kind", "source_package", "source_version"},
        )
        ...
        framework.updated_by = _actor(actor)
        _validate_model(framework)
        framework.save()
        ActivityService.record(
            tenant_id, actor, framework, "framework.updated", before, _state(framework), "", correlation_id
        )
        return framework
    ...
    def activate_framework(cls, tenant_id, actor, framework_id, transition_key, correlation_id):
        ...
        if _transition(framework, "activate", transition_key, {"draft": "active"}):
            ...
            framework.save()
            ActivityService.record(
                tenant_id, actor, framework, "framework.activated", before, _state(framework), "", correlation_id
            )
        return framework
    ...
    def archive_framework(cls, tenant_id, actor, framework_id, transition_key, correlation_id):
        ...
        if _transition(framework, "archive", transition_key, {"active": "archived"}):
            framework.updated_by = _actor(actor)
            framework.deleted_at = timezone.now()
            framework.deleted_by = _actor(actor)
            framework.save()
            ActivityService.record(
                tenant_id, actor, framework, "framework.archived", before, _state(framework), "", correlation_id
            )
        return framework
    ...
    def validate_import(tenant_id, package):
        ...
        if len(requirements) > row_limit:
            raise ComplianceValidationError(
                {"requirements": [f"Package exceeds the configured {row_limit}-row limit."]}
            )
        codes: set[str] = set()
        for index, row in enumerate(requirements):
            ...
            if code in codes:
                raise ComplianceValidationError({f"requirements.{index}.code": ["Duplicate code."]})
            codes.add(code)
            if SAFE_MARKUP.search(str(row.get("description", ""))):
                raise ComplianceValidationError(
                    {f"requirements.{index}.description": ["Unsafe markup is not allowed."]}
                )
        return {
            "valid": True,
    ...
    def import_framework(cls, tenant_id, actor, package, idempotency_key, correlation_id):
        ...
        prior = (
            ...
            .filter(action="framework.imported", after__idempotency_key=idempotency_key)
            .first()
        )
        if prior:
    ...
    def export_framework(cls, tenant_id, framework_id):
        ...
        return {
            ...
            "framework": {k: getattr(framework, k) for k in fields},
            "requirements": [
                {k: getattr(r, k) for k in req_fields}
                for r in framework.requirements.filter(deleted_at__isnull=True).order_by("sort_order", "code")
            ],
        }
...
class RequirementService:
    ...
    def create_requirement(tenant_id, actor, data, correlation_id):
        ...
        framework_id = values.pop("framework_id", values.pop("framework", None))
        ...
        _validate_model(requirement)
        requirement.save(force_insert=True)
        ActivityService.record(
            tenant_id, actor, requirement, "requirement.created", {}, _state(requirement), "", correlation_id
        )
        return requirement
    ...
    def update_requirement(cls, tenant_id, actor, requirement_id, data, correlation_id):
        ...
        requirement.save()
        ActivityService.record(
            tenant_id, actor, requirement, "requirement.updated", before, _state(requirement), "", correlation_id
        )
        return requirement
    ...
    def _change(cls, tenant_id, actor, requirement_id, command, transition_key, correlation_id):
        source = (
            ...
            if command == "restore"
            else cls.get_requirement(tenant_id, requirement_id)
        )
        requirement = ComplianceRequirement.objects.select_for_update().get(pk=source.pk)
        before = _state(requirement)
    ...
    archive_requirement = classmethod(
        lambda cls, tenant_id, actor, requirement_id, transition_key, correlation_id: cls._change(
            tenant_id, actor, requirement_id, "archive", transition_key, correlation_id
        )
    )
    restore_requirement = classmethod(
        lambda cls, tenant_id, actor, requirement_id, transition_key, correlation_id: cls._change(
            tenant_id, actor, requirement_id, "restore", transition_key, correlation_id
        )
    )
    ...
    def bulk_import(cls, tenant_id, actor, framework_id, rows, idempotency_key, correlation_id):
        ...
        created = [
            cls.create_requirement(tenant_id, actor, {**dict(row), "framework_id": framework.id}, correlation_id)
            for row in rows
        ]
        ActivityService.record(
            tenant_id,
            ...
            {},
            {"idempotency_key": idempotency_key, "count": len(created)},
            "",
            correlation_id,
        )
        return created
...
class PolicyService:
    ...
    def list_policies(tenant_id, filters=None, ordering="code"):
        ...
        for key in ("status", "category", "owner_id"):
            if filters.get(key):
                qs = qs.filter(**{key: filters[key]})
        if filters.get("review_before"):
            qs = qs.filter(next_review_date__lte=filters["review_before"])
        if filters.get("expiry_before"):
            qs = qs.filter(expiry_date__lte=filters["expiry_before"])
        if filters.get("search"):
            qs = qs.filter(
                Q(code__icontains=filters["search"])
                | Q(title__icontains=filters["search"])
    ...
    def create_policy(tenant_id, actor, data=None, correlation_id=None, **legacy):
        ...
        policy = CompliancePolicy(
            tenant_id=_uuid(tenant_id), created_by=_actor(actor), updated_by=_actor(actor), **values
        )
        _validate_model(policy)
        policy.save(force_insert=True)
        ActivityService.record(
            tenant_id, actor, policy, "policy.created", {}, _state(policy), "", correlation_id or uuid4()
        )
        return policy
    ...
    def update_policy(cls, tenant_id, actor, policy_id, data, correlation_id):
        ...
        if policy.status in {"approved", "published", "archived"}:
            raise ComplianceConflict("Policy metadata is locked in this lifecycle state.")
        values = _only(
            data,
            {
                "code",
    ...
    def create_version(cls, tenant_id, actor, policy_id, content, change_summary, idempotency_key, correlation_id):
        ...
        existing = (
            CompliancePolicyVersion.objects.for_tenant(_uuid(tenant_id))
            .filter(policy=policy, content_sha256=digest)
            .first()
        )
        if existing:
            activity = (
                ComplianceActivity.objects.for_tenant(_uuid(tenant_id))
                .filter(entity_id=existing.id, action="policy.version_created", after__idempotency_key=idempotency_key)
                .first()
            )
            if activity:
                return existing
        ...
        _validate_model(version)
        version.save(force_insert=True)
        policy.current_version = version.version
        policy.updated_by = _actor(actor)
        policy.save(update_fields=["current_version", "updated_by", "updated_at"])
        ActivityService.record(
            tenant_id,
            actor,
            version,
            ...
            {},
            {"version": version.version, "content_sha256": digest, "idempotency_key": idempotency_key},
            change_summary,
            correlation_id,
        )
        return version
    ...
    def _transition_policy(cls, tenant_id, actor, policy_id, command, transition_key, correlation_id, reason=""):
        ...
        if command == "publish":
            if not policy.owner_id or not policy.effective_date:
                raise ComplianceValidationError(
                    {"non_field_errors": ["Owner and effective date are required for publication."]}
                )
        ...
        if _transition(policy, command, transition_key, transitions[command]):
            ...
            if command == "archive":
                ...
                policy.deleted_by = _actor(actor)
            _validate_model(policy)
            policy.save()
            ActivityService.record(
                tenant_id, actor, policy, f"policy.{command}", before, _state(policy), reason, correlation_id
            )
        return policy

    submit = classmethod(
        lambda cls, tenant_id, actor, policy_id, transition_key, correlation_id: cls._transition_policy(
            tenant_id, actor, policy_id, "submit", transition_key, correlation_id
        )
    )
    request_changes = classmethod(
        lambda cls, tenant_id, actor, policy_id, reason, transition_key, correlation_id: cls._transition_policy(
            tenant_id, actor, policy_id, "request_changes", transition_key, correlation_id, reason
        )
    )
    approve = classmethod(
        lambda cls, tenant_id, actor, policy_id, transition_key, correlation_id: cls._transition_policy(
            tenant_id, actor, policy_id, "approve", transition_key, correlation_id
        )
    )
    publish = classmethod(
        lambda cls, tenant_id, actor, policy_id, transition_key, correlation_id: cls._transition_policy(
            tenant_id, actor, policy_id, "publish", transition_key, correlation_id
        )
    )
    archive = classmethod(
        lambda cls, tenant_id, actor, policy_id, transition_key, correlation_id: cls._transition_policy(
            tenant_id, actor, policy_id, "archive", transition_key, correlation_id
        )
    )
    ...
    def revise(cls, tenant_id, actor, policy_id, content, change_summary, transition_key, correlation_id):
        ...
        before = _state(policy)
        _transition(policy, "revise", transition_key, {"published": "draft"})
        policy.save()
        version = cls.create_version(
            tenant_id, actor, policy.id, content, change_summary, transition_key, correlation_id
        )
        ActivityService.record(
            tenant_id, actor, policy, "policy.revise", before, _state(policy), change_summary, correlation_id
        )
...
class MappingService:
    ...
    def list_mappings(tenant_id, filters=None, ordering="mapped_at"):
        qs = (
            ...
            .filter(deleted_at__isnull=True)
            .select_related("requirement", "policy", "policy_version")
        )
        filters = filters or {}
    ...
    def set_mapping(tenant_id, actor, requirement_id, policy_id, data, idempotency_key, correlation_id):
        ...
        values = dict(data)
        version_id = values.pop("policy_version_id", None)
        version = _get(CompliancePolicyVersion, tenant_id, version_id, include_deleted=True) if version_id else None
        if version and version.policy_id != policy.id:
            raise ComplianceValidationError({"policy_version_id": ["Version does not belong to policy."]})
        mapping = (
            RequirementPolicyMapping.objects.for_tenant(_uuid(tenant_id))
            .filter(requirement=requirement, policy=policy, deleted_at__isnull=True)
            .first()
        )
        before = _state(mapping) if mapping else {}
        if mapping is None:
            mapping = RequirementPolicyMapping(
                tenant_id=_uuid(tenant_id), requirement=requirement, policy=policy, created_by=_actor(actor)
            )
        mapping.policy_version = version
        mapping.updated_by = _actor(actor)
        mapping.mapped_at = timezone.now()
        for key, value in values.items():
            setattr(mapping, key, value)
        _validate_model(mapping)
        mapping.save()
        ActivityService.record(
            tenant_id,
            actor,
            ...
            "mapping.set",
            before,
            {**_state(mapping), "idempotency_key": idempotency_key},
            "",
            correlation_id,
        )
        return mapping
    ...
    def remove_mapping(tenant_id, actor, mapping_id, correlation_id):
        ...
        mapping.deleted_by = _actor(actor)
        mapping.updated_by = _actor(actor)
        mapping.save()
        ActivityService.record(
            tenant_id, actor, mapping, "mapping.removed", before, _state(mapping), "", correlation_id
        )
    ...
    def gap_analysis(tenant_id, framework_id, as_of=None):
        ...
        for req in requirements:
            ...
            best = (
                "full"
                if mappings.filter(coverage="full").exists()
                else "partial" if mappings.filter(coverage="partial").exists() else "none"
            )
            if best != "full":
                rows.append(
                    {
                        ...
                        "title": req.title,
                        "coverage": best,
                        "reason": "No full policy coverage",
                    }
                )
        return {
            "framework_id": _uuid(framework_id),
...
class AssessmentService:
    ...
    def record_assessment(tenant_id, actor, data, idempotency_key, correlation_id):
        ...
        if (
            assessment.status in {"partial", "non_compliant", "not_applicable"}
            and len(assessment.notes.strip()) < min_length
        ):
            raise ComplianceValidationError({"notes": [f"Must be at least {min_length} characters for this status."]})
        _validate_model(assessment)
        ...
        ActivityService.record(
            ...
            "assessment.recorded",
            {},
            {**_state(assessment), "idempotency_key": idempotency_key},
            "",
            correlation_id,
        )
        return assessment
    ...
    def list_overdue(cls, tenant_id, as_of):
        return (
            ...
            .exclude(status__in=("compliant", "not_applicable"))
        )
    ...
    def scorecard(cls, tenant_id, framework_id, as_of=None):
        ...
        requirements = list(
            ComplianceRequirement.objects.for_tenant(_uuid(tenant_id)).filter(
                framework_id=_uuid(framework_id), applicability="applicable", status="active", deleted_at__isnull=True
            )
        )
        points = 0.0
        rows = []
        for req in requirements:
            ...
            if isinstance(as_of, date) and not isinstance(as_of, datetime):
                assessment = assessment.filter(assessed_at__date__lte=as_of)
            else:
                assessment = assessment.filter(assessed_at__lte=as_of)
            current = assessment.first()
            score = (
                1.0
                if current and current.status == "compliant"
                else 0.5 if current and current.status == "partial" else 0.0
            )
            points += score
            rows.append(
                {
                    "requirement_id": req.id,
                    "code": req.code,
                    "status": current.status if current else "not_assessed",
                    "points": score,
                }
            )
        ...
        return {
            "framework_id": _uuid(framework_id),
            "score": round((points / denominator) * 100, 2) if denominator else 0.0,
            "earned_points": points,
            "possible_points": denominator,
            "formula": "applicable requirements: compliant=1, partial=0.5, all others including unassessed=0",
            "requirements": rows,
        }
...
class EvidenceService:
    ...
    def register_evidence(cls, tenant_id, actor, data, correlation_id):
        ...
        if (
            values.get("reference_kind") == "external_url"
            and not _effective_or_defaults(tenant_id)["allow_external_evidence_urls"]
        ):
            raise ComplianceValidationError(
                {"external_uri": ["External evidence URLs are disabled by active configuration."]}
            )
        if values.get("reference_kind") == "external_url":
            ...
            if (
                ...
                raise ComplianceValidationError(
                    {"external_uri": ["URL must be public HTTP(S) without embedded credentials."]}
                )
        ...
        ActivityService.record(
            tenant_id, actor, evidence, "evidence.registered", {}, _state(evidence), "", correlation_id
        )
        return evidence
    ...
    def update_evidence(cls, tenant_id, actor, evidence_id, data, correlation_id):
        ...
        evidence.updated_by = _actor(actor)
        _validate_model(evidence)
        evidence.save()
        ActivityService.record(
            tenant_id, actor, evidence, "evidence.updated", before, _state(evidence), "", correlation_id
        )
        return evidence
    ...
    def archive_evidence(cls, tenant_id, actor, evidence_id, correlation_id):
        ...
        evidence.deleted_by = _actor(actor)
        evidence.updated_by = _actor(actor)
        evidence.save()
        ActivityService.record(
            tenant_id, actor, evidence, "evidence.archived", before, _state(evidence), "", correlation_id
        )
    ...
    def validate_evidence(cls, tenant_id, evidence_id, as_of):
        ...
        return {
            ...
            "hash_valid": bool(not evidence.sha256 or SHA256_RE.fullmatch(evidence.sha256)),
            "fresh": fresh,
    ...
    def list_expiring(tenant_id, as_of, days):
        ...
        return (
            ComplianceEvidence.objects.for_tenant(_uuid(tenant_id))
            .filter(deleted_at__isnull=True, valid_until__gte=as_of, valid_until__lte=as_of + timedelta(days=days))
...
class ComplianceDashboardService:
    ...
    def summary(cls, tenant_id, framework_id=None, as_of=None):
        ...
        if framework_id:
            frameworks = frameworks.filter(pk=_uuid(framework_id))
        requirements = ComplianceRequirement.objects.for_tenant(tenant).filter(deleted_at__isnull=True, status="active")
        if framework_id:
            requirements = requirements.filter(framework_id=_uuid(framework_id))
        assessed_ids = (
            ComplianceAssessment.objects.for_tenant(tenant)
        ...
        return {
            ...
            "unassessed_requirements": requirements.exclude(id__in=assessed_ids).count(),
            "gaps": gaps,
            "review_queue": len(cls.review_queue(tenant, as_of)),
            "expiring_evidence": EvidenceService.list_expiring(tenant, as_of, warning_days).count(),
        }
    ...
    def review_queue(tenant_id, as_of=None):
        ...
        return list(
            CompliancePolicy.objects.for_tenant(_uuid(tenant_id))
            .filter(deleted_at__isnull=True, next_review_date__lte=value)
