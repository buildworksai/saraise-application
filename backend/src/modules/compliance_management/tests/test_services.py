...
def test_framework_requirement_import_export_update_and_lifecycle():
    ...
    requirements = list(
        RequirementService.list_requirements(tenant, {"framework_id": framework.id, "search": "Govern"})
    )
...
def test_policy_full_publication_revision_and_archive(tenant_a, tenant_a_user):
    ...
    policy = PolicyService.create_policy(
        ...
        tenant_a_user,
        {
            ...
            "title": "Publishable",
            "category": "General",
            "owner_id": tenant_a_user.pk,
            "effective_date": timezone.now().date(),
        },
        correlation(),
    )
    PolicyService.update_policy(tenant, tenant_a_user, policy.id, {"summary": "Reviewed policy."}, correlation())
    version = PolicyService.create_version(
        tenant, tenant_a_user, policy.id, "Policy content", "Initial", "version-1", correlation()
    )
    assert version.version == 1
    assert (
        PolicyService.create_version(
            tenant, tenant_a_user, policy.id, "Policy content", "Initial", "version-1", correlation()
        ).id
        == version.id
    )
    ...
    revised, second = PolicyService.revise(
        tenant, tenant_a_user, policy.id, "Revised content", "Annual revision", "revise", correlation()
    )
...
def test_mapping_assessment_gap_history_and_overdue():
    ...
    policy = PolicyService.create_policy(
        tenant, None, {"code": "P", "title": "Policy", "category": "General"}, correlation()
    )
    ...
    assessment = AssessmentService.record_assessment(
        ...
        None,
        {
            ...
            "notes": "Control is not implemented.",
            "source": "manual",
...
def test_evidence_crud_links_freshness_and_dependency_failure():
    ...
    evidence = EvidenceService.register_evidence(
        ...
        None,
        {
            ...
            "text_reference": "Signed reference",
    ...
    link = EvidenceService.link_requirement(
        tenant, None, evidence.id, requirement.id, {"relevance": "primary", "notes": "Primary proof"}, correlation()
    )
