...
def health_check(request):
    """
    Health check endpoint for DataMigration module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "data-migration", "checks": {}}
    ...
    try:
        cache.set("health_check_data_migration", "ok", 10)
        result = cache.get("health_check_data_migration")
        if result == "ok":
    ...
    try:
        ...
        count = MigrationJob.objects.count()
