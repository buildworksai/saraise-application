...
class SourceAdapter(Protocol):
    ...
    def inspect(
        self, tenant_id: UUID, artifact_id: UUID | None, config: Mapping[str, object], runtime: object
    ) -> Mapping[str, object]: ...
    def iter_records(
        self, tenant_id: UUID, artifact_id: UUID | None, config: Mapping[str, object], runtime: object
...
class TargetAdapter(Protocol):
    ...
    def validate_reference(
        self, entity: str, field: str, value: object, rule_type: str, config: Mapping[str, object]
    ) -> bool: ...
    def lookup(self, tenant_id: UUID, entity: str, fields: Mapping[str, object]) -> Mapping[str, object] | None: ...
    def write(
        self, tenant_id: UUID, entity: str, record: Mapping[str, object], **context: object
    ) -> Mapping[str, object]: ...
...
class AdapterRegistry:
    ...
    def register(self, key: str, adapter: Any, *, replace: bool = False) -> Any:
        if not isinstance(key, str) or not key or len(key) > 100 or "." not in key:
            raise ValueError(f"{self.kind} adapter key must be a stable namespaced identifier")
        with self._lock:
            if key in self._items and not replace:
                raise RuntimeError(f"Duplicate {self.kind} adapter registration: {key}")
    ...
    def get(self, key: str) -> Any:
        try:
            ...
            raise LookupError(f"{self.kind} adapter {key!r} is unavailable") from exc
