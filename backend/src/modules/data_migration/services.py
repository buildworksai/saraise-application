...
class ConfigurationConflict(MigrationServiceError):
    def __init__(self) -> None:
        super().__init__(
            "The migration definition changed; reload and compare before retrying.", code="VERSION_CONFLICT"
        )
...
def _validated_external_hostaddr(host: str, *, kind: str = "database") -> str:
    ...
    if (
        ...
        or host != host.strip()
        or not host
        or any(token in host for token in ("/", ",", "\x00"))
    ):
    ...
    if kind == "http":
        ...
        if (
            ...
            raise MigrationServiceError(
                "HTTP connections require a canonical HTTPS base URL without credentials.", code="DENIED_DESTINATION"
            )
...
def _snapshot_checksum(document: Mapping[str, object]) -> str:
    ...
    return hashlib.sha256(
        json.dumps(sanitized, sort_keys=True, separators=(",", ":"), default=str).encode()
    ).hexdigest()
...
def _validate_source_reference(
    tenant: UUID, actor: UUID, source_type: str, source_config: Mapping[str, object], artifact_id: object | None
) -> None:
    ...
    if source_type in {"database", "api"}:
        ...
        try:
            ...
            raise MigrationServiceError(
                "Source artifact is unavailable to this tenant.", code="INVALID_SOURCE_REFERENCE"
            ) from exc
...
class ExternalConnectionService:
    ...
    def register(tenant_id: object, actor_id: object, payload: Mapping[str, object]) -> ExternalConnection:
        ...
        if any(key in values for key in ("password", "token", "secret")):
            raise MigrationServiceError(
                "Secret material is forbidden; provide credential_ref.", code="SECRET_MATERIAL_FORBIDDEN"
            )
    ...
    def update(
        ...
        _validate_connection_destination(
            {field.name: getattr(connection, field.name) for field in connection._meta.fields}
        )
        connection.full_clean()
        connection.save()
        return connection
    ...
    def list_references(tenant_id: object) -> QuerySet[ExternalConnection]:
        return (
            ExternalConnection.objects.for_tenant(_uuid(tenant_id, "tenant_id")).filter(is_active=True).order_by("name")
        )
...
class MigrationJobService:
    ...
    def create(tenant_id: object, actor_id: object, command: Mapping[str, object]) -> MigrationJob:
        ...
        publish_event(
            ...
            payload={"version": 1, "status": job.status},
        )
        return job
    ...
    @transaction.atomic
    def update(
        tenant_id: object, job_id: object, actor_id: object, command: Mapping[str, object], expected_version: int
    ) -> MigrationJob:
        tenant, actor = _uuid(tenant_id, "tenant_id"), _uuid(actor_id, "actor_id")
        ...
        publish_event(
            ...
            actor_id=actor,
            correlation_id=correlation,
            payload={"version": job.configuration_version, "status": job.status},
        )
        return job
    ...
    def restore_deleted(tenant_id: object, job_id: object, actor_id: object) -> MigrationJob:
        ...
        publish_event(
            tenant, "data_migration.job.restored", "migration_job", job.id, actor_id=actor, payload={"status": "draft"}
        )
        return job
    ...
    def validate_definition(tenant_id: object, job_id: object, actor_id: object) -> DefinitionValidationResult:
        ...
        JOB_MACHINE.apply(
            job, "validate", transition_key=f"validate:{job.configuration_version}", context={"actor_id": str(actor)}
        )
    ...
    def archive(tenant_id: object, job_id: object, actor_id: object, transition_key: str) -> MigrationJob:
        tenant, actor = _uuid(tenant_id, "tenant_id"), _uuid(actor_id, "actor_id")
        job = _job(tenant, job_id, lock=True)
        result = JOB_MACHINE.apply(
            job, "archive", transition_key=_text(transition_key, "transition_key"), context={"actor_id": str(actor)}
        )
        publish_event(
            tenant,
    ...
    @transaction.atomic
    def restore_version(
        tenant_id: object, job_id: object, version: int, actor_id: object, expected_version: int
    ) -> MigrationJob:
        tenant, actor = _uuid(tenant_id, "tenant_id"), _uuid(actor_id, "actor_id")
        job = _job(tenant, job_id, lock=True, include_deleted=True)
        if job.configuration_version != expected_version:
...
class MigrationMappingService:
    ...
    def create(tenant_id: object, job_id: object, actor_id: object, payload: Mapping[str, object]) -> MigrationMapping:
        ...
        values.pop("tenant_id", None)
        values.pop("job", None)
        values["transform_config"] = validate_transform_config(
            str(values.get("transform_type", "identity")), values.get("transform_config", {})
        )
    ...
    @transaction.atomic
    def update(
        tenant_id: object, mapping_id: object, actor_id: object, payload: Mapping[str, object]
    ) -> MigrationMapping:
        tenant, actor = _uuid(tenant_id, "tenant_id"), _uuid(actor_id, "actor_id")
        mapping = (
        ...
        if set(payload) & {"id", "tenant_id", "job", "created_by"}:
            raise MigrationServiceError("Immutable mapping field.", code="IMMUTABLE_FIELD")
        for field, value in payload.items():
    ...
    def delete(tenant_id: object, mapping_id: object, actor_id: object) -> None:
        ...
        job = mapping.job
        mapping.delete()
        _bump_definition(job, actor, "Mapping deleted")
    ...
    def suggest_deterministic(tenant_id: object, job_id: object) -> list[dict[str, object]]:
        ...
        for source in source_fields:
            ...
            match = next(
                (field for name, field in targets.items() if re.sub(r"[^a-z0-9]", "", name) == normalized), None
            )
    ...
    def apply_suggestions(
        tenant_id: object, job_id: object, actor_id: object, suggestion_ids: Sequence[str]
    ) -> list[MigrationMapping]:
        ...
        return [
            MigrationMappingService.create(
                ...
                actor_id,
                {
                    "source_field": suggestions[key]["source_field"],
                    "target_field": suggestions[key]["target_field"],
                    "position": start + index,
                    "origin": "deterministic",
                    "confidence": Decimal(str(suggestions[key]["confidence"])),
                },
            )
            for index, key in enumerate(suggestion_ids)
        ]
...
class ValidationRuleService:
    ...
    def create(tenant_id: object, job_id: object, actor_id: object, payload: Mapping[str, object]) -> ValidationRule:
        ...
        rule = ValidationRule(tenant_id=tenant, job=job, created_by=actor, **values)
        rule.full_clean()
        rule.save()
        _bump_definition(job, actor, "Validation rule created")
        return rule
    ...
    def update(tenant_id: object, rule_id: object, actor_id: object, payload: Mapping[str, object]) -> ValidationRule:
        ...
        if set(payload) & {"id", "tenant_id", "job", "created_by"}:
            raise MigrationServiceError("Immutable rule field.", code="IMMUTABLE_FIELD")
        for field, value in payload.items():
            setattr(rule, field, value)
        rule.rule_config = validate_rule_config(rule.rule_type, rule.rule_config)
        rule.updated_by = actor
        rule.full_clean()
        rule.save()
        _bump_definition(rule.job, actor, "Validation rule updated")
        return rule
    ...
    @staticmethod
    def evaluate(
        tenant_id: object, job_id: object, mapped_record: Mapping[str, object], row_number: int
    ) -> list[RuleOutcome]:
        tenant = _uuid(tenant_id, "tenant_id")
        job = _job(tenant, job_id)
        ...
        for rule in job.validation_rules.filter(is_active=True).order_by("position"):
            config = validate_rule_config(rule.rule_type, rule.rule_config)
            value = mapped_record.get(rule.field_name)
            passed = True
            if rule.rule_type == "required":
                ...
                passed = {
                    ...
                    "number": lambda: isinstance(value, (int, float, Decimal)) and not isinstance(value, bool),
                ...
                raise MigrationServiceError(f"Unknown validation rule {rule.rule_type!r}.", code="UNKNOWN_RULE")
            outcomes.append(
                RuleOutcome(
                    rule.id,
                    rule.field_name,
                    ...
                    "" if passed else rule.error_message,
                    row_number,
                )
            )
        return outcomes
...
def _reorder_rules(
    ...
    tenant, actor = _uuid(tenant_id, "tenant_id"), _uuid(actor_id, "actor_id")
    job = _job(tenant, job_id, lock=True)
    rows = list(ValidationRule.objects.for_tenant(tenant).select_for_update().filter(job=job))
    by_id = {str(row.id): row for row in rows}
    if len(ordered_ids) != len(rows) or set(map(str, ordered_ids)) != set(by_id):
    ...
    ValidationRule.objects.bulk_update(rows, ("position", "updated_by", "updated_at"))
    _bump_definition(job, actor, "Validation rules reordered")
    return sorted(rows, key=lambda item: item.position)
...
class SourceInspectionService:
    ...
    def request_inspection(tenant_id: object, job_id: object, actor_id: object, idempotency_key: str) -> AsyncJob:
        ...
        return enqueue(
            tenant,
            actor,
            "data_migration.inspect",
            {"job_id": str(job.id), "job_version_id": str(version.id)},
            f"data-migration:inspect:{_text(idempotency_key, 'idempotency_key')}",
        )
    ...
    def preview(tenant_id: object, job_id: object, limit: int) -> PreviewResult:
        tenant = _uuid(tenant_id, "tenant_id")
        job = _job(tenant, job_id)
        configured = _configuration(tenant).preview_row_limit
        if not isinstance(limit, int) or isinstance(limit, bool) or limit < 1 or limit > min(100, configured):
            raise MigrationServiceError(
                f"Preview limit must be between 1 and {min(100, configured)}.", code="INVALID_LIMIT"
            )
        adapter = _source_adapter(f"core.{job.source_type}")
        iterator = adapter.iter_records(tenant, job.source_artifact_id, job.source_config, _configuration(tenant))
        records = []
        ...
        checksum = hashlib.sha256(json.dumps(visible, sort_keys=True, default=str).encode()).hexdigest()
...
class MigrationExecutionService:
    ...
    @transaction.atomic
    def request_run(
        tenant_id: object, job_id: object, actor_id: object, mode: str, idempotency_key: str
    ) -> MigrationRun:
        tenant, actor = _uuid(tenant_id, "tenant_id"), _uuid(actor_id, "actor_id")
        mode = _text(mode, "mode", 10)
        if mode not in ("dry_run", "commit"):
            raise MigrationServiceError("mode must be dry_run or commit", code="INVALID_MODE")
        job = _job(tenant, job_id, lock=True)
        if job.status != "ready":
            raise MigrationServiceError("Only ready jobs may run.", code="JOB_NOT_READY")
        key = _text(idempotency_key, "idempotency_key")
        existing = MigrationRun.objects.for_tenant(tenant).filter(idempotency_key=key).first()
        if existing:
            if existing.job_id != job.id or existing.mode != mode:
                raise MigrationServiceError(
                    "Idempotency key belongs to another run request.", code="IDEMPOTENCY_CONFLICT"
                )
            return existing
        profile = SourceInspectionService.inspect(
            tenant,
            job.id,
            MigrationJobVersion.objects.for_tenant(tenant).get(job=job, version=job.configuration_version).id,
        )
        ...
        run = MigrationRun.objects.create(
            ...
            created_by=actor,
            correlation_id=async_job.correlation_id,
        )
        async_job.payload = {"run_id": str(run.id)}
        async_job.save(update_fields=("payload", "updated_at"))
        publish_event(
            tenant,
            ...
            payload={"mode": mode, "status": "queued"},
        )
        return run
    ...
    def cancel(tenant_id: object, run_id: object, actor_id: object, transition_key: str) -> MigrationRun:
        ...
        if run.status == "queued":
            run = RUN_MACHINE.apply(
                run, "cancel", transition_key=_text(transition_key, "transition_key"), context={"actor_id": str(actor)}
            )
        return run
    ...
    def execute(tenant_id: object, run_id: object) -> MigrationRun:
        ...
        with transaction.atomic():
            ...
            if run is None:
                raise NotFound("Migration run not found.")
            if run.status in ("succeeded", "partial", "failed", "cancelled", "rolled_back"):
                return run
            run = RUN_MACHINE.apply(run, "start", transition_key=f"start:{run.id}")
            run.started_at = timezone.now()
            run.save(update_fields=("started_at", "updated_at"))
        job = run.job
        source = _source_adapter(f"core.{job.source_type}")
        target = _target_adapter(job.target_adapter)
        config = _configuration(tenant)
        batch: list[tuple[int, Mapping[str, object]]] = []
        try:
            for number, record in enumerate(
                source.iter_records(tenant, job.source_artifact_id, job.source_config, config), start=1
            ):
                batch.append((number, record))
                if len(batch) >= config.batch_size:
                    MigrationExecutionService._process_batch(tenant, run.id, batch, target)
                    batch = []
                if (
                    MigrationRun.objects.for_tenant(tenant)
                    .filter(pk=run.id, cancel_requested_at__isnull=False)
                    .exists()
                ):
                    with transaction.atomic():
                        locked = MigrationRun.objects.for_tenant(tenant).select_for_update().get(pk=run.id)
                        return RUN_MACHINE.apply(locked, "cancel", transition_key=f"cancel:worker:{run.id}")
            if batch:
                MigrationExecutionService._process_batch(tenant, run.id, batch, target)
            with transaction.atomic():
                run = MigrationRun.objects.for_tenant(tenant).select_for_update().get(pk=run.id)
                run.total_records = run.processed_records
                run.save(update_fields=("total_records", "updated_at"))
                command = "succeed" if run.failed_records == 0 else "partial"
                run = RUN_MACHINE.apply(run, command, transition_key=f"terminal:{run.id}")
            ...
            with transaction.atomic():
                ...
                if locked.status in ("queued", "running"):
                    locked = RUN_MACHINE.apply(locked, "fail", transition_key=f"failed:{run.id}")
        ...
        for row_number, source_record in batch:
            ...
            try:
                for mapping in mappings:
                    mapped[mapping.target_field] = _transform(
                        source_record.get(mapping.source_field),
                ...
                for outcome in failures:
                    MigrationRunIssue.objects.create(
                        ...
                        redacted_sample=_redact(mapped),
                    )
                if any(outcome.severity == "error" for outcome in failures):
                    run.failed_records += 1
                elif run.mode == "dry_run":
                    run.succeeded_records += 1
                else:
                    evidence = target.write(
                        tenant,
                        job.target_entity,
                        mapped,
                        write_mode=job.write_mode,
                    ...
                    if not evidence or not evidence.get("record_id") or not evidence.get("after_checksum"):
                        raise MigrationServiceError(
                    ...
                    run.succeeded_records += 1
                if any(outcome.severity == "warning" for outcome in failures):
                    run.warning_records += 1
            except Exception as exc:
                run.failed_records += 1
                MigrationRunIssue.objects.create(
                    tenant_id=tenant,
                    ...
                    stage="target",
                    severity="error",
                    code=getattr(exc, "code", "ROW_FAILED"),
                    message="Record processing failed; use the correlation ID for support.",
                    redacted_sample=_redact(mapped),
                )
            run.processed_records += 1
        run.total_records = max(run.total_records, run.processed_records)
        run.save(
            update_fields=(
                "total_records",
                "processed_records",
...
def _transform(value: object, kind: str, config: Mapping[str, object], record: Mapping[str, object]) -> object:
    ...
    if kind == "identity":
        return value
    if kind == "default":
        return validated["value"] if value in (None, "") else value
    if kind == "cast":
        target = validated["to"]
        if target == "string":
...
class RollbackService:
    ...
    def request(tenant_id: object, run_id: object, actor_id: object, idempotency_key: str) -> MigrationRollback:
        ...
        if run.mode != "commit" or run.status not in ("succeeded", "partial"):
            raise MigrationServiceError(
                "Only successful committed runs can be rolled back.", code="ROLLBACK_NOT_ALLOWED"
            )
        async_job = enqueue(
            tenant, actor, "data_migration.rollback", {"pending": True}, f"data-migration:rollback:{key}"
        )
        rollback = MigrationRollback.objects.create(
            tenant_id=tenant,
            run=run,
            async_job_id=async_job.id,
            idempotency_key=key,
            records_total=run.changes.filter(reversed_at__isnull=True).count(),
            requested_by=actor,
            correlation_id=async_job.correlation_id,
        )
        async_job.payload = {"rollback_id": str(rollback.id)}
        async_job.save(update_fields=("payload", "updated_at"))
        publish_event(
            tenant,
    ...
    def execute(tenant_id: object, rollback_id: object) -> MigrationRollback:
        ...
        with transaction.atomic():
            ...
            if rollback.status in ("succeeded", "failed"):
                return rollback
        ...
        try:
            for change in (
                ...
                .filter(run=rollback.run, reversed_at__isnull=True)
                .order_by("-sequence")
            ):
                adapter = target_cache.setdefault(change.target_adapter, _target_adapter(change.target_adapter))
                result = adapter.reverse(
                    tenant,
                    change.target_entity,
                    change.target_record_id,
                    operation=change.operation,
                    before_payload_encrypted=change.before_payload_encrypted,
                    expected_checksum=change.after_checksum,
                    idempotency_key=f"rollback:{rollback.id}:{change.sequence}",
                )
                if not result or not result.get("verified"):
                    raise MigrationServiceError(
                        "Rollback conflict: target record changed after migration.", code="ROLLBACK_CONFLICT"
                    )
                with transaction.atomic():
                    locked_change = MigrationChange.objects.for_tenant(tenant).select_for_update().get(pk=change.id)
                    if locked_change.reversed_at is None:
                        locked_change.mark_reversed(timezone.now())
                        MigrationRollback.objects.for_tenant(tenant).filter(pk=rollback.id).update(
                            records_reversed=models.F("records_reversed") + 1
                        )
            with transaction.atomic():
                rollback = (
                    MigrationRollback.objects.for_tenant(tenant)
                ...
                rollback = ROLLBACK_MACHINE.apply(rollback, "succeed", transition_key=f"terminal:{rollback.id}")
                rollback.completed_at = timezone.now()
                rollback.save(update_fields=("completed_at", "updated_at"))
                RUN_MACHINE.apply(rollback.run, "mark_rolled_back", transition_key=f"rollback:{rollback.id}")
            return rollback
        except Exception as exc:
            with transaction.atomic():
                rollback = MigrationRollback.objects.for_tenant(tenant).select_for_update().get(pk=rollback.id)
                rollback.records_failed = rollback.records_total - rollback.records_reversed
                rollback.failure_summary = getattr(exc, "code", "ROLLBACK_FAILED")
                rollback = ROLLBACK_MACHINE.apply(rollback, "fail", transition_key=f"failed:{rollback.id}")
                rollback.completed_at = timezone.now()
                rollback.save(update_fields=("records_failed", "failure_summary", "completed_at", "updated_at"))
            raise
...
class DataMigrationConfigurationService:
    ...
    def update(
        ...
        if unknown:
            raise MigrationServiceError(
                f"Unknown configuration fields: {', '.join(sorted(unknown))}", code="INVALID_CONFIGURATION"
            )
        ...
        publish_event(
            ...
            "data_migration_configuration",
            config.id,
            actor_id=actor,
            correlation_id=correlation,
            payload={"version": config.version, "from_version": expected_version},
        )
        return config

    @classmethod
    def preview(cls, tenant_id: object, payload: Mapping[str, object]) -> dict[str, object]:
        config = cls.get(tenant_id)
        before = config.as_document()
        proposed = {**before, **dict(payload)}
    ...
    @classmethod
    def import_document(
        cls, tenant_id: object, actor_id: object, document: Mapping[str, object], expected_version: int
    ) -> DataMigrationConfiguration:
    ...
    @classmethod
    def restore(
        cls, tenant_id: object, actor_id: object, version: int, expected_version: int
    ) -> DataMigrationConfiguration:
        tenant = _uuid(tenant_id, "tenant_id")
        audit = DataMigrationConfigurationAudit.objects.for_tenant(tenant).filter(version=version).first()
        if audit is None:
