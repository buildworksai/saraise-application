...
SOURCE_ALLOWED = {
    "csv": {"delimiter", "encoding", "header_row", "batch_size"},
    "excel": {"sheet", "header_row", "batch_size"},
    "json": {"encoding", "json_path", "batch_size"},
    "xml": {"encoding", "record_path", "batch_size"},
...
def validate_source_config(source_type: str, config: object) -> dict[str, object]:
    if source_type not in SOURCE_ALLOWED:
        raise ValueError(f"Unsupported source type: {source_type}")
    values = _object(config, "source_config")
    _only(values, SOURCE_ALLOWED[source_type], "source_config")
    _batch(values)
    if source_type in ("database", "api") and not values.get("connection_id"):
        raise ValueError("connection_id is required")
    if source_type == "database":
        if not IDENTIFIER.fullmatch(str(values.get("table", ""))):
            raise ValueError("table must be a safe identifier")
        columns = values.get("columns", [])
        if not isinstance(columns, list) or not columns or any(not IDENTIFIER.fullmatch(str(item)) for item in columns):
            raise ValueError("columns must be safe identifiers")
        if not isinstance(values.get("filters", {}), (dict, list)):
            raise ValueError("filters must contain equality filters")
    if source_type == "api":
        path = values.get("relative_path", "")
        if not isinstance(path, str) or not path.startswith("/") or path.startswith("//") or "://" in path:
            raise ValueError("relative_path must be a relative absolute-path reference")
        if values.get("method", "GET") != "GET":
...
def validate_transform_config(transform_type: str, config: object) -> dict[str, object]:
    ...
    required = {
        ...
        "date_parse": {"format"},
        "boolean_map": {"true_values", "false_values"},
    }.get(transform_type, set())
    if not required <= set(values):
        raise ValueError(f"{transform_type} requires: {', '.join(sorted(required))}")
    if transform_type == "regex_replace":
        _validate_regex(values.get("pattern"))
    return values


def _validate_regex(pattern: object) -> None:
    if not isinstance(pattern, str) or len(pattern) > 256:
        raise ValueError("regex pattern must be at most 256 characters")
...
def validate_rule_config(rule_type: str, config: object) -> dict[str, object]:
    ...
    required = {
        ...
        "referential": {"entity", "field"},
    }.get(rule_type, set())
    if not required <= set(values):
        raise ValueError(f"{rule_type} requires: {', '.join(sorted(required))}")
    if rule_type == "regex":
        _validate_regex(values.get("pattern"))
    if rule_type in ("range", "length") and values.get("min") is None and values.get("max") is None:
        raise ValueError(f"{rule_type} requires min or max")
    if rule_type == "allowed_values" and (not isinstance(values.get("values"), list) or len(values["values"]) > 1000):
        raise ValueError("allowed_values requires a bounded list")
    return values
