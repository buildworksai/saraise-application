...
def test_expected_unique_constraints_and_indexes_are_declared() -> None:
    ...
    assert {item.name for item in MigrationRun._meta.constraints} >= {
        "dm_run_idempotency_uniq",
        "dm_run_processed_lte_total",
    }
    assert {item.name for item in MigrationMapping._meta.constraints} >= {
        "dm_mapping_source_uniq",
        "dm_mapping_target_uniq",
        "dm_mapping_position_uniq",
        "dm_mapping_confidence_range",
    }
    assert {item.name for item in DataMigrationConfiguration._meta.constraints} >= {
        "dm_config_tenant_uniq",
        "dm_config_batch_range",
    }
...
def test_cross_tenant_mapping_rule_and_version_parents_are_rejected() -> None:
    ...
    version = MigrationJobVersion(
        ...
        job=job,
        version=1,
        snapshot={"tenant_id": str(tenant_a), "job_id": str(job.id)},
        change_summary="bad",
        created_by=actor,
        correlation_id="corr",
    )
...
def test_append_only_version_rejects_save_update_and_delete() -> None:
    ...
    version = MigrationJobVersion.objects.create(
        ...
        job=job,
        version=1,
        snapshot={"tenant_id": str(tenant), "job_id": str(job.id)},
...
def test_issue_sample_rejects_unredacted_pii() -> None:
    ...
    unsafe = MigrationRunIssue(
        ...
        code="EMAIL",
        message="Invalid",
        redacted_sample={"email": "person@example.test"},
    )
    ...
    safe = MigrationRunIssue(
        ...
        code="EMAIL",
        message="Invalid",
        redacted_sample={"email": "[REDACTED]"},
    )
    safe.full_clean()
