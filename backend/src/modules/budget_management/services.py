...
def _budget_for_update(tenant_id: UUID, budget_id: UUID | str) -> Budget:
    try:
        return Budget.objects.select_for_update().get(
            id=_uuid(budget_id, "budget_id"), tenant_id=tenant_id, is_deleted=False
        )
...
def _line_for_update(tenant_id: UUID, line_id: UUID | str) -> BudgetLine:
    try:
        return (
            ...
            .get(id=_uuid(line_id, "line_id"), tenant_id=tenant_id, is_deleted=False)
        )
...
def _assert_editable(budget: Budget) -> None:
    if budget.status not in EDITABLE_STATUSES:
        raise BudgetDomainError(
            "ILLEGAL_STATE", "Budget allocations and planning fields are immutable in this state", http_status=409
        )
...
def _emit(
    tenant_id: UUID, aggregate_type: str, aggregate_id: UUID, event_type: str, payload: Mapping[str, Any]
) -> OutboxEvent:
    safe = {
        key: value
        for key, value in payload.items()
        if key in {"budget_id", "actor_id", "line_count", "fiscal_year", "status", "schema_version"}
    }
...
class BudgetService:
    ...
    def create_budget(
        ...
        *,
        ...
        if Budget.objects.filter(
            tenant_id=tenant_id, is_deleted=False, fiscal_year=fiscal_year, budget_name__iexact=name
        ).exists():
            raise BudgetDomainError(
                "DUPLICATE_BUDGET_NAME", "A budget with this name already exists for the fiscal year", http_status=409
            )
        try:
        ...
        _emit(
            ...
            budget.id,
            "budget.created",
            {"budget_id": budget.id, "actor_id": actor_id, "fiscal_year": fiscal_year},
        )
        return budget
    ...
    def update_budget(
        ...
        *,
        ...
        if currency not in ISO_4217:
            raise BudgetDomainError("INVALID_CURRENCY", "currency must be an uppercase ISO 4217 code")
        ceiling = (
            _decimal(values["budget_ceiling"], "budget_ceiling") if values.get("budget_ceiling") is not None else None
        )
    ...
    def _allocation_values(data: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if (
            ...
            or not 1 <= period_number <= limits[period_type]
        ):
    ...
    def replace_allocations(
        ...
        *,
        ...
        if budget.budget_ceiling is not None and total != budget.budget_ceiling:
            raise BudgetDomainError(
                "CEILING_MISMATCH",
                "Allocation total must equal the budget ceiling",
                http_status=409,
                detail={
                    "ceiling": str(budget.budget_ceiling),
                    "allocated": str(total),
                    "difference": str(budget.budget_ceiling - total),
                },
            )
        ...
        _emit(
            ...
            "budget",
            budget.id,
            "budget.allocations_replaced",
            {"budget_id": budget.id, "actor_id": actor_id, "line_count": len(normalized)},
        )
        return budget
    ...
    def create_line(tenant_id: UUID, budget_id: UUID, actor_id: UUID, data: Mapping[str, Any]) -> BudgetLine:
        ...
        line = BudgetLine.objects.create(
            ...
            created_by=actor_id,
            updated_by=actor_id,
            variance=values["budget_amount"] - values["actual_amount"],
            **values,
        )
    ...
    def update_line(
        ...
        *,
        ...
        current = {
            ...
            for field in ("account_id", "account_code", "period_type", "period_number", "budget_amount")
        }
    ...
    def recalculate_total_budget(tenant_id: UUID, budget_id: UUID, *, actor_id: UUID | None = None) -> Budget:
        ...
        budget = _budget_for_update(tenant_id, budget_id)
        total = BudgetLine.objects.filter(tenant_id=tenant_id, budget=budget, is_deleted=False).aggregate(
    ...
    def submit_for_approval(
        tenant_id: UUID, budget_id: UUID, actor_id: UUID, *, idempotency_key: str, notes: str = ""
    ) -> Budget:
        tenant_id, actor_id = _uuid(tenant_id, "tenant_id"), _uuid(actor_id, "actor_id")
        budget = _budget_for_update(tenant_id, budget_id)
        ...
        if not lines:
            raise BudgetDomainError(
                "ALLOCATIONS_REQUIRED", "At least one active allocation is required", http_status=409
            )
        total = sum((line.budget_amount for line in lines), Decimal("0.00"))
        if budget.budget_ceiling is not None and total != budget.budget_ceiling:
            raise BudgetDomainError(
                "CEILING_MISMATCH", "Allocations must balance to the budget ceiling", http_status=409
            )
        ...
        if not request.steps or any(step.approval_level < 1 or step.approver_id == actor_id for step in request.steps):
            raise BudgetDomainError(
                "INVALID_APPROVAL_POLICY", "Workflow returned no valid independent approvers", http_status=503
            )
        ...
        result = BudgetService._apply_transition(
            ...
            notes=notes,
            workflow_request_id=request.workflow_request_id,
            _submitted_at=timezone.now(),
        )
        enqueue(
            tenant_id,
            actor_id,
            "budget_management.dispatch_submission_notification",
    ...
    def approve_budget(
        ...
        if prior_key is not None:
            if prior_key.status != "approved":
                raise BudgetDomainError(
                    "IDEMPOTENCY_CONFLICT", "Approval key belongs to another decision", http_status=409
                )
            if budget.status == "approved":
                return budget
            if BudgetApproval.objects.filter(tenant_id=tenant_id, budget=budget, decisions__isnull=True).exists():
                return budget
            return BudgetService._apply_transition(
                ...
                _approved_at=budget.approved_at or timezone.now(),
            )
        if budget.submitted_by == actor_id:
            raise BudgetDomainError(
                "SELF_APPROVAL_FORBIDDEN", "Submitters cannot approve their own budget", http_status=403
            )
        approval = (
            BudgetApproval.objects.select_for_update()
            .filter(tenant_id=tenant_id, budget=budget, approver_id=actor_id, decisions__isnull=True)
            .order_by("approval_level")
            .first()
        )
        if approval is None:
            raise BudgetDomainError(
                "APPROVAL_NOT_ASSIGNED", "No pending approval is assigned to this actor", http_status=403
            )
        ...
        if lower_pending:
            raise BudgetDomainError(
                "APPROVAL_LEVEL_PENDING", "Earlier approval levels must complete first", http_status=409
            )
    ...
    def reject_budget(tenant_id: UUID, budget_id: UUID, actor_id: UUID, *, idempotency_key: str, reason: str) -> Budget:
        ...
        if prior_key is not None:
            ...
            return BudgetService._apply_transition(
                ...
                idempotency_key,
                approval_level=prior_key.approval.approval_level,
                _rejected_at=budget.rejected_at or timezone.now(),
                _rejection_reason=budget.rejection_reason or reason,
            )
        if budget.submitted_by == actor_id:
            raise BudgetDomainError(
                "SELF_APPROVAL_FORBIDDEN", "Submitters cannot reject their own budget", http_status=403
            )
        approval = (
            BudgetApproval.objects.select_for_update()
            .filter(tenant_id=tenant_id, budget=budget, approver_id=actor_id, decisions__isnull=True)
        ...
        **metadata: Any,
        ...
        try:
            result = BUDGET_STATE_MACHINE.apply(
                ...
                command,
                tenant_id=tenant_id,
                transition_key=idempotency_key.strip(),
        ...
        _emit(
            ...
            result.id,
            event,
            {"budget_id": result.id, "actor_id": actor_id, "status": result.status},
        )
        return result
...
class BudgetControlService:
    ...
    def check_budget_availability(
        ...
        *,
        ...
        tenant_id = _uuid(tenant_id, "tenant_id")
        code, requested, on_date = (
            _clean_text(account_code, "account_code", upper=True, max_length=50),
            _decimal(amount, "amount", positive=True),
            _date(period, "period"),
        )
        ...
        matching_period = (
            models.Q(period_type="annual", period_number=1)
            | models.Q(period_type="monthly", period_number=on_date.month)
    ...
    def request_actuals_sync(tenant_id: UUID, budget_id: UUID, actor_id: UUID, *, idempotency_key: str) -> AsyncJob:
        ...
        return enqueue(
            tenant_id,
            actor_id,
            "budget_management.sync_actuals",
    ...
    def apply_actuals_snapshot(
        ...
        *,
        ...
        if OutboxEvent.objects.filter(
            tenant_id=tenant_id, aggregate_id=evidence_id, event_type="budget.actuals_evidence"
        ).exists():
            return budget
        seen: set[tuple[str, str, int]] = set()
        for raw in snapshot:
        ...
        OutboxEvent.objects.create(
            tenant_id=tenant_id,
            aggregate_type="budget_actuals_evidence",
            aggregate_id=evidence_id,
            event_type="budget.actuals_evidence",
            payload={"budget_id": str(budget.id), "schema_version": 1},
        )
    ...
    def _commitment(
        ...
        *,
        ...
        with transaction.atomic():
            ...
            if existing is not None:
                if (
                    existing.budget_line_id != line.id
                    or existing.operation != operation
                    or existing.amount != amount_value
                ):
                    raise BudgetDomainError(
            ...
            if updated < 0:
                raise BudgetDomainError(
                    "COMMITMENT_UNDERFLOW", "Released amount exceeds the committed amount", http_status=409
                )
            try:
                with transaction.atomic():
...
class VarianceAlertService:
    ...
    def request_alert_generation(
        ...
        *,
        ...
        return enqueue(
            tenant_id,
            actor_id,
            "budget_management.generate_variance_alerts",
            {
                "threshold_percentage": str(threshold),
                "alert_type": kind,
                "alert_date": timezone.localdate().isoformat(),
            },
    ...
    def generate_alerts(
        ...
        *,
        ...
        tenant_id, threshold, on_date = (
            _uuid(tenant_id, "tenant_id"),
            _percent(threshold_percentage),
            _date(alert_date, "alert_date"),
        )
        ...
        for line in lines.iterator():
            ...
            consumption_ratio = (
                None if line.budget_amount == 0 else (consumption / line.budget_amount * 100).quantize(PERCENT)
            )
            variance_percentage = (
                None
            ...
            matches = (
                (
                    ...
                    and line.budget_amount > 0
                    and consumption >= line.budget_amount * Decimal("0.80")
                    and consumption <= line.budget_amount
                )
                or (kind == "over_budget" and consumption > line.budget_amount * (Decimal("1") + threshold / 100))
                or (
                    kind == "underspend"
                    and line.budget.end_date <= on_date
                    and line.budget_amount > 0
                    and consumption_ratio is not None
                    and consumption_ratio < threshold
                )
            )
            if not matches:
                continue
