"""
Tenant Isolation Tests for CustomizationFramework module.
...
class TestCustomizationFrameworkTenantIsolation:
    """
    CRITICAL: Tenant isolation tests for CustomizationFrameworkResource model.
    ...
    def test_user_cannot_list_other_tenant_resources(self, api_client, tenant_a_user, tenant_b_user):
        ...
        resource_a = CustomizationFrameworkResource.objects.create(
        ...
        resource_b = CustomizationFrameworkResource.objects.create(
    ...
    def test_user_cannot_get_other_tenant_resource_by_id(self, api_client, tenant_a_user, tenant_b_user):
        ...
        response = api_client.get(f"/api/v1/customization-framework/resources/{resource_b.id}/")
    ...
    def test_user_cannot_update_other_tenant_resource(self, api_client, tenant_a_user, tenant_b_user):
        ...
        response = api_client.put(
            f"/api/v1/customization-framework/resources/{resource_b.id}/",
            data,
            format="json"
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    def test_user_cannot_delete_other_tenant_resource(self, api_client, tenant_a_user, tenant_b_user):
        ...
        response = api_client.delete(f"/api/v1/customization-framework/resources/{resource_b.id}/")
        assert response.status_code == status.HTTP_404_NOT_FOUND

        # Verify resource still exists
        assert CustomizationFrameworkResource.objects.filter(id=resource_b.id).exists()
