# Generated by Django 4.2.27 on 2026-01-05 10:16

import django.core.validators
import django.db.models.deletion
from django.db import migrations, models

import src.modules.tenant_management.models


class Migration(migrations.Migration):

    initial = True

    dependencies = []

    operations = [
        migrations.CreateModel(
            name="Tenant",
            fields=[
                (
                    "id",
                    models.CharField(
                        default=src.modules.tenant_management.models.generate_uuid,
                        max_length=36,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True, db_index=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "created_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who created this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "updated_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who last updated this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "name",
                    models.CharField(
                        db_index=True,
                        help_text="Tenant organization name",
                        max_length=200,
                    ),
                ),
                (
                    "slug",
                    models.CharField(
                        db_index=True,
                        help_text="URL-safe identifier",
                        max_length=100,
                        unique=True,
                    ),
                ),
                (
                    "subdomain",
                    models.CharField(
                        blank=True,
                        db_index=True,
                        help_text="Subdomain (tenant.saraise.com)",
                        max_length=100,
                        null=True,
                        unique=True,
                    ),
                ),
                (
                    "custom_domain",
                    models.CharField(
                        blank=True,
                        db_index=True,
                        help_text="Custom domain (mycompany.com)",
                        max_length=200,
                        null=True,
                        unique=True,
                    ),
                ),
                (
                    "status",
                    models.CharField(
                        choices=[
                            ("trial", "Trial"),
                            ("active", "Active"),
                            ("suspended", "Suspended"),
                            ("cancelled", "Cancelled"),
                            ("archived", "Archived"),
                        ],
                        db_index=True,
                        default="trial",
                        max_length=50,
                    ),
                ),
                (
                    "subscription_plan_id",
                    models.CharField(
                        blank=True,
                        help_text="Subscription plan ID (references billing module)",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "trial_ends_at",
                    models.DateTimeField(blank=True, help_text="Trial expiration date", null=True),
                ),
                ("subscription_start_date", models.DateField(blank=True, null=True)),
                ("subscription_end_date", models.DateField(blank=True, null=True)),
                ("primary_contact_name", models.CharField(blank=True, max_length=200)),
                (
                    "primary_contact_email",
                    models.EmailField(blank=True, max_length=254),
                ),
                ("primary_contact_phone", models.CharField(blank=True, max_length=50)),
                ("billing_email", models.EmailField(blank=True, max_length=254)),
                ("technical_email", models.EmailField(blank=True, max_length=254)),
                ("logo_url", models.URLField(blank=True, max_length=500)),
                ("website_url", models.URLField(blank=True, max_length=500)),
                ("industry", models.CharField(blank=True, max_length=100)),
                (
                    "company_size",
                    models.CharField(
                        blank=True,
                        choices=[
                            ("1-10", "1-10 employees"),
                            ("11-50", "11-50 employees"),
                            ("51-200", "51-200 employees"),
                            ("201-500", "201-500 employees"),
                            ("500+", "500+ employees"),
                        ],
                        max_length=50,
                    ),
                ),
                ("tax_id", models.CharField(blank=True, max_length=100)),
                ("timezone", models.CharField(default="UTC", max_length=100)),
                ("default_language", models.CharField(default="en", max_length=10)),
                ("default_currency", models.CharField(default="USD", max_length=10)),
                (
                    "fiscal_year_start_month",
                    models.IntegerField(
                        default=1,
                        validators=[
                            django.core.validators.MinValueValidator(1),
                            django.core.validators.MaxValueValidator(12),
                        ],
                    ),
                ),
                ("date_format", models.CharField(default="YYYY-MM-DD", max_length=50)),
                ("time_format", models.CharField(default="HH:mm:ss", max_length=50)),
                (
                    "primary_color",
                    models.CharField(blank=True, help_text="Hex color code", max_length=7),
                ),
                ("secondary_color", models.CharField(blank=True, max_length=7)),
                ("accent_color", models.CharField(blank=True, max_length=7)),
                (
                    "features_enabled",
                    models.JSONField(default=dict, help_text="Feature flags configuration"),
                ),
                (
                    "max_users",
                    models.IntegerField(
                        default=10,
                        validators=[django.core.validators.MinValueValidator(1)],
                    ),
                ),
                (
                    "max_storage_gb",
                    models.IntegerField(
                        default=10,
                        validators=[django.core.validators.MinValueValidator(1)],
                    ),
                ),
                (
                    "max_api_calls_per_day",
                    models.IntegerField(
                        default=10000,
                        validators=[django.core.validators.MinValueValidator(0)],
                    ),
                ),
                (
                    "onboarded_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who onboarded this tenant",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "metadata",
                    models.JSONField(default=dict, help_text="Additional metadata"),
                ),
            ],
            options={
                "verbose_name": "Tenant",
                "verbose_name_plural": "Tenants",
                "db_table": "tenants",
            },
        ),
        migrations.CreateModel(
            name="TenantSettings",
            fields=[
                (
                    "id",
                    models.CharField(
                        default=src.modules.tenant_management.models.generate_uuid,
                        max_length=36,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True, db_index=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "created_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who created this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "category",
                    models.CharField(
                        db_index=True,
                        help_text="Setting category (e.g., 'email', 'notifications', 'security')",
                        max_length=100,
                    ),
                ),
                ("key", models.CharField(db_index=True, max_length=200)),
                ("value", models.JSONField(help_text="Setting value")),
                ("is_encrypted", models.BooleanField(default=False)),
                (
                    "updated_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who last updated this setting",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "tenant",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="settings",
                        to="tenant_management.tenant",
                    ),
                ),
            ],
            options={
                "verbose_name": "Tenant Setting",
                "verbose_name_plural": "Tenant Settings",
                "db_table": "tenant_settings",
            },
        ),
        migrations.CreateModel(
            name="TenantResourceUsage",
            fields=[
                (
                    "id",
                    models.CharField(
                        default=src.modules.tenant_management.models.generate_uuid,
                        max_length=36,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True, db_index=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "created_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who created this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "updated_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who last updated this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "date",
                    models.DateField(db_index=True, help_text="Date for this usage record"),
                ),
                ("active_users", models.IntegerField(default=0)),
                ("api_calls", models.IntegerField(default=0)),
                (
                    "storage_used_gb",
                    models.DecimalField(decimal_places=2, default=0, max_digits=10),
                ),
                (
                    "bandwidth_used_gb",
                    models.DecimalField(decimal_places=2, default=0, max_digits=10),
                ),
                ("email_sent", models.IntegerField(default=0)),
                ("sms_sent", models.IntegerField(default=0)),
                (
                    "avg_response_time_ms",
                    models.DecimalField(blank=True, decimal_places=2, max_digits=10, null=True),
                ),
                ("error_count", models.IntegerField(default=0)),
                ("slow_query_count", models.IntegerField(default=0)),
                (
                    "tenant",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="resource_usage",
                        to="tenant_management.tenant",
                    ),
                ),
            ],
            options={
                "verbose_name": "Tenant Resource Usage",
                "verbose_name_plural": "Tenant Resource Usage",
                "db_table": "tenant_resource_usage",
            },
        ),
        migrations.CreateModel(
            name="TenantModule",
            fields=[
                (
                    "id",
                    models.CharField(
                        default=src.modules.tenant_management.models.generate_uuid,
                        max_length=36,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True, db_index=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "created_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who created this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "updated_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who last updated this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "module_name",
                    models.CharField(
                        db_index=True,
                        help_text="Module identifier (e.g., 'crm', 'accounting')",
                        max_length=100,
                    ),
                ),
                ("is_enabled", models.BooleanField(db_index=True, default=True)),
                ("installed_at", models.DateTimeField(auto_now_add=True)),
                (
                    "installed_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who installed this module",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "version",
                    models.CharField(blank=True, help_text="Module version installed", max_length=50),
                ),
                (
                    "configuration",
                    models.JSONField(default=dict, help_text="Module-specific configuration"),
                ),
                (
                    "last_used_at",
                    models.DateTimeField(blank=True, help_text="Last time module was accessed", null=True),
                ),
                (
                    "usage_count",
                    models.IntegerField(default=0, help_text="Number of times module was accessed"),
                ),
                (
                    "tenant",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="modules",
                        to="tenant_management.tenant",
                    ),
                ),
            ],
            options={
                "verbose_name": "Tenant Module",
                "verbose_name_plural": "Tenant Modules",
                "db_table": "tenant_modules",
            },
        ),
        migrations.CreateModel(
            name="TenantHealthScore",
            fields=[
                (
                    "id",
                    models.CharField(
                        default=src.modules.tenant_management.models.generate_uuid,
                        max_length=36,
                        primary_key=True,
                        serialize=False,
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True, db_index=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "created_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who created this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                (
                    "updated_by",
                    models.CharField(
                        blank=True,
                        help_text="User ID who last updated this record",
                        max_length=36,
                        null=True,
                    ),
                ),
                ("date", models.DateField(db_index=True)),
                (
                    "overall_score",
                    models.IntegerField(
                        help_text="Overall health score (0-100)",
                        validators=[
                            django.core.validators.MinValueValidator(0),
                            django.core.validators.MaxValueValidator(100),
                        ],
                    ),
                ),
                (
                    "usage_score",
                    models.IntegerField(
                        blank=True,
                        null=True,
                        validators=[
                            django.core.validators.MinValueValidator(0),
                            django.core.validators.MaxValueValidator(100),
                        ],
                    ),
                ),
                (
                    "performance_score",
                    models.IntegerField(
                        blank=True,
                        null=True,
                        validators=[
                            django.core.validators.MinValueValidator(0),
                            django.core.validators.MaxValueValidator(100),
                        ],
                    ),
                ),
                (
                    "error_score",
                    models.IntegerField(
                        blank=True,
                        null=True,
                        validators=[
                            django.core.validators.MinValueValidator(0),
                            django.core.validators.MaxValueValidator(100),
                        ],
                    ),
                ),
                (
                    "engagement_score",
                    models.IntegerField(
                        blank=True,
                        null=True,
                        validators=[
                            django.core.validators.MinValueValidator(0),
                            django.core.validators.MaxValueValidator(100),
                        ],
                    ),
                ),
                (
                    "churn_risk",
                    models.DecimalField(
                        blank=True,
                        decimal_places=2,
                        help_text="Churn risk percentage (0-100)",
                        max_digits=5,
                        null=True,
                    ),
                ),
                (
                    "at_risk_reasons",
                    models.JSONField(default=list, help_text="List of risk factors"),
                ),
                ("calculated_at", models.DateTimeField(auto_now_add=True)),
                ("metadata", models.JSONField(default=dict)),
                (
                    "tenant",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.CASCADE,
                        related_name="health_scores",
                        to="tenant_management.tenant",
                    ),
                ),
            ],
            options={
                "verbose_name": "Tenant Health Score",
                "verbose_name_plural": "Tenant Health Scores",
                "db_table": "tenant_health_scores",
            },
        ),
        migrations.AddIndex(
            model_name="tenant",
            index=models.Index(fields=["status"], name="tenants_status_80d9b6_idx"),
        ),
        migrations.AddIndex(
            model_name="tenant",
            index=models.Index(fields=["subscription_plan_id"], name="tenants_subscri_16de56_idx"),
        ),
        migrations.AddIndex(
            model_name="tenant",
            index=models.Index(fields=["created_at"], name="tenants_created_fc84fe_idx"),
        ),
        migrations.AddIndex(
            model_name="tenant",
            index=models.Index(fields=["slug"], name="tenants_slug_3181c2_idx"),
        ),
        migrations.AddIndex(
            model_name="tenantsettings",
            index=models.Index(fields=["tenant", "category"], name="tenant_sett_tenant__e0b783_idx"),
        ),
        migrations.AlterUniqueTogether(
            name="tenantsettings",
            unique_together={("tenant", "category", "key")},
        ),
        migrations.AddIndex(
            model_name="tenantresourceusage",
            index=models.Index(fields=["tenant", "date"], name="tenant_reso_tenant__d2c5e5_idx"),
        ),
        migrations.AlterUniqueTogether(
            name="tenantresourceusage",
            unique_together={("tenant", "date")},
        ),
        migrations.AddIndex(
            model_name="tenantmodule",
            index=models.Index(fields=["tenant", "is_enabled"], name="tenant_modu_tenant__dbeabe_idx"),
        ),
        migrations.AddIndex(
            model_name="tenantmodule",
            index=models.Index(fields=["module_name"], name="tenant_modu_module__65fd0f_idx"),
        ),
        migrations.AlterUniqueTogether(
            name="tenantmodule",
            unique_together={("tenant", "module_name")},
        ),
        migrations.AddIndex(
            model_name="tenanthealthscore",
            index=models.Index(fields=["tenant", "date"], name="tenant_heal_tenant__3f2ee4_idx"),
        ),
        migrations.AddIndex(
            model_name="tenanthealthscore",
            index=models.Index(fields=["overall_score"], name="tenant_heal_overall_57807c_idx"),
        ),
        migrations.AddIndex(
            model_name="tenanthealthscore",
            index=models.Index(fields=["churn_risk"], name="tenant_heal_churn_r_828d39_idx"),
        ),
        migrations.AlterUniqueTogether(
            name="tenanthealthscore",
            unique_together={("tenant", "date")},
        ),
    ]
