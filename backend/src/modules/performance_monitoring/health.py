"""
PerformanceMonitoring Health Checks
...
def health_check(request):
    """
    Health check endpoint for PerformanceMonitoring module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "performance-monitoring", "checks": {}}
    ...
    try:
        cache.set("health_check_performance_monitoring", "ok", 10)
        result = cache.get("health_check_performance_monitoring")
        if result == "ok":
    ...
    try:
        ...
        count = PerformanceMonitoringResource.objects.count()
