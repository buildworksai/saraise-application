...
STATE_MACHINES: Final = ("email_marketing.campaign", "email_marketing.recipient")
OUTBOX_FRESHNESS: Final = timedelta(minutes=5)
...
class ModuleHealthReport:
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        return {
            ...
            "live": True,
            "ready": self.ready,
            "migration": "0005_register_email_marketing_contract",
...
def _run(name: str, probe: Probe, *, critical: bool) -> Check:
    ...
    return Check(name, status, code, round((time.monotonic() - started) * 1000, 3), critical, circuit_state)
...
def _outbox(tenant_id: UUID) -> tuple[bool, str]:
    stale_before = timezone.now() - OUTBOX_FRESHNESS
...
def _gateway() -> tuple[bool, str, str]:
    result = get_delivery_gateway().health()
...
def get_module_health(tenant_id: UUID) -> ModuleHealthReport:
    ...
    checks = (
        ...
        _run("delivery_gateway", _gateway, critical=True),
...
def process_readiness_probe() -> HealthCheckResult:
    ...
    checks = (
        ...
        _run("gateway", _gateway, critical=True),
    )
