...
def register_configuration_contract(apps, schema_editor):
    ...
    entry = Registry.objects.filter(name="email_marketing", version="2.0.0").first()
    if entry is None:
        raise RuntimeError("Email-marketing v2 registry entry is required before configuration governance")
    permissions = list(dict.fromkeys([*entry.permissions, *CONFIGURATION_PERMISSIONS]))
...
def unregister_configuration_contract(apps, schema_editor):
    ...
    if entry is None:
        return
    permissions = [item for item in entry.permissions if item not in CONFIGURATION_PERMISSIONS]
    manifest = json.loads(entry.manifest_content)
    manifest["permissions"] = permissions
    manifest_content = json.dumps(manifest, sort_keys=True, separators=(",", ":"))
...
def install_governance(apps, schema_editor):
    ...
    schema_editor.execute('DROP TRIGGER IF EXISTS "em_attempt_no_delete" ON "email_delivery_attempts"')
    schema_editor.execute(
...
def remove_governance(apps, schema_editor):
    ...
    if schema_editor.connection.vendor != "postgresql":
        return
    schema_editor.execute(
        'ALTER TABLE "email_delivery_attempt_revisions" ' 'DROP CONSTRAINT IF EXISTS "em_attempt_revision_tenant_fk"'
    )
    schema_editor.execute(
        'ALTER TABLE "email_marketing_configuration_versions" '
    ...
    schema_editor.execute(
        'ALTER TABLE "email_marketing_configurations" ' 'DROP CONSTRAINT IF EXISTS "em_config_tenant_id_uniq"'
    )
    schema_editor.execute('DROP TRIGGER IF EXISTS "em_attempt_no_delete" ON "email_delivery_attempts"')
    for table in reversed(IMMUTABLE_TABLES):
        trigger = f"em_immutable_{table}"[:63]
        schema_editor.execute(f'DROP TRIGGER IF EXISTS "{trigger}" ON "{table}"')
    schema_editor.execute("DROP FUNCTION IF EXISTS email_marketing_reject_tamper()")
...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        migrations.RunPython(register_configuration_contract, unregister_configuration_contract),
