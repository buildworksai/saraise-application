...
def _post_transaction(job: AsyncJob, *, tenant_id: Any) -> dict[str, Any]:
    ...
    with transaction.atomic():
        ...
        if record.status == "posted":
            ...
            return {
                ...
                "source_journal_id": str(record.source_journal_id),
                "target_journal_id": str(record.target_journal_id),
            }
    ...
    request = DualJournalRequest(
        ...
        source=JournalRequest(
            company_id=record.source_company_id,
            external_key=f"{record.id}:source",
        ...
        target=JournalRequest(
            ...
            external_key=f"{record.id}:target",
            transaction_date=record.transaction_date,
            currency=record.currency,
            lines=(
    ...
    if not (
        result.source_accepted and result.target_accepted and result.source_journal_id and result.target_journal_id
    ):
        compensations: list[str] = []
        for company_id, journal_id in (
            (record.source_company_id, result.source_journal_id),
            (record.target_company_id, result.target_journal_id),
        ):
            if journal_id:
                reversal = ledger.reverse_journal(
                    tenant_id, company_id, journal_id, "Compensate partial intercompany posting", correlation_id
                )
        ...
        with transaction.atomic():
            ...
            IntercompanyTransaction.objects.filter(
                pk=locked.pk
            ).update(  # nosemgrep: semgrep.tenant-id-required-in-queries -- reviewed false positive; scope enforced by surrounding domain policy.  # noqa: E501
                compensation_journal_ids=compensations,
                failure_code="PARTIAL_POSTING",
            ...
            _transition(
                ...
                correlation_id=correlation_id,
                transition_key=f"job:{job.id}:failed",
            )
            _event(
                tenant_id,
                locked,
                "multi_company.transaction.failed",
                job.actor_id,
                correlation_id,
                job_id=str(job.id),
                failure_code="PARTIAL_POSTING",
    ...
    if not all(
        (
            source_check.exists,
            source_check.balanced,
            source_check.posted,
            target_check.exists,
            target_check.balanced,
            target_check.posted,
        )
    ):
    ...
    with transaction.atomic():
        ...
        IntercompanyTransaction.objects.filter(pk=locked.pk).update(
            source_journal_id=result.source_journal_id,
            target_journal_id=result.target_journal_id,
        ...
        completed = _transition(
            ...
            actor_id=job.actor_id,
            correlation_id=correlation_id,
            transition_key=f"job:{job.id}:succeeded",
        )
        _event(
            tenant_id,
            completed,
            "multi_company.transaction.posted",
            job.actor_id,
            correlation_id,
            job_id=str(job.id),
            source_journal_id=str(result.source_journal_id),
            target_journal_id=str(result.target_journal_id),
        )
    return {
        "transaction_id": str(record.id),
        "source_journal_id": str(result.source_journal_id),
...
def _reverse_transaction(job: AsyncJob, *, tenant_id: Any) -> dict[str, Any]:
    ...
    ledger = integrations.require_ledger()
    correlation_id = str(job.payload.get("correlation_id") or job.correlation_id)
    source = ledger.reverse_journal(
        tenant_id, original.source_company_id, original.source_journal_id, record.description, correlation_id
    )
    target = ledger.reverse_journal(
        tenant_id, original.target_company_id, original.target_journal_id, record.description, correlation_id
    )
    ...
    IntercompanyTransaction.objects.for_tenant(tenant_id).filter(pk=record.pk).update(
        source_journal_id=source.reversal_journal_id,
        target_journal_id=target.reversal_journal_id,
        posted_date=timezone.localdate(),
        status="posted",
    )
    return {
        "transaction_id": str(record.id),
        "source_journal_id": str(source.reversal_journal_id),
        "target_journal_id": str(target.reversal_journal_id),
    }
...
def _execute_consolidation(job: AsyncJob, *, tenant_id: Any) -> dict[str, Any]:
    ...
    try:
        for company in members:
            ...
            for line in ledger.get_trial_balance(
                tenant_id, company.id, run.period_start, run.period_end, correlation_id
            ):
                balances.append(
                    {
        ...
        minority = sum(
            (
                item.amount
                * (Decimal("100") - (item.source_company.ownership_percentage or Decimal("100")))
                / Decimal("100")
        ...
        report = {
            ...
            "elimination_total": str(elimination_total),
            "minority_interest_total": str(minority),
        }
        with transaction.atomic():
            locked = ConsolidationRun.objects.for_tenant(tenant_id).select_for_update().get(pk=run.pk)
            ConsolidationRun.objects.filter(pk=locked.pk).update(
                total_eliminations=len(eliminations),
                elimination_total=elimination_total,
                minority_interest_total=minority,
            ...
            completed = _transition(
                ...
                actor_id=job.actor_id,
                correlation_id=correlation_id,
                transition_key=f"job:{job.id}:complete",
            )
            _event(
                tenant_id,
                completed,
                "multi_company.consolidation.completed",
                job.actor_id,
                correlation_id,
                job_id=str(job.id),
            )
        ...
        with transaction.atomic():
            locked = ConsolidationRun.objects.for_tenant(tenant_id).select_for_update().get(pk=run.pk)
            ConsolidationRun.objects.filter(pk=locked.pk).update(
                failure_code=getattr(exc, "code", "CONSOLIDATION_FAILED"),
                failure_step="execution",
                failure_detail="Consolidation dependency or reconciliation failed.",
            )
            failed = _transition(
                consolidation_state_machine,
                locked,
                ...
                actor_id=job.actor_id,
                correlation_id=correlation_id,
                transition_key=f"job:{job.id}:fail",
            )
            _event(
                tenant_id,
                failed,
                "multi_company.consolidation.failed",
                job.actor_id,
                correlation_id,
                job_id=str(job.id),
                failure_code=getattr(exc, "code", "CONSOLIDATION_FAILED"),
...
def _expire_drafts(job: AsyncJob, *, tenant_id: Any) -> dict[str, int]:
    ...
    candidates = list(
        IntercompanyTransaction.objects.for_tenant(tenant_id)
        .filter(status="draft", is_deleted=False, created_at__lt=cutoff)
    ...
    for transaction_id in candidates:
        with transaction.atomic():
            ...
            _transition(
                ...
                transition_key=f"job:{job.id}:expire:{record.id}",
            )
            expired += 1
