...
from .permissions import PERMISSIONS, MultiCompanyAccessMixin
...
def _translate_service_exception(exc: Exception) -> Exception:
    ...
    if isinstance(exc, ConfigurationUnavailable):
        return OperationFailed(
            error_code=exc.code, message="Required multi-company configuration is unavailable.", http_status=503
        )
    if isinstance(exc, ConflictError):
        return OperationFailed(
            error_code=exc.code, message="The operation conflicts with current resource state.", http_status=409
        )
    if isinstance(exc, IntegrationError):
        return OperationFailed(
            error_code=exc.code, message="A required financial capability is unavailable.", http_status=503
        )
    ...
    if isinstance(exc, DomainError):
        return OperationFailed(
            error_code=exc.code, message="The requested operation could not be completed.", http_status=422
        )
    return exc
...
class CompanyV2ViewSet(TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "multi_company.company:read",
        "retrieve": "multi_company.company:read",
        "create": "multi_company.company:create",
        "partial_update": "multi_company.company:update",
        "destroy": "multi_company.company:delete",
        "deactivate": "multi_company.company:deactivate",
        "reactivate": "multi_company.company:update",
        "hierarchy": "multi_company.company:read",
        "subsidiaries": "multi_company.company:read",
        "consolidation_group": "multi_company.company:read",
    }
    action_quotas = {
        key: "multi_company.api_reads"
        for key in ("list", "retrieve", "hierarchy", "subsidiaries", "consolidation_group")
    }
    ...
    def list(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        _validate_query_keys(
            params,
            {"search", "company_code", "is_active", "parent_company_id", "consolidation_group", "currency", "ordering"},
        )
        ...
        filters["ordering"] = _ordering(
            params.get("ordering"),
            {"company_code", "company_name", "currency", "created_at", "updated_at"},
            "company_code",
        )
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        return Response(
            CompanyDetailSerializer(company, context={"request": self.request}).data, status=status.HTTP_201_CREATED
        )
    ...
    def deactivate(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        serializer = ExpectedVersionSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
    ...
    def subsidiaries(self, request: object, pk: str | None = None) -> Response:
        ...
        return (
            Response(CompanyListSerializer(rows, many=True, context={"request": self.request}).data)
            if recursive
            else self.paginated(rows, CompanyListSerializer)
        )
    ...
    def consolidation_group(self, request: object, group: str | None = None) -> Response:
        ...
        return self.paginated(
            self.service_class().get_consolidation_group(self.tenant_id(), group), CompanyListSerializer
        )
...
class CompanyAccessViewSet(TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "multi_company.company_access:read",
        "retrieve": "multi_company.company_access:read",
        "create": "multi_company.company_access:grant",
        "revoke": "multi_company.company_access:revoke",
    }
    ...
    def list(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        if params.get("company_id"):
            filters["company_id"] = _uuid(params["company_id"], "company_id")
        if params.get("active_at"):
            filters["active_at"] = _datetime(params["active_at"], "active_at")
        ...
        if filters.get("active_at"):
            rows = rows.filter(valid_from__lte=filters["active_at"]).filter(
                Q(valid_until__isnull=True) | Q(valid_until__gt=filters["active_at"])
            )
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = CompanyAccessGrantCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        value = self.service_class().grant_access(
            self.tenant_id(), self.actor_id(), self.correlation_id(), dict(serializer.validated_data)
        )
    ...
    def revoke(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        serializer = CompanyAccessRevokeSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        value = self.service_class().revoke_access(
            self.tenant_id(),
            self.kwargs["pk"],
            self.actor_id(),
            self.correlation_id(),
...
class TransactionViewSet(TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "multi_company.transaction:read",
        "retrieve": "multi_company.transaction:read",
        "create": "multi_company.transaction:create",
        "partial_update": "multi_company.transaction:update",
    ...
    def list(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        filters: dict[str, Any] = {
            key: params[key] for key in ("search", "transaction_type", "status", "currency") if params.get(key)
        }
        if "transaction_type" in filters:
            filters["transaction_type"] = _choice(
                filters["transaction_type"], "transaction_type", set(IntercompanyTransaction.TransactionType.values)
            )
        ...
        for field in ("source_company_id", "target_company_id"):
            if params.get(field):
                filters[field] = _uuid(params[field], field)
        for field in ("date_from", "date_to"):
            if params.get(field):
                filters[field] = _date(params[field], field)
        filters["ordering"] = _ordering(
            params.get("ordering"),
            {"reference", "transaction_date", "amount", "status", "created_at"},
            "-transaction_date",
        )
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        payload = dict(serializer.validated_data)
        idempotency_key = payload.pop("idempotency_key")
        value = self.service_class().create_transaction(
            self.tenant_id(), self.actor_id(), self.correlation_id(), payload, idempotency_key
        )
    ...
    def partial_update(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = TransactionUpdateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        changes = dict(serializer.validated_data)
        expected = changes.pop("expected_version")
        value = self.service_class().update_draft(
            self.tenant_id(), self.kwargs["pk"], self.actor_id(), self.correlation_id(), expected, changes
        )
        return Response(TransactionDetailSerializer(value, context={"request": self.request}).data)
    ...
    def _transition(self, serializer_type: type, method: str) -> Response:
        ...
        transition_key = data.pop("transition_key")
    ...
    @action(detail=True, methods=["post"])
    def submit(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self._transition(TransactionSubmitSerializer, "submit")

    @action(detail=True, methods=["post"])
    def approve(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self._transition(ApprovalDecisionSerializer, "record_approval")

    @action(detail=True, methods=["post"])
    def dispute(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self._transition(DisputeSerializer, "dispute")

    @action(detail=True, methods=["post"], url_path="resolve-dispute")
    def resolve_dispute(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self._transition(ResolveDisputeSerializer, "resolve_dispute")

    @action(detail=True, methods=["post"])
    def cancel(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self._transition(CancelSerializer, "cancel")
    ...
    def apply_transfer_pricing(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        serializer = ApplyTransferPricingSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        value = self.service_class().apply_transfer_pricing(
            self.tenant_id(),
            self.kwargs["pk"],
            self.actor_id(),
            self.correlation_id(),
            serializer.validated_data.get("rule_id"),
        )
        return Response(TransactionDetailSerializer(value, context={"request": self.request}).data)

    def _enqueue(self, retry: bool) -> Response:
        serializer = TransactionSubmitSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
    ...
    def retry_posting(self, request: object, pk: str | None = None) -> Response:
        del request, pk
    ...
    def reverse(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        serializer = ReverseSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
...
class ReconciliationViewSet(mixins.ListModelMixin, TenantGovernedViewSet):
    ...
    def list(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        for field in ("source_company_id", "target_company_id"):
            if field in filters:
                filters[field] = _uuid(filters[field], field)
        if "period_start" in filters:
            filters["date_from"] = _date(filters.pop("period_start"), "period_start")
        if "period_end" in filters:
            filters["date_to"] = _date(filters.pop("period_end"), "period_end")
        filters["ordering"] = _ordering(
            filters.get("ordering"), {"reference", "transaction_date", "variance", "amount"}, "-transaction_date"
        )
...
class ConsolidationRunViewSet(TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "multi_company.consolidation:read",
        "retrieve": "multi_company.consolidation:read",
        "create": "multi_company.consolidation:create",
        "partial_update": "multi_company.consolidation:update",
        ...
        "eliminations": "multi_company.elimination:read",
    ...
    def list(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        params = self.request.query_params
        _validate_query_keys(
            params, {"consolidation_group", "status", "period_start", "period_end", "reporting_currency", "ordering"}
        )
        filters = {
            key: params[key] for key in ("consolidation_group", "status", "reporting_currency") if params.get(key)
        }
        if "status" in filters:
            filters["status"] = _choice(filters["status"], "status", set(ConsolidationRun.Status.values))
        if "reporting_currency" in filters:
            filters["reporting_currency"] = _choice(
                str(filters["reporting_currency"]).upper(), "reporting_currency", set(ALLOWED_CURRENCIES)
            )
        for field in ("period_start", "period_end"):
            if params.get(field):
                filters[field] = _date(params[field], field)
        filters["ordering"] = _ordering(
            params.get("ordering"), {"created_at", "period_start", "period_end", "status", "name"}, "-created_at"
        )
        rows = self.service_class().list_runs(self.tenant_id(), filters)
        if filters.get("period_start"):
            rows = rows.filter(period_end__gte=filters["period_start"])
        if filters.get("period_end"):
            rows = rows.filter(period_start__lte=filters["period_end"])
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = ConsolidationRunCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        payload = dict(serializer.validated_data)
    ...
    def partial_update(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = ConsolidationRunUpdateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        changes = dict(serializer.validated_data)
        expected = changes.pop("expected_version")
        value = self.service_class().update_draft(
    ...
    def execute(self, request: object, pk: str | None = None) -> Response:
        ...
        serializer = TransactionSubmitSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        job = self.service_class().execute(
            self.tenant_id(),
            self.kwargs["pk"],
            self.actor_id(),
            self.correlation_id(),
    ...
    def approve(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self._command("approve")

    @action(detail=True, methods=["post"])
    def publish(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self._command("publish")

    @action(detail=True, methods=["post"])
    def cancel(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        return self._command("cancel", CancelSerializer)
    ...
    def eliminations(self, request: object, pk: str | None = None) -> Response:
        ...
        if self.request.method == "GET":
            ...
            params = self.request.query_params
            _validate_query_keys(
                params, {"elimination_type", "source_company_id", "target_company_id", "is_auto_generated"}
            )
            filters = dict(params.items())
            for field in ("source_company_id", "target_company_id"):
                if field in filters:
                    filters[field] = _uuid(filters[field], field)
            if "is_auto_generated" in filters:
                filters["is_auto_generated"] = _boolean(filters["is_auto_generated"], "is_auto_generated")
        ...
        serializer = ManualEliminationCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        value = self.service_class().create_manual_elimination(
            self.tenant_id(), self.kwargs["pk"], self.actor_id(), self.correlation_id(), dict(serializer.validated_data)
        )
...
class EliminationViewSet(mixins.RetrieveModelMixin, TenantGovernedViewSet):
    ...
    def get_queryset(self) -> QuerySet[EliminationEntry]:
        return EliminationEntry.objects.for_tenant(self.tenant_id())
...
class TransferPricingRuleViewSet(TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "multi_company.transfer_pricing:read",
        "retrieve": "multi_company.transfer_pricing:read",
        "create": "multi_company.transfer_pricing:create",
        "partial_update": "multi_company.transfer_pricing:update",
        "destroy": "multi_company.transfer_pricing:delete",
    }

    def get_queryset(self) -> QuerySet[TransferPricingRule]:
        return TransferPricingRule.objects.for_tenant(self.tenant_id()).filter(is_deleted=False)
    ...
    def list(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        filters = {
            key: params[key] for key in ("product_category", "transaction_type", "pricing_method") if params.get(key)
        }
        if "transaction_type" in filters:
            filters["transaction_type"] = _choice(
                filters["transaction_type"], "transaction_type", set(IntercompanyTransaction.TransactionType.values)
        ...
        if "pricing_method" in filters:
            filters["pricing_method"] = _choice(
                filters["pricing_method"], "pricing_method", set(TransferPricingRule.PricingMethod.values)
            )
        for field in ("source_company_id", "target_company_id"):
            if params.get(field):
                filters[field] = _uuid(params[field], field)
        if params.get("active_date"):
            filters["active_date"] = _date(params["active_date"], "active_date")
        filters["ordering"] = _ordering(
            params.get("ordering"), {"name", "effective_from", "rule_version", "created_at"}, "-effective_from"
        )
    ...
    def create(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = TransferPricingRuleCreateSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        payload = dict(serializer.validated_data)
    ...
    def partial_update(self, request: object, *args: object, **kwargs: object) -> Response:
        del request, args, kwargs
        serializer = TransferPricingRuleVersionSerializer(data=self.request.data)
        serializer.is_valid(raise_exception=True)
        changes = dict(serializer.validated_data)
        expected = changes.pop("expected_version")
        value = self.service_class().create_rule_version(
            self.tenant_id(), self.kwargs["pk"], self.actor_id(), self.correlation_id(), expected, changes
        )
        return Response(TransferPricingRuleDetailSerializer(value, context={"request": self.request}).data)
...
class TransferPriceCalculateAPIView(GovernedServiceAPIView):
    ...
    def post(self, request: Any) -> Response:
        serializer = TransferPriceCalculateSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        payload = dict(serializer.validated_data)
...
class ConfigurationVersionViewSet(TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "multi_company.configuration:read",
        "retrieve": "multi_company.configuration:read",
        "create": "multi_company.configuration:write",
        "partial_update": "multi_company.configuration:write",
        "validate_draft": "multi_company.configuration:write",
    ...
    def get_queryset(self) -> QuerySet[MultiCompanyConfigurationVersion]:
        return MultiCompanyConfigurationVersion.objects.for_tenant(self.tenant_id())
    ...
    def list(self, request: object, *args: object, **kwargs: object) -> Response:
        ...
        if not environment:
            raise ValidationError({"environment": "This filter is required."})
        environment = _choice(environment, "environment", set(MultiCompanyConfigurationVersion.Environment.values))
        return self.paginated(
            self.service_class().list_versions(self.tenant_id(), environment), ConfigurationVersionSerializer
        )
    ...
    def activate(self, request: object, pk: str | None = None) -> Response:
        del request, pk
        value = self.service_class().activate(
...
class ConfigurationExportAPIView(GovernedServiceAPIView):
    ...
    def get(self, request: Any) -> Response:
        ...
        if not environment:
            raise ValidationError({"environment": "This field is required."})
        environment = _choice(environment, "environment", set(MultiCompanyConfigurationVersion.Environment.values))
        ...
        if version is not None:
            try:
                ...
                raise ValidationError({"version": "Use an integer."}) from exc
...
class AsyncJobViewSet(mixins.RetrieveModelMixin, TenantGovernedViewSet):
    ...
    def get_permissions(self) -> list[object]:
        ...
        try:
            ...
            command = (
                ...
                .filter(pk=job_id, command__startswith="multi_company.")
    ...
    def get_queryset(self) -> QuerySet[AsyncJob]:
        return AsyncJob.objects.for_tenant(self.tenant_id()).filter(command__startswith="multi_company.")
...
class CompanyViewSet(DeprecatedV1HeadersMixin, viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer: Any) -> None:
        value = CompanyRegistryService().update_company(
            self._tenant_id(),
            serializer.instance.pk,
            str(self.request.user.id),
            str(getattr(self.request, "correlation_id", "legacy-v1")),
    ...
    def perform_destroy(self, instance: Company) -> None:
        CompanyRegistryService().delete_company(
            self._tenant_id(),
            instance.pk,
            str(self.request.user.id),
            str(getattr(self.request, "correlation_id", "legacy-v1")),
