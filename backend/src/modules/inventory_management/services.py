...
def _transition(
    instance: Any, command: str, actor_id: UUID | str, transition_key: str, graph: Mapping[str, Mapping[str, str]]
) -> Any:
...
RESERVATION_GRAPH = {
    "active": {
        name: name + ("d" if name.endswith("e") else "ed") for name in ("release", "consume", "expire", "cancel")
    }
}
...
def _apply_command(
    instance: Any, command: Mapping[str, Any], allowed: Iterable[str], *, exclude: Iterable[str] = ()
) -> None:
...
class WarehouseService:
    ...
    @classmethod
    def create(
        cls, tenant_id: UUID | str, actor_id: UUID | str, command: Mapping[str, Any], idempotency_key: str
    ) -> Warehouse:
    ...
    def create_warehouse(
        cls, tenant_id: UUID | str, warehouse_code: str, warehouse_name: str, **kwargs: Any
    ) -> Warehouse:
    ...
    @classmethod
    def archive(
        cls, tenant_id: UUID | str, warehouse_id: UUID | str, expected_version: int, actor_id: UUID | str
    ) -> Warehouse:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            warehouse = Warehouse.objects.for_tenant(tenant).select_for_update().get(pk=warehouse_id)
            ...
            if (
                StockEntry.objects.for_tenant(tenant)
                .filter(
    ...
    @classmethod
    def set_default(
        cls, tenant_id: UUID | str, warehouse_id: UUID | str, actor_id: UUID | str, transition_key: str
    ) -> Warehouse:
        tenant = _uuid(tenant_id, "tenant_id")
        ...
        with tenant_context(tenant), transaction.atomic():
            ...
            Warehouse.objects.for_tenant(tenant).exclude(pk=warehouse.pk).filter(is_default=True).update(
                is_default=False, version=F("version") + 1
            )
...
class StorageLocationService:
    ...
    @classmethod
    def archive(
        cls, tenant_id: UUID | str, location_id: UUID | str, expected_version: int, actor_id: UUID | str
    ) -> StorageLocation:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            location = StorageLocation.objects.for_tenant(tenant).select_for_update().get(pk=location_id)
    ...
    def ensure_default_location(cls, tenant_id: UUID | str, warehouse_id: UUID | str) -> StorageLocation:
        ...
        with tenant_context(tenant), transaction.atomic():
            warehouse = _tenant_object(Warehouse, tenant, warehouse_id, "warehouse_id")
            existing = (
                StorageLocation.objects.for_tenant(tenant)
                .filter(warehouse=warehouse, is_default=True, archived_at__isnull=True)
                .first()
            )
            if existing:
                return existing
...
class ItemService:
    ...
    def update(
        ...
        with tenant_context(tenant), transaction.atomic():
            ...
            if StockLedgerEntry.objects.for_tenant(tenant).filter(item=item).exists() and (
                {"tracking_mode", "valuation_method"} & set(command)
            ):
                if any(
                    command.get(name, getattr(item, name)) != getattr(item, name)
                    for name in ("tracking_mode", "valuation_method")
                ):
...
class BatchService:
    ...
    def register(
        ...
        with tenant_context(tenant), transaction.atomic():
            ...
            _apply_command(
                batch,
                command,
                {"item_id", "batch_number", "supplier_batch_number", "manufactured_on", "expires_on"},
                exclude={"item_id"},
            )
            _validate(batch)
            batch.save()
    ...
    @classmethod
    def transition(
        cls, tenant_id: UUID | str, batch_id: UUID | str, actor_id: UUID | str, command: str, transition_key: str
    ) -> Batch:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            batch = Batch.objects.for_tenant(tenant).select_for_update().get(pk=batch_id)
    ...
    activate = classmethod(
        lambda cls, tenant_id, batch_id, actor_id, transition_key: cls.transition(
            tenant_id, batch_id, actor_id, "activate", transition_key
        )
    )
    quarantine = classmethod(
        lambda cls, tenant_id, batch_id, actor_id, transition_key: cls.transition(
            tenant_id, batch_id, actor_id, "quarantine", transition_key
        )
    )
    release = classmethod(
        lambda cls, tenant_id, batch_id, actor_id, transition_key: cls.transition(
            tenant_id, batch_id, actor_id, "release", transition_key
        )
    )
    recall = classmethod(
        lambda cls, tenant_id, batch_id, actor_id, transition_key: cls.transition(
            tenant_id, batch_id, actor_id, "recall", transition_key
        )
    )
    exhaust = classmethod(
        lambda cls, tenant_id, batch_id, actor_id, transition_key: cls.transition(
            tenant_id, batch_id, actor_id, "exhaust", transition_key
        )
    )
    expire = classmethod(
        lambda cls, tenant_id, batch_id, actor_id, transition_key: cls.transition(
            tenant_id, batch_id, actor_id, "expire", transition_key
        )
    )
...
class SerialNumberService:
    ...
    def register(
        ...
        with tenant_context(tenant), transaction.atomic():
            ...
            _apply_command(
                serial,
                command,
                {"item_id", "serial_number", "manufacturer", "model_number", "warranty_starts_on", "warranty_ends_on"},
                exclude={"item_id"},
            )
            _validate(serial)
            serial.save()
    ...
    @classmethod
    def scrap(
        cls, tenant_id: UUID | str, serial_id: UUID | str, actor_id: UUID | str, transition_key: str
    ) -> SerialNumber:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            serial = SerialNumber.objects.for_tenant(tenant).select_for_update().get(pk=serial_id)
...
class StockEntryService:
    ...
    def _replace_lines(cls, tenant: UUID, entry: StockEntry, lines: list[Mapping[str, Any]]) -> None:
        ...
        for index, line_command in enumerate(lines, 1):
            ...
            line = StockEntryLine(
                tenant_id=tenant,
                stock_entry=entry,
                line_number=line_command.get("line_number", index),
                item=item,
                source_location_id=line_command.get("source_location_id"),
                destination_location_id=line_command.get("destination_location_id"),
                batch_id=line_command.get("batch_id"),
                serial_number_id=line_command.get("serial_number_id"),
                quantity=line_command.get("quantity"),
                uom=line_command.get("uom", item.base_uom),
                unit_cost=line_command.get("unit_cost"),
                notes=line_command.get("notes", ""),
            )
            for relation_name, relation_model in (
    ...
    @classmethod
    def delete_draft(
        cls, tenant_id: UUID | str, entry_id: UUID | str, expected_version: int, actor_id: UUID | str
    ) -> StockEntry:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            entry = StockEntry.objects.for_tenant(tenant).select_for_update().get(pk=entry_id)
    ...
    @classmethod
    def submit(
        cls, tenant_id: UUID | str, entry_id: UUID | str, actor_id: UUID | str, transition_key: str
    ) -> StockEntry:
    ...
    @classmethod
    def _command(
        cls, tenant_id: UUID | str, entry_id: UUID | str, actor_id: UUID | str, command: str, transition_key: str
    ) -> StockEntry:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            entry = StockEntry.objects.for_tenant(tenant).select_for_update().get(pk=entry_id)
    ...
    @classmethod
    def reverse(
        cls, tenant_id: UUID | str, entry_id: UUID | str, actor_id: UUID | str, reason: str, idempotency_key: str
    ) -> StockEntry:
        tenant = _uuid(tenant_id, "tenant_id")
        key = _required_text(idempotency_key, "idempotency_key", 255)
        with tenant_context(tenant), transaction.atomic():
            ...
            command = {
                "entry_number": f"REV-{original.entry_number}"[:50],
...
class InventoryPostingService:
    ...
    def _consume_reservations(
        ...
        remaining = quantity
        reservations = (
            StockReservation.objects.for_tenant(tenant)
            .select_for_update()
            .filter(
                status="active",
                item=line.item,
    ...
    @classmethod
    def _balance(
        cls, tenant: UUID, line: StockEntryLine, warehouse: Warehouse, location: StorageLocation
    ) -> StockBalance:
        balance = (
            StockBalance.objects.for_tenant(tenant)
            .select_for_update()
            .filter(
                item=line.item,
                warehouse=warehouse,
        ...
        return StockBalance.objects.create(
            ...
            batch=line.batch,
            serial_number=line.serial_number,
            quantity_on_hand=QTY_ZERO,
            quantity_allocated=QTY_ZERO,
            quantity_available=QTY_ZERO,
            stock_value=MONEY_ZERO,
            valuation_rate=MONEY_ZERO,
        )

    @classmethod
    def _consume_layers(
        cls, tenant: UUID, line: StockEntryLine, warehouse: Warehouse, location: StorageLocation, quantity: Decimal
    ) -> tuple[Decimal, Decimal]:
        order = "-acquired_at" if line.item.valuation_method == "lifo" else "acquired_at"
        layers = (
            StockCostLayer.objects.for_tenant(tenant)
            .select_for_update()
            .filter(
                item=line.item,
                warehouse=warehouse,
        ...
        return (value / quantity).quantize(MONEY_QUANTUM, rounding=ROUND_HALF_UP), value.quantize(
    ...
    def _movement(
        ...
        if delta > 0:
            unit_cost = (
                ...
                else (line.item.standard_cost or balance.valuation_rate or MONEY_ZERO)
            )
            ...
            unit_cost = (
                line.item.standard_cost if line.item.valuation_method == "standard_cost" else balance.valuation_rate
            )
            value_delta = (delta * unit_cost).quantize(MONEY_QUANTUM, rounding=ROUND_HALF_UP)
        ...
        if delta > 0 and line.item.valuation_method in ("fifo", "lifo"):
            StockCostLayer.objects.create(
                ...
                location=location,
                batch=line.batch,
                originating_ledger_entry=ledger,
                received_quantity=delta,
                remaining_quantity=delta,
                unit_cost=unit_cost,
                acquired_at=entry.posting_at,
            )
        return ledger, sequence + 1

    @classmethod
    def post(
        cls, tenant_id: UUID | str, entry_id: UUID | str, actor_id: UUID | str, idempotency_key: str
    ) -> StockEntry:
        tenant = _uuid(tenant_id, "tenant_id")
        key = _required_text(idempotency_key, "idempotency_key", 255)
        with tenant_context(tenant), transaction.atomic():
            ...
            lines = list(
                entry.lines.select_for_update()
                .select_related("item", "batch", "serial_number")
            ...
            sequence = (
                StockLedgerEntry.objects.for_tenant(tenant).select_for_update().aggregate(last=Max("sequence"))["last"]
                or 0
            ) + 1
            for line in lines:
                ...
                if entry.entry_type in ("issue", "scrap") or (
                    entry.entry_type in ("adjustment", "manufacturing") and line.source_location_id
                ):
                    movements.append((entry.source_warehouse, line.source_location, -line.quantity))
                elif entry.entry_type in ("receipt", "return") or (
                    entry.entry_type in ("adjustment", "manufacturing") and line.destination_location_id
                ):
                    movements.append((entry.destination_warehouse, line.destination_location, line.quantity))
                elif entry.entry_type == "transfer":
                    movements.extend(
                        (
                            (entry.source_warehouse, line.source_location, -line.quantity),
                            (entry.destination_warehouse, line.destination_location, line.quantity),
                        )
                    )
                else:
            ...
            _emit(
                ...
                {"entry_type": entry.entry_type, "line_count": len(lines)},
            )
...
class ReservationService:
    ...
    def reserve(
        ...
        with tenant_context(tenant), transaction.atomic():
            ...
            reservation = StockReservation(
                tenant_id=tenant,
                reservation_number=command.get("reservation_number"),
                reference_module=command.get("reference_module"),
                reference_type=command.get("reference_type"),
                reference_id=command.get("reference_id"),
                item=item,
                warehouse=warehouse,
                location_id=dimensions["location"],
                batch_id=dimensions["batch"],
                serial_number_id=dimensions["serial_number"],
                quantity=quantity,
            ...
            if reservation.serial_number_id:
                serial = (
                    SerialNumber.objects.for_tenant(tenant).select_for_update().get(pk=reservation.serial_number_id)
                )
    ...
    @classmethod
    def _finish(
        cls, tenant_id: UUID | str, reservation_id: UUID | str, actor_id: UUID | str, command: str, transition_key: str
    ) -> StockReservation:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            ...
            if reservation.status != "active":
                return (
                    reservation
                    if any(x.get("transition_key") == transition_key for x in reservation.transition_history)
                    else _transition(reservation, command, actor_id, transition_key, RESERVATION_GRAPH)
                )
            balance = (
                StockBalance.objects.for_tenant(tenant)
                .select_for_update()
    ...
    release = classmethod(
        lambda cls, tenant_id, reservation_id, actor_id, transition_key: cls._finish(
            tenant_id, reservation_id, actor_id, "release", transition_key
        )
    )
    consume = classmethod(
        lambda cls, tenant_id, reservation_id, actor_id, transition_key: cls._finish(
            tenant_id, reservation_id, actor_id, "consume", transition_key
        )
    )
    cancel = classmethod(
        lambda cls, tenant_id, reservation_id, actor_id, transition_key: cls._finish(
            tenant_id, reservation_id, actor_id, "cancel", transition_key
        )
    )
    ...
    def expire_due(cls, tenant_id: UUID | str, actor_id: UUID | str, *, queue_threshold: int = 100) -> int | Any:
        ...
        with tenant_context(tenant):
            due = (
                StockReservation.objects.for_tenant(tenant)
                .filter(status="active", expires_at__lte=timezone.now())
            ...
            if count > queue_threshold:
                return enqueue(
                    ...
                    f"expire:{timezone.now().date().isoformat()}",
                )
...
class CycleCountService:
    ...
    def create(
        ...
        with tenant_context(tenant), transaction.atomic():
            ...
            for index, line_data in enumerate(command.get("lines", []), 1):
                ...
                location = _tenant_object(
                    StorageLocation, tenant, line_data.get("location_id"), f"lines.{index}.location_id"
                )
                line = CycleCountLine(
                    tenant_id=tenant,
                    cycle_count=count,
                    line_number=line_data.get("line_number", index),
                    item=item,
                    location=location,
                    batch_id=line_data.get("batch_id"),
                    serial_number_id=line_data.get("serial_number_id"),
                    system_quantity=QTY_ZERO,
                )
                _validate(line)
                line.save()
            return count
    ...
    @classmethod
    def start(
        cls, tenant_id: UUID | str, count_id: UUID | str, actor_id: UUID | str, transition_key: str
    ) -> CycleCount:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            count = CycleCount.objects.for_tenant(tenant).select_for_update().get(pk=count_id)
            for line in count.lines.select_for_update().all():
                total = (
                    ...
                    .aggregate(value=Sum("quantity_on_hand"))["value"]
                    or QTY_ZERO
                )
    ...
    @classmethod
    def record_counts(
        cls, tenant_id: UUID | str, count_id: UUID | str, actor_id: UUID | str, lines: Iterable[Mapping[str, Any]]
    ) -> CycleCount:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            count = CycleCount.objects.for_tenant(tenant).select_for_update().get(pk=count_id)
    ...
    @classmethod
    def _command(
        cls, tenant_id: UUID | str, count_id: UUID | str, actor_id: UUID | str, command: str, transition_key: str
    ) -> CycleCount:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant), transaction.atomic():
            count = CycleCount.objects.for_tenant(tenant).select_for_update().get(pk=count_id)
            ...
            setattr(
                result, {"submit": "submitted_at", "approve": "approved_at"}.get(command, "updated_at"), timezone.now()
            )
            result.save()
            return result

    submit = classmethod(
        lambda cls, tenant_id, count_id, actor_id, transition_key: cls._command(
            tenant_id, count_id, actor_id, "submit", transition_key
        )
    )
    approve = classmethod(
        lambda cls, tenant_id, count_id, actor_id, transition_key: cls._command(
            tenant_id, count_id, actor_id, "approve", transition_key
        )
    )
    reject = classmethod(
        lambda cls, tenant_id, count_id, actor_id, transition_key: cls._command(
            tenant_id, count_id, actor_id, "reject", transition_key
        )
    )
    cancel = classmethod(
        lambda cls, tenant_id, count_id, actor_id, transition_key: cls._command(
            tenant_id, count_id, actor_id, "cancel", transition_key
        )
    )
        ...
        with tenant_context(tenant), transaction.atomic():
            ...
            if lines:
                entry = StockEntryService.create_draft(
                    ...
                    actor_id,
                    {
                        "entry_number": f"CC-{count.count_number}"[:50],
                        "entry_type": "adjustment",
                        "posting_at": timezone.now(),
                        "source_warehouse_id": count.warehouse_id,
                        "destination_warehouse_id": count.warehouse_id,
                        "reference_module": "inventory_management",
                        "reference_type": "cycle_count",
                        "reference_id": count.id,
                        "reason": "Approved cycle count variance",
                        "lines": lines,
                    },
                    transition_key,
                )
...
class InventoryQueryService:
    ...
    def stock_summary(cls, tenant_id: UUID | str) -> dict[str, Any]:
        tenant = _uuid(tenant_id, "tenant_id")
        with tenant_context(tenant):
            totals = StockBalance.objects.for_tenant(tenant).aggregate(
...
class InventoryConfigurationService:
    ...
    def _validate_snapshot(cls, snapshot: Mapping[str, Any]) -> dict[str, Any]:
        ...
        for field, low, high in (
            ("max_lines_per_entry", 1, 5000),
            ("reservation_ttl_minutes", 5, 10080),
    ...
    def get_effective(cls, tenant_id: UUID | str, environment: str = "development") -> InventoryConfiguration:
        ...
        with tenant_context(tenant), transaction.atomic():
            ...
            config = InventoryConfiguration.objects.create(
                tenant_id=tenant, environment=environment, status="active", **defaults
            )
            return config
    ...
    def preview(cls, tenant_id: UUID | str, environment: str, document: Mapping[str, Any]) -> dict[str, Any]:
        ...
        with tenant_context(tenant):
            ...
            changes = [
                {"field": field, "before": getattr(current, field), "after": proposed[field]}
                for field in CONFIG_FIELDS
                if getattr(current, field) != proposed[field]
            ]
            effects = sorted(
                {
                    (
                        ...
                        in (
                            ...
                            "reservations"
                            if c["field"] == "reservation_ttl_minutes"
                            else (
                                "batch_monitoring"
                                if c["field"] in ("expiry_warning_days", "auto_expire_batches")
                                else "capability_rollout"
                            )
                        )
                    )
                    for c in changes
    ...
    def create_revision(
        ...
        with tenant_context(tenant), transaction.atomic():
            config = (
                InventoryConfiguration.objects.for_tenant(tenant)
                .select_for_update()
            ...
            if config is None:
                config = InventoryConfiguration.objects.create(
                    tenant_id=tenant, environment=environment, status="draft", **cls._defaults()
                )
            revision_number = (config.revisions.aggregate(last=Max("revision"))["last"] or 0) + 1
            revision = InventoryConfigurationRevision.objects.create(
                tenant_id=tenant,
                configuration=config,
                revision=revision_number,
                snapshot=snapshot,
                change_reason=_required_text(change_reason, "change_reason", 5000),
                changed_by_id=_uuid(actor_id, "actor_id"),
    ...
    @classmethod
    def activate(
        cls, tenant_id: UUID | str, environment: str, revision: int, actor_id: UUID | str, transition_key: str
    ) -> InventoryConfiguration:
        tenant = _uuid(tenant_id, "tenant_id")
        _required_text(transition_key, "transition_key", 255)
        with tenant_context(tenant), transaction.atomic():
...
class InventoryBulkService:
    @classmethod
    def enqueue_import(
        cls, tenant_id: UUID | str, actor_id: UUID | str, resource_type: str, document_ref: str, idempotency_key: str
    ):
        tenant = _uuid(tenant_id, "tenant_id")
        ...
        with tenant_context(tenant), transaction.atomic():
            return enqueue(
                ...
                actor_id,
                "inventory.import",
                {"resource_type": resource, "document_ref": reference},
                idempotency_key,
            )
