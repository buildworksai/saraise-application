"""
BillingSubscriptions Health Checks
...
def health_check(request):
    """
    Health check endpoint for BillingSubscriptions module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "billing-subscriptions", "checks": {}}
    ...
    try:
        cache.set("health_check_billing_subscriptions", "ok", 10)
        result = cache.get("health_check_billing_subscriptions")
        if result == "ok":
