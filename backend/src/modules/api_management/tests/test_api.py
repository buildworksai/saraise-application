"""
API Integration Tests for ApiManagement module.
...
class TestTenantBaseModelViewSet:
    ...
    def test_list_resources(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.get("/api/v1/api-management/resources/")
    ...
    def test_create_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.post("/api/v1/api-management/resources/", data, format="json")
    ...
    def test_get_resource_detail(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.get(f"/api/v1/api-management/resources/{resource.id}/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.put(f"/api/v1/api-management/resources/{resource.id}/", data, format="json")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_delete_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.delete(f"/api/v1/api-management/resources/{resource.id}/")
        assert response.status_code == status.HTTP_204_NO_CONTENT

        # Verify resource is deleted
        assert not TenantBaseModel.objects.filter(id=resource.id).exists()

    def test_activate_resource(self, authenticated_client, tenant_user):
        """Test activating a resource through its tenant-scoped action."""
        tenant_id = get_user_tenant_id(tenant_user)
        resource = TenantBaseModel.objects.create(
            tenant_id=tenant_id,
        ...
        response = authenticated_client.post(f"/api/v1/api-management/resources/{resource.id}/activate/")

        assert response.status_code == status.HTTP_200_OK
        resource.refresh_from_db()
        assert resource.is_active is True

    def test_deactivate_resource(self, authenticated_client, tenant_user):
        """Test deactivating a resource through its tenant-scoped action."""
        tenant_id = get_user_tenant_id(tenant_user)
        resource = TenantBaseModel.objects.create(
            tenant_id=tenant_id,
        ...
        response = authenticated_client.post(f"/api/v1/api-management/resources/{resource.id}/deactivate/")
