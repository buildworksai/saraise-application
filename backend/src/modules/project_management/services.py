...
def _snapshot(instance: Any) -> dict[str, Any]:
    ...
    for field in instance._meta.concrete_fields:
        ...
        if isinstance(value, (uuid.UUID, Decimal, date)) or hasattr(value, "isoformat"):
            value = str(value)
...
def _fingerprint(action: str, data: Any) -> str:
    def canonical(value: Any) -> Any:
        ...
        if isinstance(value, (uuid.UUID, Decimal, date)):
            return str(value)
        return value

    return hashlib.sha256(
        json.dumps({"action": action, "data": canonical(data)}, sort_keys=True, separators=(",", ":")).encode()
    ).hexdigest()
...
class ActivityService:
    ...
    def record(
        ...
        *,
        ...
        if action in emitted:
            ...
            event = OutboxEvent(
                tenant_id=_tenant(tenant_id),
                aggregate_type="project_management",
                aggregate_id=_uuid(entity_id),
    ...
    def replay(tenant_id: Any, action: str, idempotency_key: str, fingerprint: str, model: Any) -> Any:
        ...
        activity = (
            ProjectActivity.objects.for_tenant(_tenant(tenant_id))
            .filter(action=action, metadata__idempotency_key=idempotency_key)
            .first()
        )
        if not activity:
            return None
        if activity.metadata.get("fingerprint") != fingerprint:
            raise IdempotencyConflictError()
        try:
            return model.objects.for_tenant(_tenant(tenant_id)).get(pk=activity.entity_id)
...
def _project(tenant_id: Any, project_id: Any, include_archived: bool = False, lock: bool = False) -> Project:
    ...
    try:
        return qs.get(pk=_uuid(project_id, "project_id"))
    except Project.DoesNotExist as exc:
        raise ProjectManagementError("Project was not found.", "NOT_FOUND") from exc
...
class ProjectService:
    ...
    def create_project(
        ...
        **legacy: Any,
        ...
        if data is None:
            data = legacy
            if actor_id is not None and not legacy and isinstance(actor_id, Mapping):
                data, actor_id = actor_id, None
        ...
        if not re.fullmatch(config.project_code_pattern, payload["project_code"]):
            raise ProjectManagementError("project_code does not match tenant configuration.", "INVALID_PROJECT_CODE")
    ...
    def update_project(
        ...
        if "project_code" in payload:
            ...
            if not re.fullmatch(_active_configuration(tenant_id).project_code_pattern, payload["project_code"]):
                raise ProjectManagementError(
                    "project_code does not match tenant configuration.", "INVALID_PROJECT_CODE"
                )
    ...
    def transition_project(
        ...
        transitions = {
            ...
            "resume": ({ProjectStatus.ON_HOLD}, ProjectStatus.ACTIVE),
            "complete": ({ProjectStatus.ACTIVE, ProjectStatus.ON_HOLD}, ProjectStatus.COMPLETED),
            "cancel": ({ProjectStatus.PLANNING, ProjectStatus.ACTIVE, ProjectStatus.ON_HOLD}, ProjectStatus.CANCELLED),
        }
        if command not in transitions:
            raise ProjectManagementError("Unknown project transition.", "UNKNOWN_COMMAND")
        sources, target = transitions[command]
        if project.status in {ProjectStatus.COMPLETED, ProjectStatus.CANCELLED} or project.status not in sources:
            raise ProjectManagementError(f"{command} is not allowed from {project.status}.", "ILLEGAL_TRANSITION")
        if command == "activate" and (not project.start_date or not project.project_manager_id):
            raise ProjectManagementError("Activation requires a start date and project manager.", "ACTIVATION_GUARD")
        if command == "complete":
            if (
                ...
                raise ProjectManagementError("Complete or cancel all active tasks first.", "INCOMPLETE_TASKS")
            if (
                ProjectMilestone.objects.for_tenant(_tenant(tenant_id))
                .filter(
                    project=project, archived_at__isnull=True, achieved_date__isnull=True, cancelled_at__isnull=True
                )
                .exists()
            ):
                raise ProjectManagementError("Achieve or cancel all active milestones first.", "INCOMPLETE_MILESTONES")
        before = _snapshot(project)
        prior = project.status
        project.status = target
        project.version += 1
        project.transition_history = [
            *project.transition_history,
            {
                ...
                "reason": reason,
                "actor_id": str(_actor(actor_id)),
                "correlation_id": _correlation(transition_key),
    ...
    def duplicate_project(
        ...
        for task in (
            ...
            copy = TaskService.create_task(
                ...
                f"{idempotency_key}:task:{task.id}",
            )
            task_map[task.id] = copy
        for member in ProjectMember.objects.for_tenant(_tenant(tenant_id)).filter(
            project=source, archived_at__isnull=True
        ):
            ProjectMemberService.add_member(
                ...
                f"{idempotency_key}:member:{member.id}",
            )
        for milestone in ProjectMilestone.objects.for_tenant(_tenant(tenant_id)).filter(
            project=source, archived_at__isnull=True
        ):
            MilestoneService.create_milestone(
                ...
                f"{idempotency_key}:milestone:{milestone.id}",
            )
        return clone
    ...
    def get_project_summary(tenant_id: Any, project_id: Any) -> dict[str, Any]:
        ...
        time_total = TimeEntry.objects.for_tenant(_tenant(tenant_id)).filter(
            project=project, archived_at__isnull=True
        ).aggregate(v=models.Sum("hours_worked"))["v"] or Decimal("0")
        total = tasks.count()
        ...
        return {
            ...
            "progress_percentage": Decimal(done * 100 / total).quantize(Decimal("0.01")) if total else Decimal("0.00"),
            "milestone_count": milestones.count(),
            "achieved_milestone_count": milestones.filter(achieved_date__isnull=False).count(),
            "time_hours": time_total,
            "next_due_date": tasks.filter(due_date__isnull=False)
            .order_by("due_date")
    ...
    def get_portfolio_summary(tenant_id: Any) -> dict[str, Any]:
        ...
        budget_rows = (
            projects.exclude(budget__isnull=True)
            .values("currency")
            .annotate(total=models.Sum("budget"))
            .order_by("currency")
        )
        return {
            ...
            "overdue_task_count": tasks.filter(due_date__lt=today)
            .exclude(status__in=[TaskStatus.DONE, TaskStatus.CANCELLED])
            .count(),
            "blocked_task_count": tasks.filter(status=TaskStatus.BLOCKED).count(),
            "upcoming_milestone_count": milestones.filter(
                target_date__gte=today, achieved_date__isnull=True, cancelled_at__isnull=True
            ).count(),
...
class TaskService:
    ...
    def _payload(tenant_id: Any, data: Mapping[str, Any], instance: Task | None = None) -> dict[str, Any]:
        ...
        if parent_id:
            payload["parent_task"] = (
                Task.objects.for_tenant(_tenant(tenant_id))
                .filter(pk=_uuid(parent_id), archived_at__isnull=True)
                .first()
            )
        if parent_id and not payload["parent_task"]:
            raise ProjectManagementError("Parent task was not found.", "NOT_FOUND")
        if "task_code" in payload:
            payload["task_code"] = _required(payload["task_code"], "task_code", 50).upper()
        return payload
    ...
    def create_task(tenant_id: Any, actor_id: Any, data: Mapping[str, Any], idempotency_key: str) -> Task:
        ...
        if not re.fullmatch(config.task_code_pattern, payload.get("task_code", "")):
            raise ProjectManagementError("task_code does not match tenant configuration.", "INVALID_TASK_CODE")
    ...
    def update_task(
        ...
        _version(task, expected_version)
        payload = TaskService._payload(tenant_id, data, task)
    ...
    def transition_task(
        ...
        try:
            ...
            raise ProjectManagementError("Task was not found.", "NOT_FOUND") from exc
        fp = _fingerprint(
            "task.transition", {"id": task_id, "command": command, "reason": reason, "target": target_state}
        )
        ...
        allowed = {
            ...
            "submit_review": {TaskStatus.IN_PROGRESS},
            "request_changes": {TaskStatus.REVIEW},
            "complete": {TaskStatus.IN_PROGRESS, TaskStatus.REVIEW},
            "block": {TaskStatus.TODO, TaskStatus.IN_PROGRESS, TaskStatus.REVIEW},
            "cancel": {TaskStatus.TODO, TaskStatus.IN_PROGRESS, TaskStatus.REVIEW, TaskStatus.BLOCKED},
        }
        if command == "unblock":
            if target_state not in {TaskStatus.TODO, TaskStatus.IN_PROGRESS}:
                raise ProjectManagementError("Unblock target must be todo or in_progress.", "INVALID_TARGET_STATE")
            target, allowed[command] = target_state, {TaskStatus.BLOCKED}
        if command not in allowed or task.status not in allowed[command]:
            raise ProjectManagementError(f"{command} is not allowed from {task.status}.", "ILLEGAL_TRANSITION")
        if command == "block" and not str(reason).strip():
            raise ProjectManagementError("A block reason is required.", "BLOCK_REASON_REQUIRED")
        prior = task.status
        before = _snapshot(task)
        task.status = target
    ...
    def _archive(tenant_id: Any, actor_id: Any, task_id: Any, expected_version: Any, key: str, restore: bool) -> Task:
        ...
        ActivityService.record(
            ...
            after=_snapshot(task),
            idempotency_key=key,
            fingerprint=_fingerprint(action, {"id": task_id}),
        )
        return task

    archive_task = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, task_id, expected_version, idempotency_key: TaskService._archive(
                tenant_id, actor_id, task_id, expected_version, idempotency_key, False
            )
        )
    )
    restore_task = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, task_id, expected_version, idempotency_key: TaskService._archive(
                tenant_id, actor_id, task_id, expected_version, idempotency_key, True
            )
        )
    )
...
class ProjectMemberService:
    ...
    def add_member(tenant_id: Any, actor_id: Any, data: Mapping[str, Any], idempotency_key: str) -> ProjectMember:
        ...
        ActivityService.record(
            ...
            idempotency_key=idempotency_key,
            fingerprint=_fingerprint("member.create", data),
        )
        return member
    ...
    def update_member(
        ...
        for field in {"role", "allocation_percentage", "joined_at", "left_at"} & data.keys():
        ...
        ActivityService.record(
            ...
            after=_snapshot(member),
            idempotency_key=idempotency_key,
            fingerprint=_fingerprint("member.update", data),
        )
        return member
    ...
    def _archive(tenant_id: Any, actor_id: Any, member_id: Any, key: str, restore: bool) -> ProjectMember:
        ...
        ActivityService.record(
            ...
            after=_snapshot(member),
            idempotency_key=key,
            fingerprint=_fingerprint("member.archive", {"id": member_id, "restore": restore}),
        )
        return member

    archive_member = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, member_id, idempotency_key: ProjectMemberService._archive(
                tenant_id, actor_id, member_id, idempotency_key, False
            )
        )
    )
    restore_member = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, member_id, idempotency_key: ProjectMemberService._archive(
                tenant_id, actor_id, member_id, idempotency_key, True
            )
        )
    )
...
class TimeEntryService:
    ...
    def _payload(tenant_id: Any, data: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if task_id:
            try:
                payload["task"] = Task.objects.for_tenant(_tenant(tenant_id)).get(
                    pk=_uuid(task_id), project=payload["project"], archived_at__isnull=True
                )
            except Task.DoesNotExist as exc:
                raise ProjectManagementError(
                    "Task must belong to the selected project.", "TASK_PROJECT_MISMATCH"
                ) from exc
        return payload
    ...
    def _validate(tenant_id: Any, payload: dict[str, Any], exclude: Any = None) -> None:
        ...
        if entry_date > timezone.localdate() and not config.allow_future_time_entries:
            raise ProjectManagementError("Future time entries are disabled.", "FUTURE_TIME_DISABLED")
        if config.require_time_description and not str(payload.get("description", "")).strip():
            raise ProjectManagementError("A time description is required.", "DESCRIPTION_REQUIRED")
        hours = Decimal(str(payload["hours_worked"]))
        qs = (
            TimeEntry.objects.for_tenant(_tenant(tenant_id))
            .select_for_update()
            .filter(employee_id=payload["employee_id"], entry_date=entry_date, archived_at__isnull=True)
        )
        if exclude:
            qs = qs.exclude(pk=exclude)
        total = qs.aggregate(v=models.Sum("hours_worked"))["v"] or Decimal("0")
        if hours <= 0 or hours > 24 or total + hours > config.max_daily_hours:
            raise ProjectManagementError("Hours exceed the configured daily limit.", "DAILY_HOURS_LIMIT")
    ...
    def create_time_entry(
        ...
        **legacy: Any,
        ...
        if entry.task_id:
            TaskService.recalculate_actual_hours(tenant_id, entry.task_id)
        return entry
    ...
    def update_time_entry(
        ...
        old_task = entry.task_id
        merged = {
            "project": data.get("project", entry.project_id),
            "task": data.get("task", entry.task_id),
            "employee_id": data.get("employee_id", entry.employee_id),
            ...
            "billable": data.get("billable", entry.billable),
        }
        payload = TimeEntryService._payload(tenant_id, merged)
        TimeEntryService._validate(tenant_id, payload, entry.id)
        for k, v in payload.items():
            setattr(entry, k, v)
        entry.version += 1
        _save(entry)
        ActivityService.record(
            ...
            after=_snapshot(entry),
            idempotency_key=idempotency_key,
            fingerprint=_fingerprint("time.update", data),
        )
        if old_task:
            TaskService.recalculate_actual_hours(tenant_id, old_task)
        if entry.task_id and entry.task_id != old_task:
            TaskService.recalculate_actual_hours(tenant_id, entry.task_id)
        return entry
    ...
    def _archive(
        ...
        ActivityService.record(
            ...
            after=_snapshot(entry),
            idempotency_key=key,
            fingerprint=_fingerprint("time.archive", {"id": entry_id, "restore": restore}),
        )
        if entry.task_id:
            TaskService.recalculate_actual_hours(tenant_id, entry.task_id)
        return entry

    archive_time_entry = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, entry_id, expected_version, idempotency_key: TimeEntryService._archive(
                tenant_id, actor_id, entry_id, expected_version, idempotency_key, False
            )
        )
    )
    restore_time_entry = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, entry_id, expected_version, idempotency_key: TimeEntryService._archive(
                tenant_id, actor_id, entry_id, expected_version, idempotency_key, True
            )
        )
    )
...
class MilestoneService:
    ...
    def create_milestone(
        ...
        ActivityService.record(
            ...
            idempotency_key=idempotency_key,
            fingerprint=_fingerprint("milestone.create", data),
        )
        return milestone
    ...
    def update_milestone(
        ...
        ActivityService.record(
            ...
            after=_snapshot(milestone),
            idempotency_key=idempotency_key,
            fingerprint=_fingerprint("milestone.update", data),
        )
        return milestone
    ...
    def _get(tenant_id: Any, milestone_id: Any, lock: bool = False) -> ProjectMilestone:
        ...
        try:
            return qs.get(pk=_uuid(milestone_id))
    ...
    def _lifecycle(
        ...
        if action == "achieve":
            ...
            if milestone.project.start_date and achieved < milestone.project.start_date:
                raise ProjectManagementError("Achieved date cannot precede project start.", "INVALID_DATE")
        ...
        ActivityService.record(
            ...
            after=_snapshot(milestone),
            idempotency_key=key,
            fingerprint=_fingerprint(f"milestone.{action}", {"id": milestone_id, "date": achieved_date}),
        )
        return milestone

    achieve_milestone = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, milestone_id, achieved_date, idempotency_key: MilestoneService._lifecycle(
                tenant_id, actor_id, milestone_id, "achieve", idempotency_key, achieved_date
            )
        )
    )
    reopen_milestone = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, milestone_id, idempotency_key: MilestoneService._lifecycle(
                tenant_id, actor_id, milestone_id, "reopen", idempotency_key
            )
        )
    )
    cancel_milestone = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, milestone_id, idempotency_key: MilestoneService._lifecycle(
                tenant_id, actor_id, milestone_id, "cancel", idempotency_key
            )
        )
    )
    ...
    def _archive(
        ...
        ActivityService.record(
            ...
            after=_snapshot(milestone),
            idempotency_key=key,
            fingerprint=_fingerprint("milestone.archive", {"id": milestone_id, "restore": restore}),
        )
        return milestone

    archive_milestone = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, milestone_id, expected_version, idempotency_key: MilestoneService._archive(
                tenant_id, actor_id, milestone_id, expected_version, idempotency_key, False
            )
        )
    )
    restore_milestone = staticmethod(
        transaction.atomic(
            lambda tenant_id, actor_id, milestone_id, expected_version, idempotency_key: MilestoneService._archive(
                tenant_id, actor_id, milestone_id, expected_version, idempotency_key, True
            )
        )
    )
...
class ConfigurationService:
    FIELDS = {
        ...
        "enforce_project_date_bounds",
        "allow_future_time_entries",
        "require_time_description",
        "default_billable",
        "enabled_views",
        "paid_extension_rollout",
    }
    DEFAULTS = {
        "default_currency": "USD",
        "project_code_pattern": r"^[A-Z][A-Z0-9-]{0,49}$",
        "task_code_pattern": r"^[A-Z][A-Z0-9-]{0,49}$",
        "max_daily_hours": Decimal("12.00"),
    ...
    def _validate(values: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if unknown:
            raise ProjectManagementError(
                f"Unknown configuration fields: {', '.join(sorted(unknown))}.", "UNKNOWN_FIELD"
            )
        merged = {**ConfigurationService.DEFAULTS, **values}
        currency = str(merged["default_currency"]).upper()
        if len(currency) != 3 or not currency.isalpha():
            raise ProjectManagementError("default_currency must be a three-letter code.", "INVALID_CURRENCY")
        ...
        for name in ("project_code_pattern", "task_code_pattern"):
            ...
            if (
                ...
                raise ProjectManagementError(f"{name} must be a safe anchored regular expression.", "UNSAFE_REGEX")
            try:
                re.compile(pattern)
            except re.error as exc:
                raise ProjectManagementError(f"{name} is invalid.", "UNSAFE_REGEX") from exc
        merged["max_daily_hours"] = Decimal(str(merged["max_daily_hours"]))
        merged["max_allocation_percentage"] = Decimal(str(merged["max_allocation_percentage"]))
        if not Decimal("0") < merged["max_daily_hours"] <= Decimal("24"):
            raise ProjectManagementError("max_daily_hours must be between 0 and 24.", "SAFE_LIMIT")
        if not Decimal("0") < merged["max_allocation_percentage"] <= Decimal("100"):
            raise ProjectManagementError("max_allocation_percentage must be between 0 and 100.", "SAFE_LIMIT")
        views = merged["enabled_views"]
        if (
            not isinstance(views, list)
            or len(views) != len(set(views))
            or not set(views) <= {"list", "board", "calendar", "timeline"}
        ):
            raise ProjectManagementError(
                "enabled_views contains unsupported or duplicate values.", "INVALID_JSON_SCHEMA"
            )
        rollout = merged["paid_extension_rollout"]
        if not isinstance(rollout, dict) or any(
            not isinstance(k, str) or not isinstance(v, (bool, list, str)) for k, v in rollout.items()
        ):
            raise ProjectManagementError("paid_extension_rollout has an invalid schema.", "INVALID_JSON_SCHEMA")
        return merged
    ...
    def get_active(tenant_id: Any, environment: str) -> ProjectManagementConfigurationVersion:
        ...
        if config.active_version_id:
            return ProjectManagementConfigurationVersion.objects.for_tenant(tenant).get(pk=config.active_version_id)
        system = uuid.UUID(int=0)
        version = ProjectManagementConfigurationVersion(
            tenant_id=tenant,
            configuration=config,
            version=1,
            state=ConfigurationState.ACTIVE,
            change_summary="Initial safe defaults",
    ...
    def create_draft(
        ...
        version = (
            ProjectManagementConfigurationVersion.objects.filter(
                tenant_id=_tenant(tenant_id), configuration=active.configuration
            ).aggregate(v=models.Max("version"))["v"]
            + 1
        )
        draft = ProjectManagementConfigurationVersion(
            tenant_id=_tenant(tenant_id),
            configuration=active.configuration,
            version=version,
            state=ConfigurationState.DRAFT,
            change_summary=_required(change_summary, "change_summary", 500),
    ...
    def simulate(tenant_id: Any, draft_id: Any) -> dict[str, Any]:
        ...
        except ProjectManagementConfigurationVersion.DoesNotExist as exc:
            raise ProjectManagementError("Draft was not found.", "NOT_FOUND") from exc
        config = ConfigurationService._validate({f: getattr(draft, f) for f in ConfigurationService.FIELDS})
        invalid_projects = (
            Project.objects.for_tenant(_tenant(tenant_id))
            .filter(archived_at__isnull=True)
            .exclude(project_code__regex=config["project_code_pattern"])
            .count()
        )
        invalid_tasks = (
            Task.objects.for_tenant(_tenant(tenant_id))
            .filter(archived_at__isnull=True)
            .exclude(task_code__regex=config["task_code_pattern"])
            .count()
        )
        over_allocated = (
            ProjectMember.objects.for_tenant(_tenant(tenant_id))
            .filter(archived_at__isnull=True, allocation_percentage__gt=config["max_allocation_percentage"])
            .count()
        )
        return {
            "valid": not any([invalid_projects, invalid_tasks, over_allocated]),
            "errors": [],
            "affected_records": {
                "invalid_project_codes": invalid_projects,
                "invalid_task_codes": invalid_tasks,
    ...
    def publish(
        ...
        try:
            draft = (
                ...
                .select_for_update()
                .get(pk=_uuid(draft_id), state=ConfigurationState.DRAFT)
            )
        except ProjectManagementConfigurationVersion.DoesNotExist as exc:
            raise ProjectManagementError("Draft was not found.", "NOT_FOUND") from exc
        ...
        ActivityService.record(
            ...
            idempotency_key=idempotency_key,
            fingerprint=_fingerprint("configuration.publish", {"id": draft.id}),
        )
        return draft
    ...
    def export_document(tenant_id: Any, environment: str) -> dict[str, Any]:
        ...
        return {
            ...
            "values": {
                f: str(getattr(active, f)) if isinstance(getattr(active, f), Decimal) else getattr(active, f)
                for f in ConfigurationService.FIELDS
            },
        }
    ...
    def import_document(
        ...
        if (
            ...
            raise ProjectManagementError("Unsupported configuration document.", "INVALID_DOCUMENT")
        return ConfigurationService.create_draft(
            tenant_id,
            actor_id,
            document.get("environment", ConfigurationService.runtime_environment()),
            document["values"],
    ...
    def rollback(
        ...
        try:
            ...
            raise ProjectManagementError("Configuration version was not found.", "NOT_FOUND") from exc
        values = {f: getattr(target, f) for f in ConfigurationService.FIELDS}
        draft = ConfigurationService.create_draft(
            tenant, actor_id, target.configuration.environment, values, f"Rollback to version {target.version}"
        )
        return ConfigurationService.publish(tenant, actor_id, draft.id, idempotency_key)
