...
class ExtensionResult:
    ...
    @classmethod
    def unavailable(cls, code="PROVIDER_UNAVAILABLE"):
        return cls("unavailable", None, code)
...
class ProjectValidator(Protocol):
    ...
    def validate(
        self, context: ProjectExtensionContext, operation: str, changes: Mapping[str, object]
    ) -> ExtensionResult: ...
...
class ProjectDetailPanel:
    ...
    slot: str
    module: str
    minimum_spi_version: str = SPI_VERSION
...
class ExtensionRegistry:
    ...
    def register(self, provider):
        ...
        if not key:
            raise ValueError("Extension provider key is required.")
        if key in self._providers:
            raise ValueError(f"Duplicate extension provider key: {key}")
