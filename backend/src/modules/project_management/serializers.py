...
class ProjectDetailSerializer(AllowedActionsMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        allowed_actions = ("update", "transition", "archive", "duplicate")
...
class ProjectUpdateSerializer(ProjectCreateSerializer):
    project_code = serializers.CharField(max_length=50, required=False)
...
class TaskDetailSerializer(AllowedActionsMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        allowed_actions = ("update", "transition", "reorder", "archive")


class TaskCreateSerializer(StrictSerializer):
    project = serializers.UUIDField()
    task_code = serializers.CharField(max_length=50)
    ...
    priority = serializers.ChoiceField(choices=("critical", "high", "medium", "low"), required=False)
...
class TaskUpdateSerializer(TaskCreateSerializer):
    ...
    task_code = serializers.CharField(max_length=50, required=False)
...
class ProjectMilestoneCreateSerializer(StrictSerializer):
    project = serializers.UUIDField()
    milestone_name = serializers.CharField(max_length=255)
...
class ProjectMilestoneUpdateSerializer(StrictSerializer):
    milestone_name = serializers.CharField(max_length=255, required=False)
...
class TransitionSerializer(StrictSerializer):
    ...
    target_state = serializers.ChoiceField(choices=("todo", "in_progress"), required=False)
...
class IdempotencySerializer(StrictSerializer):
    idempotency_key = serializers.CharField(max_length=255)


class ReorderTaskSerializer(ArchiveRestoreSerializer):
    position = serializers.IntegerField(min_value=1)


class DuplicateProjectSerializer(IdempotencySerializer):
    project_code = serializers.CharField(max_length=50)
    project_name = serializers.CharField(max_length=255)


class MilestoneAchieveSerializer(IdempotencySerializer):
...
class ConfigurationVersionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ProjectManagementConfigurationVersion
        fields = (
            "id",
            "environment",
            "version",
...
class ConfigurationPublishSerializer(IdempotencySerializer):
    pass


class ConfigurationRollbackSerializer(IdempotencySerializer):
...
class ProjectSummarySerializer(StrictSerializer):
    ...
    time_hours = serializers.DecimalField(max_digits=12, decimal_places=2)
