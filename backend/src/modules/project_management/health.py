...
def get_module_health(tenant_id):
    ...
    return {
        "status": "healthy" if all(v in {"healthy", "ready", "not_enabled"} for v in checks.values()) else "degraded",
...
class ProjectManagementHealthView(GovernedAPIViewMixin, APIView):
    ...
    def get(self, request):
        ...
        return Response(
            result,
            status=status.HTTP_200_OK if result["status"] != "unhealthy" else status.HTTP_503_SERVICE_UNAVAILABLE,
        )
