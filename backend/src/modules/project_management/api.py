    ...
    def handle_exception(self, exc):
        if isinstance(exc, StaleVersionError):
            ...
            exc = OperationFailed(
                ...
                message="The idempotency key conflicts with a prior command.",
                http_status=status.HTTP_409_CONFLICT,
            )
            ...
            if exc.code == "NOT_FOUND":
                ...
                exc = ValidationError({"code": exc.code, "message": exc.messages[0]})
...
class CrudViewSet(
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        self.archive_service(
            ...
            self.actor_id(),
            instance.id,
            int(payload.get("version", getattr(instance, "version", 1))),
            self.idempotency_key(payload),
        )
...
class ProjectViewSet(CrudViewSet):
    ...
    action_permissions = {
        "list": "project_management.project:read",
        "retrieve": "project_management.project:read",
        "summary": "project_management.project:read",
        "create": "project_management.project:create",
        "partial_update": "project_management.project:update",
        "destroy": "project_management.project:archive",
        "restore": "project_management.project:restore",
        "duplicate": "project_management.project:duplicate",
        **{x: "project_management.project:transition" for x in ("activate", "hold", "resume", "complete", "cancel")},
    }
    action_quotas = {
        "create": "project_management.project_writes",
        "partial_update": "project_management.project_writes",
        "destroy": "project_management.project_writes",
    }
    ...
    def get_queryset(self):
        ...
        qs = (
            (Project.all_objects if include_archived else Project.objects).for_tenant(self.tenant_id()).select_related()
        )
        p = self.request.query_params
        if p.get("search"):
            qs = qs.filter(Q(project_code__icontains=p["search"][:100]) | Q(project_name__icontains=p["search"][:100]))
        if p.get("status"):
            qs = qs.filter(status=p["status"])
        if p.get("manager_id"):
            qs = qs.filter(project_manager_id=p["manager_id"])
        if p.get("start_from"):
            qs = qs.filter(start_date__gte=_date(p["start_from"], "start_from"))
        if p.get("start_to"):
            qs = qs.filter(start_date__lte=_date(p["start_to"], "start_to"))
        ordering = p.get("ordering", "-updated_at")
        allowed = {"project_code", "project_name", "start_date", "updated_at"}
    ...
    def create(self, request):
        ...
        project = ProjectService.create_project(
            self.tenant_id(), self.actor_id(), serializer.validated_data, self.idempotency_key(request.data)
        )
    ...
    def partial_update(self, request, pk=None):
        ...
        project = ProjectService.update_project(
            self.tenant_id(), self.actor_id(), pk, data, version, self.idempotency_key({"idempotency_key": key})
        )
    ...
    def _transition(self, request, pk, command):
        ...
        serializer.is_valid(raise_exception=True)
        project = ProjectService.transition_project(
            self.tenant_id(),
            self.actor_id(),
            pk,
            command,
    ...
    def restore(self, request, pk=None):
        ...
        project = ProjectService.restore_project(
            ...
            self.idempotency_key(serializer.validated_data),
        )
        return Response(ProjectDetailSerializer(project).data)
    ...
    def duplicate(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        project = ProjectService.duplicate_project(
            self.tenant_id(),
            self.actor_id(),
            pk,
            serializer.validated_data["project_code"],
...
class TaskViewSet(CrudViewSet):
    ...
    archive_service = staticmethod(TaskService.archive_task)
    action_permissions = {
        "list": "project_management.task:read",
        "retrieve": "project_management.task:read",
        "create": "project_management.task:create",
        "partial_update": "project_management.task:update",
        "destroy": "project_management.task:archive",
        "restore": "project_management.task:restore",
        "reorder": "project_management.task:reorder",
        **{
            x: "project_management.task:transition"
            for x in ("start", "submit_review", "request_changes", "complete", "block", "unblock", "cancel")
        },
    }
    ...
    def get_queryset(self):
        ...
        qs = (
            (Task.all_objects if include else Task.objects)
        ...
        if p.get("search"):
            qs = qs.filter(Q(task_code__icontains=p["search"][:100]) | Q(task_name__icontains=p["search"][:100]))
        if p.get("due_from"):
            qs = qs.filter(due_date__gte=_date(p["due_from"], "due_from"))
        if p.get("due_to"):
            qs = qs.filter(due_date__lte=_date(p["due_to"], "due_to"))
        if p.get("overdue") and _bool(p["overdue"], "overdue"):
            qs = qs.filter(due_date__lt=timezone.localdate()).exclude(status__in=["done", "cancelled"])
        ordering = p.get("ordering", "position")
        allowed = {"position", "due_date", "priority", "updated_at"}
        if ordering.lstrip("-") not in allowed:
            raise ValidationError({"ordering": "Unsupported ordering field."})
        return qs.order_by(ordering, "id")
    ...
    def create(self, request):
        ...
        obj = TaskService.create_task(
            self.tenant_id(), self.actor_id(), s.validated_data, self.idempotency_key(request.data)
        )
    ...
    def partial_update(self, request, pk=None):
        ...
        v = d.pop("version")
        k = d.pop("idempotency_key")
        obj = TaskService.update_task(
    ...
    def _transition(self, r, pk, command):
        ...
        s.is_valid(raise_exception=True)
        obj = TaskService.transition_task(
            self.tenant_id(),
            self.actor_id(),
            pk,
            command,
            s.validated_data["transition_key"],
            s.validated_data.get("reason", ""),
            s.validated_data.get("target_state"),
        )
        return Response(TaskDetailSerializer(obj).data)
    ...
    def reorder(self, r, pk=None):
        ...
        obj = TaskService.reorder_task(
            ...
            self.actor_id(),
            pk,
            s.validated_data["position"],
            s.validated_data["version"],
            self.idempotency_key(s.validated_data),
        )
        return Response(TaskDetailSerializer(obj).data)
    ...
    def restore(self, r, pk=None):
        ...
        obj = TaskService.restore_task(
            self.tenant_id(), self.actor_id(), pk, s.validated_data["version"], self.idempotency_key(s.validated_data)
        )
        return Response(TaskDetailSerializer(obj).data)
...
class ProjectMemberViewSet(CrudViewSet):
    ...
    list_serializer_class = ProjectMemberListSerializer
    detail_serializer_class = ProjectMemberDetailSerializer
    serializer_class = ProjectMemberCreateSerializer
    action_permissions = {
        "list": "project_management.member:read",
        "retrieve": "project_management.member:read",
        "create": "project_management.member:create",
        "partial_update": "project_management.member:update",
        "destroy": "project_management.member:archive",
    ...
    def get_queryset(self):
        ...
        include = bool(p.get("include_archived") and _bool(p["include_archived"], "include_archived"))
        qs = (
            (ProjectMember.all_objects if include else ProjectMember.objects)
            .for_tenant(self.tenant_id())
        ...
        if ordering.lstrip("-") not in {"joined_at", "role", "updated_at"}:
            raise ValidationError({"ordering": "Unsupported ordering field."})
        return qs.order_by(ordering, "id")
    ...
    def create(self, r):
        ...
        o = ProjectMemberService.add_member(
            self.tenant_id(), self.actor_id(), s.validated_data, self.idempotency_key(r.data)
        )
    ...
    def partial_update(self, r, pk=None):
        ...
        d = dict(s.validated_data)
        k = d.pop("idempotency_key")
        o = ProjectMemberService.update_member(
            self.tenant_id(), self.actor_id(), pk, d, self.idempotency_key({"idempotency_key": k})
        )
    ...
    def restore(self, r, pk=None):
        ...
        o = ProjectMemberService.restore_member(
            self.tenant_id(), self.actor_id(), pk, self.idempotency_key(s.validated_data)
        )
        return Response(ProjectMemberDetailSerializer(o).data)
...
class TimeEntryViewSet(CrudViewSet):
    ...
    list_serializer_class = TimeEntryListSerializer
    detail_serializer_class = TimeEntryDetailSerializer
    serializer_class = TimeEntryCreateSerializer
    archive_service = staticmethod(TimeEntryService.archive_time_entry)
    action_permissions = {
        "list": "project_management.time_entry:read",
        "retrieve": "project_management.time_entry:read",
        "create": "project_management.time_entry:create",
        "partial_update": "project_management.time_entry:update",
        "destroy": "project_management.time_entry:archive",
    ...
    def get_queryset(self):
        ...
        include = bool(p.get("include_archived") and _bool(p["include_archived"], "include_archived"))
        qs = (
            (TimeEntry.all_objects if include else TimeEntry.objects)
            .for_tenant(self.tenant_id())
            .select_related("project", "task")
        )
        for k in ("project_id", "task_id", "employee_id", "billable"):
            if p.get(k) is not None and p.get(k) != "":
                qs = qs.filter(**{k: _bool(p[k], k) if k == "billable" else p[k]})
        if p.get("entry_from"):
            qs = qs.filter(entry_date__gte=_date(p["entry_from"], "entry_from"))
        if p.get("entry_to"):
            qs = qs.filter(entry_date__lte=_date(p["entry_to"], "entry_to"))
        ordering = p.get("ordering", "-entry_date")
        if ordering.lstrip("-") not in {"entry_date", "created_at"}:
            raise ValidationError({"ordering": "Unsupported ordering field."})
        return qs.order_by(ordering, "id")
    ...
    def partial_update(self, r, pk=None):
        ...
        v = d.pop("version")
        k = d.pop("idempotency_key")
        o = TimeEntryService.update_time_entry(
            self.tenant_id(), self.actor_id(), pk, d, v, self.idempotency_key({"idempotency_key": k})
        )
...
class ProjectMilestoneViewSet(CrudViewSet):
    ...
    list_serializer_class = ProjectMilestoneListSerializer
    detail_serializer_class = ProjectMilestoneDetailSerializer
    serializer_class = ProjectMilestoneCreateSerializer
    archive_service = staticmethod(MilestoneService.archive_milestone)
    action_permissions = {
        "list": "project_management.milestone:read",
        "retrieve": "project_management.milestone:read",
        "create": "project_management.milestone:create",
        "partial_update": "project_management.milestone:update",
        "destroy": "project_management.milestone:archive",
    ...
    def get_queryset(self):
        ...
        include = bool(p.get("include_archived") and _bool(p["include_archived"], "include_archived"))
        qs = (
            (ProjectMilestone.all_objects if include else ProjectMilestone.objects)
            .for_tenant(self.tenant_id())
            .select_related("project")
        )
        if p.get("project_id"):
            qs = qs.filter(project_id=p["project_id"])
        if p.get("search"):
            qs = qs.filter(milestone_name__icontains=p["search"][:100])
        if p.get("achieved"):
            qs = qs.filter(achieved_date__isnull=not _bool(p["achieved"], "achieved"))
        if p.get("overdue") and _bool(p["overdue"], "overdue"):
            qs = qs.filter(target_date__lt=timezone.localdate(), achieved_date__isnull=True, cancelled_at__isnull=True)
        if p.get("target_from"):
            qs = qs.filter(target_date__gte=_date(p["target_from"], "target_from"))
        if p.get("target_to"):
            qs = qs.filter(target_date__lte=_date(p["target_to"], "target_to"))
        ordering = p.get("ordering", "target_date")
        if ordering.lstrip("-") not in {"target_date", "milestone_name"}:
            raise ValidationError({"ordering": "Unsupported ordering field."})
        return qs.order_by(ordering, "id")
    ...
    def partial_update(self, r, pk=None):
        ...
        v = d.pop("version")
        k = d.pop("idempotency_key")
        o = MilestoneService.update_milestone(
            self.tenant_id(), self.actor_id(), pk, d, v, self.idempotency_key({"idempotency_key": k})
        )
    ...
    def achieve(self, r, pk=None):
        ...
        o = MilestoneService.achieve_milestone(
            ...
            self.actor_id(),
            pk,
            s.validated_data["achieved_date"],
            self.idempotency_key(s.validated_data),
        )
        return Response(ProjectMilestoneDetailSerializer(o).data)
...
class ProjectActivityViewSet(TenantGovernedViewSet, mixins.ListModelMixin, mixins.RetrieveModelMixin):
    ...
    list_serializer_class = ProjectActivitySerializer
    detail_serializer_class = ProjectActivitySerializer
...
class ConfigurationVersionViewSet(TenantGovernedViewSet, mixins.ListModelMixin, mixins.RetrieveModelMixin):
    ...
    def get_queryset(self):
        ...
        if p.get("environment"):
            qs = qs.filter(configuration__environment=p["environment"])
        if p.get("state"):
...
class ConfigurationViewSet(TenantGovernedViewSet):
    ...
    action_permissions = {
        "list": "project_management.configuration:read",
        "drafts": "project_management.configuration:write",
        "simulate": "project_management.configuration:simulate",
        "publish": "project_management.configuration:publish",
        "rollback": "project_management.configuration:rollback",
        "export": "project_management.configuration:export",
    ...
    def publish(self, r, draft_id=None):
        ...
        o = ConfigurationService.publish(
            self.tenant_id(), self.actor_id(), draft_id, self.idempotency_key(s.validated_data)
        )
    ...
    def rollback(self, r):
        ...
        o = ConfigurationService.rollback(
            self.tenant_id(),
            self.actor_id(),
            s.validated_data["target_version"],
            self.idempotency_key(s.validated_data),
        )
        return Response(ConfigurationVersionSerializer(o).data)
