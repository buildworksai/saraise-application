...
from .models import Agent, AgentExecution, AgentManagementConfiguration, AgentSchedulerTask
...
class GovernedTenantViewSet(GovernedAPIViewMixin, viewsets.GenericViewSet):
    ...
    def tenant_id_for_query(self) -> UUID | None:
        """Resolve tenant context without turning an unscoped read into an existence oracle."""

        tenant = get_user_tenant_id(getattr(self.request, "user", None))
        try:
            return UUID(str(tenant)) if tenant else None
        except (ValueError, TypeError, AttributeError):
            return None
    ...
    def correlation_id(self) -> UUID:
        try:
            return UUID(str(correlation_id_for_request(self.request)))
    ...
    def check_permissions(self, request: object) -> None:
        ...
        permission = self.permission_map.get(getattr(self, "action", ""), self.required_permission)
        if getattr(self, "action", "") == "evaluate" and bool(getattr(request, "data", {}).get("red_team")):
            permission = "ai.evaluation:red_team"
...
class AgentViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, GovernedTenantViewSet):
    ...
    def _transition(self, request, command: str) -> Response:
        ...
        if command == "activate":
            ...
            value = AgentService.disable_agent(
                self.tenant_id(), self.actor_id(), agent.id, values["reason"], values["transition_key"]
            )
        else:
            value = AgentService.retire_agent(
                self.tenant_id(), self.actor_id(), agent.id, values["reason"], values["transition_key"]
            )
    ...
    def evaluate(self, request, pk=None):
        ...
        job = method(
            self.tenant_id(), self.actor_id(), agent.id, values["suite_key"], values["idempotency_key"]
        ).unwrap()
...
class AgentExecutionViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, GovernedTenantViewSet):
    ...
    permission_map = {
        "list": "ai.execution:view",
        "retrieve": "ai.execution:view",
        "pause": "ai.agent:pause",
...
class ScheduleViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, GovernedTenantViewSet):
    ...
    def cancel(self, request, pk=None):
        ...
        serializer = TransitionKeySerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        value = ScheduleService.cancel_schedule(
            self.tenant_id(), self.actor_id(), schedule.id, serializer.validated_data["transition_key"]
        )
...
class ApprovalRequestViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, GovernedTenantViewSet):
    ...
    permission_map = {
        "list": "ai.approval:view",
        "retrieve": "ai.approval:view",
        "create": "ai.approval:request",
        "approve": "ai.approval:decide",
        "reject": "ai.approval:decide",
    ...
    def _decision(self, request, command):
        ...
        values = serializer.validated_data
        if command == "approve":
            value = ApprovalService.approve(self.tenant_id(), self.actor_id(), approval.id, values["transition_key"])
        elif command == "reject":
            value = ApprovalService.reject(
                self.tenant_id(), self.actor_id(), approval.id, values["reason"], values["transition_key"]
            )
        else:
            value = ApprovalService.cancel(self.tenant_id(), self.actor_id(), approval.id, values["transition_key"])
...
class SoDPolicyViewSet(ServiceCrudViewSet):
    queryset = SoDPolicy.objects.none()
    serializer_class = SoDPolicySerializer
    write_serializer_class = SoDPolicyWriteSerializer
    permission_map = {
        "list": "ai.governance:view",
        "retrieve": "ai.governance:view",
        "create": "ai.governance:manage",
        "update": "ai.governance:manage",
        "partial_update": "ai.governance:manage",
    ...
    def _write(self, request, updating, partial=False):
        ...
        serializer.is_valid(raise_exception=True)
        value = (
            SoDService.update_policy(self.tenant_id(), self.actor_id(), obj.id, serializer.validated_data)
            if obj
            else SoDService.create_policy(self.tenant_id(), self.actor_id(), serializer.validated_data)
        )
...
class ToolViewSet(ServiceCrudViewSet):
    ...
    permission_map = {
        "list": "ai.tool:view",
        "retrieve": "ai.tool:view",
        "create": "ai.tool:register",
        "update": "ai.tool:update",
        "partial_update": "ai.tool:update",
        "destroy": "ai.tool:delete",
    ...
    def _write(self, request, updating, partial=False):
        ...
        serializer.is_valid(raise_exception=True)
        value = (
            ToolService.update_tool(self.tenant_id(), self.actor_id(), obj.id, serializer.validated_data)
            if obj
            else ToolService.register_tool(self.tenant_id(), self.actor_id(), serializer.validated_data)
        )
    ...
    def validate(self, request, pk=None):
        ...
        values = serializer.validated_data
        return Response(
            ToolService.validation_diagnostic(self.tenant_id(), tool.id, values["direction"], values["value"])
        )


class EgressRuleViewSet(ServiceCrudViewSet):
    queryset = EgressRule.objects.none()
    serializer_class = EgressRuleSerializer
    permission_map = {
        "list": "ai.governance:view",
        "retrieve": "ai.governance:view",
        "create": "ai.governance:manage",
    ...
    def _write(self, request, updating, partial=False):
        ...
        serializer.is_valid(raise_exception=True)
        value = (
            EgressService.update_rule(self.tenant_id(), self.actor_id(), obj.id, serializer.validated_data)
            if obj
            else EgressService.create_rule(self.tenant_id(), self.actor_id(), serializer.validated_data)
        )
...
class SecretViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, GovernedTenantViewSet):
    ...
    serializer_class = SecretMetadataSerializer
    permission_map = {
        "list": "ai.secret:view",
        "retrieve": "ai.secret:view",
        "create": "ai.secret:manage",
        "rotate": "ai.secret:manage",
    ...
    def rotate(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        value = SecretService.rotate_secret(
            self.tenant_id(),
            self.actor_id(),
            secret.id,
...
class KillSwitchViewSet(mixins.ListModelMixin, mixins.RetrieveModelMixin, GovernedTenantViewSet):
    queryset = KillSwitch.objects.none()
    ...
    def create(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        values = serializer.validated_data
        value = KillSwitchService.activate(
            self.tenant_id(),
            self.actor_id(),
            values["scope"],
            values.get("scope_id"),
    ...
    def deactivate(self, request, pk=None):
        ...
        serializer.is_valid(raise_exception=True)
        values = serializer.validated_data
        value = KillSwitchService.deactivate(
            self.tenant_id(), self.actor_id(), switch.id, values["reason"], values["transition_key"]
        )
        return Response(KillSwitchSerializer(value).data)
...
class SoDViolationViewSet(TenantReadOnlyViewSet):
    queryset = SoDViolation.objects.none()
    serializer_class = SoDViolationSerializer


class ToolInvocationViewSet(TenantReadOnlyViewSet):
    queryset = ToolInvocation.objects.none()
    serializer_class = ToolInvocationSerializer


class EgressRequestViewSet(TenantReadOnlyViewSet):
    queryset = EgressRequest.objects.none()
    serializer_class = EgressRequestSerializer


class SecretAccessViewSet(TenantReadOnlyViewSet):
    queryset = SecretAccess.objects.none()
    serializer_class = SecretAccessSerializer


class QuotaUsageViewSet(TenantReadOnlyViewSet):
    queryset = QuotaUsage.objects.none()
    serializer_class = QuotaUsageSerializer
    permission_map = {"list": "ai.usage:view", "retrieve": "ai.usage:view"}


class ShardSaturationViewSet(TenantReadOnlyViewSet):
    queryset = ShardSaturation.objects.none()
    serializer_class = ShardSaturationSerializer
    permission_map = {"list": "ai.usage:view", "retrieve": "ai.usage:view"}


class TokenUsageViewSet(TenantReadOnlyViewSet):
    queryset = TokenUsage.objects.none()
    serializer_class = TokenUsageSerializer
    permission_map = {"list": "ai.usage:view", "retrieve": "ai.usage:view"}


class CostRecordViewSet(TenantReadOnlyViewSet):
    queryset = CostRecord.objects.none()
    serializer_class = CostRecordSerializer
    permission_map = {"list": "ai.usage:view", "retrieve": "ai.usage:view"}


class AuditEventViewSet(TenantReadOnlyViewSet):
    queryset = AuditEvent.objects.none()
    serializer_class = AuditEventSerializer
    permission_map = {"list": "ai.audit:view", "retrieve": "ai.audit:view"}


class AuditTrailViewSet(TenantReadOnlyViewSet):
    queryset = AuditTrail.objects.none()
...
class CostSummaryViewSet(TenantReadOnlyViewSet):
    queryset = CostSummary.objects.none()
    ...
    def recalculate(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        values = serializer.validated_data
        job = UsageService.enqueue_cost_recalculation(self.tenant_id(), self.actor_id(), values, self.correlation_id())
        return Response(AsyncJobSerializer(job).data, status=status.HTTP_202_ACCEPTED)
...
class ConfigurationViewSet(GovernedTenantViewSet):
    queryset = AgentManagementConfiguration.objects.none()
    serializer_class = ConfigurationSerializer
    permission_map = {
        "list": "ai.configuration:view",
        "retrieve": "ai.configuration:view",
        "create": "ai.configuration:manage",
        "preview": "ai.configuration:manage",
        "update": "ai.configuration:manage",
        "partial_update": "ai.configuration:manage",
        "versions": "ai.configuration:view",
        "rollback": "ai.configuration:rollback",
        "import_document": "ai.configuration:import",
    ...
    def list(self, request, *args, **kwargs):
        value = ConfigurationService.current(
            ...
            self.actor_id(),
            self.correlation_id(),
            request.query_params.get("environment", "production"),
        )
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer.is_valid(raise_exception=True)
        values = serializer.validated_data
        value = ConfigurationService.replace(
            self.tenant_id(),
            self.actor_id(),
            self.correlation_id(),
            values["document"],
