...
from .services import EXECUTION_MACHINE, AgentServiceError, EvaluationService, ExecutionService, _transition
...
def execute_agent_job(job: AsyncJob) -> dict[str, object]:
    ...
    if runner is None:
        ExecutionService.fail(
            ...
            execution.id,
            "RUNNER_UNAVAILABLE",
            "The configured runner is unavailable.",
            f"job:{job.id}:unavailable",
        )
    ...
    try:
        ...
        ExecutionService.fail(
            job.tenant_id, execution.id, "RUNNER_FAILED", "The configured runner failed.", f"job:{job.id}:failed"
        )
        raise AgentServiceError("RUNNER_FAILED", "The configured runner failed.")
    completed = ExecutionService.complete(
        job.tenant_id, execution.id, result, {"runner_key": execution.agent.runner_key}, f"job:{job.id}:complete"
    )
