"""
API Integration Tests for BackupDisasterRecovery module.
...
class TestBackupDisasterRecoveryResourceViewSet:
    """Test BackupDisasterRecoveryResourceViewSet CRUD operations."""
    ...
    def test_list_resources(self, authenticated_client, tenant_user):
        ...
        BackupDisasterRecoveryResource.objects.create(
            tenant_id=tenant_id,
        ...
        response = authenticated_client.get(f"/api/v1/backup-disaster-recovery/resources/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_create_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.post(
            f"/api/v1/backup-disaster-recovery/resources/",
            data,
            format="json"
        )
    ...
    def test_get_resource_detail(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.get(f"/api/v1/backup-disaster-recovery/resources/{resource.id}/")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.put(
            f"/api/v1/backup-disaster-recovery/resources/{resource.id}/",
            data,
            format="json"
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_delete_resource(self, authenticated_client, tenant_user):
        ...
        response = authenticated_client.delete(f"/api/v1/backup-disaster-recovery/resources/{resource.id}/")
        assert response.status_code == status.HTTP_204_NO_CONTENT
        
        # Verify resource is deleted
        assert not BackupDisasterRecoveryResource.objects.filter(id=resource.id).exists()
