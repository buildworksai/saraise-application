"""
BackupDisasterRecovery Health Checks
...
def health_check(request):
    """
    Health check endpoint for BackupDisasterRecovery module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "backup-disaster-recovery", "checks": {}}
    ...
    try:
        cache.set("health_check_backup_disaster_recovery", "ok", 10)
        result = cache.get("health_check_backup_disaster_recovery")
        if result == "ok":
    ...
    try:
        ...
        count = BackupDisasterRecoveryResource.objects.count()
