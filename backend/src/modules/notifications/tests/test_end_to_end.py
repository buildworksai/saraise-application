...
def test_unavailable_provider_records_failure_evidence_without_false_success():
    ...
    document["channels"] = {
        **document["channels"],
        "email": {**document["channels"]["email"], "enabled": True, "adapter_key": "not_installed"},
    }
    NotificationConfigurationService.update(
        tenant, "development", actor, document, "Exercise explicit provider failure"
    )
    template = NotificationTemplateService.create_template(
        tenant,
        actor,
    ...
    operation = NotificationDispatchService.enqueue(
        tenant,
        actor,
        {
            "template_id": template.id,
