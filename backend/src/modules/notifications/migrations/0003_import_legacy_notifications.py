...
def import_legacy(apps, schema_editor):
    ...
    for legacy in LegacyNotification.objects.all().iterator():
        ...
        action_url = (
            legacy.action_url
            if isinstance(legacy.action_url, str)
            and legacy.action_url.startswith("/")
            and not legacy.action_url.startswith("//")
            else ""
        )
        ...
        if len(str(metadata).encode()) > 65536:
            metadata = {
                "migration_provenance": {**PROVENANCE, "source_id": str(legacy.pk), "metadata_quarantined": True}
            }
        Notification.objects.update_or_create(
            ...
            defaults={
                ...
                "notification_type": (
                    legacy.type
                    if legacy.type in {"info", "success", "warning", "error", "workflow", "approval", "system"}
                    else "info"
                ),
                "category": f"legacy.{legacy.type}"[:100],
                "title": legacy.title,
                "message": legacy.message,
                "status": "read" if legacy.read else "unread",
                "read_at": legacy.read_at if legacy.read else None,
        ...
        for channel, channel_field in channel_fields.items():
            for category, category_field in category_fields.items():
                Preference.objects.update_or_create(
                    ...
                    category=category,
                    defaults={
                        "enabled": bool(getattr(legacy, channel_field) and getattr(legacy, category_field)),
                        "digest_mode": "immediate",
                        "timezone": "UTC",
                        "migration_provenance": {**PROVENANCE, "source_id": str(legacy.pk)},
                    },
                )
    ...
    for legacy in LegacyToken.objects.all().iterator():
        ...
        Endpoint.objects.update_or_create(
            ...
            defaults={
                "user_id": legacy.user_id,
                "device_type": legacy.device_type,
                "address_ciphertext": _encrypt(legacy.token),
                "display_name": legacy.device_id or f"Imported {legacy.device_type} device",
                "secret_ref": "",
                "is_active": legacy.is_active,
                "last_used_at": legacy.last_used_at,
                "created_by": _actor(legacy.tenant_id),
                "migration_provenance": {**PROVENANCE, "source_id": str(legacy.pk)},
            },
        )
