"""
BlockchainTraceability Health Checks
...
def health_check(request):
    """
    Health check endpoint for BlockchainTraceability module.

    Returns:
    - 200 OK if healthy
    - 503 Service Unavailable if unhealthy
    """
    health_status = {"status": "healthy", "module": "blockchain-traceability", "checks": {}}
    ...
    try:
        cache.set("health_check_blockchain_traceability", "ok", 10)
        result = cache.get("health_check_blockchain_traceability")
        if result == "ok":
    ...
    try:
        ...
        count = BlockchainTraceabilityResource.objects.count()
