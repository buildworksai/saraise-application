...
This guide provides instructions for using the Process Mining module.
...
#### Endpoint: [Name]
**Path:** `/api/v1/process-mining/[path]`
**Method:** POST
...
This document describes all integration points for the Process Mining module, including internal module integrations, external system integrations, and webhook events.

---

## Integration Overview

The Process Mining module integrates with:
...
|-------|----------|---------|----------|
| [event.name] | `/api/v1/process-mining/webhooks/[path]` | [Handler function] | [Use case] |
...
This document describes the comprehensive demo data included with the Process Mining module for testing and training purposes.

## Overview

The demo data seed script (`backend/scripts/seed_process-mining_demo.py`) creates a fully functional Process Mining setup for the demo tenant `demo@saraise.com` with:
...
**Usage:**
```bash
python backend/scripts/seed_process-mining_demo.py --size basic
```
...
**Usage:**
```bash
python backend/scripts/seed_process-mining_demo.py --size full
```
...
**File:** `backend/scripts/seed_process-mining_demo.py`

**Usage:**
```bash
# Basic demo
python backend/scripts/seed_process-mining_demo.py --size basic --tenant demo@saraise.com

# Full demo
python backend/scripts/seed_process-mining_demo.py --size full --tenant demo@saraise.com

# Custom count
python backend/scripts/seed_process-mining_demo.py --count 50 --tenant demo@saraise.com
```
...
#### `generate_process-mining_data(count)`
**Purpose:** Generate [entity type] records
**Parameters:**
- `count` (int): Number of records to generate

```python
def generate_process-mining_data(count: int):
    """Generate demo data"""
    # Implementation
    pass
```
...
```bash
python backend/scripts/seed_process-mining_demo.py --reset --tenant demo@saraise.com
```
...
3. **Run:**
```bash
python backend/scripts/seed_process-mining_demo.py --custom
```
