...
# Backup Disaster Recovery - User Guide
...
This guide provides instructions for using the Backup Disaster Recovery module.
...
<!-- SPDX-License-Identifier: Apache-2.0 -->
# Backup & Disaster Recovery - Customization Guide
...
This guide explains how to customize the Backup & Disaster Recovery module to meet your specific business requirements using SARAISE's low-code customization features.

## Customization Options

The Backup & Disaster Recovery module supports extensive customization through:
...
### Common Custom Fields for Backup & Disaster Recovery
...
# hooks.py
hooks = {
    "backup-disaster-recovery.[Resource Type].before_save": "backup-disaster-recovery.customizations.[resource_type].before_save"
}
```
...
#### Endpoint: [Name]
**Path:** `/api/v1/backup-disaster-recovery/[path]`
**Method:** POST
...
- Prefix custom fields with module name: `backup-disaster-recovery_custom_field`
...
<!-- SPDX-License-Identifier: Apache-2.0 -->
# Backup & Disaster Recovery - Integration Guide
...
This document describes all integration points for the Backup & Disaster Recovery module, including internal module integrations, external system integrations, and webhook events.

---

## Integration Overview

The Backup & Disaster Recovery module integrates with:
...
|-------|----------|---------|----------|
| [event.name] | `/api/v1/backup-disaster-recovery/webhooks/[path]` | [Handler function] | [Use case] |
...
<!-- SPDX-License-Identifier: Apache-2.0 -->
# Backup & Disaster Recovery - Demo Data
...
This document describes the comprehensive demo data included with the Backup & Disaster Recovery module for testing and training purposes.

## Overview

The demo data seed script (`backend/scripts/seed_backup-disaster-recovery_demo.py`) creates a fully functional Backup & Disaster Recovery setup for the demo tenant `demo@saraise.com` with:
...
**Usage:**
```bash
python backend/scripts/seed_backup-disaster-recovery_demo.py --size basic
```
...
**Usage:**
```bash
python backend/scripts/seed_backup-disaster-recovery_demo.py --size full
```
...
**File:** `backend/scripts/seed_backup-disaster-recovery_demo.py`

**Usage:**
```bash
# Basic demo
python backend/scripts/seed_backup-disaster-recovery_demo.py --size basic --tenant demo@saraise.com

# Full demo
python backend/scripts/seed_backup-disaster-recovery_demo.py --size full --tenant demo@saraise.com

# Custom count
python backend/scripts/seed_backup-disaster-recovery_demo.py --count 50 --tenant demo@saraise.com
```
...
#### `generate_backup-disaster-recovery_data(count)`
**Purpose:** Generate [entity type] records
**Parameters:**
- `count` (int): Number of records to generate

```python
def generate_backup-disaster-recovery_data(count: int):
    """Generate demo data"""
    # Implementation
    pass
```
...
```bash
python backend/scripts/seed_backup-disaster-recovery_demo.py --reset --tenant demo@saraise.com
```
...
3. **Run:**
```bash
python backend/scripts/seed_backup-disaster-recovery_demo.py --custom
```
