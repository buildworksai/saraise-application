...
# Ai Provider Configuration - User Guide
...
This guide provides instructions for using the Ai Provider Configuration module.
...
<!-- SPDX-License-Identifier: Apache-2.0 -->
# AI Provider Configuration - Customization Guide
...
This guide explains how to customize the AI Provider Configuration module to meet your specific business requirements using SARAISE's low-code customization features.

## Customization Options

The AI Provider Configuration module supports extensive customization through:
...
### Common Custom Fields for AI Provider Configuration
...
# hooks.py
hooks = {
    "ai-provider-configuration.[Resource Type].before_save": "ai-provider-configuration.customizations.[resource_type].before_save"
}
```
...
#### Endpoint: [Name]
**Path:** `/api/v1/ai-provider-configuration/[path]`
**Method:** POST
...
- Prefix custom fields with module name: `ai-provider-configuration_custom_field`
...
<!-- SPDX-License-Identifier: Apache-2.0 -->
# AI Provider Configuration - Integration Guide
...
This document describes all integration points for the AI Provider Configuration module, including internal module integrations, external system integrations, and webhook events.

---

## Integration Overview

The AI Provider Configuration module integrates with:
...
|-------|----------|---------|----------|
| [event.name] | `/api/v1/ai-provider-configuration/webhooks/[path]` | [Handler function] | [Use case] |
...
<!-- SPDX-License-Identifier: Apache-2.0 -->
# AI Provider Configuration - Demo Data
...
This document describes the comprehensive demo data included with the AI Provider Configuration module for testing and training purposes.

## Overview

The demo data seed script (`backend/scripts/seed_ai-provider-configuration_demo.py`) creates a fully functional AI Provider Configuration setup for the demo tenant `demo@saraise.com` with:
...
**Usage:**
```bash
python backend/scripts/seed_ai-provider-configuration_demo.py --size basic
```
...
**Usage:**
```bash
python backend/scripts/seed_ai-provider-configuration_demo.py --size full
```
...
**File:** `backend/scripts/seed_ai-provider-configuration_demo.py`

**Usage:**
```bash
# Basic demo
python backend/scripts/seed_ai-provider-configuration_demo.py --size basic --tenant demo@saraise.com

# Full demo
python backend/scripts/seed_ai-provider-configuration_demo.py --size full --tenant demo@saraise.com

# Custom count
python backend/scripts/seed_ai-provider-configuration_demo.py --count 50 --tenant demo@saraise.com
```
...
#### `generate_ai-provider-configuration_data(count)`
**Purpose:** Generate [entity type] records
**Parameters:**
- `count` (int): Number of records to generate

```python
def generate_ai-provider-configuration_data(count: int):
    """Generate demo data"""
    # Implementation
    pass
```
...
```bash
python backend/scripts/seed_ai-provider-configuration_demo.py --reset --tenant demo@saraise.com
```
...
3. **Run:**
```bash
python backend/scripts/seed_ai-provider-configuration_demo.py --custom
```
